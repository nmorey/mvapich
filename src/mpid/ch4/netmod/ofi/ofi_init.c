...
/*
 * include malloc.h for mallopt functions.
 * xopen_lim.h needed to prevent compile errors in CMA.
 */
#include <malloc.h>
...
cvars:
    ...
    - name        : MPIR_CVAR_MALLOC_ENABLE_MMAP
      category    : MVAPICH
      type        : int
      default     : 0
      class       : none
      ...
      scope       : MPI_T_SCOPE_LOCAL
      description : >-
        Set to 0 to disable the use of MMAP by the standard memory allocator.
        Doing so provides cache performance improvements for most cases. Setting
        to 1 will return to the system default behaviour.

    - name        : MPIR_CVAR_MALLOC_HEAP_STEP
      category    : MVAPICH
      type        : int
      ...
      scope       : MPI_T_SCOPE_LOCAL
      description : >-
        Sets the size by which the malloc program heap is expanded by the
        memory allocator when new memory is required by the program. A higher
        value will generally provide better cache performance, but will
        result in a higher memory footprint. The program heap exists on a
        per-processes basis. Setting this value to zero will use the system
        default.
...
int MPIDI_OFI_init_local(int *tag_bits)
{
    ...
    /*Setting FI_UNIVERSE_SIZE to the total number of MPI processes.  This hint
     * to libfabric optimizes small message latency for smaller process counts*/
    char fi_env_str_buf[10];
    snprintf(fi_env_str_buf, sizeof(fi_env_str_buf), "%d\n", MPIR_Process.size);
    setenv("FI_UNIVERSE_SIZE", fi_env_str_buf, 1);

    /* set malloc options to get better cache behaviour */
    if (!MPIR_CVAR_MALLOC_ENABLE_MMAP) {
        mallopt(M_MMAP_MAX, 0);
    }
    if (MPIR_CVAR_MALLOC_HEAP_STEP > 0) {
        mallopt(M_TOP_PAD, MPIR_CVAR_MALLOC_HEAP_STEP);
    }
