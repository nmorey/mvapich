...
#if 0
/* TODO: FIXME: Work on this */
int MPIDI_MVP_ReqHandler_UnpackSRBufComplete(MPIDI_MVP_ep_t * vc, MPIR_Request * rreq, int *complete)
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_MVP_REQHANDLER_UNPACKSRBUFCOMPLETE);

    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_MVP_REQHANDLER_UNPACKSRBUFCOMPLETE);

    MPIDI_MVP_Request_unpack_srbuf(rreq);

    if (MPIDI_Request_get_type(MPIDI_MVP_REQUEST_FROM_MPICH(rreq)) == MPIDI_REQUEST_TYPE_PUT_RECV) {
        mpi_errno = MPIDI_MVP_ReqHandler_PutRecvComplete(vc, rreq, complete);
    }
    else if (MPIDI_Request_get_type(MPIDI_MVP_REQUEST_FROM_MPICH(rreq)) == MPIDI_REQUEST_TYPE_ACCUM_RECV) {
        mpi_errno = MPIDI_MVP_ReqHandler_AccumRecvComplete(vc, rreq, complete);
    }
    else if (MPIDI_Request_get_type(MPIDI_MVP_REQUEST_FROM_MPICH(rreq)) == MPIDI_REQUEST_TYPE_GET_ACCUM_RECV) {
        mpi_errno = MPIDI_MVP_ReqHandler_GaccumRecvComplete(vc, rreq, complete);
    }
    else if (MPIDI_Request_get_type(MPIDI_MVP_REQUEST_FROM_MPICH(rreq)) == MPIDI_REQUEST_TYPE_FOP_RECV) {
        mpi_errno = MPIDI_MVP_ReqHandler_FOPRecvComplete(vc, rreq, complete);
    }
    else {
  ...
  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_MVP_REQHANDLER_UNPACKSRBUFCOMPLETE);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
}
...
int MPIDI_MVP_MRAIL_Prepare_rndv(MPIDI_MVP_ep_t *vc, MPIR_Request *req)
{
     ...
     * then that decision is respected */
        ...
        MPIDI_MVP_REQUEST(req, mrail).protocol = rdma_rndv_protocol;
    }
#endif
    ...
    if (
        ...
        if (IS_VC_SMP(vc)) {
            ...
            PRINT_DEBUG(DEBUG_RNDV_verbose>1,
                    ...
                    vc->pg_rank, MPIDI_MVP_REQUEST(req, mrail).rndv_buf_sz, MPIDI_MVP_REQUEST(req, mrail).rndv_buf);
            reg_entry =
                dreg_register(MPIDI_MVP_REQUEST(req, mrail).rndv_buf, MPIDI_MVP_REQUEST(req, mrail).rndv_buf_sz);
            if (NULL == reg_entry) {
                PRINT_DEBUG(DEBUG_RNDV_verbose>1,
...
#ifdef _ENABLE_UD_
                if (MVP_RNDV_PROTOCOL_UD_ZCOPY == MPIDI_MVP_REQUEST(req, mrail).protocol) {
                    MPIDI_MVP_REQUEST(req, mrail).num_hcas = rdma_num_hcas;
                }
#endif
                MPIDI_MVP_REQUEST(req, mrail).d_entry = reg_entry;
                PRINT_DEBUG(DEBUG_RNDV_verbose>1,
                        "registration success for buf: %p, d_entry: %p\n",
                        MPIDI_MVP_REQUEST(req, mrail).rndv_buf, MPIDI_MVP_REQUEST(req, mrail).d_entry);
            }
#endif
        }
