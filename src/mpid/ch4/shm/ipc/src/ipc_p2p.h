...
/* BEGIN MVAPICH split routines - used to RPUT/COOP completions */
int MPIDI_IPC_remote_complete(MPIR_Request *rreq, MPIDI_IPCI_type_t ipc_type);
#define MPIDI_IPC_local_complete(rreq, ipc_type)                               \
    (MPIDIG_REQUEST(rreq, req->target_cmpl_cb)(rreq))
/* END MVAPICH split routines */
...
MPL_STATIC_INLINE_PREFIX int MPIDI_IPCI_do_lmt_cts(
    MPIDI_IPCI_ipc_attr_t *ipc_attr, MPIR_Request *sreq_ptr, MPIR_Request *rreq)
{
    int mpi_errno = MPI_SUCCESS;

    MPIDI_IPC_cts_t am_hdr;
    int is_contig;

    int local_vci = MPIDIG_REQUEST(rreq, req->local_vci);
    int remote_vci = MPIDIG_REQUEST(rreq, req->remote_vci);

    MPIDI_Datatype_check_contig(MPIDIG_REQUEST(rreq, datatype), is_contig);
    ...
    CH4_CALL(am_send_hdr_reply(rreq->comm, rreq->status.MPI_SOURCE,
                               MPIDI_IPC_CTS, &am_hdr, sizeof(am_hdr),
...
fn_exit:
    ...
    return mpi_errno;
fn_fail:
    /* Need to send ack and complete the request so we don't block sender and
     * receiver */
                                                        ...
                                                        MPIR_Request * rreq)
{
    ...
    /* BEGIN: MVAPICH CMA RPUT/COOP */
    if (ipc_hdr->ipc_type == MPIDI_IPCI_TYPE__CMA &&
        !MPIR_CVAR_ENABLE_GPU &&
        HANDLE_IS_BUILTIN(MPIDIG_REQUEST(rreq, datatype)) &&
        ipc_hdr->is_contig &&
        ((MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL ==
            MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL_auto &&
            src_data_sz >= MPIR_CVAR_CH4_IPC_CMA_COOP_THRESHOLD) ||
         MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL ==
        ...
        mpi_errno =
            MPIDI_CMA_get_ipc_attr(MPIDIG_REQUEST(rreq, buffer),
                                   MPIDIG_REQUEST(rreq, count),
                                   MPIDIG_REQUEST(rreq, datatype), &ipc_attr);
        MPIR_ERR_CHECK(mpi_errno);

        mpi_errno = MPIDI_IPCI_do_lmt_cts(&ipc_attr, sreq_ptr, rreq);
        MPIR_ERR_CHECK(mpi_errno);
        ...
        if ((MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL == 
            ...
            MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL ==
                MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL_coop) {
            mpi_errno =
                MPIDI_CMA_copy_data_coop(ipc_hdr, rreq, src_data_sz, 1);
            MPIR_ERR_CHECK(mpi_errno);
            mpi_errno = MPIDI_IPC_remote_complete(rreq, ipc_hdr->ipc_type);
            MPIR_ERR_CHECK(mpi_errno);
        }
        goto fn_cts;
    }
...
MPL_STATIC_INLINE_PREFIX int MPIDI_IPCI_handle_lmt_send(MPIDI_IPC_hdr *ipc_hdr,
                                                        size_t data_sz,
                                                        MPIR_Request *sreq,
                                                        MPIR_Request *rreq_ptr)
{
    ...
    if ((MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL ==
        ...
        MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL ==
            MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL_coop) {
        mpi_errno = MPIDI_CMA_copy_data_coop(ipc_hdr, sreq, data_sz, 0);
    } else if (MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL ==
               MPIR_CVAR_CH4_IPC_RNDV_PROTOCOL_rput) {
        mpi_errno = MPIDI_CMA_copy_data_rput(ipc_hdr, sreq, data_sz);
    }
    return mpi_errno;
}
