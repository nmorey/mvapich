...
#define _COMMON_PRINT_(FMT, args...)                                           \
    do {                                                                       \
        fprintf(stderr, "[%s][%s] " FMT, get_output_prefix(), __func__,        \
                ##args);                                                       \
    } while (0)
...
#if defined(_GNU_SOURCE) && (!defined(__USE_XOPEN2K) || defined(__USE_GNU))
#define MVP_STRERROR_R(ERRCODE, buf, BUF_SIZE, str)                            \
    str = (char *)strerror_r(ERRCODE, buf, BUF_SIZE);
#else
#define MVP_STRERROR_R(ERRCODE, buf, BUF_SIZE, str)                            \
...
#define PRINT_ERROR_ERRNO(FMT, ERRCODE, args...)                               \
    do {                                                                       \
        ...
        MVP_STRERROR_R(ERRCODE, err_msg, MAX_ERR_MSG, str);                    \
...
#define CHECK_ERROR_ERRNO(COND, FMT, errno, args...)                           \
    do {                                                                       \
        if (!(COND)) {                                                         \
            PRINT_ERROR_ERRNO("At %s:%d: " FMT, errno, __FILE__, __LINE__,     \
                              ##args);                                         \
            abort();                                                           \
        }                                                                      \
    } while (0)
...
extern void mvp_print_mem_usage();
