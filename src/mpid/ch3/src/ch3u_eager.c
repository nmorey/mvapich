                                 ...
                                 struct iovec *hdr_iov, int n_hdr_iov)
{
    ...
    if (mv2_enable_device && sreq->dev.OnDataAvail ==
        ...
        iov[1].iov_base = (void *)sreq->dev.tmpbuf;
        iov[1].iov_len = sreq->dev.segment_size;
        iov_n = 1;
    ...
    if (vc->smp.local_nodes != -1
        ...
        *buflen = eager_pkt->data_sz;
        ...
        if (contig_avail >= data_len) {
            data_buf = (void *) 
                ((unsigned long)smp_device_region_recv[vc->smp.local_nodes] + cur_t);
            cur_t += data_len;
        }
        else {
                    ...
                    intptr_t *buflen, MPIR_Request **rreqp )
{
    ...
    if (vc->smp.local_nodes != -1 && eager_pkt->in_device_region == 1 &&
        ...
        *buflen += eager_pkt->data_sz;
        ...
        contig_avail = smp_device_region_size - cur_t;
        if (contig_avail >= data_len) {
            data = (void *) ((unsigned long)smp_device_region_recv[vc->smp.local_nodes] + cur_t);
            cur_t += data_len;
        }
        else {
...
#endif
    {
        data = (char *)pkt + sizeof(MPIDI_CH3_Pkt_t);
    }
