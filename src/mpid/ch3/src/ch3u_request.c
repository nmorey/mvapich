...
MPIR_Request MPID_Request_direct[MPIR_REQUEST_PREALLOC] __attribute__((__aligned__(64))) = {{0}};
MPIR_Object_alloc_t MPID_Request_mem = {
    0, 0, 0, 0, MPIR_REQUEST, sizeof(MPIR_Request), MPID_Request_direct,
   ...
   "post-upv: first=%" PRIdPTR ", last=%" PRIdPTR ", iov_n=%d, iov_offset=%lld",
    ...
    if (rreq->dev.iov_count == 0)
    {
        ...
        if (mv2_enable_device && is_device_buffer(rreq->dev.iov[0].iov_base)) {
            ...
            rreq->dev.iov[0].iov_base = rreq->dev.tmpbuf + 
                rreq->dev.tmpbuf_off;
            rreq->dev.iov[0].iov_len = rreq->dev.segment_size;
            rreq->dev.iov_offset = 0;
            rreq->dev.iov_count = 1;
    ...
    if (unpack_sz > 0)
    {
        if (dt_contig)
        {
            ...
            if (rreq->mrail.is_eager_vbuf_queued == 1) {
                MPIR_Assert(rreq->mrail.eager_unexp_size == unpack_sz);
        ...
        else
        {
            ...
            if (rreq->mrail.is_eager_vbuf_queued == 1) {
                ...
                while ((unpack_sz > 0) && (v != NULL)) { 
                    ...
                    MPIR_Typerep_unpack(v->unexp_data_buf, unpack_sz, 
 ...
 *
 * TODO: remove this function once we are sure it is no longer needed
 */
void MPID_Request_set_completed( MPIR_Request *req )
{
