 ...
 */
MPID_nem_netmod_funcs_t MPIDI_nem_ib_funcs = {MPID_nem_ib_init,
 ...
 *
    \todo Need to add more stuff here
          Look at InitPG in mvapich/trunk/src/mpid/ch3/src/mpid_init.c
 */
int MPID_nem_ib_pmi_init()
{
    ...
    /* -------------------------------------- From InitPG in
    mvapich/trunk/src/mpid/ch3/src/mpid_init.c pmi_errno =
    UPMI_GET_APPNUM(&appnum); if (pmi_errno != UPMI_SUCCESS) {
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPID_nem_ib_allocate_memory(int pg_rank, int pg_size)
{
    int mpi_errno = MPI_SUCCESS;
    process_info.polling_group_size = 0;

    if (rdma_polling_set_limit > 0) {
        process_info.polling_set = (MPIDI_VC_t **)MPL_malloc(
            rdma_polling_set_limit * sizeof(MPIDI_VC_t *));
    } else {
        process_info.polling_set =
            (MPIDI_VC_t **)MPL_malloc(pg_size * sizeof(MPIDI_VC_t *));
    }
...
int MPID_nem_ib_init(MPIDI_PG_t *pg_p, int pg_rank, char **bc_val_p,
                     int *val_max_sz_p)
{
    int mpi_errno = MPI_SUCCESS;
    char *value = NULL;
    ...
    if (mpi_errno != MPI_SUCCESS) {
        MPIR_ERR_SETFATALANDJUMP1(
            ...
            "Failed to get control params after hca_init");
    }
    ...
    if (mpi_errno != MPI_SUCCESS) {
        MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_INTERN, "**fail",
                                  "**fail %s",
                                  "Failed to init process_init_info");
    }
    ...
    if (!mvapich_minit()) {
        ...
        if (mpi_errno != MPI_SUCCESS) {
            MPIR_ERR_SETFATALANDJUMP1(
                mpi_errno, MPI_ERR_INTERN, "**fail", "**fail %s",
                "Failed to initialize registration cache");
        }
    } else {
    ...
    mpi_errno = MPID_nem_ib_allocate_memory(pg_rank, pg_p->size);
    if (mpi_errno != MPI_SUCCESS) {
        MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_INTERN, "**fail",
    ...
    if (process_info.has_srq) {
        ...
        if (mpi_errno != MPI_SUCCESS) {
            MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_INTERN, "**fail",
                                      "**fail %s",
                                      "Failed to allocate memory for srq");
        }
    }
    ...
    if ((value = getenv("MVP_SHOW_ENV_INFO")) != NULL) {
        mvp_show_env_info = atoi(value);
    }
...
static int ib_ckpt_restart(void)
{
    ...
    /* Now we can restart */
    mpi_errno = MPID_nem_ib_init(MPIDI_Process.my_pg, MPIDI_Process.my_pg_rank,
                                 &bc_val, &val_max_sz);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);

    /* publish business card */
    mpi_errno = MPIDI_PG_SetConnInfo(MPIDI_Process.my_pg_rank,
                                     (const char *)publish_bc_orig);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
