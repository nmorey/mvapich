...
int MPID_nem_ib_lmt_start_recv(struct MPIDI_VC *VC, struct MPIR_Request *rreq,
                               struct iovec s_cookie);
int MPIDI_CH3U_Post_data_receive_found_temp(MPIR_Request *rreq);
...
#define MPIDI_CH3I_MRAIL_SET_COOKIE_RNDV(_cookie_buf, _req)                    \
    {                                                                          \
        ...
        if ((MVP_LMT_PROTOCOL_RPUT == (_cookie_buf)->protocol) ||              \
            (MVP_LMT_PROTOCOL_RGET == (_cookie_buf)->protocol)) {              \
...
#define MPIDI_CH3I_MRAIL_REVERT_RPUT(_sreq)                                    \
    {                                                                          \
        if (MVP_LMT_PROTOCOL_RGET == REQ_FIELD(_sreq, protocol))               \
            REQ_FIELD(_sreq, protocol) = MVP_LMT_PROTOCOL_RPUT;                \
    }
                               ...
                               struct iovec s_cookie)
{
    ...
    r_cookie_buf = MPL_malloc(sizeof(MPID_nem_ib_lmt_cookie));
    rndv_info = (MPID_nem_ib_lmt_cookie *)s_cookie.iov_base;
    ...
    if (rreq->dev.recv_data_sz == 0) {
        ...
        if (mpi_errno != MPI_SUCCESS && rreq != NULL) {
            MPIR_ERR_SETANDJUMP(mpi_errno, MPI_ERR_OTHER, "**nem|loadsendiov");
        }
    }
    ...
    if (MPIDI_NEM_RNDV_PROTOCOL_IS_READ(rreq)) {
        /*         mpi_errno = MPIDI_CH3_Prepare_rndv_get(vc, rreq);
                 ...
                 mpi_errno = MPIDI_CH3_Rndv_transfer(vc,
                          NULL, rreq, NULL, rts_pkt); //rts_pkt needs to be
           modified if (mpi_errno != MPI_SUCCESS && rreq != NULL) {
...
int MPIDI_CH3U_Post_data_receive_found_temp(MPIR_Request *rreq)
{
    int dt_contig;
    MPI_Aint dt_true_lb;
    intptr_t userbuf_sz ATTRIBUTE((unused));
    ...
    MPIDI_Datatype_get_info(rreq->dev.user_count, rreq->dev.datatype, dt_contig,
    ...
    /*
        if (rreq->dev.recv_data_sz <= userbuf_sz) {
            ...
            MPL_DBG_MSG_FMT(CH3_OTHER,VERBOSE,(MPL_DBG_FDEST,
                   "receive buffer too small; message truncated, msg_sz="
       MPIDI_MSG_SZ_FMT ", userbuf_sz=" MPIDI_MSG_SZ_FMT,
    ...
    */
    ...
    if (dt_contig && data_sz == rreq->dev.recv_data_sz) {
        ...
        MPL_DBG_MSG(CH3_OTHER, VERBOSE, "IOV loaded for contiguous read");
        ...
        /* user buffer is not contiguous or is too small to hold
           the entire message */
        MPL_DBG_MSG(CH3_OTHER, VERBOSE, "IOV loaded for non-contiguous read");
...
int MPIDI_CH3I_MRAIL_Prepare_rndv(MPIDI_VC_t *vc, MPIR_Request *req)
{
    ...
    if (MVP_LMT_PROTOCOL_RPUT == MVP_RNDV_PROTOCOL) {
        REQ_FIELD(req, protocol) = MVP_LMT_PROTOCOL_RPUT;
    } else if (MVP_LMT_PROTOCOL_RGET == MVP_RNDV_PROTOCOL) {
        REQ_FIELD(req, protocol) = MVP_LMT_PROTOCOL_RGET;
    } else {
        REQ_FIELD(req, protocol) = MVP_LMT_PROTOCOL_R3;
    }
    ...
    if ((MVP_LMT_PROTOCOL_RPUT == REQ_FIELD(req, protocol)) ||
        (MVP_LMT_PROTOCOL_RGET == REQ_FIELD(req, protocol))) {
        DEBUG_PRINT("[cts] size registered %d, addr %p\n",
                    REQ_FIELD(req, rndv_buf_sz), REQ_FIELD(req, rndv_buf));
        d_entry = dreg_register(REQ_FIELD(req, rndv_buf),
                                REQ_FIELD(req, rndv_buf_sz));

        if (NULL == d_entry) {
            REQ_FIELD(req, protocol) = MVP_LMT_PROTOCOL_R3;
            if (1 == REQ_FIELD(req, rndv_buf_alloc)) {
                MPL_free(REQ_FIELD(req, rndv_buf));
                REQ_FIELD(req, rndv_buf_alloc) = 0;
                REQ_FIELD(req, rndv_buf_sz) = 0;
...
int MPIDI_nem_ib_Finish_request(MPIR_Request *rreq)
{
    REQ_FIELD(rreq, completion_counter)++;

    if (REQ_FIELD(rreq, protocol) == MVP_LMT_PROTOCOL_RGET) {
        return 1;
    }
...
int MPID_nem_ib_lmt_done_recv(struct MPIDI_VC *VC, struct MPIR_Request *rreq)
{
    ...
    if (REQ_FIELD(rreq, rndv_buf_alloc) == 1) {
        ...
        while (rreq->dev.OnDataAvail ==
            ...
            /*** added ***/
            if (TRUE == complete &&
                MVP_LMT_PROTOCOL_R3 == REQ_FIELD(rreq, protocol)) {
    ...
    MPIDI_NEM_IB_RREQ_RNDV_FINISH(rreq);
    if ((mpi_errno = MPIDI_CH3U_Handle_recv_req(VC, rreq, &complete)) !=
        MPI_SUCCESS) {
        mpi_errno =
...
int MPIDI_nem_ib_lmt_r3_recv_data(MPIDI_VC_t *vc, vbuf *buffer)
{
    ...
    if (!(MVP_LMT_PROTOCOL_R3 == REQ_FIELD(rreq, protocol) ||
          MVP_LMT_PROTOCOL_RPUT == REQ_FIELD(rreq, protocol))) {
        int rank;
        UPMI_GET_RANK(&rank);

        DEBUG_PRINT("[rank %d]get wrong req protocol, req %p, protocol %d\n",
                    rank, rreq, REQ_FIELD(rreq, protocol));
        MPIR_Assert(MVP_LMT_PROTOCOL_R3 == REQ_FIELD(rreq, protocol) ||
                    MVP_LMT_PROTOCOL_RPUT == REQ_FIELD(rreq, protocol));
    }

    REQ_FIELD(rreq, protocol) = MVP_LMT_PROTOCOL_R3;
    ...
    if (MPIDI_nem_ib_request_adjust_iov(rreq, nb)) {
        ...
        if (TRUE == complete) {
            REQ_FIELD(rreq, protocol) = MVP_LMT_PROTOCOL_RENDEZVOUS_UNSPECIFIED;
        }
    }
fn_exit:
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
void MPIDI_nem_ib_lmt_r3_recv_ack(MPIDI_VC_t *vc, void *vstart)
{
    ...
    MPIR_Assert(VC_FIELD(vc, pending_r3_data) == 0);
