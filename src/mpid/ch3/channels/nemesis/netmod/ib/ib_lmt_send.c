                                 ...
                                 struct MPIR_Request *req)
{
    ...
    MPIDI_Datatype_get_info(req->dev.user_count, req->dev.datatype, dt_contig,
    ...
    if (dt_contig) {
        ...
        req->dev.iov[0].iov_base =
            (void *)((char *)req->dev.user_buf + dt_true_lb);
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
void vbuf_init_rndv_rput(vbuf *v, void *local_address, uint32_t lkey,
                         void *remote_addr, uint32_t rkey, intptr_t len,
                         int rail)
{
...
int MPIDI_nem_ib_rput(struct MPIDI_VC *vc, struct MPIR_Request *sreq)
{
            ...
            if (sreq->dev.OnDataAvail == MPIDI_CH3_ReqHandler_SendReloadIOV) {
                ...
                if (mpi_errno != MPI_SUCCESS) {
                    mpi_errno = MPIR_Err_create_code(
                        mpi_errno, MPIR_ERR_FATAL, FCNAME, __LINE__,
                        MPI_ERR_OTHER, "**ch3|loadsendiov", 0);
                    goto fn_exit;
                }
            } else {
                complete = 1;
    ...
    while (REQ_FIELD(sreq, rndv_buf_off) < REQ_FIELD(sreq, rndv_buf_sz)) {
        ...
        vbuf_init_rndv_rput(
            v,
            (void *)REQ_FIELD(sreq, rndv_buf) + REQ_FIELD(sreq, rndv_buf_off),
            (REQ_FIELD(sreq, d_entry)->memhandle[hca_num])->lkey,
            REQ_FIELD(sreq, remote_addr) + REQ_FIELD(sreq, rndv_buf_off),
            REQ_FIELD(sreq, rkey[hca_num]), nbytes, rail);
        ...
        if (!VC_FIELD(vc, connection)->rails[rail].send_wqes_avail ||
            ...
            if (REQ_FIELD(sreq, rndv_buf_off) < REQ_FIELD(sreq, rndv_buf_sz)) {
    ...
    do {
        do {
            ...
            if (process_info.has_srq) {
                while (VC_FIELD(vc, pending_r3_data) >=
                               ...
                               struct iovec r_cookie)
{
    /*all variable declarations must be done before state declaration*/
    int mpi_errno = MPI_SUCCESS;
    MPID_nem_ib_lmt_cookie *cookie =
        (MPID_nem_ib_lmt_cookie *)r_cookie.iov_base;
    ...
    if (cookie->protocol == MVP_LMT_PROTOCOL_R3) {
        ...
        REQ_FIELD(sreq, protocol) = MVP_LMT_PROTOCOL_R3;
    }
    ...
    switch (REQ_FIELD(sreq, protocol)) {
        ...
        case MVP_LMT_PROTOCOL_RGET:
            ...
            break;
        default:
            mpi_errno = MPIR_Err_create_code(0, MPIR_ERR_FATAL, FCNAME,
...
void MPID_nem_lmt_ib_process_rndv()
{
    ...
    while (flowlist) {
        ...
        while (sreq != NULL) {
            MPIR_Assert(MVP_LMT_PROTOCOL_RPUT == REQ_FIELD(sreq, protocol));
            mpi_errno = MPIDI_nem_ib_rput(flowlist, sreq);
            if (mpi_errno == MPI_SUCCESS) {
                MPID_nem_lmt_send_DONE(flowlist, sreq);
                RENDEZVOUS_DONE(flowlist);
                sreq =
                    (MPIR_Request *)(VC_FIELD(flowlist, connection)->sreq_head);
            } else {
