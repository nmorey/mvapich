...
static inline vbuf *MPIDI_CH3I_RDMA_poll(MPIDI_VC_t *vc)
{
    ...
    v = &(VC_FIELD(vc, connection)
              ->rfp.RDMA_recv_buf[VC_FIELD(vc, connection)->rfp.p_RDMA_recv]);
    head = v->head_flag;

    if (*head && VC_FIELD(vc, connection)->rfp.p_RDMA_recv !=
                     VC_FIELD(vc, connection)->rfp.p_RDMA_recv_tail) {
        ...
        DEBUG_PRINT("[recv: poll rdma] recv %d, tail %d, size %d\n",
                    vc->pg_rank, VC_FIELD(vc, connection)->rfp.p_RDMA_recv,
                    VC_FIELD(vc, connection)->rfp.p_RDMA_recv_tail, *head);
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIDI_nem_ib_handle_read_individual(MPIDI_VC_t *vc, vbuf *buffer,
                                        int *header_type)
{
    int mpi_errno = MPI_SUCCESS;
    int header_size = 0;
    ...
    mpi_errno =
        MPIDI_nem_ib_parse_header(vc, buffer, (void *)(&header), &header_size);
    if (mpi_errno) {
        MPIR_ERR_POP(mpi_errno);
    }
    ...
    *header_type = header->type;
    ...
    switch (((MPIDI_nem_ib_pkt_comm_header *)buffer->iheader)->type) {
        case MPIDI_NEM_PKT_LMT_RTS:
            ...
            mpi_errno =
                pktArray[header->type](vc, buffer->pheader, &(lmt_len), &req);
            goto fn_exit;
    ...
    if ((mpi_errno = MPIDI_CH3U_Handle_recv_pkt(vc, (void *)header, &buflen,
    ...
    if (VC_FIELD(vc, recv_active) != NULL) {
        ...
        if (packetized_recv == 1) {
            if ((mpi_errno = MPIDI_nem_ib_packetized_recv_req(
                     vc, VC_FIELD(vc, recv_active))) != MPI_SUCCESS) {
                MPIR_ERR_POP(mpi_errno);
            }
        }
        ...
        if ((mpi_errno = MPIDI_nem_ib_fill_request(VC_FIELD(vc, recv_active),
                                                   buffer, header_size, &nb)) !=
            MPI_SUCCESS) {
            MPIR_ERR_POP(mpi_errno);
        }
        ...
        DEBUG_PRINT(
            ...
            req->dev.iov_count, req->dev.iov[0].iov_len, VBUF_BUFFER_SIZE);
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIDI_nem_ib_read_progress(MPIDI_VC_t **vc_pptr, vbuf **v_ptr,
                               int is_blocking)
{
    ...
    if (pending_vc != NULL) {
        type = MPIDI_nem_ib_waiting_msg(pending_vc, v_ptr, is_blocking);
        ...
        *vc_pptr = recv_vc_ptr;
        switch (type) {
            ...
            case (T_CHANNEL_OUT_OF_ORDER_ARRIVE):
                ...
                /* DEBUG_PRINT("get out of order progress seqnum %d, expect
                   %d\n",
                            (*v_ptr)->seqnumber, recv_vc_ptr->seqnum_recv); */

                type = MPIDI_nem_ib_waiting_msg(recv_vc_ptr, v_ptr, 1);
                if (type == T_CHANNEL_CONTROL_MSG_ARRIVE) {
...
/* coalesce codes */
    ...
    if ((mpi_errno = MPIDI_nem_ib_handle_read_individual(
             vc, buffer, &header_type)) != MPI_SUCCESS) {
        MPIR_ERR_POP(mpi_errno);
    }
#endif /* defined(MPIDI_MRAILI_COALESCE_ENABLED) */
...
int MPID_nem_ib_poll(int in_blocking_poll)
{
    ...
    if ((mpi_errno = MPIDI_nem_ib_read_progress(
             &vc_ptr, &buffer, in_blocking_poll)) != MPI_SUCCESS) {
        MPIR_ERR_POP(mpi_errno);
    }
    ...
    if (vc_ptr == NULL) {
        ...
        if ((mpi_errno = MPIDI_nem_ib_handle_read(vc_ptr, buffer)) !=
            MPI_SUCCESS) {
            MPIR_ERR_POP(mpi_errno);
        }
    }
...
{
    return ((MPIDI_nem_ib_pkt_comm_header *)((vbuf *)v)->iheader)->type ==
           MPIDI_CH3_PKT_NOOP;
}
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIDI_nem_ib_cq_poll(vbuf **vbuf_handle, MPIDI_VC_t *vc_req, int receiving,
                         int is_blocking)
{
    int ne, ret, err = 0;
    *vbuf_handle = NULL;
        ...
        for (cq_choice = 0; cq_choice < num_cqs; ++cq_choice) {
            ...
            if (ne < 0) {
                ...
                if (!is_send_completion && process_info.has_srq) {
                    vc = (void *)(unsigned long)(((MPIDI_nem_ib_pkt_comm_header
                                                       *)((vbuf *)v)
                                                      ->iheader)
                                                     ->vc_addr);
                    v->vc = vc;
                    v->rail =
                        ((MPIDI_nem_ib_pkt_comm_header *)((vbuf *)v)->iheader)
                            ->rail;
                }
                    ...
                    *vbuf_handle = v;
                    ...
                    if (process_info.has_srq) {
                        ...
                        if (srq_info.posted_bufs[i] <= rdma_credit_preserve) {
                            ...
                            srq_info.posted_bufs[i] +=
                                MPIDI_nem_ib_post_srq_buffers(
                                    mvp_srq_fill_size - srq_info.posted_bufs[i],
                                    i);
                        }
                        ...
                        if (srq_info.srq_zero_post_counter[i] >= 1) {
                            pthread_mutex_lock(
                                &srq_info.srq_post_mutex_lock[i]);
                    ...
                    DEBUG_PRINT(
                        "[cq_poll]: get seqnum = %d, expect = %d, seqnum in ib "
                        "= %d, "
                        " v->content_size = %d, type = %d\n",
                        seqnum, VC_FIELD(vc, seqnum_recv),
                        ((MPIDI_nem_ib_pkt_comm_header *)v->iheader)->seqnum,
                        v->content_size,
                    ...
                    if (!process_info.has_srq) {
                        if (PKT_IS_NOOP(v)) {
                            ...
                            --VC_FIELD(vc, connection)
                                  ->srp.credits[v->rail]
                                  .local_credit;
                        } else if (VC_FIELD(vc, connection)
                                           ->srp.credits[v->rail]
                                           .preposts < rdma_rq_size &&
                                   VC_FIELD(vc, connection)
                                               ->srp.credits[v->rail]
                                               .preposts +
                ...
                *vbuf_handle = NULL;
                ...
                if (rdma_use_blocking && is_blocking &&
...
#endif
                    do {
                        ret = ibv_get_cq_event(hca_list[i].comp_channel, &ev_cq,
                                               &ev_ctx);
                        if (ret && errno != EINTR) {
                    ...
                    if (num_cqs == 1) {
                        ...
                        if (ibv_req_notify_cq(hca_list[i].cq_hndl, 0)) {
                            ibv_error_abort(
                                IBV_RETURN_ERR,
                        ...
                        if (ev_cq == hca_list[i].send_cq_hndl) {
                            ibv_ack_cq_events(hca_list[i].send_cq_hndl, 1);

                            if (ibv_req_notify_cq(hca_list[i].send_cq_hndl,
                                                  0)) {
                                ibv_error_abort(
                                    IBV_RETURN_ERR,
                                    "Couldn't request for CQ notification\n");
                            }
                        } else if (ev_cq == hca_list[i].recv_cq_hndl) {
                            ibv_ack_cq_events(hca_list[i].recv_cq_hndl, 1);

                            if (ibv_req_notify_cq(hca_list[i].recv_cq_hndl,
                                                  0)) {
                                ibv_error_abort(
                                    IBV_RETURN_ERR,
                                    "Couldn't request for CQ notification\n");
...
int MPIDI_nem_ib_get_next_vbuf(MPIDI_VC_t **vc_ptr, vbuf **vbuf_ptr)
{
    ...
    *vbuf_ptr = NULL;
    ...
    for (i = 0; i < process_info.polling_group_size; ++i) {
        ...
        seq = GetSeqNumVbuf(
            VC_FIELD(vc, cmanager)
                ->msg_channels[INDEX_LOCAL(VC_FIELD(vc, cmanager), 0)]
                .v_queue_head);
        ...
        if (seq == PKT_IS_NULL) {
            v = &(
                VC_FIELD(vc, connection)
                    ->rfp
                    .RDMA_recv_buf[VC_FIELD(vc, connection)->rfp.p_RDMA_recv]);
            head = v->head_flag;
            ...
            if (*head && VC_FIELD(vc, connection)->rfp.p_RDMA_recv !=
                ...
                ** poll next connection */
                ...
                if (++VC_FIELD(vc, connection)->rfp.p_RDMA_recv >=
                    num_rdma_buffer) {
                    VC_FIELD(vc, connection)->rfp.p_RDMA_recv = 0;
                }
                ...
                *head = 0;
                ...
                seq = GetSeqNumVbuf(v);
                DEBUG_PRINT(
                    "[get rfp packet] seq = %d, expect %d, seqnum in ib = %d, "
                    "v->iheader->type = %d\n",
                    seq, VC_FIELD(vc, seqnum_recv),
                    ((MPIDI_nem_ib_pkt_comm_header *)v->iheader)->seqnum,
                    ((MPIDI_nem_ib_pkt_comm_header *)v->iheader)->type);
                ...
                if (seq == VC_FIELD(vc, seqnum_recv)) {
                    ...
                    *vc_ptr = v->vc;
                    ...
                    VQUEUE_ENQUEUE(VC_FIELD(vc, cmanager),
                                   INDEX_LOCAL(VC_FIELD(vc, cmanager), 0), v);
                    continue;
                }
            } else {
                continue;
        ...
        if (seq == VC_FIELD(vc, seqnum_recv)) {
            *vbuf_ptr = VQUEUE_DEQUEUE(VC_FIELD(vc, cmanager),
                                       INDEX_LOCAL(VC_FIELD(vc, cmanager), 0));
            *vc_ptr = (*vbuf_ptr)->vc;
            VC_FIELD(vc, seqnum_recv)++;
            type = T_CHANNEL_EXACT_ARRIVE;
            goto fn_exit;
...
int MPIDI_nem_ib_waiting_msg(MPIDI_VC_t *vc, vbuf **vbuf_handle, int blocking)
{
    ...
    *vbuf_handle = NULL;
    ...
    if (blocking) {
        DEBUG_PRINT("{entering} solve_out_of_order next expected %d, channel "
                    "%d, head %p (%d)\n",
