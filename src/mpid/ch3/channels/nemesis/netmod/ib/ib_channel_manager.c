...
int MPIDI_nem_ib_init_cmanager(int rank, int size)
{
    ...
    cmanagers = (MPID_nem_ib_channel_manager *)MPL_malloc(
        size * sizeof(MPID_nem_ib_channel_manager));
    ...
    for (i = 0; i < size; i++) {
        ...
        cmanagers[i].msg_channels =
            MPL_malloc(sizeof *cmanagers[i].msg_channels *
                       (cmanagers[i].num_channels + 1));
        if (!cmanagers[i].msg_channels) {
            ibv_error_abort(GEN_EXIT_ERR, "No resource for msg channels\n");
        }

        memset(cmanagers[i].msg_channels, 0,
               sizeof *cmanagers[i].msg_channels *
                   (cmanagers[i].num_channels + 1));
