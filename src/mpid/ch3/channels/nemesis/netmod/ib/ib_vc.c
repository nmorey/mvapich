...
/*const char *business_card)*/
{
    ...
    */
    ...
    VC_FIELD(vc, connection)->rfp.cached_incoming =
        MPL_malloc(sizeof(MPIDI_CH3_Pkt_send_t));
    VC_FIELD(vc, connection)->rfp.cached_outgoing =
        MPL_malloc(sizeof(MPIDI_CH3_Pkt_send_t));
    VC_FIELD(vc, connection)->rfp.cached_incoming_iheader =
        MPL_malloc(sizeof(MPIDI_nem_ib_pkt_comm_header));
    VC_FIELD(vc, connection)->rfp.cached_outgoing_iheader =
        MPL_malloc(sizeof(MPIDI_nem_ib_pkt_comm_header));
    memset(VC_FIELD(vc, connection)->rfp.cached_outgoing, 0,
           sizeof(MPIDI_CH3_Pkt_send_t));
    memset(VC_FIELD(vc, connection)->rfp.cached_incoming, 0,
           sizeof(MPIDI_CH3_Pkt_send_t));
    memset(VC_FIELD(vc, connection)->rfp.cached_outgoing_iheader, 0,
           sizeof(MPIDI_nem_ib_pkt_comm_header));
    memset(VC_FIELD(vc, connection)->rfp.cached_incoming_iheader, 0,
           sizeof(MPIDI_nem_ib_pkt_comm_header));
#endif

    if (pg_size < rdma_eager_limit)
        VC_FIELD(vc, connection)->rfp.eager_start_cnt =
            rdma_polling_set_threshold + 1;
    else
    ...
    for (i = 0; i < rdma_num_rails; i++) {
        ...
        VC_FIELD(vc, connection)->rails[i].send_wqes_avail =
        ...
        VC_FIELD(vc, connection)->srp.credits[i].remote_credit =
            rdma_initial_credits;
        VC_FIELD(vc, connection)->srp.credits[i].remote_cc =
            rdma_initial_credits;
        VC_FIELD(vc, connection)->srp.credits[i].local_credit = 0;
        VC_FIELD(vc, connection)->srp.credits[i].preposts =
            rdma_initial_prepost_depth;

        if (!process_info.has_srq) {
            VC_FIELD(vc, connection)->srp.credits[i].initialized =
                (uint8_t)(rdma_prepost_depth == rdma_initial_prepost_depth);
        } else {
        ...
        VC_FIELD(vc, connection)->srp.credits[i].rendezvous_packets_expected =
            0;
