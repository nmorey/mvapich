    ...
    while (1) {
        ...
        switch (event.event_type) {
            ...
            case IBV_EVENT_SRQ_LIMIT_REACHED:
                ...
                post_new = srq_info.posted_bufs[hca_num] - post_new;

                pthread_spin_unlock(&srq_info.srq_post_spin_lock);

                if (!post_new) {
                    pthread_mutex_lock(&srq_info.srq_post_mutex_lock[hca_num]);
                    ...
                    while (srq_info.srq_zero_post_counter[hca_num] >= 1) {
                        ...
                        pthread_cond_wait(
                            &srq_info.srq_post_cond[hca_num],
                            &srq_info.srq_post_mutex_lock[hca_num]);
                    }
                    pthread_mutex_unlock(
                        &srq_info.srq_post_mutex_lock[hca_num]);
                } else {
                    /* Was able to post some, so erase old counter */
                    if (srq_info.srq_zero_post_counter[hca_num]) {
                ...
                if (ibv_modify_srq(hca_list[hca_num].srq_hndl, &srq_attr,
                                   IBV_SRQ_LIMIT)) {
                    ibv_va_error_abort(
                        GEN_EXIT_ERR,
                        "Couldn't modify SRQ limit (%u) after posting %d\n",
                        mvp_srq_limit, post_new);
                }
                ...
                break;
            default:
                fprintf(stderr, "Got unknown event %d ... continuing ...\n",
                        event.event_type);
        }
#ifdef _ENABLE_XRC_
        if (xrc_event) {
...
int MPID_nem_ib_allocate_srq()
{
    ...
    for (; hca_num < ib_hca_num_hcas; ++hca_num) {
        ...
        srq_info.posted_bufs[hca_num] =
            MPIDI_nem_ib_post_srq_buffers(mvp_srq_fill_size, hca_num);

        {
            ...
            pthread_create(&srq_info.async_thread[hca_num], NULL,
                           (void *)async_thread,
...
int MPIDI_nem_ib_post_srq_buffers(int num_bufs, int hca_num)
{
    int i = 0;
    vbuf *v = NULL;
    ...
    for (; i < num_bufs; ++i) {
        ...
        if (ibv_post_srq_recv(hca_list[hca_num].srq_hndl, &v->desc.u.rr,
                              &bad_wr)) {
