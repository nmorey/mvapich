                       ...
                       struct ibv_device_attr *dev_attr)
{
    ...
    if (port_attr->active_mtu < rdma_default_mtu) {
        MPL_error_printf(
            "Active MTU is %d, MVP_DEFAULT_MTU set to %d. See User Guide\n",
            port_attr->active_mtu, rdma_default_mtu);
        ret = 1;
    }

    if (dev_attr->max_qp_rd_atom < rdma_default_qp_ous_rd_atom) {
        MPL_error_printf("Max MVP_DEFAULT_QP_OUS_RD_ATOM is %d, set to %d\n",
    ...
    if (process_info.has_srq) {
        if (dev_attr->max_srq_sge < rdma_default_max_sg_list) {
            MPL_error_printf("Max MVP_DEFAULT_MAX_SG_LIST is %d, set to %d\n",
                             dev_attr->max_srq_sge, rdma_default_max_sg_list);
            ret = 1;
        }

        if (dev_attr->max_srq_wr < mvp_srq_alloc_size) {
            MPL_error_printf("Max MVP_SRQ_SIZE is %d, set to %d\n",
                             dev_attr->max_srq_wr, (int)mvp_srq_alloc_size);
            ret = 1;
        }
    } else {
        ...
        if (dev_attr->max_qp_wr < rdma_default_max_send_wqe) {
            MPL_error_printf("Max MVP_DEFAULT_MAX_SEND_WQE is %d, set to %d\n",
                             dev_attr->max_qp_wr,
                             (int)rdma_default_max_send_wqe);
            ret = 1;
        }
    }
    if (dev_attr->max_cqe < rdma_default_max_cq_size) {
        MPL_error_printf("Max MVP_DEFAULT_MAX_CQ_SIZE is %d, set to %d\n",
 ...
 *      Failure:    ERROR (-1).
 */
static int rdma_find_active_port(struct ibv_context *context,
    ...
    for (j = 1; j <= RDMA_DEFAULT_MAX_PORTS; ++j) {
        if ((!ibv_query_port(context, j, &port_attr)) &&
            ...
            if (!strncmp(dev_name, "cxgb3", 5) ||
                !strncmp(dev_name, "cxgb4", 5) || port_attr.lid) {
...
static char *port_state_str[] = {"???", "Down", "Initializing", "Armed",
                                 "Active"};

static char *port_phy_state_str[] = {
    "No state change",           "Sleep",  "Polling",           "Disabled",
    "PortConfigurationTraining", "LinkUp", "LinkErrorRecovery", "PhyTest"};
#endif
 ...
 * mvapich/trunk/src/mpid/ch3/channels/mrail/src/gen2/rdma_iba_priv.c
 *
 * Store all the HCA info in mvp_nem_dev_info_t->hca[hca_num]
 *
 * Output:
...
int MPID_nem_ib_init_hca()
{
    ...
    if (num_devices > MAX_NUM_HCAS) {
        MPL_error_printf(
            "WARNING: found %d IB devices, the maximum is %d (MAX_NUM_HCAS). ",
            num_devices, MAX_NUM_HCAS);
        num_devices = MAX_NUM_HCAS;
    }

    if (ib_hca_num_hcas > num_devices) {
        MPL_error_printf("WARNING: user requested %d IB devices, the available "
                         "number is %d. ",
    ...
    for (nHca = 0; nHca < ib_hca_num_hcas; nHca++) {
        ...
        if ((rdma_iba_hca[0] == 0) ||
            (strncmp(rdma_iba_hca, RDMA_IBA_NULL_HCA, 32) == 0) ||
            (ib_hca_num_hcas > 1)) {
            /* User hasn't specified any HCA name, or the number of HCAs is
             * greater then 1 */
            ...
            while (dev_list[dev_count]) {
                if (!strncmp(ibv_get_device_name(dev_list[dev_count]),
                             rdma_iba_hca, 32)) {
        ...
        if (process_info.use_iwarp_mode) {
            if ((mpi_errno = rdma_cm_get_hca_type(process_info.use_iwarp_mode,
                                                  &process_info.hca_type)) !=
                MPI_SUCCESS) {
        ...
        {
            process_info.hca_type = hca_list[nHca].hca_type =
                mvp_get_hca_type(hca_list[nHca].ib_dev);
            process_info.arch_hca_type =
                mvp_get_arch_hca_type(hca_list[nHca].ib_dev);
        }
    }

    if (!strncmp(rdma_iba_hca, RDMA_IBA_NULL_HCA, 32) &&
        (ib_hca_num_hcas == 1) && (num_devices > nHca) &&
        (rdma_find_active_port(hca_list[0].nic_context,
                               hca_list[nHca].ib_dev) == -1)) {
         ...
         * (rdma_num_hcas), and the first device does not have an active port.
         * So try to find some other device with an active port.
         */
        int j;
        for (j = 0; dev_list[j]; j++) {
            ...
            if (ib_dev) {
                ...
                if (rdma_find_active_port(hca_list[0].nic_context, ib_dev) !=
                    -1) {
                    hca_list[0].ib_dev = ib_dev;
                    hca_list[0].ptag = ibv_alloc_pd(hca_list[0].nic_context);
                    if (!hca_list[0].ptag) {
                        MPIR_ERR_SETFATALANDJUMP2(
                            mpi_errno, MPI_ERR_OTHER, "**fail", "%s%d",
                            "Failed to alloc pd number ", nHca);
                    }
                }
            }
        }
    ...
    for (nHca = 0; nHca < ib_hca_num_hcas; nHca++) {
        ...
        if (rdma_default_port < 0 || ib_hca_num_ports > 1) {
            ...
            for (nPort = 1; nPort <= RDMA_DEFAULT_MAX_PORTS; nPort++) {
                if ((!ibv_query_port(hca_list[nHca].nic_context, nPort,
                                     &port_attr)) &&
                    port_attr.state == IBV_PORT_ACTIVE &&
                    (port_attr.lid || (!port_attr.lid && use_iboeth))) {
                    if (use_iboeth) {
                        if (ibv_query_gid(hca_list[nHca].nic_context, nPort, 0,
                                          &hca_list[nHca].gids[k])) {
                        ...
                        DEBUG_PRINT("[%d] %s(%d): Getting gid[%d][%d] for"
                                    ...
                                    " intf_id = %llx\r\n",
                                    process_info.rank, __FUNCTION__, __LINE__,
                                    nHca, k, k,
            ...
            if (k < ib_hca_num_ports) {
                MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_OTHER,
                                          "**activeports", "**activeports %d",
                                          ib_hca_num_ports);
            }
        } else {
            if (ibv_query_port(hca_list[nHca].nic_context, rdma_default_port,
                               &port_attr) ||
            ...
            if (use_iboeth) {
                if (ibv_query_gid(hca_list[nHca].nic_context, 0, 0,
                                  &hca_list[nHca].gids[0])) {
                    /* new error function needed */
                    MPIR_ERR_SETFATALANDJUMP1(
                        mpi_errno, MPI_ERR_OTHER, "**fail",
                        "Failed to retrieve gid on rank %d", process_info.rank);
                }
        ...
        if (rdma_use_blocking) {
            hca_list[nHca].comp_channel =
                ibv_create_comp_channel(hca_list[nHca].nic_context);
            ...
            hca_list[nHca].cq_hndl = ibv_create_cq(
                hca_list[nHca].nic_context, rdma_default_max_cq_size, NULL,
                hca_list[nHca].comp_channel, 0);
            ...
            hca_list[nHca].cq_hndl =
                ibv_create_cq(hca_list[nHca].nic_context,
        ...
        /* to decouple process_info, may need to store has_srq to hca
         * structure??? */
        if (process_info.has_srq) {
