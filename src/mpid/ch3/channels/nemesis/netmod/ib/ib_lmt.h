...
typedef enum {
    MVP_LMT_PROTOCOL_RENDEZVOUS_UNSPECIFIED = 0,
    MVP_LMT_PROTOCOL_EAGER,
    MVP_LMT_PROTOCOL_R3,
    MVP_LMT_PROTOCOL_RPUT,
    MVP_LMT_PROTOCOL_RGET,
...
typedef struct {
    ...
    /* rndv (MPIDI_CH3I_MRAILI_RNDV_INFO_DECL) in original rts,cts pkt, now used
     * as cookie */
...
#define REQ_FIELD(req, field)                                                  \
    (((MPID_nem_ib_rndv_info *)(&(req)->ch.netmod_area))->field)
...
#define MPIDI_NEM_RNDV_PROTOCOL_IS_READ(rreq)                                  \
    (MVP_LMT_PROTOCOL_RGET == REQ_FIELD(rreq, protocol))
    ...
    {                                                                          \
        if (rreq != NULL) {                                                    \
            ...
            if (1 == REQ_FIELD(rreq, rndv_buf_alloc) &&                        \
                ...
                REQ_FIELD(rreq, rndv_buf) = NULL;                              \
                REQ_FIELD(rreq, rndv_buf_off) = REQ_FIELD(rreq, rndv_buf_sz) = \
                    0;                                                         \
            ...
            REQ_FIELD(rreq, protocol) =                                        \
                MVP_LMT_PROTOCOL_RENDEZVOUS_UNSPECIFIED;                       \
        }                                                                      \
    }
