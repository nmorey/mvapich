...
struct addr_packet {
    ...
    int lid;
    int rail;
    mvp_arch_hca_type arch_hca_type;
    union ibv_gid gid;
...
static struct ibv_qp *create_qp(struct ibv_pd *pd, struct ibv_cq *scq,
                                struct ibv_cq *rcq)
{
...
static int get_pkey_index(uint16_t pkey, int hca_num, int port_num,
                          uint16_t *index)
{
    uint16_t i = 0;
    ...
    for (i = 0; i < dev_attr.max_pkeys; ++i) {
        uint16_t curr_pkey;
        ibv_query_pkey(hca_list[hca_num].nic_context, (uint8_t)port_num, (int)i,
                       &curr_pkey);
...
int MPID_nem_ib_init_connection(int rank, int size)
{
    ...
    conn_info.size = size;
    conn_info.rank = rank;
    conn_info.connections = (MPID_nem_ib_connection_t *)MPL_malloc(
        size * sizeof(MPID_nem_ib_connection_t));
...
int MPID_nem_ib_free_conn_info(int size)
{
    ...
    if (size > 1) {
        if ((mpi_errno = rdma_cleanup_startup_ring()) != MPI_SUCCESS) {
...
int MPID_nem_ib_alloc_process_init_info()
{
    ...
    info->gid = (union ibv_gid **)MPL_malloc(size * sizeof(union ibv_gid *));
    info->hostid = (int **)MPL_malloc(size * sizeof(int *));
    info->qp_num_rdma = (uint32_t **)MPL_malloc(size * sizeof(uint32_t *));
    info->arch_hca_type =
        (mvp_arch_hca_type *)MPL_malloc(size * sizeof(mvp_arch_hca_type));
    ...
    for (i = 0; i < size; ++i) {
        info->qp_num_rdma[i] = (uint32_t *)MPL_malloc(rails * sizeof(uint32_t));
        info->lid[i] = (uint16_t *)MPL_malloc(rails * sizeof(uint16_t));
        info->gid[i] =
            (union ibv_gid *)MPL_malloc(rails * sizeof(union ibv_gid));
...
int MPID_nem_ib_setup_conn(MPIDI_PG_t *pg)
{
    ...
    for (curRank = 0; curRank < size; curRank++) {
        ...
        conn_info.connections[curRank].rails =
            MPL_malloc(sizeof *conn_info.connections->rails * rdma_num_rails);
        conn_info.connections[curRank].srp.credits = MPL_malloc(
            sizeof *(conn_info.connections->srp.credits) * rdma_num_rails);
        ...
        if (conn_info.init_info)
            conn_info.init_info->arch_hca_type[curRank] =
                process_info.arch_hca_type;

        for (rail_index = 0; rail_index < rdma_num_rails; rail_index++) {
            int hca_index, port_index;
            hca_index = rail_index / (rdma_num_rails / ib_hca_num_hcas);
            port_index = (rail_index / (rdma_num_rails /
                                        (ib_hca_num_hcas * ib_hca_num_ports))) %
                         ib_hca_num_ports;
            ...
            conn_info.connections[curRank].rails[rail_index].qp_hndl =
                ibv_create_qp(hca_list[hca_index].ptag, &attr);

            if (!conn_info.connections[curRank].rails[rail_index].qp_hndl) {
                MPIR_ERR_SETFATALANDSTMT2(
                    mpi_errno, MPI_ERR_OTHER, goto fn_fail, "**fail", "%s%d",
                    "Failed to create qp for rank ", curRank);
            }

            if (conn_info.init_info) {
                conn_info.init_info->lid[curRank][rail_index] =
                    hca_list[hca_index].lids[port_index];
                conn_info.init_info->gid[curRank][rail_index] =
                    hca_list[hca_index].gids[port_index];
                conn_info.init_info->qp_num_rdma[curRank][rail_index] =
                    conn_info.connections[curRank]
                        .rails[rail_index]
                        .qp_hndl->qp_num;
...
int MPID_nem_ib_pmi_exchange()
{
    ...
    for (i = 0; i < conn_info.size; i++) {
        ...
        buf += 16;
        /* still about vc_addr here. vc should have not be initialized right
         * now*/
        sprintf(buf, "%016llx",
                (long long unsigned int)conn_info.init_info->vc_addr[i]);
        buf += 16;
        ...
        value is not visible to other processes until 'UPMI_KVS_COMMIT()' is
        called. The function may complete locally.  After 'UPMI_KVS_COMMIT()' is
        called, the value may be retrieved by calling 'UPMI_KVS_GET()'.  All
        keys put to a keyval space must be unique to the keyval space.  You may
        not put more than once with the same key.
        */
        error = UPMI_KVS_PUT(kvsname, mvp_pmi_key, mvp_pmi_val);
        ...
        /*
        This function commits all previous puts since the last
        'UPMI_KVS_COMMIT()' into the specified keyval space. It is a process
        local operation.
        */
    ...
    for (i = 0; i < conn_info.size; i++) {
        ...
        if (conn_info.rank == i) {
            conn_info.init_info->lid[i][0] =
                get_local_lid(hca_list[0].nic_context, rdma_default_port);
        ...
        error =
            UPMI_KVS_GET(kvsname, mvp_pmi_key, mvp_pmi_val, mvp_pmi_max_vallen);
        if (error != UPMI_SUCCESS) {
                        ...
                        MPIDI_PG_t *pg, int pg_rank)
{
    ...
    if (mpi_errno) {
        MPIR_ERR_SETFATALANDJUMP2(mpi_errno, MPI_ERR_OTHER, "**getaddrinfo",
    ...
    for (rail_index = 0; rail_index < rdma_num_rails; rail_index++) {
        ...
        send_packet =
            addr_packet_buffer(addr_poolProxy, index_to_send, pg_size);
        ...
        for (i = 0; i < pg_size; i++) {
            if (i == pg_rank) {
                ...
                conn_info.init_info->arch_hca_type[i] =
                    process_info.arch_hca_type;
            } else {
                ...
                send_packet->val[i].sr_qp_num =
                    conn_info.init_info->qp_num_rdma[i][rail_index];
                send_packet->val[i].vc_addr =
                    (uintptr_t)conn_info.init_info->vc_addr[i];
                send_packet->arch_hca_type =
                    conn_info.init_info->arch_hca_type[i];
            }
        }
        ...
        for (i = 0; i < pg_size - 1; i++) {
            ...
            sr.next = NULL;
            sg_entry_s.addr = (uintptr_t)addr_packet_buffer(
                addr_poolProxy, index_to_send, pg_size);
            ...
            while (1) {
                ...
                if (ne < 0) {
                    ...
                    if (rc.wr_id < MPD_WINDOW) {
                        ...
                        conn_info.init_info
                            ->lid[recv_packet->rank][rail_index] =
                            recv_packet->lid;
                        conn_info.init_info
                            ->gid[recv_packet->rank][rail_index] =
                            recv_packet->gid;
                        conn_info.init_info
                            ->hostid[recv_packet->rank][rail_index] =
                            recv_packet->host_id;
                        ...
                        */
                        ...
                        conn_info.init_info
                            ->qp_num_rdma[recv_packet->rank][rail_index] =
                        ...
                        rr.next = NULL;
                        sg_entry_r.lkey = addr_hndl->lkey;
                        sg_entry_r.addr = (uintptr_t)addr_packet_buffer(
                            addr_poolProxy, rr.wr_id, pg_size);
                        sg_entry_r.length = addr_packet_size(pg_size);
                        if (ibv_post_recv(process_info.boot_qp_hndl[0], &rr,
                                          &bad_wr_r)) {
                            MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_INTERN,
                                                      "**fail", "**fail %s",
                                                      "Error posting recv!\n");
        ...
        if (ne == 1) {
            if (rc.status != IBV_WC_SUCCESS) {
                MPIR_ERR_SETFATALANDJUMP2(
                    mpi_errno, MPI_ERR_INTERN, "**fail", "**fail %s %d",
...
int rdma_ring_boot_exchange(MPIDI_PG_t *pg, int pg_rank)
{
    ...
    addr_hndl = ibv_reg_mr(hca_list[0].ptag, addr_pool,
                           MPD_WINDOW * addr_packet_size(pg_size),
    ...
    if (conn_info.size > 1) {
        if (process_info.has_ring_startup) {
            ...
            if ((mpi_errno = MPID_nem_ib_pmi_exchange()) != MPI_SUCCESS) {
                MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_INTERN, "**fail",
                                          "**fail %s",
                                          "Failed to get HCA attrs");
            }
            rdma_param_handle_heterogeneity(conn_info.init_info->arch_hca_type,
                                            conn_info.size);
        }
    }
...
int MPID_nem_ib_establish_conn()
{
    ...
    for (i = 0; i < size; i++) {
        ...
        conn_info.connections[i].remote_vc_addr =
            conn_info.init_info->vc_addr[i];
        for (rail_index = 0; rail_index < rdma_num_rails; rail_index++) {
                                        ...
                                        void *alladdrs)
{
    ...
    CHECK_UNEXP((len_local > mvp_pmi_max_vallen),
                "local address length is larger then string length");

    /* Be sure to use different keys for different processes */
    MPIR_Memset(attr_buff, 0, IBA_PMI_ATTRLEN * sizeof(char));
    snprintf(attr_buff, IBA_PMI_ATTRLEN, "MVAPICH-%04d", pg_rank);

    /* put the kvs into PMI */
    MPL_strncpy(mvp_pmi_key, attr_buff, mvp_pmi_max_keylen);
    MPL_strncpy(mvp_pmi_val, temp_localaddr, mvp_pmi_max_vallen);
    MPIDI_PG_GetConnKVSname(&kvsname);
    ret = UPMI_KVS_PUT(kvsname, mvp_pmi_key, mvp_pmi_val);
    ...
    for (i = 0; i < 2; i++) {
        ...
        snprintf(attr_buff, IBA_PMI_ATTRLEN, "MVAPICH-%04d", j);
        MPL_strncpy(mvp_pmi_key, attr_buff, mvp_pmi_max_keylen);

        ret =
            UPMI_KVS_GET(kvsname, mvp_pmi_key, mvp_pmi_val, mvp_pmi_max_vallen);
        CHECK_UNEXP((ret != 0), "UPMI_KVS_GET error \n");
        MPL_strncpy(val_buff, mvp_pmi_val, mvp_pmi_max_vallen);

        /* Simple sanity check before stashing it to the alladdrs */
        len_remote = strlen(val_buff);
        CHECK_UNEXP((len_remote < len_local),
                    "remote length is smaller than local length");
...
static int _setup_ib_boot_ring(struct init_addr_inf *neighbor_addr, int port)
{
    struct ibv_qp_attr qp_attr;
    uint32_t qp_attr_mask = 0;
    int i;
    ...
    ret = ibv_modify_qp(
        process_info.boot_qp_hndl[0], &qp_attr,
        (IBV_QP_STATE | IBV_QP_PKEY_INDEX | IBV_QP_PORT | IBV_QP_ACCESS_FLAGS));
    CHECK_RETURN(ret, "Could not modify boot qp to INIT");

    ret = ibv_modify_qp(
        process_info.boot_qp_hndl[1], &qp_attr,
        (IBV_QP_STATE | IBV_QP_PKEY_INDEX | IBV_QP_PORT | IBV_QP_ACCESS_FLAGS));
    CHECK_RETURN(ret, "Could not modify boot qp to INIT");
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (!use_iboeth && (rdma_3dtorus_support || rdma_path_sl_query)) {
            ...
            /* don't know our local LID yet, so set it to 0 to let
               mvp_get_path_rec_sl do the lookup */
        ...
        ret =
            ibv_modify_qp(process_info.boot_qp_hndl[i], &qp_attr, qp_attr_mask);
...
int rdma_setup_startup_ring(int pg_rank, int pg_size)
{
    ...
    process_info.boot_cq_hndl = ibv_create_cq(
        hca_list[0].nic_context, rdma_default_max_cq_size, NULL, NULL, 0);
    if (!process_info.boot_cq_hndl) {
        MPIR_ERR_SETFATALANDSTMT1(mpi_errno, MPI_ERR_OTHER, goto out, "**fail",
                                  "**fail %s", "cannot create cq");
    }

    process_info.boot_qp_hndl[0] = create_qp(
        hca_list[0].ptag, process_info.boot_cq_hndl, process_info.boot_cq_hndl);
    if (!process_info.boot_qp_hndl[0]) {
        MPIR_ERR_SETFATALANDSTMT2(mpi_errno, MPI_ERR_OTHER, goto out, "**fail",
                                  "%s%d", "Fail to create qp on rank ",
                                  pg_rank);
    }

    process_info.boot_qp_hndl[1] = create_qp(
        hca_list[0].ptag, process_info.boot_cq_hndl, process_info.boot_cq_hndl);
...
#if defined(USE_IBOETH)
    if (use_iboeth) {
        gid = get_local_gid(mvp_MPIDI_CH3I_RDMA_Process.nic_context[0], port);
        sprintf(ring_qp_out,
                "%016llx:%016llx:%08x:%08x:", gid.global.subnet_prefix,
                gid.global.interface_id, proc->boot_qp_hndl[0]->qp_num,
    ...
    {
        sprintf(ring_qp_out,
                "%08x:%08x:%08x:", get_local_lid(hca_list[0].nic_context, port),
                process_info.boot_qp_hndl[0]->qp_num,
                process_info.boot_qp_hndl[1]->qp_num);

        DEBUG_PRINT("After setting LID: %d, qp0: %x, qp1: %x\n",
                    get_local_lid(hca_list[0].nic_context, port),
                    process_info.boot_qp_hndl[0]->qp_num,
                    process_info.boot_qp_hndl[1]->qp_num);
    }

    bootstrap_len = strlen(ring_qp_out);
    _rdma_pmi_exchange_addresses(pg_rank, pg_size, ring_qp_out, bootstrap_len,
                                 ring_qp_in);
    ...
    if (use_iboeth) {
        sscanf(&ring_qp_in[0], "%016llx:%016llx:%08x:%08x:",
               ...
               &neighbor_addr[0].qp_num[0], &neighbor_addr[0].qp_num[1]);
        sscanf(&ring_qp_in[53], "%016llx:%016llx:%08x:%08x:",
               ...
               &neighbor_addr[1].qp_num[0], &neighbor_addr[1].qp_num[1]);
...
#endif
    {
        sscanf(&ring_qp_in[0], "%08x:%08x:%08x:", &neighbor_addr[0].lid,
               &neighbor_addr[0].qp_num[0], &neighbor_addr[0].qp_num[1]);
        sscanf(&ring_qp_in[27], "%08x:%08x:%08x:", &neighbor_addr[1].lid,
               &neighbor_addr[1].qp_num[0], &neighbor_addr[1].qp_num[1]);
    }
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int rdma_ring_based_allgather(void *sbuf, int data_size, int pg_rank,
                              void *rbuf, int pg_size)
{
    struct ibv_mr *addr_hndl;
    int i;
    int mpi_errno = MPI_SUCCESS;
    addr_hndl = ibv_reg_mr(hca_list[0].ptag, rbuf, data_size * pg_size,
                           IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
    ...
    /* Now start exchanging data*/
    {
        ...
        for (i = 0; i < MPD_WINDOW; i++) {
            ...
            sg_entry_r.lkey = addr_hndl->lkey;
            sg_entry_r.addr =
                (uintptr_t)(rbufProxy + data_size * recv_post_index);
        ...
        while (recv_comp_index != (pg_rank + 1) % pg_size ||
            ...
            if (send_post_index != (pg_rank + 1) % pg_size &&
                (recv_comp_index == send_post_index ||
                 is_A_on_left_of_B(recv_comp_index, send_post_index, pg_rank,
                                   pg_size)) &&
                credit > 0) {
                sr.opcode = IBV_WR_SEND;
                sr.send_flags = IBV_SEND_SIGNALED;
                ...
                sr.next = NULL;
                sg_entry_s.addr =
                    (uintptr_t)(rbufProxy + data_size * send_post_index);
                sg_entry_s.length = data_size;
                sg_entry_s.lkey = addr_hndl->lkey;

                if (ibv_post_send(process_info.boot_qp_hndl[1], &sr,
                                  &bad_wr_s)) {
                    MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_INTERN,
                                              "**fail", "**fail %s",
                                              "Error posting send!\n");
            ...
            if (ne < 0) {
                MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_INTERN, "**fail",
                                          "**fail %s", "Poll CQ failed!\n");
            } else if (ne > 1) {
                MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_INTERN, "**fail",
                                          "**fail %s", "Got more than one\n");
            } else if (ne == 1) {
                if (rc.status != IBV_WC_SUCCESS) {
                    if (rc.status == IBV_WC_RETRY_EXC_ERR) {
                        DEBUG_PRINT("Got IBV_WC_RETRY_EXC_ERR\n");
...
int MPID_nem_ib_setup_startup_ring(MPIDI_PG_t *pg, int rank)
{
    ...
    if (pg_size > 1) {
        ...
        mpi_errno = rdma_ring_based_allgather(
            &my_arch_hca_type, sizeof(my_arch_hca_type), rank,
            conn_info.init_info->arch_hca_type, pg_size);
        if (mpi_errno) {
            MPIR_ERR_POP(mpi_errno);
        }
        /* Check heterogeneity */
...
int mvp_allocate_pmi_keyval(void)
{
    ...
    mvp_pmi_key = MPL_malloc(mvp_pmi_max_keylen + 1);
    mvp_pmi_val = MPL_malloc(mvp_pmi_max_vallen + 1);
