...
uint32_t mvp_srq_alloc_size = 4096;
uint32_t mvp_srq_fill_size = 256;
uint32_t mvp_srq_limit = 30;
...
static void get_hca_user_parameters()
{
    ...
    value = getenv("MVP_IBA_HCA");
    if (value != NULL) {
        strncpy(rdma_iba_hca, value, sizeof(rdma_iba_hca));
    }

    value = getenv("MVP_NUM_HCAS");
    if (value != NULL) {
...
static int check_hsam_parameters()
{
    ...
    if ((value = getenv("MVP_NUM_QP_PER_PORT")) != NULL) {
...
int MPID_nem_ib_get_control_params_after_hcainit()
{
    ...
    int size;
    UPMI_GET_SIZE(&size);
    process_info.has_srq =
        (value = getenv("MVP_USE_SRQ")) != NULL ? !!atoi(value) : 1;
#ifdef _ENABLE_XRC_
    if (USE_XRC) {
    ...
    if (process_info.has_srq &&
        (hca_list[0].hca_type != MVP_HCA_QLGIC_PATH_HT) &&
        (hca_list[0].hca_type != MVP_HCA_QLGIC_QIB) &&
        (hca_list[0].hca_type != MVP_HCA_MLX_PCI_X) &&
        (hca_list[0].hca_type != MVP_HCA_IBM_EHCA)
#if defined(RDMA_CM)
...
#else  /* defined(CKPT) */
    if ((value = getenv("MVP_USE_RDMA_FAST_PATH")) != NULL) {
    ...
    process_info.has_ring_startup =
        (value = getenv("MVP_USE_RING_STARTUP")) != NULL ? !!atoi(value) : 1;

#if !defined(DISABLE_PTMALLOC)
    process_info.has_lazy_mem_unregister =
        (value = getenv("MVP_USE_LAZY_MEM_UNREGISTER")) != NULL ?
            !!atoi(value) :
            1;
...
#else /* defined(CKPT) */
    process_info.has_one_sided =
        (value = getenv("MVP_USE_RDMA_ONE_SIDED")) != NULL ? !!atoi(value) : 1;

#endif /* defined(CKPT) */

    if ((value = getenv("MVP_RNDV_EXT_SENDQ_SIZE")) != NULL) {
    ...
    if ((value = getenv("MVP_COALESCE_THRESHOLD")) != NULL) {
    ...
    if ((value = getenv("MVP_USE_COALESCE")) != NULL) {
        rdma_use_coalesce = !!atoi(value);
    }

    if (hca_list[0].hca_type == MVP_HCA_MLX_CX_SDR ||
        hca_list[0].hca_type == MVP_HCA_MLX_CX_DDR) {
        rdma_use_coalesce = 0;
    }

    if ((value = getenv("MVP_USE_SHARED_MEM")) != NULL) {
        rdma_use_smp = !!atoi(value);
    }

    if ((value = getenv("MVP_USE_IBOETH")) != NULL) {
        use_iboeth = !!atoi(value);
        if (1 == process_info.has_ring_startup) {
            if (0 == my_rank) {
                MPL_usage_printf(
                    "Ring start up cannot be used in IBoEth mode."
                    "Falling back to PMI exchange.\r\n"
                    "You can also set MVP_USE_RING_STARTUP=0.\r\n");
            }
        ...
        if (!rdma_use_smp) {
            if (0 == my_rank) {
                MPL_usage_printf("IBoEth mode cannot function without SHMEM."
                                 ...
                                 "Please do NOT set MVP_USE_SHARED_MEM=0.\r\n");
            }
            rdma_use_smp = 1;
        }
    }
    ...
    if (!USE_XRC) {
#endif
        if ((value = getenv("MVP_USE_BLOCKING")) != NULL) {
    ...
    if ((value = getenv("MVP_SPIN_COUNT")) != NULL) {
    ...
    if ((value = getenv("MVP_R3_THRESHOLD")) != NULL) {
    ...
    if ((value = getenv("MVP_R3_NOCACHE_THRESHOLD")) != NULL) {
    ...
    if ((value = getenv("MVP_MAX_R3_PENDING_DATA")) != NULL) {
 ...
 * "mvapich/trunk/src/mpid/ch3/channels/mrail/src/gen2/ibv_param.c"
 */
int MPID_nem_ib_set_default_params()
{
    ...
    if (MVP_IS_ARCH_HCA_TYPE(process_info.arch_hca_type,
                             MVP_ARCH_INTEL_XEON_E5630_8, MVP_HCA_MLX_CX_QDR)) {
    ...
    else if (MVP_IS_ARCH_HCA_TYPE(process_info.arch_hca_type,
    ...
    else if (MVP_IS_ARCH_HCA_TYPE(process_info.arch_hca_type, MVP_ARCH_ANY,
                                  MVP_HCA_MLX_CX_QDR)) {
        rdma_vbuf_total_size = 16 * 1024 + EAGER_THRESHOLD_ADJUST;
        rdma_fp_buffer_size = 5 * 1024;
        rdma_iba_eager_threshold = VBUF_BUFFER_SIZE;
        ...
        rdma_get_fallback_threshold = 0;
                                  ...
                                  MVP_HCA_IBM_EHCA) ||
             MVP_IS_ARCH_HCA_TYPE(process_info.arch_hca_type, MVP_ARCH_ANY,
                                  MVP_HCA_MLX_PCI_X)) {
        rdma_vbuf_total_size = 12 * 1024 + EAGER_THRESHOLD_ADJUST;
        rdma_iba_eager_threshold = VBUF_BUFFER_SIZE;
        rdma_eagersize_1sc = 4 * 1024;
    ...
    num_rdma_buffer = 16;
    if (process_info.hca_type == MVP_HCA_QLGIC_PATH_HT ||
        process_info.hca_type == MVP_HCA_QLGIC_QIB) {
        rdma_default_qp_ous_rd_atom = 1;
    } else {
        rdma_default_qp_ous_rd_atom = 4;
    }

    if (process_info.hca_type == MVP_HCA_IBM_EHCA) {
        rdma_max_inline_size = -1;
    } else if ((process_info.hca_type == MVP_HCA_CHELSIO_T3) ||
               (process_info.hca_type == MVP_HCA_CHELSIO_T4)) {
        rdma_max_inline_size = 64;
    } else {
        rdma_max_inline_size = 128 + INLINE_THRESHOLD_ADJUST;
    }

    if (process_info.hca_type == MVP_HCA_MLX_PCI_EX_DDR) {
        rdma_default_mtu = IBV_MTU_2048;

    } else if (process_info.hca_type == MVP_HCA_MLX_CX_QDR) {
        rdma_default_mtu = IBV_MTU_2048;
    } else {
        rdma_default_mtu = IBV_MTU_1024;
    }

    if ((process_info.hca_type == MVP_HCA_CHELSIO_T3) ||
        (process_info.hca_type == MVP_HCA_CHELSIO_T4)) {
        /* Trac #423 */
...
int MPID_nem_ib_get_user_params()
{
    ...
    if ((value = getenv("MVP_DEFAULT_MTU")) != NULL) {
    ...
    if ((value = getenv("MVP_PIN_POOL_SIZE")) != NULL) {
        rdma_pin_pool_size = (int)atoi(value);
    }
    if ((value = getenv("MVP_MAX_INLINE_SIZE")) != NULL) {
    ...
    if ((value = getenv("MVP_DEFAULT_MAX_CQ_SIZE")) != NULL) {
        rdma_default_max_cq_size = (int)atoi(value);
    }
    if ((value = getenv("MVP_NUM_RDMA_BUFFER")) != NULL) {
        num_rdma_buffer = (int)atoi(value);
    }
    if ((value = getenv("MVP_POLLING_SET_THRESHOLD")) != NULL &&
        process_info.has_adaptive_fast_path) {
        rdma_polling_set_threshold = atoi(value);
    }
    if ((value = getenv("MVP_RDMA_EAGER_LIMIT")) != NULL &&
        process_info.has_adaptive_fast_path) {
        rdma_eager_limit = atoi(value);
        if (rdma_eager_limit < 0)
            rdma_eager_limit = 0;
    }
    if ((value = getenv("MVP_POLLING_SET_LIMIT")) != NULL &&
        process_info.has_adaptive_fast_path) {
    ...
    if ((value = getenv("MVP_VBUF_TOTAL_SIZE")) != NULL) {
        rdma_vbuf_total_size = user_val_to_bytes(value, "MVP_VBUF_TOTAL_SIZE");
        if (rdma_vbuf_total_size <= 2 * sizeof(int))
            rdma_vbuf_total_size = 2 * sizeof(int);
    }

    if ((value = getenv("MVP_RDMA_FAST_PATH_BUF_SIZE")) != NULL &&
        process_info.has_adaptive_fast_path) {
    ...
    if ((value = getenv("MVP_SRQ_MAX_SIZE")) != NULL) {
        mvp_srq_alloc_size = (uint32_t)atoi(value);
    }

    if ((value = getenv("MVP_SRQ_SIZE")) != NULL) {
        mvp_srq_fill_size = (uint32_t)atoi(value);
    }

    if ((value = getenv("MVP_SRQ_LIMIT")) != NULL) {
        mvp_srq_limit = (uint32_t)atoi(value);
    ...
    if ((value = getenv("MVP_IBA_EAGER_THRESHOLD")) != NULL) {
        rdma_iba_eager_threshold =
            user_val_to_bytes(value, "MVP_IBA_EAGER_THRESHOLD");
    }

    if ((value = getenv("MVP_STRIPING_THRESHOLD")) != NULL) {
    ...
    if ((value = getenv("MVP_NDREG_ENTRIES")) != NULL) {
        ...
        rdma_ndreg_entries =
            ((RDMA_NDREG_ENTRIES + 2 * pg_size < rdma_ndreg_entries_max) ?
                 RDMA_NDREG_ENTRIES + 2 * pg_size :
                 rdma_ndreg_entries_max);
    }
    if ((value = getenv("MVP_VBUF_MAX")) != NULL) {
        rdma_vbuf_max = atoi(value);
    }
    if ((value = getenv("MVP_INITIAL_PREPOST_DEPTH")) != NULL) {
        rdma_initial_prepost_depth = atoi(value);
    }
    if ((value = getenv("MVP_PREPOST_DEPTH")) != NULL) {
        rdma_prepost_depth = atoi(value);
    }
    if ((value = getenv("MVP_VBUF_POOL_SIZE")) != NULL) {
        rdma_vbuf_pool_size = atoi(value);
    }
    if ((value = getenv("MVP_DREG_CACHE_LIMIT")) != NULL) {
    ...
    if ((value = getenv("MVP_VBUF_SECONDARY_POOL_SIZE")) != NULL) {
...
int MPID_nem_ib_get_control_params()
{
    ...
    if ((value = getenv("MVP_LOG_ARCH_HCA_TYPE")) != NULL) {
        rdma_log_arch_hca_type = !!atoi(value);
    }

    /* Start HSAM Parameters */
    if ((value = getenv("MVP_USE_HSAM")) != NULL) {
    ...
    if ((value = getenv("MVP_USE_QOS")) != NULL) {
        rdma_use_qos = !!atoi(value);
    }

    if ((value = getenv("MVP_3DTORUS_SUPPORT")) != NULL) {
        rdma_3dtorus_support = !!atoi(value);
    }

    if ((value = getenv("MVP_PATH_SL_QUERY")) != NULL) {
        rdma_path_sl_query = !!atoi(value);
    }

    if ((value = getenv("MVP_NUM_SLS")) != NULL) {
    ...
    if ((value = getenv("MVP_NUM_SA_QUERY_RETRIES")) != NULL) {
    ...
    process_info.has_apm =
        (value = getenv("MVP_USE_APM")) != NULL ? (int)atoi(value) : 0;
    apm_tester =
        (value = getenv("MVP_USE_APM_TEST")) != NULL ? (int)atoi(value) : 0;

    /* Scheduling Parameters */
    if ((value = getenv("MVP_SM_SCHEDULING")) != NULL) {
    ...
    if (process_info.use_rdma_cm) {
        ...
        if ((value = getenv("MVP_ON_DEMAND_THRESHOLD")) != NULL) {
...
void mvp_print_env_info()
{
    mvp_arch_type arch_type = MVP_GET_ARCH(process_info.arch_hca_type);
    mvp_hca_type hca_type = MVP_GET_HCA(process_info.arch_hca_type);
    ...
    fprintf(stderr, "\tMVP_RDMA_FAST_PATH_BUF_SIZE    : %d\n",
            rdma_fp_buffer_size);
    fprintf(stderr, "\tMVP_EAGERSIZE_1SC              : %ld\n",
            rdma_eagersize_1sc);
...
#undef FUNCNAME
#define FUNCNAME MVP_get_arch_hca_type
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
mvp_arch_hca_type MVP_get_arch_hca_type() { return process_info.arch_hca_type; }

int mvp_get_heterogeneity() { return process_info.heterogeneity; }
