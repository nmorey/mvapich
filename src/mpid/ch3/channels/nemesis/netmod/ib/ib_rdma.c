...
{
    struct ibv_mr *mr =
        ibv_reg_mr(hca_list[hca_num].ptag, buf, len,
...
int vbuf_fast_rdma_alloc(MPIDI_VC_t *c, int dir)
{
    ...
    if (num_rdma_buffer) {
        ...
        if (!(vbuf_ctrl_buf = MPL_aligned_alloc(
                  vbuf_alignment, sizeof(struct vbuf) * num_rdma_buffer),
              MPL_MEM_BUFFER)) {
            DEBUG_PRINT("malloc failed: vbuf in vbuf_fast_rdma_alloc\n");
            goto fn_fail;
        }

        MPIR_Memset(vbuf_ctrl_buf, 0, sizeof(struct vbuf) * num_rdma_buffer);
        ...
        if (dir == 0) {
            ...
            for (i = 0; i < ib_hca_num_hcas; i++)
                VC_FIELD(c, connection)->rfp.RDMA_send_buf_mr[i] =
                    mem_handle[i];
            /* set pointers */
            ...
            for (i = 0; i < ib_hca_num_hcas; i++)
                VC_FIELD(c, connection)->rfp.RDMA_recv_buf_mr[i] =
                    mem_handle[i];
        }
    }
fn_exit:
