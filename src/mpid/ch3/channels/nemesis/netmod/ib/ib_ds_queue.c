...
int MPID_nem_ib_queue_init(MPID_nem_ib_queue_t **q)
{
    ...
    *q = MPL_malloc(sizeof(MPID_nem_ib_queue_t));
    ...
    if (NULL == *q) {
        MPIR_CHKMEM_SETERR(mpi_errno, sizeof(MPID_nem_ib_queue_t),
                           "IB Module Queue");
    }
...
int MPID_nem_ib_queue_new_elem(MPID_nem_ib_queue_elem_t **e, void *init_ptr)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    if (NULL == *e) {
        MPIR_CHKMEM_SETERR(mpi_errno, sizeof(MPID_nem_ib_queue_elem_t),
...
void MPID_nem_ib_queue_dequeue(MPID_nem_ib_queue_t *q,
...
void MPID_nem_ib_queue_enqueue(MPID_nem_ib_queue_t *q,
                               MPID_nem_ib_queue_elem_t *e)
{
    if (NULL == q->tail) {
        q->head = e;
...
void MPID_nem_ib_queue_free(MPID_nem_ib_queue_t *q, MPID_nem_ib_queue_elem_t *e)
{
    e->next = q->free_queue;
    q->free_queue = e;
}
...
int MPID_nem_ib_queue_alloc(MPID_nem_ib_queue_t *q,
                            MPID_nem_ib_queue_elem_t **e)
{
    int mpi_errno = MPI_SUCCESS;
...
void MPID_nem_ib_queue_finalize(MPID_nem_ib_queue_t *q)
{
    MPID_nem_ib_queue_elem_t *e;
