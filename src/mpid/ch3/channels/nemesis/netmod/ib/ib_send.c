                                           ...
                                           int iov_offset, size_t nb)
{
    ...
    MPIR_Memcpy(sreq->dev.iov, iov, iov_count * sizeof(MPL_IOV));
                                        ...
                                        int _rail, char *err_string)
{
    ...
    if ((_v->desc.sg_entry.length <= rdma_max_inline_size) &&
        ...
        _v->desc.u.sr.send_flags =
            (enum ibv_send_flags)(IBV_SEND_SIGNALED | IBV_SEND_INLINE);
...
void MRAILI_Ext_sendq_enqueue(MPIDI_VC_t *c, int rail, vbuf *v)
{
    ...
    if (VC_FIELD(c, connection)->rails[rail].ext_sendq_size >
...
static inline void MRAILI_Ext_sendq_send(MPIDI_VC_t *c, int rail)
{
    ...
    while (VC_FIELD(c, connection)->rails[rail].send_wqes_avail &&
        ...
        MPIDI_nem_ib_POST_SR(v, VC_FIELD(c, connection), rail,
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIDI_nem_ib_packetized_send(MPIDI_VC_t *vc, MPIR_Request *sreq)
{
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIDI_nem_ib_fill_start_buffer(vbuf *v, struct iovec *iov, int n_iov)
{
...
int MPIDI_nem_ib_post_send(MPIDI_VC_t *vc, vbuf *v, int rail)
{
    ...
    DEBUG_PRINT("[post send] credit %d,type %d, "
                "backlog %d, wqe %d, nb will be %d, seqnum = %d\n",
                VC_FIELD(vc, connection)->srp.credits[rail].remote_credit,
                p->type, VC_FIELD(vc, connection)->srp.credits[0].backlog.len,
    ...
    if (VC_FIELD(vc, connection)->srp.credits[rail].remote_credit > 0 ||
        ...
        ibv_backlog_queue_t *q =
            &(VC_FIELD(vc, connection)->srp.credits[rail].backlog);
        BACKLOG_ENQUEUE(q, v);
        MPIDI_FUNC_EXIT(MPID_STATE_IB_POST_SEND);
        return MPI_MRAIL_MSG_QUEUED;
    }
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIDI_nem_ib_eager_send(MPIDI_VC_t *vc, struct iovec *iov, int n_iov,
                            int pkt_len, int *num_bytes_ptr, vbuf **buf_handle)
{
    int mpi_errno = MPI_SUCCESS;
    uint16_t seqnum;
    ...
    if (MPIDI_nem_ib_fast_rdma_ok(vc, pkt_len)) {
        *num_bytes_ptr = pkt_len;
        ...
        return MPIDI_nem_ib_fast_rdma_send_complete(vc, iov, n_iov,
                                                    num_bytes_ptr, buf_handle);
    }
    ...
    *num_bytes_ptr =
        MPIDI_nem_ib_fill_start_buffer(v, iov, n_iov) + IB_PKT_HEADER_LENGTH;
    ...
    if (!MPID_nem_ib_vc_send_paused(vc_ib)) {
#endif
        /* The RDMA implementation uses a fixed length header, the size of which
        ...
        if (SENDQ_EMPTY(VC_FIELD(vc, send_queue))) {
            ...
            if (sreq->dev.OnDataAvail == MPIDI_CH3_ReqHandler_SendReloadIOV) {
                ...
                tmpbuf = MPL_malloc(sreq->dev.segment_size + pkt_len);
                databuf = tmpbuf;
                pkt_len = 0;
            ...
            if (mpi_errno == MPI_SUCCESS) {
                ...
                if (nb == 0) {
                    ...
                    if (!complete) {
                        ...
                        VC_FIELD(vc, send_active) =
                            SENDQ_HEAD(VC_FIELD(vc, send_queue));
                    }
                }
            } else if (MPI_MRAIL_MSG_QUEUED == mpi_errno) {
                buf->sreq = (void *)sreq;
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPID_nem_ib_iStartContigMsg_paused(MPIDI_VC_t *vc, void *hdr,
                                       intptr_t hdr_sz, void *data,
                                       intptr_t data_sz,
                                       MPIR_Request **sreq_ptr)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    if (SENDQ_EMPTY(VC_FIELD(vc, send_queue))) {
        ...
        mpi_errno = MPIDI_nem_ib_eager_send(vc, iov, n_iov, pkt_len, &nb, &buf);
        mpi_errno = MPI_SUCCESS DEBUG_PRINT(
            "[istartmsg_paused] mpierr %d, nb %d\n", mpi_errno, nb);
        if (mpi_errno == MPI_SUCCESS) {
            DEBUG_PRINT("[send path] eager send return %d bytes\n", nb);
            /* eager poking */
            /* if (nb == pkt_len)
                               ...
                               intptr_t hdr_sz)
{
    ...
    if (!MPID_nem_ib_vc_send_paused(vc_ib)) {
        ...
        if (SENDQ_EMPTY(VC_FIELD(vc, send_queue))) {
            ...
            if (sreq->dev.OnDataAvail == MPIDI_CH3_ReqHandler_SendReloadIOV) {
                /*reload iov, allocate packing buffer of size =  data size +
                 * header size*/
                tmpbuf = MPL_malloc(sreq->dev.segment_size +
                                    sizeof(MPIDI_CH3_Pkt_t));
                databuf = tmpbuf;
                ...
                /* First copy whatever has already been in iov set */
                for (i = 0; i < n_iov; i++) {
                    MPIR_Memcpy(tmpbuf, iov[i].iov_base, iov[i].iov_len);
                    tmpbuf = (void *)((unsigned long)tmpbuf + iov[i].iov_len);
                ...
                do {
                    ...
                    for (i = 0; i < n_iov; i++) {
                        MPIR_Memcpy(tmpbuf, sreq->dev.iov[i].iov_base,
                                    sreq->dev.iov[i].iov_len);
                        tmpbuf = (void *)((unsigned long)tmpbuf +
                                          sreq->dev.iov[i].iov_len);
                        pkt_len += sreq->dev.iov[i].iov_len;
                    }
                } while (sreq->dev.OnDataAvail ==
                         MPIDI_CH3_ReqHandler_SendReloadIOV);
                ...
                MPIR_Assert(pkt_len ==
                            (sreq->dev.segment_size + sizeof(MPIDI_CH3_Pkt_t)));
            ...
            mpi_errno =
                MPIDI_nem_ib_eager_send(vc, iov, n_iov, pkt_len, &nb, &vbuf);
            DEBUG_PRINT("[istartmsgv] mpierr %d, nb %d\n", mpi_errno, nb);
            if (MPI_SUCCESS == mpi_errno) {
                DEBUG_PRINT("[send path] eager send return %d bytes\n", nb);
                if (nb != 0) {
...
int MRAILI_Backlog_send(MPIDI_VC_t *vc, int rail)
{
    ...
    while ((q->len > 0) &&
        ...
        MPIDI_nem_ib_POST_SR(v, VC_FIELD(vc, connection), rail,
                             "ibv_post_sr (MRAILI_Backlog_push)");
    }
...
int MRAILI_Process_send(void *vbuf_addr)
{
    ...
    if (VC_FIELD(vc, free_vc)) {
        if (VC_FIELD(vc, connection)->rails[v->rail].send_wqes_avail ==
    ...
    switch (p->type) {
        ...
        case MPIDI_CH3_PKT_GET_ACCUM_RESP:
            req = v->sreq;
            v->sreq = NULL;
            DEBUG_PRINT("[process send type: %d] complete for eager msg, req "
                        "%p, seqnum = %d\n",
                        p->type, req, p->seqnum);
            if (req != NULL) {
        ...
        case MPIDI_NEM_PKT_LMT_DONE: {
            ...
            MPIR_Request *orig_req;
            done = (MPID_nem_pkt_lmt_done_t *)(v->pheader);
            MPIR_Request_get_ptr(done->sreq_id, orig_req);
            /* MPIR_Request_get_ptr(done->req_id, orig_req); */
            if (orig_req == NULL) {
            ...
            int dt_contig;
            intptr_t data_sz ATTRIBUTE((unused));
            MPI_Aint dt_true_lb ATTRIBUTE((unused));
            MPIR_Datatype *dt_ptr;
            MPIDI_Datatype_get_info(orig_req->dev.user_count,
                                    orig_req->dev.datatype, dt_contig, data_sz,
                                    dt_ptr, dt_true_lb);
            if (!dt_contig)
                if (REQ_FIELD(orig_req, rndv_buf) != NULL)
                    MPL_free(REQ_FIELD(orig_req, rndv_buf));
...
int MPIDI_nem_ib_fast_rdma_fill_start_buf(MPIDI_VC_t *vc, struct iovec *iov,
                                          int n_iov, int *num_bytes_ptr)
{
    ...
    MPIDI_CH3_Pkt_send_t *cached =
        VC_FIELD(vc, connection)->rfp.cached_outgoing;
    MPIDI_nem_ib_pkt_comm_header *cached_iheader =
        VC_FIELD(vc, connection)->rfp.cached_outgoing_iheader;
#endif
    MPIDI_CH3_Pkt_send_t *header;
    vbuf *v = &(
        VC_FIELD(vc, connection)
            ->rfp.RDMA_send_buf[VC_FIELD(vc, connection)->rfp.phead_RDMA_send]);
    void *vstart;
    void *data_buf;
    ...
    /* it's also need to be added in !MVP_DISABLE_HEADER_CACHING situation*/
    *num_bytes_ptr = 0;
    v->sreq = NULL;
    ...
    if ((header->type == MPIDI_CH3_PKT_EAGER_SEND) &&
        ...
        (VC_FIELD(vc, connection)->rfp.rdma_credit ==
         cached_iheader->rdma_credit)) {
...
#endif
    {
        ...
        *num_bytes_ptr += iov[0].iov_len + IB_PKT_HEADER_LENGTH;
        avail -= iov[0].iov_len;
        v->pheader = vstart;
    }
...
#define FCNAME MPL_QUOTE(FUNCNAME)
/* INOUT: num_bytes_ptr holds the pkt_len as input parameter */
int MPIDI_nem_ib_fast_rdma_send_complete(MPIDI_VC_t *vc, struct iovec *iov,
                                         int n_iov, int *num_bytes_ptr,
                                         vbuf **vbuf_handle)
{
    ...
    DEBUG_PRINT("[send: rdma_send] local vbuf %p, remote start %p, size %d, "
                "iheader = %p\n",
    ...
    *((VBUF_FLAG_TYPE *)(v->buffer + post_len)) = flag;
    ...
    DEBUG_PRINT(
        "[send: rdma_send] lkey %p, rkey %p, len %d, flag %d\n",
        VC_FIELD(vc, connection)
            ->rfp
            .RDMA_send_buf_mr[VC_FIELD(vc, connection)->rails[rail].hca_index]
            ->lkey,
        VC_FIELD(vc, connection)->rfp.RDMA_remote_buf_rkey, post_len,
        *v->head_flag);
    ...
    VBUF_SET_RDMA_ADDR_KEY(
        ...
        rstart,
        VC_FIELD(vc, connection)
            ->rfp.RDMA_remote_buf_rkey
                [VC_FIELD(vc, connection)->rails[rail].hca_index]);
    ...
    if ((NULL != hca_list[VC_FIELD(vc, connection)->rails[rail].hca_index]
                     .send_cq_hndl) &&
        (process_info.global_used_send_cq >= rdma_default_max_cq_size)) {
        /* We are monitoring CQ's and there is CQ overflow */
        cq_overflow = 1;
...
int MPIDI_nem_ib_fast_rdma_ok(MPIDI_VC_t *vc, int len)
{
    ...
    if (num_rdma_buffer < 2 ||
        ...
        VC_FIELD(vc, connection)
                ->rfp
                .RDMA_send_buf[VC_FIELD(vc, connection)->rfp.phead_RDMA_send]
                .padding == BUSY_FLAG) {
...
int MPIDI_nem_ib_post_srq_send(MPIDI_VC_t *vc, vbuf *v, int rail)
{
    ...
    if (srq_info.posted_bufs[hca_num] <= rdma_credit_preserve) {
        srq_info.posted_bufs[hca_num] += MPIDI_nem_ib_post_srq_buffers(
            mvp_srq_fill_size - srq_info.posted_bufs[hca_num], hca_num);
    }
...
void vbuf_address_send(MPIDI_VC_t *vc)
{
    ...
    p->type = 0;
    p->rdma_address =
        (unsigned long)VC_FIELD(vc, connection)->rfp.RDMA_recv_buf_DMA;
    ...
    for (i = 0; i < ib_hca_num_hcas; i++) {
        ...
        p->rdma_hndl[i] =
            VC_FIELD(vc, connection)->rfp.RDMA_recv_buf_mr[i]->rkey;
    }
    vbuf_init_send(v, sizeof(MPIDI_nem_ib_pkt_address) + IB_PKT_HEADER_LENGTH,
                   rail);
...
void vbuf_address_reply_send(MPIDI_VC_t *vc, uint8_t data)
{
    ...
    vbuf *v = get_vbuf();
    MPIDI_nem_ib_pkt_address_reply *p =
        (MPIDI_nem_ib_pkt_address_reply *)v->pheader;
    MPIDI_nem_ib_pkt_comm_header *pi = v->iheader;
    ...
    vbuf_init_send(
        v, sizeof(MPIDI_nem_ib_pkt_address_reply) + IB_PKT_HEADER_LENGTH, rail);
    process_info.post_send(vc, v, rail);
}
...
int MPIDI_nem_ib_lmt_r3_ack_send(MPIDI_VC_t *vc)
{
    ...
    vbuf_init_send(
        v, sizeof(MPIDI_CH3_Pkt_rndv_r3_ack_t) + IB_PKT_HEADER_LENGTH, rail);
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPID_nem_ib_send_queued(MPIDI_VC_t *vc,
                            MPIDI_nem_ib_request_queue_t *send_queue)
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_Request *sreq;
    intptr_t offset;
    ...
    MPL_DBG_MSG_P(CH3_CHANNEL, VERBOSE, "vc = %p", vc);
    MPIR_Assert(vc != NULL);
    ...
    while (!MPIDI_CH3I_Sendq_empty(*send_queue)) {
        ...
        MPL_DBG_MSG_P(CH3_CHANNEL, VERBOSE, "Sending %p", sreq);
        ...
        for (iov = &sreq->dev.iov[sreq->dev.iov_offset];
             iov < &sreq->dev.iov[sreq->dev.iov_offset + sreq->dev.iov_count];
             ++iov) {
        ...
        if (!complete) {
            ...
            if (!reqFn) {
                ...
                MPID_Request_complete(sreq);
                MPL_DBG_MSG(CH3_CHANNEL, VERBOSE, ".... complete");
...
void mvp_shm_coll_prepare_post_send(uint64_t local_rdma_addr,
