...
#define MPID_DEV_SHARP_REQUEST_DECL SHARP_REQ_HANDLE *sharp_req;

#define MPIDI_CH3_SHARP_REQUEST_INIT(_req) (_req)->sharp_req = NULL;
...
typedef enum DEVICE_IPC_STATUS {
    MVP_DEVICE_IPC_UNINITIALIZED,
    MVP_DEVICE_IPC_ENABLED,
    MVP_DEVICE_IPC_DISABLED
} mvp_device_ipc_status_t;
#endif
...
#if defined(_ENABLE_CUDA_) && defined(HAVE_CUDA_IPC)
    mvp_device_ipc_status_t can_access_peer;
#endif
} MPIDI_CH3I_SMP_VC;
 ...
 */
#define MPIDI_CH3_CA_ENUM MPIDI_CH3I_CA_HANDLE_PKT, MPIDI_CH3I_CA_END_RDMA
...
#define MPIDI_CH3_WIN_DECL                                                     \
    ...
    volatile int poll_flag; /* flag to indicate if polling for one sided       \
    ...
    MPL_shm_hnd_t shm_mutex_segment_handle; /* handle to interprocess mutex    \
                                               memory region */                \
    void *info_shm_base_addr;      /* base address of shared memory region for \
                                      window info */                           \
    MPI_Aint info_shm_segment_len; /* size of shared memory region for window  \
                                      info */                                  \
    MPL_shm_hnd_t info_shm_segment_handle; /* handle to shared memory region   \
                                              for window info */

extern int mvp_create_dummy_request();
extern int mvp_free_dummy_request();
