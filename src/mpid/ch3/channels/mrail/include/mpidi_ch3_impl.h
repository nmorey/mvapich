...
extern MPIDI_VC_t *mvp_read_progress_pending_vc;

extern int mvp_smp_fast_write_contig(MPIDI_VC_t *vc, const void *buf,
...
#define MPIDI_CH3I_SendQ_enqueue(vc, req)                                      \
    {                                                                          \
        /* MT - not thread safe! */                                            \
        if (MPL_DBG_SELECTED(MPIDI_CH3_DBG_CHANNEL, VERBOSE)) {                \
            MPL_DBG_OUT_FMT(MPIDI_CH3_DBG_CHANNEL,                             \
                            (MPL_DBG_FDEST,                                    \
                             "SendQ_enqueue_head vc=%08p req=0x%08x", vc,      \
                             req->handle));                                    \
        }                                                                      \
        req->dev.next = NULL;                                                  \
...
#define MPIDI_CH3I_SendQ_dequeue(vc)                                           \
    {                                                                          \
        ...
        if (MPL_DBG_SELECTED(MPIDI_CH3_DBG_CHANNEL, VERBOSE)) {                \
            MPL_DBG_OUT_FMT(MPIDI_CH3_DBG_CHANNEL,                             \
                            (MPL_DBG_FDEST,                                    \
                             "SendQ_dequeue vc=%08p req=0x%08x", vc,           \
                             vc->ch.sendq_head));                              \
        }                                                                      \
...
#define MPIDI_CH3I_CM_SendQ_enqueue(vc, req)                                   \
    {                                                                          \
        ...
        if (MPL_DBG_SELECTED(MPIDI_CH3_DBG_CHANNEL, VERBOSE)) {                \
            MPL_DBG_OUT_FMT(MPIDI_CH3_DBG_CHANNEL,                             \
                            (MPL_DBG_FDEST,                                    \
                             "CM_SendQ_enqueue vc=%08p req=0x%08x", vc,        \
                             vc->ch.cm_sendq_head));                           \
        }                                                                      \
        req->dev.next = NULL;                                                  \
...
#define MPIDI_CH3I_CM_SendQ_dequeue(vc)                                        \
    {                                                                          \
        ...
        MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL, VERBOSE,                        \
                        (MPL_DBG_FDEST, "CM_SendQ_dequeue vc=%08p req=0x%08x", \
                         vc, vc->ch.cm_sendq_head));                           \
...
#define MPIDI_CH3I_CM_One_Sided_SendQ_enqueue(vc, v)                           \
    {                                                                          \
        /* MT - not thread safe! */                                            \
        MPL_DBG_MSG_FMT(                                                       \
            MPIDI_CH3_DBG_CHANNEL, VERBOSE,                                    \
            (MPL_DBG_FDEST, "CM_SendQ_enqueue vc=%08p vbuf=0x%08x", vc, v));   \
        v->desc.next = NULL;                                                   \
...
#define MPIDI_CH3I_CM_One_Sided_SendQ_empty(vc)                                \
...
int MPIDI_CH3I_Request_adjust_iov(MPIR_Request *, intptr_t);
...
/*
MPIDI_CH3I_RDMA_init_process_group is called before the RDMA channel is
initialized.  It's job is to initialize the process group and determine the
process rank in the group.  has_parent is used by spawn and you can set it to
FALSE.  You can copy the implementation found in the shm directory for this
function if you want to use the PMI interface for process management
(recommended).
*/
int MPIDI_CH3I_RDMA_init_process_group(int *has_parent, MPIDI_PG_t **pg_pptr,
                                       int *pg_rank_ptr);
...
*/
int MPIDI_CH3I_RDMA_read_datav(MPIDI_VC_t *vc, struct iovec *iov, int n,
                               int *num_bytes_ptr);
...
int MPIDI_CH3_Rendezvous_rget_recv_finish(MPIDI_VC_t *, MPIR_Request *);

int MPIDI_CH3_Rendezvous_rget_send_finish(MPIDI_VC_t *,
                                          MPIDI_CH3_Pkt_rget_finish_t *);

int MPIDI_CH3_Packetized_send(MPIDI_VC_t *vc, MPIR_Request *);
...
int MPIDI_CH3_Packetized_recv_req(MPIDI_VC_t *vc, MPIR_Request *);

int MPIDI_CH3_Rendezvous_unpack_data(MPIDI_VC_t *vc, MPIR_Request *);
#ifdef _ENABLE_UD_
...
/* Mrail interfaces*/
int MPIDI_CH3I_MRAIL_Prepare_rndv(MPIDI_VC_t *vc, MPIR_Request *rreq);
...
int MPIDI_CH3I_MRAILI_Process_device_finish(
    MPIDI_VC_t *vc, MPIR_Request *rreq, MPIDI_CH3_Pkt_rput_finish_t *rf_pkt);
...
int MPIDI_CH3I_MRAIL_Finish_request(MPIR_Request *);
 ...
 *  * rndv recv */
void MPIDI_CH3I_CR_req_enqueue(MPIR_Request *req, MPIDI_VC_t *vc);

void MPIDI_CH3I_CR_req_dequeue(MPIR_Request *req);
...
#define MPIDI_CH3I_SMP_SendQ_enqueue(vc, req)                                  \
    {                                                                          \
        ...
        PRINT_DEBUG(DEBUG_SHM_verbose > 1,                                     \
                    "SendQ_enqueue vc: %p, dst: %d, req: %p, type: %d, "       \
                    "ch.reqtype: %d\n",                                        \
                    vc, vc->pg_rank, req, MPIDI_Request_get_type(req),         \
                    req->ch.reqtype);                                          \
        req->dev.next = NULL;                                                  \
...
#define MPIDI_CH3I_SMP_SendQ_enqueue_head(vc, req)                             \
    {                                                                          \
        /* MT - not thread safe! */                                            \
        PRINT_DEBUG(DEBUG_SHM_verbose > 1,                                     \
                    "SendQ_enqueue_head vc: %p, dst: %d, req: %p, type: %d, "  \
                    "ch.reqtype: %d\n",                                        \
                    vc, vc->pg_rank, req, MPIDI_Request_get_type(req),         \
                    req->ch.reqtype);                                          \
        MPIR_Request_add_ref(req);                                             \
...
#define MPIDI_CH3I_SMP_SendQ_dequeue(vc)                                       \
    {                                                                          \
        MPIR_Request *req = vc->smp.sendq_head;                                \
        /* MT - not thread safe! */                                            \
        PRINT_DEBUG(DEBUG_SHM_verbose > 1,                                     \
                    "SendQ_dequeue vc: %p, dst: %d, req: %p, type: %d, "       \
                    "ch.reqtype: %d\n",                                        \
                    vc, vc->pg_rank, req, MPIDI_Request_get_type(req),         \
                    req->ch.reqtype);                                          \
...
typedef enum { SMP_DMA_NONE, SMP_DMA_LIMIC, SMP_DMA_CMA } smp_dma_flag_t;

typedef enum { ONE_FREE = 1, TWO_FREE } smp_ctrl_avail_flag_t;

typedef enum { NO_FALLBACK = 0, FALLBACK } smp_fallback_flag_t;
...
int MPIDI_CH3I_SMP_readv_rndv_cont(MPIDI_VC_t *recv_vc_ptr,
                                   ...
                                   int index, void *l_header, void *c_header,
...
int MPIDI_CH3I_SMP_readv_rndv(MPIDI_VC_t *recv_vc_ptr, const struct iovec *iov,
                              ...
                              void *c_header, size_t *num_bytes_ptr,
                              smp_dma_flag_t dma_flag);
...
#define MPIDI_CH3I_SHM_MUTEX_LOCK(win_ptr)                                     \
    do {                                                                       \
        ...
        MPIR_ERR_CHKANDJUMP1(pt_err, mpi_errno, MPI_ERR_OTHER,                 \
                             "**pthread_lock", "**pthread_lock %s",            \
                             strerror(pt_err));                                \
    } while (0)
...
#define MPIDI_CH3I_SHM_MUTEX_UNLOCK(win_ptr)                                   \
    do {                                                                       \
        ...
        MPIR_ERR_CHKANDJUMP1(pt_err, mpi_errno, MPI_ERR_OTHER,                 \
                             "**pthread_unlock", "**pthread_unlock %s",        \
                             strerror(pt_err));                                \
    } while (0)
...
#define MPIDI_SHM_Wins_next_and_continue(elem)                                 \
    {                                                                          \
        elem = elem->next;                                                     \
        continue;                                                              \
    }

static inline int MPIDI_CH3I_SHM_Wins_append(MPIDI_SHM_Wins_list_t *list,
                                             MPIR_Win *win)
{
 ...
 */
static inline void MPIDI_CH3I_SHM_Wins_unlink(MPIDI_SHM_Wins_list_t *list,
                                              MPIR_Win *shm_win)
{
