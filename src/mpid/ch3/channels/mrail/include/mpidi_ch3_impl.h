...
{                                                                           \
    ...
    if (MPL_DBG_SELECTED(MPIDI_CH3_DBG_CHANNEL,VERBOSE)) {                  \
        MPL_DBG_OUT_FMT(MPIDI_CH3_DBG_CHANNEL, (MPL_DBG_FDEST,              \
...
{                                                                       \
    ...
    MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL,VERBOSE,(MPL_DBG_FDEST,       \
                    "CM_SendQ_dequeue vc=%08p req=0x%08x", vc,          \
                    vc->ch.cm_sendq_head));                             \
...
{                                                                           \
    /* MT - not thread safe! */                                             \
    MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL,VERBOSE,(MPL_DBG_FDEST,           \
                    "CM_SendQ_enqueue vc=%08p vbuf=0x%08x", vc, v));        \
    v->desc.next = NULL;                                                    \
...
int MPIDI_CH3I_RDMA_put_datav(MPIDI_VC_t *vc, 
        struct iovec *iov, int n, int *num_bytes_ptr);
...
void MRAILI_RDMA_Put_finish_device(MPIDI_VC_t * vc,
          MPIR_Request * sreq, int rail, int is_device_pipeline,
            int device_pipeline_finish, int offset);
int MPIDI_CH3I_MRAILI_Process_device_finish(MPIDI_VC_t * vc,
            MPIR_Request * rreq, MPIDI_CH3_Pkt_rput_finish_t * rf_pkt);
...
int MPIDI_CH3I_SMP_readv_rndv_device(MPIDI_VC_t *recv_vc_ptr, MPIR_Request *req,
    const struct iovec * iov, const int iov_count, int index, size_t *num_bytes_ptr, 
    int is_cont);
