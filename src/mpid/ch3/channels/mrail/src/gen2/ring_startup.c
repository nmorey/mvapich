...
static int _setup_ib_boot_ring(struct init_addr_inf *neighbor_addr,
                               ...
                               int port, int mvp_port_is_ethernet)
{
    ...
    ret = ibv_ops.modify_qp(
        proc->boot_qp_hndl[0], &qp_attr,
        (IBV_QP_STATE | IBV_QP_PKEY_INDEX | IBV_QP_PORT | IBV_QP_ACCESS_FLAGS));
    CHECK_RETURN(ret, "Could not modify boot qp to INIT");

    ret = ibv_ops.modify_qp(
        proc->boot_qp_hndl[1], &qp_attr,
        (IBV_QP_STATE | IBV_QP_PKEY_INDEX | IBV_QP_PORT | IBV_QP_ACCESS_FLAGS));
    CHECK_RETURN(ret, "Could not modify boot qp to INIT");
    ...
    if (use_iboeth || mvp_port_is_ethernet) {
...
int rdma_ring_exchange_host_id(MPIDI_PG_t *pg, int pg_rank, int pg_size)
{
    ...
    mpi_errno = rdma_ring_based_allgather(&my_hostid, sizeof my_hostid, pg_rank,
                                          hostid_all, pg_size,
...
static inline int ring_rdma_get_hca_context(
    struct mvp_MPIDI_CH3I_RDMA_Process_t *proc)
{
...
int rdma_setup_startup_ring(struct mvp_MPIDI_CH3I_RDMA_Process_t *proc,
                            int pg_rank, int pg_size)
{
    ...
    int mpi_errno = MPI_SUCCESS;
    int port;
    int mvp_port_is_ethernet = 0;
    char *value = NULL;

    if (!ring_rdma_get_hca_context(proc)) {
        MPIR_ERR_SETFATALANDSTMT1(mpi_errno, MPI_ERR_OTHER, goto out, "**fail",
                                  "**fail %s", "cannot retrieve hca device");
    }

    port = _find_active_port(proc->boot_context, &mvp_port_is_ethernet);
    if (port < 0) {
        MPIR_ERR_SETFATALANDSTMT1(mpi_errno, MPI_ERR_OTHER, goto out, "**fail",
                                  "**fail %s", "could not find active port");
    }

    proc->boot_cq_hndl = ibv_ops.create_cq(
        proc->boot_context, rdma_default_max_cq_size, NULL, NULL, 0);
    ...
    proc->boot_qp_hndl[0] =
        create_qp(proc->boot_ptag, proc->boot_cq_hndl, proc->boot_cq_hndl);
    ...
    if (use_iboeth || mvp_port_is_ethernet) {
        ...
        PRINT_DEBUG(
            DEBUG_CM_verbose,
            "After setting GID: %" PRIx64 ":%" PRIx64 ", qp0: %x, qp1: %x\n",
            (unsigned long)gid.global.subnet_prefix,
            (unsigned long)gid.global.interface_id,
            proc->boot_qp_hndl[0]->qp_num, proc->boot_qp_hndl[1]->qp_num);
    } else {
        sprintf(ring_qp_out,
                "%08x:%08x:%08x:", get_local_lid(proc->boot_context, port),
                proc->boot_qp_hndl[0]->qp_num, proc->boot_qp_hndl[1]->qp_num);

        PRINT_DEBUG(
            DEBUG_CM_verbose, "After setting LID: %d, qp0: %x, qp1: %x\n",
            get_local_lid(proc->boot_context, port),
            proc->boot_qp_hndl[0]->qp_num, proc->boot_qp_hndl[1]->qp_num);
    }
    ...
    if (use_iboeth || mvp_port_is_ethernet) {
        ...
        PRINT_DEBUG(DEBUG_CM_verbose,
                    "After retrieving GID: %" PRIx64 ":%" PRIx64
                    ", qp0: %x, qp1: %x\n",
                    (unsigned long)neighbor_addr[0].gid.global.subnet_prefix,
                    (unsigned long)neighbor_addr[0].gid.global.interface_id,
                    neighbor_addr[0].qp_num[0], neighbor_addr[0].qp_num[1]);
    ...
    mpi_errno =
        _setup_ib_boot_ring(neighbor_addr, proc, port, mvp_port_is_ethernet);
    UPMI_BARRIER();
...
int rdma_cleanup_startup_ring(struct mvp_MPIDI_CH3I_RDMA_Process_t *proc)
{
    int mpi_errno = MPI_SUCCESS;
                              ...
                              struct mvp_MPIDI_CH3I_RDMA_Process_t *proc)
{
    ...
    addr_hndl =
        ibv_ops.reg_mr(proc->boot_ptag, rbuf, data_size * pg_size,
            ...
            if (ne < 0) {
                ...
                if (rc.wr_id < pg_size) {
                    ...
                    if (recv_post_index != pg_rank) {
                        ...
                        if (ibv_post_recv(proc->boot_qp_hndl[0], &rr,
                                          &bad_wr_r)) {
                            MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_INTERN,
                                                      "**fail", "**fail %s",
                                                      "Error posting recv!\n");
...
int rdma_ring_boot_exchange(struct mvp_MPIDI_CH3I_RDMA_Process_t *proc,
                            MPIDI_PG_t *pg, int pg_rank,
                            struct process_init_info *info)
{
    ...
    int mpi_errno = MPI_SUCCESS;
    addr_pool =
        MPL_malloc(MPD_WINDOW * addr_packet_size(pg_size), MPL_MEM_OTHER);
    addr_hndl = ibv_ops.reg_mr(
        proc->boot_ptag, addr_pool, MPD_WINDOW * addr_packet_size(pg_size),
        IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
    ...
    mpi_errno =
        _ring_boot_exchange(addr_hndl, addr_pool, proc, pg, pg_rank, info);
    if (mpi_errno) {
        MPIR_ERR_POP(mpi_errno);
    }
    ibv_ops.dereg_mr(addr_hndl);
