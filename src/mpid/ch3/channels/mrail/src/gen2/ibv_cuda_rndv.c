...
int MPIDI_CH3I_MRAIL_Prepare_rndv_device(MPIDI_VC_t *vc, MPIR_Request *sreq,
                                         MPIR_Request *rreq)
{
    ...
    if (MRAILI_PROTOCOL_RPUT == MRAILI_PROTOCOL) {
    ...
    req->mrail.num_device_blocks =
        ROUNDUP(req->mrail.rndv_buf_sz, mvp_device_stage_block_size);
    for (i = 0;
         i < MIN(req->mrail.num_device_blocks, mvp_device_num_rndv_blocks);
         i++) {
        GET_VBUF_BY_OFFSET_WITHOUT_LOCK(req->mrail.device_vbuf[i],
                                        MVP_CUDA_VBUF_POOL_OFFSET);
    }
    req->mrail.device_block_offset = 0;
    req->mrail.num_remote_device_pending =
        MIN(req->mrail.num_device_blocks, mvp_device_num_rndv_blocks);
...
void MPIDI_CH3_DEVICE_IPC_Rendezvous_push(MPIDI_VC_t *vc, MPIR_Request *sreq)
{
    ...
    while (1) {
        if (cudaipc_shared_data[shared_index + i].sync_flag !=
            CUDAIPC_BUF_EMPTY) {
            break;
        }
        ...
        CUDA_CHECK(cudaStreamWaitEvent(
            strm, cudaipc_local_data[local_index + i].ipcEvent, 0));
        if (cudaipc_memcpy_sync) {
            MVP_MPIDI_Memcpy_Device(cudaipc_local_data[local_index + i].buffer,
                                    src, nbytes, cudaMemcpyDefault);
        } else {
            MVP_MPIDI_Memcpy_Device_Async(
                cudaipc_local_data[local_index + i].buffer, src, nbytes,
                cudaMemcpyDefault, strm);
        }
        CUDA_CHECK(cudaEventRecord(cudaipc_local_data[local_index + i].ipcEvent,
                                   strm));

        PRINT_DEBUG(CUDAIPC_DEBUG,
                    "cudaipc data-in: src:%p bytes:%d,"
                    "stage index:%d\n",
        ...
        if (sreq->mrail.rndv_buf_off == sreq->mrail.rndv_buf_sz) {
            if (cudaipc_memcpy_sync) {
                ...
                if (sreq->mrail.rndv_buf_alloc == 1 &&
                    ...
                    /* a temporary host rndv buffer would have been allocated
                       only when the sender buffers is noncontiguous and is in
                       the host memory */
                    MPIR_Assert(sreq->mrail.device_transfer_mode ==
                                HOST_TO_DEVICE);
                    MVP_MPIDI_Free_Device_Pinned_Host(sreq->mrail.rndv_buf);
...
void MPIDI_CH3_DEVICE_IPC_Rendezvous_recv(MPIDI_VC_t *vc, MPIR_Request *rreq)
{
    ...
    while (1) {
        ...
        CUDA_CHECK(cudaStreamWaitEvent(
            strm, cudaipc_remote_data[local_index + i].ipcEvent, 0));
        if (cudaipc_memcpy_sync) {
            MVP_MPIDI_Memcpy_Device(dst,
                                    cudaipc_remote_data[local_index + i].buffer,
                                    nbytes, cudaMemcpyDefault);
        } else {
            MVP_MPIDI_Memcpy_Device_Async(
                dst, cudaipc_remote_data[local_index + i].buffer, nbytes,
                cudaMemcpyDefault, strm);
        }
        CUDA_CHECK(cudaEventRecord(
            cudaipc_remote_data[local_index + i].ipcEvent, strm));

        PRINT_DEBUG(CUDAIPC_DEBUG,
                    "cudaipc data-out: dst:%p bytes:%d, stage index:%d\n", dst,
                    nbytes, shared_index + i);
        ...
        if (rreq->mrail.rndv_buf_off == rreq->mrail.rndv_buf_sz) {
            if (cudaipc_memcpy_sync) {
                ...
                CUDA_CHECK(
                    cudaEventRecord(rreq->mrail.device_event->event, strm));
                                                      ...
                                                      MPIR_Request *sreq)
{
    if (vc->smp.local_rank == -1 ||
        ...
        sreq->mrail.device_transfer_mode == NONE || sreq->dev.iov_count > 1 ||
        sreq->dev.iov[0].iov_len < mvp_device_ipc_threshold) {
        return 0;
    }
                                                     ...
                                                     MPIR_Request *sreq)
{
    ...
    if (sreq->mrail.rndv_buf_alloc == 1 && sreq->mrail.rndv_buf != NULL) {
    ...
    if (1 == sreq->dev.iov_count &&
        ...
        MVP_MPIDI_Malloc_Device_Pinned_Host(sreq->mrail.rndv_buf,
                                            sreq->mrail.rndv_buf_sz);
...
int MPIDI_CH3I_MRAIL_Prepare_rndv_device_ipc(MPIDI_VC_t *vc, MPIR_Request *sreq)
{
    int status = 1;
    size_t size;
    ...
    sreq->mrail.protocol = MRAILI_PROTOCOL_RGET;
    sreq->mrail.d_entry = NULL;

    cuerr = cuMemGetAddressRange(&baseptr, &size,
                                 (CUdeviceptr)sreq->dev.iov[0].iov_base);
    ...
    sreq->mrail.ipc_displ =
        (uint64_t)sreq->dev.iov[0].iov_base - (uint64_t)baseptr;
...
int MPIDI_CH3I_MRAIL_Rndv_transfer_device_ipc(
    MPIDI_VC_t *vc, MPIR_Request *rreq,
    MPIDI_CH3_Pkt_rndv_req_to_send_t *rts_pkt)
{
    ...
    MPIR_Memcpy(&rreq->mrail.ipc_eventhandle, &rts_pkt->rndv.ipc_eventhandle,
                sizeof(cudaIpcEventHandle_t));
    MPIR_Memcpy(&rreq->mrail.ipc_memhandle, &rts_pkt->rndv.ipc_memhandle,
                sizeof(cudaIpcMemHandle_t));
    base_ptr = rts_pkt->rndv.ipc_baseptr;
    size = rts_pkt->rndv.ipc_size;

    cudaipc_register(base_ptr, size, vc->smp.local_rank,
                     rreq->mrail.ipc_memhandle, &reg);
    ...
    if (cuda_event == NULL) {
        ...
        CUDA_LIST_ADD(cuda_event, busy_cuda_event_list_head,
                      busy_cuda_event_list_tail);
    }

    MVP_MPIDI_Memcpy_Device_Async(rreq->mrail.rndv_buf, remote_buf,
                                  rreq->mrail.rndv_buf_sz, cudaMemcpyDefault,
                                  stream_d2h);
...
void MPIDI_CH3I_MRAIL_Send_device_cts_conti(MPIDI_VC_t *vc, MPIR_Request *req)
{
    ...
    PRINT_DEBUG(DEBUG_CUDA_verbose,
                "Send CTS conti : offset: %d"
                " no.of buffers:%d req:%p\n",
                req->mrail.device_block_offset,
                req->mrail.num_remote_device_pending, req);
}

void MPIDI_CH3_Rendezvous_device_cts_conti(
    MPIDI_VC_t *vc, MPIDI_CH3_Pkt_device_cts_cont_t *cts_pkt)
{
    int i, hca_index;
    MPIR_Request *sreq;
...
void MRAILI_RDMA_Put_finish_device(MPIDI_VC_t *vc, MPIR_Request *sreq, int rail,
                                   int is_device_pipeline,
                                                   ...
                                                   MPIR_Request *sreq)
{
    ...
    if (sreq->mrail.device_transfer_mode == DEVICE_TO_DEVICE ||
        ...
        sreq->mrail.num_device_blocks =
            ROUNDUP(sreq->mrail.rndv_buf_sz, mvp_device_stage_block_size);
        ...
        for (; i < sreq->mrail.num_device_blocks; i++) {
            ...
            if (i == (sreq->mrail.num_device_blocks - 1)) {
                if (sreq->mrail.rndv_buf_sz % mvp_device_stage_block_size) {
                    nbytes =
            ...
            GET_VBUF_BY_OFFSET_WITHOUT_LOCK(cuda_event->device_vbuf_head,
                                            MVP_CUDA_VBUF_POOL_OFFSET);
            sreq->mrail.num_send_device_copy++;
            MVP_MPIDI_Memcpy_Device_Async(
                cuda_event->device_vbuf_head->buffer,
                (const void *)(sreq->dev.iov[0].iov_base +
                               mvp_device_stage_block_size * i),
                nbytes, cudaMemcpyDeviceToHost, stream_d2h);
            ...
            if (cuda_error != cudaSuccess) {
                ibv_error_abort(IBV_RETURN_ERR, "cudaEventRecord failed\n");
            }
        }
            ...
            if (nbytes <= rdma_large_msg_rail_sharing_threshold) {
                ...
                MRAILI_RDMA_Put(
                    ...
                    ((dreg_entry *)sreq->mrail.d_entry)
                        ->memhandle[rail_index]
                        ->lkey,
                    (char *)(sreq->mrail.device_remote_addr
                                 [sreq->mrail.num_remote_device_done]),
                    sreq->mrail
                        .device_remote_rkey[sreq->mrail.num_remote_device_done]
                                           [rail_index],
                    nbytes, rail);
            } else {
                ...
                for (rail = 0; rail < rdma_num_rails; rail++) {
                    ...
                    stripe_size = (rail < rdma_num_rails - 1) ?
                                      stripe_avg :
                                      (nbytes - offset);
                    ...
                    MRAILI_RDMA_Put(
                        ...
                        (char *)(sreq->mrail.rndv_buf) +
                            ...
                            mvp_device_stage_block_size * i + offset,
                        ((dreg_entry *)sreq->mrail.d_entry)
                            ->memhandle[rail_index]
                            ->lkey,
                        (char *)(sreq->mrail.device_remote_addr
                                     [sreq->mrail.num_remote_device_done]) +
                            offset,
                        sreq->mrail.device_remote_rkey
                            [sreq->mrail.num_remote_device_done][rail_index],
                        stripe_size, rail);
                }
            }

            for (rail = 0; rail < rdma_num_rails; rail++) {
                MRAILI_RDMA_Put_finish_device(vc, sreq, rail, is_pipeline,
                                              is_finish,
                                              mvp_device_stage_block_size * i);
            }
                                            ...
                                            MPIDI_CH3_Pkt_rput_finish_t *rf_pkt)
{
    ...
    displacement = rf_pkt->device_pipeline_offset / mvp_device_stage_block_size;
    ...
    nbytes = (rreq->mrail.rndv_buf_sz - rf_pkt->device_pipeline_offset <
              mvp_device_stage_block_size) ?
                 (rreq->mrail.rndv_buf_sz - rf_pkt->device_pipeline_offset) :
                 mvp_device_stage_block_size;

    mvp_device_event_t *cuda_event = NULL;
    if (rreq->mrail.device_event == NULL) {
    ...
    cuda_event->is_finish =
        (rreq->mrail.pipeline_nm == rreq->mrail.num_device_blocks) ? 1 : 0;
    ...
    if (rreq->mrail.num_remote_device_pending == 0 &&
        ...
        rreq->mrail.device_block_offset += mvp_device_num_rndv_blocks;
        MPIDI_CH3I_MRAIL_Send_device_cts_conti(vc, rreq);
    }

    MVP_MPIDI_Memcpy_Device_Async(rreq->mrail.rndv_buf +
                                      rf_pkt->device_pipeline_offset,
                                  (const void *)(device_vbuf->buffer), nbytes,
    ...
    PRINT_DEBUG(DEBUG_CUDA_verbose > 1,
                "RECV cudaMemcpyAsync :%d "
                "req:%p strm:%p\n",
                rreq->mrail.num_remote_device_done, rreq, cuda_event);
                                      ...
                                      MPIR_Request *rreq)
{
    ...
    if (mvp_device_use_ipc && mvp_device_use_ipc_stage_buffer &&
        vc->smp.can_access_peer == MVP_DEVICE_IPC_ENABLED) {
        if ((rreq->mrail.protocol == MRAILI_PROTOCOL_CUDAIPC &&
    ...
    switch (rreq->mrail.protocol) {
        ...
        case MRAILI_PROTOCOL_CUDAIPC: {
#if defined(HAVE_CUDA_IPC)
            cts_pkt->rndv.protocol = MRAILI_PROTOCOL_CUDAIPC;
            MPIDI_CH3I_MRAIL_Prepare_rndv_recv_cuda_ipc_buffered(vc, rreq);
#else
            PRINT_ERROR(
                "CUDAIPC rndv is requested, but CUDA IPC is not supported\n");
            mpi_errno = -1;
#endif
            break;
        }
        case MRAILI_PROTOCOL_R3: {
