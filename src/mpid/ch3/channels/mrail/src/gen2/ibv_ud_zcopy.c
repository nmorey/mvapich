                    ...
                    MPIR_Request * sreq, mv2_ud_zcopy_info_t *zcopy_info)
{
    ...
    PRINT_DEBUG(DEBUG_ZCY_verbose>1, "sending zcopy finish message to:%d on %d HCAs\n",
                vc->pg_rank, sreq->mrail.num_hcas);
    ...
    for (hca_index = 0; hca_index < sreq->mrail.num_hcas; ++hca_index) {
        ...
        PRINT_DEBUG(DEBUG_ZCY_verbose>1, "sending zcopy finish message to:%d on HCAs %d\n",
...
static inline void MRAILI_Rndv_send_zcopy_ack(MPIDI_VC_t * vc,  MPIR_Request * rreq)
{
    ...
    PRINT_DEBUG(DEBUG_ZCY_verbose>1, "sending zcopy ack message to:%d on %d HCAs\n",
                vc->pg_rank, rreq->mrail.num_hcas);
...
static inline void mv2_flush_zcopy_rndv_qp(mv2_rndv_qp_t *rqp, int hca_index, int num_to_flush)
{
    int ne, count;
    ...
    if(mv2_ud_qp_transition(rqp->ud_qp[hca_index], hca_index)) {
...
static inline void mv2_ud_post_zcopy_recv(MPIR_Request *req, mv2_ud_zcopy_info_t *zcopy_info)
{
    ...
    /* For segments that are not fully divisible, send out the remainder through the first rail */
    for (hca_num = 0; hca_num < req->mrail.num_hcas; ++hca_num) {
        posts_required_per_hca[hca_num] = (posts_required / req->mrail.num_hcas);
        PRINT_DEBUG(DEBUG_ZCY_verbose>0 ,"Posts for HCA %d = %d\n", hca_num, posts_required_per_hca[hca_num]);
    }
    posts_required_per_hca[0] += posts_required - (posts_required_per_hca[0]*req->mrail.num_hcas);
    ...
    while (posted_buffers < posts_required) {
        for (hca_num = 0; hca_num < req->mrail.num_hcas; ++hca_num) {
            if (posted_buffers_per_hca[hca_num] >= posts_required_per_hca[hca_num]) {
                continue;
            }
            PRINT_DEBUG(DEBUG_ZCY_verbose>0 ,"Preparing recv WQE HCA %d. posted_buffers = %d, req = %d\n",
                        hca_num, posted_buffers_per_hca[hca_num], posts_required_per_hca[hca_num]);
            for (i = 0; i < rdma_ud_zcopy_push_segment-1; i++) {
                ...
                sge_entry[i * 2].lkey = ((dreg_entry *)
                        (zcopy_info->grh_mr))->memhandle[hca_num]->lkey;
                ...
                sge_entry[i * 2 + 1].lkey =  ((dreg_entry *) 
                            (req->mrail.d_entry))->memhandle[hca_num]->lkey;
            ...
            PRINT_DEBUG(DEBUG_ZCY_verbose>0 ,"Posted %d/%d bytes %d (%d overall) for HCA %d\n",
                                ...
                                vbuf *resend_buf, int hca_index, int *found)
{
    ...
    ne = ibv_poll_cq(zcopy_info->rndv_ud_cqs[hca_index], rdma_ud_zcopy_push_segment, wc_list);
    if (ne < 0) {
                    ...
                    MPIR_Request * sreq, mv2_ud_zcopy_info_t *zcopy_info)
{
    ...
    for (hca_num = 0; hca_num < sreq->mrail.num_hcas; ++hca_num) {
        posts_required_per_hca[hca_num] = (posts_required / sreq->mrail.num_hcas);
        PRINT_DEBUG(DEBUG_ZCY_verbose>0 ,"Posts for HCA %d = %d\n", hca_num, posts_required_per_hca[hca_num]);
    }
    posts_required_per_hca[0] += posts_required - (posts_required_per_hca[0]*sreq->mrail.num_hcas);
    PRINT_DEBUG(DEBUG_ZCY_verbose>0 ,"Posts for HCA 0 = %d\n", posts_required_per_hca[0]);
    ...
    while (sreq->mrail.rndv_buf_off < sreq->mrail.rndv_buf_sz) {
        for (hca_num = 0; hca_num < sreq->mrail.num_hcas; ++hca_num) {
            ...
            do {
                mv2_ud_zcopy_poll_cq(zcopy_info, ud_ctx, NULL, hca_num, &found);
            } while (ud_ctx->send_wqes_avail < rdma_ud_zcopy_push_segment || found);
            ...
            for (i = 0; i < rdma_ud_zcopy_push_segment-1; i++) {
                ...
                sg_entry[i].lkey   = ((dreg_entry *)
                                sreq->mrail.d_entry)->memhandle[hca_num]->lkey;
            ...
            PRINT_DEBUG(DEBUG_ZCY_verbose>0 ,"Posted %d bytes %d/%d for HCA %d\n",
                        bytes_to_post, posted_buffers_per_hca[hca_num],
    ...
    for (hca_num = 0; hca_num < sreq->mrail.num_hcas; ++ hca_num) {
        PRINT_DEBUG(DEBUG_ZCY_verbose>1, "Posted rndv send posts:%d on HCA %d to:%d\n",
...
void MPIDI_CH3_Rendezvous_zcopy_finish(MPIDI_VC_t * vc,
                             MPIDI_CH3_Pkt_zcopy_finish_t * zcopy_finish)
{
    int i, ne, posted_buffers, count = 0, complete, hca_num = 0, num_retries = 0;
    int count_per_hca[MAX_NUM_HCAS] = {0}, posted_buffers_per_hca[MAX_NUM_HCAS] = {0};
    ...
    for (hca_num = 0; hca_num < rreq->mrail.num_hcas; ++hca_num) {
        posted_buffers_per_hca[hca_num] = (posted_buffers / rreq->mrail.num_hcas);
...
zcopy_recv_polling:
    for (hca_num = 0; hca_num < rreq->mrail.num_hcas; ++hca_num) {
        ...
        do {
            PRINT_DEBUG(DEBUG_ZCY_verbose>0 ,"Polling CQ for %d. %d/%d\n",
                        hca_num, count_per_hca[hca_num], posted_buffers_per_hca[hca_num]);
            ne = ibv_poll_cq(rqp->ud_cq[hca_num], posted_buffers_per_hca[hca_num], wc);
            if (ne < 0) {
                ibv_error_abort(IBV_RETURN_ERR, "Error in polling recv RNDV UD QP\n");
            } else if (ne > 0) {
                PRINT_DEBUG(DEBUG_ZCY_verbose>0 ,"Rvcd %d completions on CQ for HCA %d\n", ne, hca_num);
                for (i=0; i<ne; i++) {
        ...
        } while (!empty && posted_buffers_per_hca[hca_num] != count_per_hca[hca_num]);

        PRINT_DEBUG(DEBUG_ZCY_verbose>1, "Done polling RNDV UD. got %d of %d. remote:%d\n",
                                    count, posted_buffers, vc->pg_rank);

        if (count_per_hca[hca_num] != posted_buffers_per_hca[hca_num] || out_of_order) {
            PRINT_DEBUG(DEBUG_ZCY_verbose, "Zcopy Rndv recv failed. "
    ...
    if (need_resend) {
        for (hca_num = 0; hca_num < rreq->mrail.num_hcas; ++hca_num) {
            rqp = rreq->mrail.rndv_qp_entry;
            mv2_flush_zcopy_rndv_qp(rqp, hca_num, posted_buffers_per_hca[hca_num]-count_per_hca[hca_num]);
        }
