...
int MPIDI_CH3I_MRAILI_Recv_addr(MPIDI_VC_t *vc, void *vstart)
{
    ...
    int ret;
#ifdef _ENABLE_XRC_
    if (USE_XRC && (0 == xrc_rdmafp_init || VC_XST_ISSET(vc, XF_CONN_CLOSING)))
...
int MPIDI_CH3I_MRAILI_Recv_addr_reply(MPIDI_VC_t *vc, void *vstart)
{
    ...
    if (pkt->reply_data == RDMA_FP_SENDBUFF_ALLOC_FAILED ||
        ...
        MPIR_Assert(mvp_MPIDI_CH3I_RDMA_Process.polling_group_size <
                    rdma_polling_set_limit);

        mvp_MPIDI_CH3I_RDMA_Process
            .polling_set[mvp_MPIDI_CH3I_RDMA_Process.polling_group_size] = vc;
        mvp_MPIDI_CH3I_RDMA_Process.polling_group_size++;
