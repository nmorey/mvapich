...
#endif
    {
        for (i = 0; i < n_iov; i++) {
            ...
            if (avail >= iov[i].iov_len) {
                ...
                MPIR_Memcpy(ptr, iov[i].iov_base,
                        (iov[i].iov_len));
                len += (iov[i].iov_len);
                avail -= (iov[i].iov_len);
