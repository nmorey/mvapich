...
/* TODO: Ticket #1433 */
void FLUSH_SQUEUE_NOINLINE(MPIDI_VC_t *vc) { FLUSH_SQUEUE(vc); }
#endif /*defined(MPIDI_MRAILI_COALESCE_ENABLED)*/

int MPIDI_CH3I_RDMA_put_datav(MPIDI_VC_t *vc, struct iovec *iov, int n,
                              int *num_bytes_ptr)
{
    int mpi_errno = MPI_SUCCESS;
    /* all variable must be declared before the state declarations */
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_CH3I_PUT_DATAV);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_PUT_DATAV);

    /* Insert implementation here */
    PRINT_ERROR("MPIDI_CH3I_RDMA_put_datav is not implemented\n");
    exit(EXIT_FAILURE);

    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_PUT_DATAV);
    return mpi_errno;
}

int MPIDI_CH3I_RDMA_read_datav(MPIDI_VC_t *recv_vc_ptr, struct iovec *iov,
                               int iovlen, int *num_bytes_ptr)
{
    int mpi_errno = MPI_SUCCESS;
    /* all variable must be declared before the state declarations */
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_CH3I_RDMA_READ_DATAV);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_RDMA_READ_DATAV);

    /* Insert implementation here */
    PRINT_ERROR("MPIDI_CH3I_RDMA_read_datav Function not implemented\n");
    exit(EXIT_FAILURE);

    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_RDMA_READ_DATAV);
    return mpi_errno;
}

int mvp_post_srq_buffers(int num_bufs, int hca_num)
{
    int i = 0;
    vbuf *v = NULL;
    struct ibv_recv_wr *bad_wr = NULL;
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_POST_SRQ_BUFFERS);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_POST_SRQ_BUFFERS);

    if (num_bufs > mvp_srq_fill_size) {
        ibv_va_error_abort(GEN_ASSERT_ERR, "Try to post %d to SRQ, max %d\n",
                           num_bufs, mvp_srq_fill_size);
    }

    for (; i < num_bufs; ++i) {
        if ((v = get_vbuf_by_offset(MVP_RECV_VBUF_POOL_OFFSET)) == NULL) {
            break;
        }
        ...
        if (ibv_post_srq_recv(mvp_MPIDI_CH3I_RDMA_Process.srq_hndl[hca_num],
                              &v->desc.u.rr, &bad_wr)) {
            release_vbuf(v);
            break;
        }
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_POST_SRQ_BUFFERS);
    return i;
}

#ifdef _ENABLE_UD_
int mvp_post_ud_recv_buffers(int num_bufs, mvp_ud_ctx_t *ud_ctx)
{
    int i = 0, ret = 0;
    vbuf *v = NULL;
    struct ibv_recv_wr *bad_wr = NULL;
    int max_ud_bufs =
        (rdma_use_ud_srq) ? mvp_ud_srq_fill_size : rdma_default_max_ud_recv_wqe;

    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_POST_RECV_BUFFERS);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_POST_RECV_BUFFERS);
    ...
    PRINT_DEBUG(DEBUG_UD_verbose > 0,
                "Posted %d buffers of size:%d to UD QP on HCA %d\n", num_bufs,
                rdma_default_ud_mtu, ud_ctx->hca_num);

    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_POST_RECV_BUFFERS);
    return i;
}
#endif /*_ENABLE_UD_*/
...
int MRAILI_Fill_start_buffer(vbuf *v, struct iovec *iov, int n_iov)
{
    ...
    int len = 0;
#ifdef _ENABLE_UD_
    if (MVP_USE_UD_HYBRID && v->transport == IB_TRANSPORT_UD) {
        avail = MRAIL_MAX_UD_SIZE - v->content_size;
    }
#endif

    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MRAILI_FILL_START_BUFFER);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MRAILI_FILL_START_BUFFER);

    DEBUG_PRINT("buffer: %p, content size: %d\n", v->buffer, v->content_size);

#ifdef _ENABLE_CUDA_
    if (mvp_enable_device && n_iov > 1 && is_device_buffer(iov[1].iov_base)) {
        /* in the case of GPU buffers, there is only one data iov, if data is
         * non-contiguous it should have been packed before this */
        MPIR_Assert(n_iov == 2);

        MPIR_Memcpy(ptr, iov[0].iov_base, (iov[0].iov_len));
        len += (iov[0].iov_len);
        avail -= (iov[0].iov_len);
        ptr = (void *)((unsigned long)ptr + iov[0].iov_len);

        if (avail >= iov[1].iov_len) {
            MVP_MPIDI_Memcpy_Device(ptr, iov[1].iov_base, iov[1].iov_len,
                                    deviceMemcpyDeviceToHost);
            len += iov[1].iov_len;
        } else {
            MVP_MPIDI_Memcpy_Device(ptr, iov[1].iov_base, avail,
                                    deviceMemcpyDeviceToHost);
            len += avail;
            avail = 0;
        }
...
#endif
    {
        for (i = 0; i < n_iov; i++) {
            ...
            if (avail >= iov[i].iov_len) {
                ...
                MPIR_Memcpy(ptr, iov[i].iov_base, (iov[i].iov_len));
                len += (iov[i].iov_len);
                avail -= (iov[i].iov_len);
                ptr = (void *)((unsigned long)ptr + iov[i].iov_len);
            } else {
                MPIR_Memcpy(ptr, iov[i].iov_base, avail);
                len += avail;
                avail = 0;
                break;
            }
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MRAILI_FILL_START_BUFFER);
    return len;
}

int MPIDI_CH3I_MRAILI_rget_finish(MPIDI_VC_t *vc, struct iovec *iov, int n_iov,
                                  ...
                                  int rail)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_CH3I_MRAILI_RGET_FINISH);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_MRAILI_RGET_FINISH);

    if (likely(nbytes <= DEFAULT_MEDIUM_VBUF_SIZE)) {
        GET_VBUF_BY_OFFSET_WITHOUT_LOCK(v, MVP_MEDIUM_DATA_VBUF_POOL_OFFSET);
    } else {
        GET_VBUF_BY_OFFSET_WITHOUT_LOCK(v, MVP_LARGE_DATA_VBUF_POOL_OFFSET);
    }
    *buf_handle = v;
    *num_bytes_ptr = MRAILI_Fill_start_buffer(v, iov, n_iov);
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_MRAILI_RGET_FINISH);
    return mpi_errno;
}

int MPIDI_CH3I_MRAILI_rput_complete(MPIDI_VC_t *vc, struct iovec *iov,
                                    int n_iov, int *num_bytes_ptr,
                                    vbuf **buf_handle, int rail)
{
    vbuf *v;
    int mpi_errno;

    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_CH3I_MRAILI_RPUT_COMPLETE);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_MRAILI_RPUT_COMPLETE);

    MRAILI_Get_buffer(vc, v, iov->iov_len);
    *buf_handle = v;
    ...
    *num_bytes_ptr = MRAILI_Fill_start_buffer(v, iov, n_iov);
    ...
    mpi_errno = post_send(vc, v, rail);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_MRAILI_RPUT_COMPLETE);
    return mpi_errno;
}
...
int MRAILI_Backlog_send(MPIDI_VC_t *vc, int rail)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MRAILI_BACKLOG_SEND);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MRAILI_BACKLOG_SEND);
    ...
    while ((q->len > 0) && (vc->mrail.srp.credits[rail].remote_credit > 0)) {
        vbuf *v = NULL;
        MPIDI_CH3I_MRAILI_Pkt_comm_header *p;
        MPIR_Assert(q->vbuf_head != NULL);
        BACKLOG_DEQUEUE(q, v);
        ...
        if (likely(vc->mrail.rails[rail].send_wqes_avail > 0 && !cq_overflow)) {
            ...
            IBV_POST_SR(v, vc, rail, "ibv_post_sr (MRAILI_Backlog_send)");
        } else {
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MRAILI_BACKLOG_SEND);
    return 0;
}

int MRAILI_Flush_wqe(MPIDI_VC_t *vc, vbuf *v, int rail)
{
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MRAILI_FLUSH_WQE);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MRAILI_FLUSH_WQE);
    FLUSH_RAIL(vc, rail);
    if (!vc->mrail.rails[rail].send_wqes_avail) {
        MRAILI_Ext_sendq_enqueue(vc, rail, v);
        MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MRAILI_FLUSH_WQE);
...
int MRAILI_Process_send(void *vbuf_addr)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MRAILI_PROCESS_SEND);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MRAILI_PROCESS_SEND);
    ...
    if (v->transport == IB_TRANSPORT_RC) {
        if (v->padding == RDMA_ONE_SIDED) {
            ...
            if ((mpi_errno = MRAILI_Handle_one_sided_completions(v)) !=
                MPI_SUCCESS) {
        ...
        if (vc->free_vc) {
            if (vc->mrail.rails[v->rail].send_wqes_avail ==
                ...
                MPIR_Memset(vc, 0, sizeof(MPIDI_VC_t));
                MPL_free(vc);
        ...
        if (v->eager) {
            ...
            if (vc->mrail.outstanding_eager_vbufs < rdma_coalesce_threshold) {
        ...
        if (v->padding == RPUT_VBUF_FLAG) {
            req = (MPIR_Request *)v->sreq;

            PRINT_DEBUG(DEBUG_RNDV_verbose,
                        "Processing RPUT completion "
                        "req: %p, protocol: %d, local: %d, remote: %d\n",
                        req, req->mrail.protocol, req->mrail.local_complete,
                        req->mrail.remote_complete);
            ...
            if (mvp_MPIDI_CH3I_RDMA_Process.has_hsam) {
                ...
                req->mrail.stripe_finish_time[v->rail] = time_taken;
            }
#endif /*_ENABLE_HSAM_*/
#ifdef _ENABLE_CUDA_
            if (mvp_enable_device && v->orig_vbuf != NULL) {
        ...
        if (v->padding == RGET_VBUF_FLAG) {
            ...
            if (req->mrail.rndv_buf_sz >
                ...
                MRAILI_RDMA_Get_finish(vc, (MPIR_Request *)v->sreq, v->rail);
            }
    ...
    switch (p->type) {
        ...
        case MPIDI_CH3_PKT_RPUT_FINISH:
            ...
            if (!mvp_enable_device || process_rput_finish)
#endif
            {
                ...
                PRINT_DEBUG(DEBUG_RNDV_verbose,
                            "Processing RPUT FIN completion "
                            "req: %p, protocol: %d, local: %d, remote: %d\n",
                            req, req->mrail.protocol, req->mrail.local_complete,
                            req->mrail.remote_complete);
                ...
                if (MPIDI_CH3I_MRAIL_Finish_request(req)) {
                    ...
                    if (mvp_MPIDI_CH3I_RDMA_Process.has_hsam &&
                        ((req->mrail.rndv_buf_sz >
                          rdma_large_msg_rail_sharing_threshold))) {
                        /* Adjust the weights of different paths according to
                         * the timings obtained for the stripes */
        ...
        case MPIDI_CH3_PKT_MCST_INIT:
            PRINT_DEBUG(DEBUG_MCST_verbose > 4, "mcast send completion\n");
        ...
        case MPIDI_CH3_PKT_ZCOPY_ACK:
            PRINT_DEBUG(DEBUG_SEND_verbose > 1, "[process send] get %d\n",
                        p->type);
            /* In MPIDI_CH3I_MRAILI_Eager_send, we save the request object in
             * the vbuf in case send operation could not be completed due to
             * lack of WQEs. In this scenario, we should call
             * MPIDI_CH3U_Handle_send_req upon sucessful completion of the send
             * operation. This assert is to make sure that we do not release a
             * vbuf which needs additional handling without doing it. */
            if (v->sreq != NULL) {
                PRINT_ERROR(
                    "Releasing vbuf for packet %s without handling sreq\n",
                    MPIDI_CH3_Pkt_type_to_string[p->type]);
            }
            MPIR_Assert(v->sreq == NULL);
            if (v->padding == NORMAL_VBUF_FLAG) {
                MRAILI_Release_vbuf(v);
            } else {
                v->padding = FREE_FLAG;
        ...
        default:
            ...
            ibv_va_error_abort(IBV_STATUS_ERR,
                               ...
                               "MRAILI_Process_send MPIDI_CH3_PKT_FOP: %d",
                               p->type, MPIDI_CH3_PKT_FOP);
    }
    DEBUG_PRINT("return from process send\n");

fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MRAILI_PROCESS_SEND);
    return mpi_errno;
...
void MRAILI_Send_noop(MPIDI_VC_t *c, int rail)
{
    ...
    vbuf *v = get_vbuf_by_offset(MVP_RECV_VBUF_POOL_OFFSET);

    MPIDI_CH3I_MRAILI_Pkt_noop *p = (MPIDI_CH3I_MRAILI_Pkt_noop *)v->pheader;

    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MRAILI_SEND_NOOP);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MRAILI_SEND_NOOP);

    p->type = MPIDI_CH3_PKT_NOOP;
    vbuf_init_send(v, sizeof(MPIDI_CH3I_MRAILI_Pkt_noop), rail);
    post_send(c, v, rail);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MRAILI_SEND_NOOP);
}

int MRAILI_Send_noop_if_needed(MPIDI_VC_t *vc, int rail)
{
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MRAILI_SEND_NOOP_IF_NEEDED);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MRAILI_SEND_NOOP_IF_NEEDED);
    ...
    if (vc->mrail.srp.credits[rail].local_credit >=
        ...
        (vc->mrail.srp.credits[rail].remote_cc <= rdma_credit_preserve &&
         vc->mrail.srp.credits[rail].local_credit >=
             rdma_credit_notify_threshold)) {
        MRAILI_Send_noop(vc, rail);
    }
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MRAILI_SEND_NOOP_IF_NEEDED);
    return MPI_SUCCESS;
}

void MRAILI_RDMA_Get(MPIDI_VC_t *vc, vbuf *v, char *local_addr, uint32_t lkey,
                     char *remote_addr, uint32_t rkey, int nbytes, int rail)
{
    char cq_overflow = 0;

    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MRAILI_RDMA_GET);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MRAILI_RDMA_GET);
    ...
    vbuf_init_rget(v, (void *)local_addr, lkey, remote_addr, rkey, nbytes,
                   rail);
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MRAILI_RDMA_GET);
    return;
}

void MRAILI_RDMA_Put(MPIDI_VC_t *vc, vbuf *v, char *local_addr, uint32_t lkey,
                     char *remote_addr, uint32_t rkey, int nbytes, int rail)
{
    char cq_overflow = 0;

    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MRAILI_RDMA_PUT);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MRAILI_RDMA_PUT);

    DEBUG_PRINT("MRAILI_RDMA_Put: RDMA write, "
                "remote addr %p, rkey %p, nbytes %d, hca %d\n",
                remote_addr, rkey, nbytes, vc->mrail.rails[rail].hca_index);

    vbuf_init_rput(v, (void *)local_addr, lkey, remote_addr, rkey, nbytes,
                   rail);
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MRAILI_RDMA_PUT);
    return;
}
...
void vbuf_address_reply_send(MPIDI_VC_t *vc, uint8_t data)
{
    ...
    vbuf *v = NULL;
    GET_VBUF_BY_OFFSET_WITHOUT_LOCK(v, MVP_SMALL_DATA_VBUF_POOL_OFFSET);
    MPIDI_CH3_Pkt_address_reply_t *p =
        (MPIDI_CH3_Pkt_address_reply_t *)v->pheader;
...
int mvp_shm_coll_post_send(vbuf *v, int rail, MPIDI_VC_t *vc)
{
    char cq_overflow = 0;
    int mpi_errno = MPI_SUCCESS;
                                    ...
                                    MPIDI_VC_t *vc)
{
    ...
    XRC_FILL_SRQN_FIX_CONN(v, vc, rail);
    mvp_shm_coll_post_send(v, rail, vc);
    ...
    for (i = 0; i < rdma_num_hcas; i++) {
        if (mem_handle[i] != NULL) {
            if (deregister_memory(mem_handle[i])) {
                ibv_error_abort(IBV_RETURN_ERR, "deregistration failed\n");
            }
            mem_handle[i] = NULL;
        }
    }
