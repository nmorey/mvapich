...
static int hca_check(int rank)
{
    ...
    for (hcan = 0; hcan < rdma_num_hcas; hcan++) {
        ...
        for (p=0; p<ca.numports; p++) {
            ...
            if (port!=0) {
                printf( "<hcaport rank='%d' hca='%d' port='%d' state='%s
        (%d)'/>\n", rank, hcan, port->portnum, (uint)port->state <= 4 ?
        port_state_str[port->state] : "???", (uint)port->state);
            }
        }
        umad_release_ca( &ca );
       */
