...
static inline int MRAILI_Coalesce_ok(MPIDI_VC_t *vc, int rail)
{
    if (unlikely(
            ...
            (mvp_MPIDI_CH3I_RDMA_Process.has_srq ||
...
static inline void MRAILI_Ext_sendq_enqueue(MPIDI_VC_t *c, int rail, vbuf *v)
{
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MRAILI_EXT_SENDQ_ENQUEUE);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MRAILI_EXT_SENDQ_ENQUEUE);
    ...
    PRINT_DEBUG(DEBUG_SEND_verbose > 1,
                "[ibv_send] enqueue, head %p, tail %p\n",
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MRAILI_EXT_SENDQ_ENQUEUE);
}
...
static inline void MRAILI_Ext_sendq_send(MPIDI_VC_t *c, int rail)
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MRAILI_EXT_SENDQ_SEND);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MRAILI_EXT_SENDQ_SEND);

#ifdef _ENABLE_XRC_
    MPIR_Assert(!USE_XRC || VC_XST_ISUNSET(c, XF_INDIRECT_CONN));
#endif

    CHECK_CQ_OVERFLOW(cq_overflow, c, rail);

    while (c->mrail.rails[rail].send_wqes_avail && !cq_overflow &&
           c->mrail.rails[rail].ext_sendq_head) {
    ...
    PRINT_DEBUG(DEBUG_SEND_verbose > 1,
                "[ibv_send] dequeue, head %p, tail %p\n",
                c->mrail.rails[rail].ext_sendq_head,
                c->mrail.rails[rail].ext_sendq_tail);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MRAILI_EXT_SENDQ_SEND);
}
...
static inline vbuf *MRAILI_Get_Vbuf(MPIDI_VC_t *vc, size_t pkt_len)
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MRAILI_GET_VBUF);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MRAILI_GET_VBUF);
    ...
    if (unlikely(NULL != vc->mrail.coalesce_vbuf)) {
        ...
        {
            coalesc_buf_size =
                ((vbuf_pool_t *)vc->mrail.coalesce_vbuf->pool_index)->buf_size;
        }
    ...
    if (likely(NULL == temp_v)) {
        ...
        PRINT_DEBUG(DEBUG_SEND_verbose > 1,
                    "incrementing the outstanding eager vbufs: eager %d\n",
    ...
    MPIR_Assert(temp_v != NULL);

    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MRAILI_GET_VBUF);
    return temp_v;
}
                                                  ...
                                                  int *num_bytes_ptr)
{
    ...
    *num_bytes_ptr = 0;
    ...
    PRINT_DEBUG(DEBUG_SEND_verbose > 1,
                "Header info, tag %d, rank %d, context_id %d\n",
                header->match.parts.tag, header->match.parts.rank,
                header->match.parts.context_id);
#ifndef MVP_DISABLE_HEADER_CACHING
    ...
    if ((header->type == MPIDI_CH3_PKT_EAGER_SEND) &&
        ...
        if (header->sender_req_id == cached->sender_req_id) {
            ...
            /*
               DEBUG_PRINT
               ("[send: fill buf], head cached, head_flag %p, vstart %p, length
               %d", &v->head_flag, vstart, len -
            ...
            if (iov[0].iov_len - sizeof(MPIDI_CH3_Pkt_eager_send_t))
                MPIR_Memcpy(data_buf,
                            (void *)((uintptr_t)iov[0].iov_base +
                                     sizeof(MPIDI_CH3_Pkt_eager_send_t)),
                            iov[0].iov_len -
                                sizeof(MPIDI_CH3_Pkt_eager_send_t));

            data_buf = (void *)((uintptr_t)data_buf + iov[0].iov_len -
                                sizeof(MPIDI_CH3_Pkt_eager_send_t));
            ...
            *num_bytes_ptr += sizeof(MPIDI_CH3I_MRAILI_Pkt_fast_eager);
            ...
            DEBUG_PRINT("[send: fill buf], head cached, head_flag %p, vstart "
                        "%p, length %d\n",
                        &v->head_flag, vstart,
                        len - sizeof(MPIDI_CH3_Pkt_eager_send_t) +
    ...
    {
        vstart = v->buffer;
        DEBUG_PRINT("[send: fill buf], head not cached, v %p, vstart %p, "
                    "length %d, header size %d\n",
                    v, vstart, len, iov[0].iov_len);
        MPIR_Memcpy(vstart, header, iov[0].iov_len);
#ifndef MVP_DISABLE_HEADER_CACHING
        if (header->type == MPIDI_CH3_PKT_EAGER_SEND &&
            ((len - sizeof(MPIDI_CH3_Pkt_eager_send_t)) <=
             MAX_SIZE_WITH_HEADER_CACHING)) {
            MPIR_Memcpy(cached, header, sizeof(MPIDI_CH3_Pkt_eager_send_t));
            ++vc->mrail.rfp.cached_miss;
        }
#endif
        data_buf = (void *)((unsigned long)vstart + iov[0].iov_len);
...
#ifdef _ENABLE_CUDA_
    if (rdma_enable_cuda && n_iov > 1 && is_device_buffer(iov[1].iov_base)) {
        /* in the case of GPU buffers, there is only one data iov, if data is
         * non-contiguous it should have been packed before this */
        MPIR_Assert(n_iov == 2);

        MVP_MPIDI_Memcpy_Device(data_buf, iov[1].iov_base, iov[1].iov_len,
                                cudaMemcpyDeviceToHost);
        *num_bytes_ptr += iov[1].iov_len;
        avail -= iov[1].iov_len;
    ...
    {
        for (i = 1; i < n_iov; i++) {
            if (avail >= iov[i].iov_len) {
                MPIR_Memcpy(data_buf, iov[i].iov_base, iov[i].iov_len);
                data_buf = (void *)((unsigned long)data_buf + iov[i].iov_len);
                *num_bytes_ptr += iov[i].iov_len;
                avail -= iov[i].iov_len;
            } else if (avail > 0) {
                MPIR_Memcpy(data_buf, iov[i].iov_base, avail);
    ...
    PRINT_DEBUG(DEBUG_SEND_verbose > 1,
                "[send: fill buf], num bytes copied %d\n", *num_bytes_ptr);
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIDI_CH3I_MRAILI_FAST_RDMA_FILL_START_BUF);
    return MPI_SUCCESS;
}
                                                            ...
                                                            vbuf **vbuf_handle)
{
    ...
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIDI_CH3I_MRAILI_FAST_RDMA_SEND_COMPLETE);
    ...
    PRINT_DEBUG(
        DEBUG_SEND_verbose > 1,
        "[send: rdma_send] local vbuf %p, remote start %p, align size %d\n", v,
        rstart, post_len);
    ...
    *((VBUF_FLAG_TYPE *)(v->buffer + post_len)) = flag;
    ...
    PRINT_DEBUG(DEBUG_SEND_verbose > 1,
                "incrementing the outstanding eager vbufs: RFP %d\n",
                vc->mrail.outstanding_eager_vbufs);

    /* generate a completion, following statements should have been executed
     * during initialization */
    post_len += VBUF_FAST_RDMA_EXTRA_BYTES;

    PRINT_DEBUG(
        DEBUG_SEND_verbose > 1,
        "[send: rdma_send] lkey %x, rkey %p, len %d, flag %lu\n",
        vc->mrail.rfp.RDMA_send_buf_mr[vc->mrail.rails[rail].hca_index]->lkey,
        vc->mrail.rfp.RDMA_remote_buf_rkey, post_len, *v->head_flag);

    VBUF_SET_RDMA_ADDR_KEY(
        v, post_len, v->head_flag,
        vc->mrail.rfp.RDMA_send_buf_mr[vc->mrail.rails[rail].hca_index]->lkey,
        rstart,
    ...
    if (likely(vc->mrail.rails[rail].send_wqes_avail > 0 && !cq_overflow)) {
        ...
        *vbuf_handle = v;
        ...
        PRINT_DEBUG(
            DEBUG_SEND_verbose > 1,
            "[send: rdma_send] Warning! no send wqe or send cq available\n");
        MRAILI_Ext_sendq_enqueue(vc, rail, v);
        *vbuf_handle = v;
        return MPI_MRAIL_MSG_QUEUED;
    }

    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIDI_CH3I_MRAILI_FAST_RDMA_SEND_COMPLETE);
    return MPI_SUCCESS;
}
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int MPIDI_CH3I_MRAILI_Fast_rdma_ok(MPIDI_VC_t *vc, intptr_t len)
{
    int i = 0;

    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIDI_CH3I_MRAILI_FAST_RDMA_OK);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIDI_CH3I_MRAILI_FAST_RDMA_OK);
    ...
    if (unlikely(num_rdma_buffer < 2 ||
                 ...
                 vc->mrail.rfp.RDMA_send_buf[vc->mrail.rfp.phead_RDMA_send]
                         .padding == BUSY_FLAG ||
                 MRAILI_Coalesce_ok(vc, 0))) /* We can only coalesce with
                                                send/recv. */
    {
        MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIDI_CH3I_MRAILI_FAST_RDMA_OK);
        return 0;
    }
...
static inline int post_srq_send(MPIDI_VC_t *vc, vbuf *v, int rail)
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_POST_SRQ_SEND);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_POST_SRQ_SEND);
    ...
    MPIR_Assert(v->transport == IB_TRANSPORT_RC);
    ...
    if (likely(vc->mrail.rails[rail].send_wqes_avail > 0 && !cq_overflow)) {
        ...
        MRAILI_Ext_sendq_enqueue(vc, rail, v);
        MPIR_FUNC_TERSE_EXIT(MPID_STATE_POST_SRQ_SEND);
        return MPI_MRAIL_MSG_QUEUED;
    }
...
static inline int post_nosrq_send(MPIDI_VC_t *vc, vbuf *v, int rail)
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_POST_SEND);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_POST_SEND);
    PRINT_DEBUG(DEBUG_SEND_verbose > 1,
                ...
                vc->mrail.rails[rail].send_wqes_avail, v->desc.sg_entry.length);
    ...
    if (vc->mrail.srp.credits[rail].remote_credit > 0 ||
        ...
        if (likely(vc->mrail.rails[rail].send_wqes_avail > 0 && !cq_overflow)) {
            ...
            MRAILI_Ext_sendq_enqueue(vc, rail, v);
            MPIR_FUNC_TERSE_EXIT(MPID_STATE_POST_SEND);
            return MPI_MRAIL_MSG_QUEUED;
        }
    } else {
...
static inline int post_hybrid_send(MPIDI_VC_t *vc, vbuf *v, int rail)
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_POST_HYBRID_SEND);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_POST_HYBRID_SEND);
    ...
    switch (v->transport) {
        ...
        default:
            PRINT_DEBUG(DEBUG_UD_verbose > 1,
                        "Invalid IB transport protocol\n");
            return -1;
    }

    MPIR_FUNC_TERSE_EXIT(MPID_STATE_POST_HYBRID_SEND);
    return 0;
}
#endif /* _ENABLE_UD_ */
...
#endif
    {
        if (likely(mvp_use_post_srq_send)) {
                                               ...
                                               vbuf **buf_handle)
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIDI_CH3I_MRAILI_EAGER_SEND);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIDI_CH3I_MRAILI_EAGER_SEND);

    /* first we check if we can take the RDMA FP */
    if (likely(MPIDI_CH3I_MRAILI_Fast_rdma_ok(vc, pkt_len))) {
        *num_bytes_ptr = pkt_len;
        MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIDI_CH3I_MRAILI_EAGER_SEND);
        return MPIDI_CH3I_MRAILI_Fast_rdma_send_complete(
            vc, iov, n_iov, num_bytes_ptr, buf_handle);
    }
    ...
    PRINT_DEBUG(DEBUG_SEND_verbose > 1,
                "[eager send]vbuf addr %p, buffer: %p\n", v, v->buffer);
    *num_bytes_ptr = MRAILI_Fill_start_buffer(v, iov, n_iov);
    ...
    if (vc->ch.state != MPIDI_CH3I_VC_STATE_IDLE) {
         ...
         * (MPIDI_CH3I_MRAILI_Pkt_comm_header *) v->pheader;*/
        MPIDI_CH3I_CR_msg_log_queue_entry_t *entry;
        if (rdma_use_coalesce) {
            entry = MSG_LOG_QUEUE_TAIL(vc);
            if (entry->buf == v) /*since the vbuf is already filled, no need to
                                    queue it again*/
            {
    ...
    if (likely(vc->mrail.coalesce_vbuf != v)) {
        PRINT_DEBUG(DEBUG_SEND_verbose > 1,
                    "[eager send] len %d, selected rail hca %d, rail %d\n",
                    *num_bytes_ptr, vc->mrail.rails[v->rail].hca_index,
                    v->rail);
        vbuf_init_send(v, *num_bytes_ptr, v->rail);
        post_send(vc, v, v->rail);
    } else {
        MPIDI_CH3I_MRAILI_Pkt_comm_header *p =
            (MPIDI_CH3I_MRAILI_Pkt_comm_header *)(v->buffer + v->content_size -
                                                  *num_bytes_ptr);
...
#undef FUNCNAME
#define FUNCNAME mvp_eager_fast_send
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int mvp_eager_fast_send(MPIDI_VC_t *vc, const void *buf,
                                      ...
                                      MPIR_Request **sreq_p)
{
    ...
    rail = MRAILI_Send_select_rail(vc, data_sz +
                                           sizeof(MPIDI_CH3_Pkt_eager_send_t));
...
#undef FUNCNAME
#define FUNCNAME mvp_eager_fast_coalesce_send
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int mvp_eager_fast_coalesce_send(MPIDI_VC_t *vc, const void *buf,
                                               intptr_t data_sz, int rank,
                                               ...
                                               MPIR_Request **sreq_p)
{
    ...
    /* Copy data */
    ptr = (void *)v->buffer + v->content_size +
          sizeof(MPIDI_CH3_Pkt_eager_send_t);
...
#undef FUNCNAME
#define FUNCNAME mvp_eager_fast_rfp_send
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int mvp_eager_fast_rfp_send(MPIDI_VC_t *vc, const void *buf,
                                          intptr_t data_sz, int rank, int tag,
                                          MPIR_Comm *comm, int context_offset,
                                          MPIR_Request **sreq_p)
{
    ...
    if (unlikely(!MPIDI_CH3I_MRAILI_Fast_rdma_ok(vc, data_sz +
                                                         sizeof(*eager_pkt)))) {
        if (likely(rdma_use_coalesce)) {
            return mvp_eager_fast_coalesce_send(vc, buf, data_sz, rank, tag,
                                                comm, context_offset, sreq_p);
        } else {
            return mvp_eager_fast_send(vc, buf, data_sz, rank, tag, comm,
                                       context_offset, sreq_p);
        }
    }
    ...
    /* Compute size of pkt */
    num_bytes_ptr = iov[0].iov_len + iov[1].iov_len;
...
#undef FUNCNAME
#define FUNCNAME mvp_eager_fast_wrapper
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int mvp_eager_fast_wrapper(MPIDI_VC_t *vc, const void *buf,
                                         intptr_t data_sz, int rank, int tag,
                                         MPIR_Comm *comm, int context_offset,
                                         MPIR_Request **sreq_p)
{
    if (likely(vc->smp.local_nodes < 0)) {
        ...
        if (vc->use_eager_fast_rfp_fn) {
            return mvp_eager_fast_rfp_send(vc, buf, data_sz, rank, tag, comm,
                                           context_offset, sreq_p);
        ...
        /* Intra-node scenario */
        return mvp_smp_fast_write_contig(vc, buf, data_sz, rank, tag, comm,
                                         context_offset, sreq_p);
    }
}
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int MPIDI_CH3I_MRAIL_Parse_header(MPIDI_VC_t *vc, vbuf *v,
                                                void **pkt, int *header_size)
{
    void *vstart;
    ...
    int ret;
    MPIR_FUNC_TERSE_STATE_DECL(MPIDI_STATE_CH3I_MRAIL_PARSE_HEADER);
    MPIR_FUNC_TERSE_ENTER(MPIDI_STATE_CH3I_MRAIL_PARSE_HEADER);
    ...
    *header_size = MPIDI_CH3_Pkt_size_index[header->type];
    ...
    switch (header->type) {
        ...
        case (MPIDI_CH3_PKT_FAST_EAGER_SEND_WITH_REQ): {
            ...
            MPIDI_CH3_Pkt_eager_send_t *eager_header =
                (MPIDI_CH3_Pkt_eager_send_t *)vc->mrail.rfp.cached_incoming;
            ...
            if (MPIDI_CH3_PKT_FAST_EAGER_SEND == header->type) {
                ...
                *header_size =
                ...
                eager_header->sender_req_id =
                    ((MPIDI_CH3I_MRAILI_Pkt_fast_eager_with_req *)vstart)
                        ->sender_req_id;
            }
        ...
        case (MPIDI_CH3_PKT_EAGER_SEND): {
            ...
            if (v->padding != NORMAL_VBUF_FLAG &&
                ((v->content_size - sizeof(MPIDI_CH3_Pkt_eager_send_t)) <=
                 MAX_SIZE_WITH_HEADER_CACHING)) {
                /* Only cache header if the packet is from RdMA path
                 * XXXX: what is R3_FLAG?
                 */
                MPIR_Memcpy((vc->mrail.rfp.cached_incoming), vstart,
                            sizeof(MPIDI_CH3_Pkt_eager_send_t));
            }
#endif
    ...
    PRINT_DEBUG(DEBUG_CHM_verbose > 1,
                "Before set credit, vc: %p, v->rail: %d, "
                "pkt: %p, pheader: %p\n",
                vc, v->rail, pkt, v->pheader);

    SET_CREDIT((&(((MPIDI_CH3_Pkt_t *)(*pkt))->eager_send)), vc, (v->rail),
               v->transport);
    ...
    if (vc->mrail.state & MRAILI_RC_CONNECTED &&
        ...
        vc->mrail.rfp.RDMA_recv_buf == NULL && num_rdma_buffer &&
        !vc->mrail.rfp.rdma_failed) {
        if ((mvp_MPIDI_CH3I_RDMA_Process.polling_group_size +
             rdma_pending_conn_request) < rdma_polling_set_limit) {
            vc->mrail.rfp.eager_start_cnt++;
            if (rdma_polling_set_threshold < vc->mrail.rfp.eager_start_cnt) {
                MPICM_lock();
#ifdef _ENABLE_XRC_
                if (xrc_rdmafp_init && USE_XRC &&
                    VC_XST_ISUNSET(vc, XF_SEND_IDLE)) {
                    if (VC_XSTS_ISUNSET(vc, XF_START_RDMAFP | XF_CONN_CLOSING |
                                                XF_DPM_INI)) {
                        PRINT_DEBUG(DEBUG_XRC_verbose > 0,
                                    "Trying to FP to %d st: %d xr: 0x%08x",
                                    vc->pg_rank, vc->ch.state,
                                    vc->ch.xrc_flags);
                        ...
                        goto fn_exit;
                           ...
                           (xrc_rdmafp_init &&
                            VC_XSTS_ISUNSET(vc, XF_DPM_INI | XF_CONN_CLOSING |
                                                    XF_START_RDMAFP) &&
...
fn_exit:
    MPIR_FUNC_TERSE_EXIT(MPIDI_STATE_CH3I_MRAIL_PARSE_HEADER);
    return mpi_errno;
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int MPIDI_CH3I_MRAIL_Fill_Request(MPIR_Request *req, vbuf *v,
                                                int header_size, int *nb)
{
    ...
    void *data_buf;
    int i;
    MPIR_FUNC_TERSE_STATE_DECL(MPIDI_STATE_CH3I_MRAIL_FILL_REQUEST);
    MPIR_FUNC_TERSE_ENTER(MPIDI_STATE_CH3I_MRAIL_FILL_REQUEST);
...
#ifdef _ENABLE_CUDA_
    if (rdma_enable_cuda && is_device_buffer(iov[0].iov_base)) {
        *nb = 0;
        MPIR_Assert(req->dev.iov_offset == 0 && n_iov == 1);
        MVP_MPIDI_Memcpy_Device(iov[0].iov_base, data_buf, iov[0].iov_len,
                                cudaMemcpyHostToDevice);
        *nb += iov[0].iov_len;
        len_avail -= iov[0].iov_len;
    } else {
#endif

        *nb = 0;
        for (i = req->dev.iov_offset; i < n_iov; i++) {
            if (len_avail >= (intptr_t)iov[i].iov_len && iov[i].iov_len != 0) {
    ...
    DEBUG_PRINT("[recv:fill request] about to return form request, nb %d\n",
                *nb);
    MPIR_FUNC_TERSE_EXIT(MPIDI_STATE_CH3I_MRAIL_FILL_REQUEST);
    return MPI_SUCCESS;
}
