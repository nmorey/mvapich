...
vbuf_pool_t mvp_srq_repost_pool;
volatile int rdma_num_vbuf_pools = 0;
int mvp_waiting_for_vbuf = 0;
...
vbuf_pool_t mvp_ud_srq_repost_pool;
...
void mvp_print_vbuf_usage_usage()
{
    ...
    for (i = 0; i < rdma_num_vbuf_pools; i++) {
        ...
        PRINT_INFO(DEBUG_VBUF_verbose > 0,
                   "[Pool: %d, Size:%lu] num_bufs:%u, tot_mem:%ld kB,"
                   " num_get = %ld, num_freed = %ld\n",
                   i, (long unsigned int)rdma_vbuf_pools[i].buf_size,
                   rdma_vbuf_pools[i].num_allocated, (size / 1024),
    ...
    PRINT_INFO(DEBUG_VBUF_verbose,
               "Reposted VBUF stats: num_freed = %ld, num_get = %ld\n",
               mvp_srq_repost_pool.num_freed, mvp_srq_repost_pool.num_get);
    ...
    for (i = 0; i < rdma_num_ud_vbuf_pools; i++) {
        ...
        PRINT_INFO(DEBUG_VBUF_verbose > 0,
                   "[UD Pool: %d, Size:%lu] num_bufs:%u, tot_mem:%ld kB,"
                   " num_get = %ld, num_freed = %ld\n",
                   i, (long unsigned int)rdma_ud_vbuf_pools[i].buf_size,
                   rdma_ud_vbuf_pools[i].num_allocated, (size / 1024),
                   rdma_ud_vbuf_pools[i].num_get,
                   rdma_ud_vbuf_pools[i].num_freed);
    }
...
int init_vbuf_lock(void)
{
    ...
    if (pthread_spin_init(&vbuf_lock, 0)) {
        mpi_errno =
            MPIR_Err_create_code(mpi_errno, MPIR_ERR_FATAL, "init_vbuf_lock",
                                 __LINE__, MPI_ERR_OTHER, "**fail", "%s: %s",
                                 "pthread_spin_init", strerror(errno));
    }
...
#if defined(RDMA_CM)
        || mvp_MPIDI_CH3I_RDMA_Process.use_rdma_cm_on_demand
...
void deallocate_vbuf_region(void)
{
    ...
    if (mvp_enable_device) {
        MVP_MPIDI_Device_CtxGetCurrent(&active_context);
    }
#endif
    ...
    for (i = 0; i < rdma_num_vbuf_pools; i++) {
        ...
        while (curr) {
            ...
            if (active_context != NULL &&
                (rdma_vbuf_pools[i].index == MVP_CUDA_VBUF_POOL_OFFSET ||
                 (rdma_eager_devicehost_reg &&
                  rdma_vbuf_pools[i].index <= MVP_RECV_VBUF_POOL_OFFSET))) {
...
#ifdef _ENABLE_CUDA_
            if (active_context != NULL &&
                (rdma_ud_vbuf_pools[i].index == MVP_CUDA_VBUF_POOL_OFFSET ||
                 (rdma_eager_devicehost_reg &&
                  rdma_ud_vbuf_pools[i].index <= MVP_RECV_VBUF_POOL_OFFSET))) {
                ibv_device_unregister(curr->malloc_buf_start);
            }
#endif
            if (rdma_enable_hugepage && curr->vbuf_shmid >= 0) {
...
fn_fail:
    ...
    if (rdma_enable_hugepage >= 2) {
        fprintf(stderr,
                "[%d] Failed to allocate buffer from huge pages. "
...
static inline int reregister_vbuf_pool(vbuf_pool_t *rdma_vbuf_pool)
{
    ...
    PRINT_DEBUG(DEBUG_CR_verbose > 0,
                "index = %u; initial_count = %u;"
    ...
    while (region) {
...
#ifdef _ENABLE_CUDA_
        if (mvp_enable_device &&
            (rdma_vbuf_pool->index == MVP_CUDA_VBUF_POOL_OFFSET ||
             (rdma_eager_devicehost_reg &&
              rdma_vbuf_pool->index <= MVP_RECV_VBUF_POOL_OFFSET))) {
        ...
        for (i = 0; i < rdma_num_hcas; ++i) {
            if (g_atomics_support) {
                region->mem_handle[i] = ibv_ops.reg_mr(
                    ptag_save[i], vbuf_buffer, nvbufs * buf_size,
...
int allocate_vbuf_pool(vbuf_pool_t *rdma_vbuf_pool, int nvbufs)
{
    ...
    PRINT_DEBUG(
        DEBUG_VBUF_verbose,
        "Allocating a new vbuf region with %d VBUFs of size %d. Current VBUFs "
        "in pool: %d, Max VBUFs allowed for pool: %d, Pool Index = %d\n",
        nvbufs, rdma_vbuf_pool->buf_size, rdma_vbuf_pool->num_allocated,
        rdma_vbuf_pool->max_num_buf, rdma_vbuf_pool->index);
    ...
    if (rdma_vbuf_pool->max_num_buf > 0) {
        ...
        if (nvbufs <= 0) {
            PRINT_DEBUG(DEBUG_VBUF_verbose,
                        "max vbuf pool size reached for size : %d \n",
                        buf_size);
            mpi_errno = -1;
            return mpi_errno;
        }
    ...
    if (rdma_enable_hugepage == 0 || result != 0) {
        result = 0;
        vbuf_buffer = (void *)MPL_aligned_alloc(
            alignment_dma, nvbufs * buf_size, MPL_MEM_OTHER);
        region->vbuf_shmid = -1;
    }
    ...
    if (rdma_enable_hugepage) {
        num_bufs = nvbufs;
        result = alloc_hugepage_region(&region->vbuf_struct_shmid, &vbuf_struct,
                                       &num_bufs, sizeof(struct vbuf));
    }
    /* do posix_memalign if enable hugepage disabled or failed */
    if (rdma_enable_hugepage == 0 || result != 0) {
        result = 0;
        vbuf_struct = (void **)MPL_aligned_alloc(
            alignment_vbuf, nvbufs * sizeof(struct vbuf), MPL_MEM_OTHER);
    ...
    MPIR_T_PVAR_COUNTER_INC(MVP, mvp_vbuf_allocated, nvbufs);
    MPIR_T_PVAR_LEVEL_INC(MVP, mvp_vbuf_available, nvbufs);

    region->malloc_start = vbuf_struct;
    region->malloc_buf_start = vbuf_buffer;
    region->malloc_end =
        (void *)((char *)vbuf_struct + nvbufs * sizeof(struct vbuf));
    region->malloc_buf_end = (void *)((char *)vbuf_buffer + nvbufs * buf_size);
    ...
    PRINT_DEBUG(DEBUG_VBUF_verbose,
                "VBUF region of size %d with %d VBUFs, Total: %d, Free: %d, "
                "Cumulative Freed: %ld, Cumulative Got: %ld\n",
                rdma_vbuf_pool->buf_size, nvbufs, rdma_vbuf_pool->num_allocated,
                rdma_vbuf_pool->num_free, rdma_vbuf_pool->num_freed,
    ...
    for (i = 0; i < nvbufs; ++i) {
        ...
        cur->head_flag =
            (VBUF_FLAG_TYPE *)((char *)vbuf_buffer + (i + 1) * buf_size -
                               sizeof(*cur->head_flag));
        cur->buffer = (unsigned char *)((char *)vbuf_buffer + i * buf_size);
        cur->eager = 0;
        cur->content_size = 0;
        cur->coalesce = 0;
    }
...
void register_cuda_vbuf_regions()
{
    ...
    MPIR_Assert(mvp_device_dynamic_init == 1);

    for (i = 0; i < rdma_num_vbuf_pools; i++) {
        rdma_vbuf_pool = &rdma_vbuf_pools[i];
        if (rdma_vbuf_pool->index == MVP_CUDA_VBUF_POOL_OFFSET ||
            (rdma_eager_devicehost_reg &&
             rdma_vbuf_pool->index <= MVP_RECV_VBUF_POOL_OFFSET)) {
            region = rdma_vbuf_pool->region_head;
            while (region != NULL) {
                ibv_device_register(region->malloc_buf_start,
                                    (uint64_t)region->malloc_buf_end -
                                        (uint64_t)region->malloc_buf_start);
                region = region->next;
            }
        }
    }
...
static inline int size_to_offset(size_t message_size)
{
    ...
    for (i = MVP_SMALL_DATA_VBUF_POOL_OFFSET; i < rdma_num_vbuf_pools; i++) {
...
vbuf *get_vbuf(size_t message_size)
{
    ...
    if (unlikely(NULL == rdma_vbuf_pool->free_head)) {
        if (allocate_vbuf_pool(rdma_vbuf_pool, rdma_vbuf_pool->incr_count) !=
            0) {
            ibv_va_error_abort(GEN_EXIT_ERR, "vbuf pool allocation failed");
        }
    }

    MVP_GET_AND_INIT_RC_VBUF(v, rdma_vbuf_pool);
...
void release_vbuf(vbuf *v)
{
    ...
    MPIR_Assert(v->padding == NORMAL_VBUF_FLAG ||
                v->padding == RPUT_VBUF_FLAG || v->padding == RGET_VBUF_FLAG ||
                v->padding == RDMA_ONE_SIDED || v->padding == COLL_VBUF_FLAG);

    PRINT_DEBUG(DEBUG_VBUF_verbose > 1,
                "release_vbuf: %p releasing free_head %p padding %d\n", v,
                rdma_vbuf_pool->free_head, v->padding);
    /* Reset VBUF parameters */
    MVP_RESET_VBUF(v);
    /* Release VBUF */
    MVP_RELEASE_VBUF(v, MVP_VBUF_POOL(v));
...
int allocate_vbufs(struct ibv_pd *ptag[])
{
    ...
    if (allocate_vbuf_pools(ptag) != 0) {
        ibv_va_error_abort(GEN_EXIT_ERR, "VBUF region allocation failed.\n")
    }
...
void MRAILI_Release_vbuf(vbuf *v)
{
    ...
    if (v->transport == IB_TRANSPORT_RC) {
        if (likely(MVP_VBUF_POOL(v)->index != MVP_RECV_VBUF_POOL_OFFSET)) {
            /* This is a send buf. We don't need a lock to release it */
            MVP_RELEASE_VBUF_NO_LOCK(v);
        } else if (likely(mvp_MPIDI_CH3I_RDMA_Process.has_srq)) {
            MVP_RELEASE_VBUF(v, &mvp_srq_repost_pool);
            if (mvp_srq_repost_pool.num_free &&
                mvp_srq_repost_pool.num_free >= rdma_credit_preserve) {
        ...
        if (likely(MVP_VBUF_POOL(v)->index != MVP_RECV_UD_VBUF_POOL_OFFSET)) {
            /* This is a send buf. We don't need a lock to release it */
            MPIR_Assert(v->pending_send_polls == 0);
        ...
        {
            /*
             * TODO: eventually we want to be using MVP_REPOST_UD_VBUF_TO_SRQ
...
vbuf *get_ud_vbuf_by_offset(int offset)
{
    ...
    MVP_GET_AND_INIT_UD_VBUF(v, rdma_vbuf_pool);
...
void vbuf_reregister_all()
{
    ...
    for (i = 0; i < rdma_num_hcas; ++i) {
        ptag_save[i] = mvp_MPIDI_CH3I_RDMA_Process.ptag[i];
    }

    for (i = 0; i < rdma_num_vbuf_pools; i++) {
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "Reregistering vbuf pool %d, count = %d\n", i,
                    rdma_vbuf_pools[i].initial_count);
