...
typedef enum { FREE = 2, BUSY } put_get_list_entry_stat;
 ...
 * shandle_tail entries on mvp_connection_t and the list is linked
...
#define RENDEZVOUS_IN_PROGRESS(c, s)                                           \
    {                                                                          \
        ...
        if (NULL == (c)->mrail.sreq_tail) {                                    \
            ...
            ((MPIR_Request *)(c)->mrail.sreq_tail)->mrail.next_inflow =        \
                (void *)(s);                                                   \
        }                                                                      \
...
#define RENDEZVOUS_DONE(c)                                                     \
    {                                                                          \
        ...
        (c)->mrail.sreq_head =                                                 \
            ((MPIR_Request *)(c)->mrail.sreq_head)->mrail.next_inflow;         \
...
#define MPIDI_CH3I_MRAIL_REVERT_RPUT(_sreq)                                    \
    {                                                                          \
        if (MRAILI_PROTOCOL_RGET == (_sreq)->mrail.protocol)                   \
            (_sreq)->mrail.protocol = MRAILI_PROTOCOL_RPUT;                    \
    }
...
#define MPIDI_CH3I_MRAIL_SET_PKT_RNDV_DEVICE(_pkt, _req)                       \
    {                                                                          \
        ...
        (_pkt)->rndv.num_device_blocks =                                       \
            MIN(((_req)->mrail.num_device_blocks -                             \
                 (_req)->mrail.device_block_offset),                           \
                mvp_device_num_rndv_blocks);                                   \
        if ((MRAILI_PROTOCOL_RPUT == (_pkt)->rndv.protocol) ||                 \
            (MRAILI_PROTOCOL_RGET == (_pkt)->rndv.protocol)) {                 \
            for (_i = 0; _i < (_pkt)->rndv.num_device_blocks; _i++) {          \
                (_pkt)->rndv.buffer_addr[_i] =                                 \
                    (_req)->mrail.device_vbuf[_i]->buffer;                     \
                for (_k = 0; _k < rdma_num_hcas; _k++) {                       \
                    (_pkt)->rndv.buffer_rkey[_i][_k] =                         \
                        (_req)                                                 \
                            ->mrail.device_vbuf[_i]                            \
                            ->region->mem_handle[_k]                           \
                            ->rkey;                                            \
                }                                                              \
            }                                                                  \
            (_pkt)->rndv.device_block_offset =                                 \
                (_req)->mrail.device_block_offset;                             \
        }                                                                      \
    }
#endif
...
#define MPIDI_CH3I_MRAIL_SET_PKT_RNDV_DEVICE_IPC(_pkt, _req)                   \
    {                                                                          \
        if (MRAILI_PROTOCOL_RGET == (_pkt)->rndv.protocol &&                   \
...
int mvp_post_srq_buffers(int, int);
