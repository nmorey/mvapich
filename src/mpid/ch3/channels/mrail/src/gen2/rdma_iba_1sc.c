...
#ifdef _ENABLE_XRC_
#define IS_XRC_SEND_IDLE_UNSET(vc_ptr)                                         \
...
#define ONESIDED_RDMA_POST(_v, _vc_ptr, _save_vc, _rail)                       \
    do {                                                                       \
        if (unlikely(!(IS_RC_CONN_ESTABLISHED((_vc_ptr))) ||                   \
            ...
            if (MRAILI_Flush_wqe((_vc_ptr), (_v), (_rail)) !=                  \
                -1) { /* message not enqueued */                               \
                --(_vc_ptr)->mrail.rails[(_rail)].send_wqes_avail;             \
                IBV_POST_SR((_v), (_vc_ptr), (_rail),                          \
                            "Failed to post rma put");                         \
            }                                                                  \
            if ((_save_vc)) {                                                  \
                (_vc_ptr) = (_save_vc);                                        \
            }                                                                  \
...
/*20 is to hold rma_shmid, rank, mvp and other punctuations */
#define SHM_FILENAME_LEN (sizeof(SHM_DIR) + PID_CHAR_LEN + 20)

unsigned short rma_shmid = 100;

typedef enum { SINGLE = 0, STRIPE, REPLICATE } rail_select_t;
...
static int Post_Get_Put_Get_List(MPIR_Win *, intptr_t, dreg_entry *,
                                 MPIDI_VC_t *, vbuf **, void *local_buf[],
                                 void *remote_buf[], void *user_buf[],
                                 intptr_t length, uint32_t lkeys[],
                                 uint32_t rkeys[], rail_select_t rail_select,
                                 int target);

static int Post_Put_Put_Get_List(MPIR_Win *, intptr_t, dreg_entry *,
                                 MPIDI_VC_t *, vbuf **, void *local_buf[],
                                 void *remote_buf[], intptr_t length,
                                 uint32_t lkeys[], uint32_t rkeys[],
                                 rail_select_t rail_select, int target);

static int iba_put(MPIDI_RMA_Op_t *, MPIR_Win *, intptr_t);
static int iba_get(MPIDI_RMA_Op_t *, MPIR_Win *, intptr_t);
static int iba_fetch_and_add(MPIDI_RMA_Op_t *, MPIR_Win *, intptr_t);
static int iba_compare_and_swap(MPIDI_RMA_Op_t *, MPIR_Win *, intptr_t);

int iba_lock(MPIR_Win *, MPIDI_RMA_Op_t *, int);
int iba_unlock(MPIR_Win *, MPIDI_RMA_Op_t *, int);
...
int mvp_allocate_shm_local(int size, void **rnt_buf)
{
    ...
    shm_file = (char *)MPL_malloc(SHM_FILENAME_LEN, MPL_MEM_OTHER);
    if (shm_file == NULL) {
        MPIR_ERR_SETANDSTMT1(mpi_errno, MPI_ERR_OTHER, goto fn_exit, "**fail",
                             "**fail %s", "malloc failed");
    }

    sprintf(shm_file, "%smvp-%d-%d-%d.tmp", SHM_DIR, MPIDI_Process.my_pg_rank,
            getpid(), rma_shmid);
    rma_shmid++;
    ...
    mem_ptr = mmap(0, size, PROT_READ | PROT_WRITE,
    ...
    MPIR_Memset(mem_ptr, 0, size);

    *rnt_buf = mem_ptr;

    /*adding buffer to the list*/
    shm_buffer_ptr =
        (shm_buffer *)MPL_malloc(sizeof(shm_buffer), MPL_MEM_BUFFER);
    MPIR_Memcpy(shm_buffer_ptr->filename, shm_file, SHM_FILENAME_LEN);
...
int mvp_deallocate_shm_local(void *ptr)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    if (curr_ptr == NULL) {
        MPIR_ERR_SETANDSTMT1(mpi_errno, MPI_ERR_OTHER, goto fn_exit, "**fail",
                             "**fail %s", "buffer not found in shm list");
    }
    ...
    if (munmap(curr_ptr->ptr, curr_ptr->size)) {
        ibv_error_abort(
            GEN_EXIT_ERR,
            "rdma_iba_1sc: munmap failed in mvp_deallocate_shm_local");
    }
    close(curr_ptr->fd);
...
int mvp_find_and_deallocate_shm(shm_buffer **list)
{
    int mpi_errno = MPI_SUCCESS;
    shm_buffer *curr_ptr, *prev_ptr;
        ...
        if (curr_ptr->ref_count == 0) {
            ...
            if (munmap(curr_ptr->ptr, curr_ptr->size)) {
                ibv_error_abort(GEN_EXIT_ERR, "rdma_iba_1sc: \
                      mvp_find_and_deallocate_shm_local");
            }
            close(curr_ptr->fd);
...
int mvp_rma_allocate_shm(int size, int g_rank, int *shmem_fd, void **rnt_buf,
                         MPIR_Comm *comm_ptr)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    length = strlen(rma_shmem_dir) +
             7 /*this is to hold MVP and the unsigned short rma_shmid*/
             + PID_CHAR_LEN + 6 /*this is for the hyphens and extension */;

    rma_shmem_file = (char *)MPL_malloc(length, MPL_MEM_OTHER);

    if (g_rank == 0) {
        sprintf(rma_shmem_file, "%smvp-%d-%d.tmp", rma_shmem_dir, getpid(),
                rma_shmid);
        rma_shmid++;
    }
    ...
    *shmem_fd = shm_open(rma_shmem_file, O_CREAT | O_RDWR, S_IRWXU);
    ...
    rma_shared_memory =
        mmap(0, size, PROT_READ | PROT_WRITE, MAP_SHARED, *shmem_fd, 0);
    ...
    MPIR_Memset(rma_shared_memory, 0, size);
    *rnt_buf = rma_shared_memory;

fn_exit:
    mpi_errno1 = MPIR_Barrier_impl(comm_ptr, &errflag);
    if (mpi_errno1 != MPI_SUCCESS) {
        ibv_error_abort(GEN_EXIT_ERR, "rdma_iba_1sc: error calling barrier");
    }
    if (*shmem_fd != -1) {
        shm_unlink(rma_shmem_file);
    }
    if (rma_shmem_file) {
        MPL_free(rma_shmem_file);
    }
    return mpi_errno;
}

void mvp_rma_deallocate_shm(void *addr, int size)
{
    if (munmap(addr, size)) {
        DEBUG_PRINT("munmap failed in mvp_rma_deallocate_shm with error: %d \n",
                    errno);
        ibv_error_abort(GEN_EXIT_ERR, "rdma_iba_1sc: mvp_rma_deallocate_shm");
    }
}
...
void *MPIDI_CH3I_Alloc_mem(size_t size, MPID_Info *info)
{
    ...
    ptr = MPL_malloc(size, MPL_MEM_OTHER);
...
void MPIDI_CH3I_Free_mem(void *ptr)
{
    ...
    if (SMP_INIT) {
        mpi_errno = mvp_deallocate_shm_local(ptr);
        ...
        mvp_find_and_deallocate_shm(&shm_buffer_rlist);
    } else {
        MPL_free(ptr);
    }
}
...
void MPIDI_CH3I_RDMA_start(MPIR_Win *win_ptr, int start_grp_size,
...
int MPIDI_CH3I_RDMA_finish_rma(MPIR_Win *win_ptr)
{
    int mpi_errno = MPI_SUCCESS;
...
inline int MPIDI_CH3I_RDMA_try_rma_op_fast(
    int type, void *origin_addr, int origin_count, MPI_Datatype origin_datatype,
    int target_rank, MPI_Aint target_disp, int target_count,
    MPI_Datatype target_datatype, void *compare_addr, void *result_addr,
    MPIR_Win *win_ptr)
{
    intptr_t complete = 0;
    MPIDI_VC_t *vc = NULL;
    ...
    if (SMP_INIT && vc->smp.local_nodes != -1 &&
        !mvp_MPIDI_CH3I_RDMA_Process.force_ib_atomic) {
        goto fn_exit;
    }

    intptr_t size, target_type_size, origin_type_size;
    ...
    switch (type) {
        case MPIDI_CH3_PKT_PUT: {
            ...
            r_key = win_ptr->win_rkeys[target_rank * rdma_num_hcas + rail];

            remote_addr =
                (char *)win_ptr->basic_info_table[target_rank].base_addr +
            ...
            vbuf_init_rma_put(v, local_addr, l_key, remote_addr, r_key, size,
                              rail);
        ...
        case MPIDI_CH3_PKT_GET: {
            ...
            vbuf_init_rma_get(v, local_addr, l_key, remote_addr, r_key, size,
                              rail);
            ONESIDED_RDMA_POST(v, vc, NULL, rail);
        ...
        case MPIDI_CH3_PKT_FOP: {
            ...
            if (g_atomics_support && aligned && origin_type_size == 8) {
                ++win_ptr->rma_issued;
                GET_VBUF_BY_OFFSET_WITHOUT_LOCK(
                    v, MVP_SMALL_DATA_VBUF_POOL_OFFSET);
                ...
                *fetch_addr = 0;
                vbuf_init_rma_fetch_and_add(v, (void *)fetch_addr, l_key,
                                            remote_addr, r_key, add_value,
                                            rail);
                ONESIDED_RDMA_POST(v, vc, NULL, rail);
        ...
        case MPIDI_CH3_PKT_CAS_IMMED: {
            ...
            if (g_atomics_support && aligned && origin_type_size == 8) {
                ...
                *((uint64_t *)return_addr) = 0;
                vbuf_init_rma_compare_and_swap(v, return_addr, l_key,
                                               remote_addr, r_key,
...
void mvp_init_rank_for_barrier(MPIR_Win **win_ptr)
{
    ...
    (*win_ptr)->shm_l2g_rank =
        (int *)MPL_malloc(g_smpi.num_local_nodes * sizeof(int), MPL_MEM_OTHER);
...
int MPIDI_CH3I_barrier_in_rma(MPIR_Win **win_ptr, int rank, int node_size,
                              int comm_size)
{
...
void MPIDI_CH3I_RDMA_try_rma(MPIR_Win *win_ptr, MPIDI_RMA_Target_t *target)
{
    ...
    MPIR_Comm *comm_ptr = NULL;
#if defined(RDMA_CM)
    has_iwarp = mvp_MPIDI_CH3I_RDMA_Process.use_iwarp_mode;
#endif
    if (SMP_INIT) {
        comm_ptr = win_ptr->comm_ptr;
    }
    ...
    while (curr_ptr != NULL) {
        ...
        intptr_t size, origin_type_size, target_type_size;
        switch (curr_ptr->pkt.type) {
            ...
            case MPIDI_CH3_PKT_PUT: {
                ...
                MPIDI_CH3_PKT_RMA_GET_TARGET_DATATYPE(
                    curr_ptr->pkt, target_datatype, mpi_errno);
                target_dt_derived =
                    HANDLE_GET_KIND(target_datatype) != HANDLE_KIND_BUILTIN ?
                        1 :
                        0;
                origin_dt_derived =
                    HANDLE_GET_KIND(curr_ptr->origin_datatype) !=
                            HANDLE_KIND_BUILTIN ?
                        1 :
                        0;
                MPIR_Datatype_get_size_macro(target_datatype, target_type_size);

                MPIR_Datatype_get_size_macro(curr_ptr->origin_datatype,
                                             origin_type_size);
                ...
                if (!origin_dt_derived && !target_dt_derived &&
                    ...
                    MPIDI_CH3I_RMA_Ops_free_elem(
                        win_ptr, &(target->pending_net_ops_list_head),
                        curr_ptr);
                    curr_ptr = next_ptr;
            ...
            case MPIDI_CH3_PKT_FOP_IMMED: {
                ...
                if (g_atomics_support && origin_type_size == 8 &&
                    curr_ptr->origin_datatype != MPI_DOUBLE && aligned &&
                    !has_iwarp && curr_ptr->pkt.fop.op == MPI_SUM &&
                    curr_ptr->ureq == NULL) {
                    next_ptr = curr_ptr->next;
            ...
            case MPIDI_CH3_PKT_CAS_IMMED: {
                 ...
                 * IB atomic operations require aligned address*/
                if (g_atomics_support && origin_type_size == 8 && aligned &&
                    !has_iwarp && curr_ptr->ureq == NULL) {
                    next_ptr = curr_ptr->next;
            ...
            case MPIDI_CH3_PKT_GET: {
                ...
                int target_count;
                MPIDI_CH3_PKT_RMA_GET_TARGET_COUNT(curr_ptr->pkt, target_count,
                                                   mpi_errno);
                size = target_count * target_type_size;
...
int MPIDI_CH3I_RDMA_post(MPIR_Win *win_ptr, int target_rank)
{
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_CH3I_RDMA_POST);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_RDMA_POST);
    int mpi_errno = MPI_SUCCESS;

    char *origin_addr, *remote_addr;
    MPIDI_VC_t *tmp_vc;
    MPIR_Comm *comm_ptr;
    uint32_t i, size, hca_index, r_key[MAX_NUM_SUBRAILS],
    ...
    *((int *)origin_addr) = 1;
    ...
    for (i = 0; i < rdma_num_rails; ++i) {
        ...
        r_key[i] =
            win_ptr->post_flag_rkeys[target_rank * rdma_num_hcas + hca_index];
    }

    Post_Put_Put_Get_List(win_ptr, -1, NULL, tmp_vc, &v, (void *)&origin_addr,
                          (void *)&remote_addr, size, l_key, r_key, SINGLE,
                          target_rank);
...
fn_fail:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_RDMA_POST);
    return mpi_errno;
}

void MPIDI_CH3I_RDMA_win_create(void *base, MPI_Aint size, int comm_size,
                                int my_rank, MPIR_Win **win_ptr,
                                MPIR_Comm *comm_ptr)
{
    ...
    if (mvp_MPIDI_CH3I_RDMA_Process.enable_rma_fast_path == 1) {
        (*win_ptr)->enable_fast_path = 1;
    }

    if (!mvp_MPIDI_CH3I_RDMA_Process.has_one_sided) {
        (*win_ptr)->fall_back = 1;
        goto fn_exit;
    }
    ...
    win_info_exchange = MPL_malloc(comm_size * sizeof(win_info), MPL_MEM_OTHER);
    ...
    cc_ptrs_exchange = MPL_malloc(
        comm_size * sizeof(uintptr_t) * rdma_num_rails, MPL_MEM_OTHER);
    if (!cc_ptrs_exchange) {
    ...
    (*win_ptr)->completion_counter = MPL_malloc(
        sizeof(long long) * comm_size * rdma_num_rails, MPL_MEM_OTHER);
    ...
    for (i = 0; i < rdma_num_hcas; ++i) {
        win_info_exchange[my_rank].completion_counter_rkeys[i] =
            (uint32_t)(*win_ptr)
                ->completion_counter_dreg_entry->memhandle[i]
                ->rkey;
    }

    /*Register buffer for post flags : from target to origin */
    (*win_ptr)->post_flag =
        (int *)MPL_malloc(comm_size * sizeof(int), MPL_MEM_OTHER);
    ...
    for (i = 0; i < rdma_num_hcas; ++i) {
        ...
        DEBUG_PRINT("the rank [%d] post_flag rkey before exchange is %x\n",
                    my_rank, win_info_exchange[my_rank].post_flag_rkeys[i]);
    }
    ...
    ret = MPIR_Allgather_impl(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,
                              win_info_exchange, sizeof(win_info), MPI_BYTE,
                              comm_ptr, &errflag);
    if (ret != MPI_SUCCESS) {
    ...
    if (fallback_trigger) {
        MPL_free(win_info_exchange);
        MPL_free(cc_ptrs_exchange);
        dreg_unregister((*win_ptr)->post_flag_dreg_entry);
        MPL_free((*win_ptr)->post_flag);
        dreg_unregister((*win_ptr)->completion_counter_dreg_entry);
        MPL_free((*win_ptr)->completion_counter);
    ...
    win_elem_t *new_element =
        (win_elem_t *)MPL_malloc(sizeof(win_elem_t), MPL_MEM_OTHER);
    new_element->next = NULL;

    if (mvp_win_list) {
        new_element->prev = mvp_win_list->prev;
        mvp_win_list->prev->next = new_element;
        mvp_win_list->prev = new_element;
        new_element->next = NULL;
    } else {
        mvp_win_list = new_element;
        mvp_win_list->prev = new_element;
        mvp_win_list->next = NULL;
    }
    ...
    ret = MPIR_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, cc_ptrs_exchange,
                         rdma_num_rails * sizeof(uintptr_t), MPI_BYTE, comm_ptr,
                         &errflag);
    if (ret != MPI_SUCCESS) {
    ...
    (*win_ptr)->win_rkeys = (uint32_t *)MPL_malloc(
        comm_size * sizeof(uint32_t) * rdma_num_hcas, MPL_MEM_OTHER);
    ...
    (*win_ptr)->completion_counter_rkeys = (uint32_t *)MPL_malloc(
        comm_size * sizeof(uint32_t) * rdma_num_hcas, MPL_MEM_OTHER);
    if (!(*win_ptr)->completion_counter_rkeys) {
        DEBUG_PRINT("Error malloc win->completion_counter_rkeys when creating "
                    "windows\n");
        ibv_error_abort(GEN_EXIT_ERR, "rdma_iba_1sc");
    }

    /* Now allocate the completion counter array for all other processes */
    (*win_ptr)->all_completion_counter = (long long **)MPL_malloc(
        comm_size * sizeof(long long *) * rdma_num_rails, MPL_MEM_OTHER);
    ...
    (*win_ptr)->post_flag_rkeys = (uint32_t *)MPL_malloc(
        comm_size * sizeof(uint32_t) * rdma_num_hcas, MPL_MEM_OTHER);
    if (!(*win_ptr)->post_flag_rkeys) {
        DEBUG_PRINT(
            "error malloc win->post_flag_rkeys when creating windows\n");
        ibv_error_abort(GEN_EXIT_ERR, "rdma_iba_1sc");
    }
    ...
    for (i = 0; i < comm_size; ++i) {
        for (j = 0; j < rdma_num_hcas; ++j) {
            ...
            (*win_ptr)->win_rkeys[arrIndex] = win_info_exchange[i].win_rkeys[j];
    ...
    for (i = 0; i < comm_size; ++i) {
        for (j = 0; j < rdma_num_rails; ++j) {
            ...
            (*win_ptr)->all_completion_counter[arrIndex] =
                (long long *)((size_t)(cc_ptrs_exchange[arrIndex]) +
                              sizeof(long long) * my_rank * rdma_num_rails);
        }
    }

    post_flag_ptr_send =
        (uintptr_t *)MPL_malloc(comm_size * sizeof(uintptr_t), MPL_MEM_OTHER);
    ...
    ret = MPIR_Alltoall_impl(post_flag_ptr_send, sizeof(uintptr_t), MPI_BYTE,
                             post_flag_ptr_recv, sizeof(uintptr_t), MPI_BYTE,
                             comm_ptr, &errflag);
    if (ret != MPI_SUCCESS) {
    ...
    MPL_free(win_info_exchange);
    MPL_free(cc_ptrs_exchange);
    MPL_free(post_flag_ptr_send);
    MPL_free(post_flag_ptr_recv);
    ...
    (*win_ptr)->put_get_list = (MPIDI_CH3I_RDMA_put_get_list *)MPL_malloc(
        rdma_default_put_get_list_size * sizeof(MPIDI_CH3I_RDMA_put_get_list),
        MPL_MEM_OTHER);
    if (!(*win_ptr)->put_get_list) {
        DEBUG_PRINT("Fail to malloc space for window put get list\n");
        ibv_error_abort(GEN_EXIT_ERR, "rdma_iba_1sc");
    }
    /* Initialize the status of all entries in put_get_list to avoid garbage
     * value */
    ...
    (*win_ptr)->put_get_list_size_per_process =
        (int *)MPL_malloc(sizeof(int) * comm_size, MPL_MEM_OTHER);
    if (!(*win_ptr)->put_get_list_size_per_process) {
        DEBUG_PRINT(
            "Fail to malloc space for window put get list per process\n");
        ibv_error_abort(GEN_EXIT_ERR, "rdma_iba_1sc");
    }
    MPIR_Memset((*win_ptr)->put_get_list_size_per_process, 0,
...
void MPIDI_CH3I_RDMA_win_free(MPIR_Win **win_ptr)
{
    ...
    MPL_free((*win_ptr)->win_rkeys);
    MPL_free((*win_ptr)->completion_counter_rkeys);
    MPL_free((*win_ptr)->post_flag);
    MPL_free((*win_ptr)->post_flag_rkeys);
    MPL_free((*win_ptr)->remote_post_flags);
    MPL_free((*win_ptr)->put_get_list);
    MPL_free((*win_ptr)->put_get_list_size_per_process);

    MPL_free((*win_ptr)->completion_counter);
    MPL_free((*win_ptr)->all_completion_counter);
}

int MPIDI_CH3I_RDMA_set_CC(MPIR_Win *win_ptr, int target_rank)
{
    int i;
    int mpi_errno = MPI_SUCCESS;
    ...
    for (i = 0; i < rdma_num_rails; ++i) {
        GET_VBUF_BY_OFFSET_WITHOUT_LOCK(v[i], MVP_SMALL_DATA_VBUF_POOL_OFFSET);
        local_addr[i] = (void *)v[i]->buffer;
        hca_index = i / rdma_num_rails_per_hca;
        remote_addr[i] =
            (void *)(uintptr_t)(win_ptr->all_completion_counter
                                    [target_rank * rdma_num_rails + i]);
        *((long long *)local_addr[i]) = 1;
        l_key2[i] = v[i]->region->mem_handle[hca_index]->lkey;
        r_key2[i] =
            win_ptr->completion_counter_rkeys[target_rank * rdma_num_hcas +
                                              hca_index];
    }

    Post_Put_Put_Get_List(win_ptr, -1, NULL, tmp_vc, &v[0], local_addr,
                          remote_addr, sizeof(long long), l_key2, r_key2,
                          REPLICATE, target_rank);
    return mpi_errno;
}

static int Post_Put_Put_Get_List(MPIR_Win *winptr, intptr_t size,
                                 dreg_entry *dreg_tmp, MPIDI_VC_t *vc_ptr,
                                 vbuf **allocated_v, void *local_buf[],
                                 void *remote_buf[], intptr_t length,
                                 uint32_t lkeys[], uint32_t rkeys[],
                                 rail_select_t rail_select, int target)
{
    int mpi_errno = MPI_SUCCESS;
    int rail, i, index;
    intptr_t count, bytes_per_rail, posting_length;
    void *local_address, *remote_address;
    vbuf *v;
    MPIDI_VC_t *save_vc = vc_ptr;

    while (winptr->put_get_list[winptr->put_get_list_tail].status == BUSY) {
        winptr->put_get_list_tail =
            (winptr->put_get_list_tail + 1) % rdma_default_put_get_list_size;
    }
    ...
    if (rail_select == STRIPE) { /* stripe the message across rails */
        /*post data in chunks of mvp_MPIDI_CH3I_RDMA_Process.maxtransfersize as
          long as the total size per rail is larger than that*/
        count = 0;
        bytes_per_rail = length / rdma_num_rails;
        while (bytes_per_rail > mvp_MPIDI_CH3I_RDMA_Process.maxtransfersize) {
            for (i = 0; i < rdma_num_rails; ++i) {
                GET_VBUF_BY_OFFSET_WITHOUT_LOCK(
                    v, MVP_SMALL_DATA_VBUF_POOL_OFFSET);
                if (NULL == v) {
                ...
                local_address =
                    (void *)((char *)local_buf[0] +
                             count * rdma_num_rails *
                                 mvp_MPIDI_CH3I_RDMA_Process.maxtransfersize +
                             i * mvp_MPIDI_CH3I_RDMA_Process.maxtransfersize);
                remote_address =
                    (void *)((char *)remote_buf[0] +
                             count * rdma_num_rails *
                                 mvp_MPIDI_CH3I_RDMA_Process.maxtransfersize +
                             i * mvp_MPIDI_CH3I_RDMA_Process.maxtransfersize);

                vbuf_init_rma_put(
                    v, local_address, lkeys[i], remote_address, rkeys[i],
                    mvp_MPIDI_CH3I_RDMA_Process.maxtransfersize, i);
                ONESIDED_RDMA_POST(v, vc_ptr, save_vc, i);
            }

            ++count;
            length -=
                rdma_num_rails * mvp_MPIDI_CH3I_RDMA_Process.maxtransfersize;
            bytes_per_rail = length / rdma_num_rails;
        }

        /* Post remaining data as length <
         * rdma_num_rails*mvp_MPIDI_CH3I_RDMA_Process.maxtransfersize Still
         * stripe if length > rdma_large_msg_rail_sharing_threshold*/
        if (length < rdma_large_msg_rail_sharing_threshold) {
            ...
            vbuf_init_rma_put(v, local_address, lkeys[rail], remote_address,
            ...
            for (i = 0; i < rdma_num_rails; ++i) {
                ...
                if (i < rdma_num_rails - 1) {
                    ...
                    posting_length =
                        length - (rdma_num_rails - 1) * bytes_per_rail;
                }
...
static int Post_Get_Put_Get_List(MPIR_Win *winptr, intptr_t size,
                                 dreg_entry *dreg_tmp, MPIDI_VC_t *vc_ptr,
                                 vbuf **allocated_v, void *local_buf[],
                                 void *remote_buf[], void *user_buf[],
                                 intptr_t length, uint32_t lkeys[],
                                 ...
                                 int target)
{
    int mpi_errno = MPI_SUCCESS;
    int i, rail, index;
    intptr_t posting_length, bytes_per_rail, count;
    void *local_address, *remote_address;
    vbuf *v;
    MPIDI_VC_t *save_vc = vc_ptr;
    ...
    if (rail_select == STRIPE) { /*stripe across the rails*/
        ...
        if (length < rdma_large_msg_rail_sharing_threshold) {
            ...
            vbuf_init_rma_get(v, local_address, lkeys[rail], remote_address,
                              rkeys[rail], length, rail);
            v->list = (void *)(&(winptr->put_get_list[index]));
            v->vc = (void *)vc_ptr;
    ...
    if (v->target_rank == -1) {
        ...
        switch (list_entry->op_type) {
            case (SIGNAL_FOR_PUT): {
                ...
                --(list_win_ptr->put_get_list_size_per_process
                       [list_entry->target_rank]);
                break;
            }
            case (SIGNAL_FOR_GET): {
                size = list_entry->data_size;
                ...
                if (origin_addr == NULL) {
                    MPIR_Assert(size > rdma_eagersize_1sc);
                    --(list_entry->completion);
                    if (list_entry->completion == 0) {
                        dreg_unregister(dreg_tmp);
                    }
                    ...
                    MPIR_Assert(size <= rdma_eagersize_1sc);
                    MPIR_Assert(target_addr != NULL);
                    MPIR_Memcpy(target_addr, origin_addr, size);
                    list_entry->status = FREE;
                }
                --(list_win_ptr->put_get_list_size);
                --(list_win_ptr->rma_issued);
...
#ifdef INFINIBAND_VERBS_EXP_H
                if (g_atomics_support_be) {
                    *((uint64_t *)v->result_addr) =
                        ntohll(*((uint64_t *)v->buffer));
                } else
#endif /* INFINIBAND_VERBS_EXP_H */
                {
...
static int iba_put(MPIDI_RMA_Op_t *rma_op, MPIR_Win *win_ptr, intptr_t size)
{
    char *remote_address;
    int mpi_errno = MPI_SUCCESS;
    int hca_index;
    uint32_t r_key[MAX_NUM_SUBRAILS], l_key1[MAX_NUM_SUBRAILS],
        l_key[MAX_NUM_SUBRAILS];
    int i;
    dreg_entry *tmp_dreg = NULL;
    char *origin_addr;
    ...
    MPIR_Assert(rma_op->pkt.type == MPIDI_CH3_PKT_PUT_IMMED ||
                rma_op->pkt.type == MPIDI_CH3_PKT_PUT);
    remote_address = rma_op->pkt.put.addr;

    if (likely(size <= rdma_eagersize_1sc)) {
        MVP_GET_RC_VBUF(v, size);
        origin_addr = (char *)v->buffer;
        MPIR_Memcpy(origin_addr, (char *)rma_op->origin_addr, size);
    ...
    for (i = 0; i < rdma_num_rails; ++i) {
        hca_index = i / rdma_num_rails_per_hca;
        r_key[i] =
            win_ptr->win_rkeys[rma_op->target_rank * rdma_num_hcas + hca_index];
        l_key1[i] = l_key[hca_index];
    }

    if (size < rdma_large_msg_rail_sharing_threshold ||
        size < rdma_eagersize_1sc) {
...
int iba_get(MPIDI_RMA_Op_t *rma_op, MPIR_Win *win_ptr, intptr_t size)
{
    int mpi_errno = MPI_SUCCESS;
    int hca_index;
    uint32_t r_key[MAX_NUM_SUBRAILS], l_key1[MAX_NUM_SUBRAILS],
    ...
    MPIR_Assert(rma_op->pkt.type == MPIDI_CH3_PKT_GET);
    ...
    if (size < rdma_large_msg_rail_sharing_threshold ||
        size < rdma_eagersize_1sc) {
        Post_Get_Put_Get_List(win_ptr, size, tmp_dreg, tmp_vc, &v,
                              (void *)&target_addr, (void *)&remote_addr,
                              (void *)&user_addr, size, l_key1, r_key, SINGLE,
                              rma_op->target_rank);
    } else {
        Post_Get_Put_Get_List(win_ptr, size, tmp_dreg, tmp_vc, &v,
                              (void *)&target_addr, (void *)&remote_addr,
                              (void *)&user_addr, size, l_key1, r_key, STRIPE,
                              rma_op->target_rank);
    }
...
int iba_compare_and_swap(MPIDI_RMA_Op_t *rma_op, MPIR_Win *win_ptr,
                         intptr_t size)
{
    ...
    MPIR_Assert(rma_op->pkt.type == MPIDI_CH3_PKT_CAS_IMMED);
    ...
    win_ptr->put_get_list_tail =
        (win_ptr->put_get_list_tail + 1) % rdma_default_put_get_list_size;
...
int iba_fetch_and_add(MPIDI_RMA_Op_t *rma_op, MPIR_Win *win_ptr, intptr_t size)
{
    ...
    MPIR_Assert(rma_op->pkt.type == MPIDI_CH3_PKT_FOP_IMMED ||
    ...
    *fetch_addr = 0;
    ...
    vbuf_init_rma_fetch_and_add(v, (void *)fetch_addr, l_key, remote_addr,
                                r_key, add_value, rail);
    ONESIDED_RDMA_POST(v, vc_ptr, NULL, rail);
