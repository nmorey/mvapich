...
/* TODO: move this into ch3/channels/mrail/include/mvp_mpidi_ibv_cuda_util.h
 * since it has channel specific implementations of various functions.
 * The generic device interfaces are added in ch3/include/mvp_mpid_cuda_util.h
 */
...
#define MVP_DEVICE_FLAG(_flag) cuda##_flag
...
typedef struct mvp_device_event {
    cudaEvent_t event;
    ...
    void *smp_ptr;
    struct mvp_device_event *next, *prev;
} mvp_device_event_t;
...
int allocate_cuda_event(mvp_device_event_t **);    /* allocate single event */
void deallocate_cuda_event(mvp_device_event_t **); /* deallocate single event */
...
void release_cuda_event(mvp_device_event_t *);
extern mvp_device_event_t *free_cuda_event_list_head;
extern mvp_device_event_t *busy_cuda_event_list_head;
extern mvp_device_event_t *busy_cuda_event_list_tail;

typedef struct MPIDI_PG MPIDI_PG_t;
void cudaipc_allocate_shared_region(MPIDI_PG_t *pg, int num_processes,
                                    int my_rank);
void cudaipc_allocate_ipc_region(MPIDI_PG_t *pg, int num_processes,
                                 int my_rank);
...
#define MVP_MPIDI_Malloc_Device(_buf, _size)                                   \
    do {                                                                       \
...
#define MVP_MPIDI_Free_Device(_buf)                                            \
    do {                                                                       \
        cudaError_t cuerr = cudaSuccess;                                       \
        cuerr = cudaFree(_buf);                                                \
        if (cuerr != cudaSuccess) {                                            \
...
#define MVP_MPIDI_Malloc_Device_Pinned_Host(_buf, _size)                       \
    do {                                                                       \
        cudaError_t cuerr = cudaSuccess;                                       \
...
#define MVP_MPIDI_Free_Device_Pinned_Host(_buf)                                \
    do {                                                                       \
        cudaError_t cuerr = cudaSuccess;                                       \
...
#define MVP_MPIDI_Memcpy_Device_Async(_dst, _src, _size, _type, _stream)       \
    do {                                                                       \
        cudaError_t cuerr = cudaSuccess;                                       \
...
#define MVP_MPIDI_Memcpy_Device(_dst, _src, _size, _type)                      \
    do {                                                                       \
        cudaError_t cuerr = cudaSuccess;                                       \
...
#define MVP_MPIDI_Device_CtxGetCurrent(_ctx)                                   \
    do {                                                                       \
        CU_CHECK(cuCtxGetCurrent(_ctx));                                       \
    } while (0)

#define MVP_MPIDI_DeviceEventCreate(_event)                                    \
    do {                                                                       \
        CUDA_CHECK(cudaEventCreate(_event));                                   \
    } while (0)

#define MVP_MPIDI_DeviceEventCreateWithFlags(_event, _flags)                   \
    do {                                                                       \
        CUDA_CHECK(cudaEventCreateWithFlags(_event, _flags));                  \
    } while (0)

#define MVP_MPIDI_Device_EventRecord(_event, _stream)                          \
    do {                                                                       \
        CUDA_CHECK(cudaEventRecord(_event, _stream));                          \
    } while (0)

#define MVP_MPIDI_Device_EventSynchronize(_event)                              \
    do {                                                                       \
        CUDA_CHECK(cudaEventSynchronize(_event));                              \
    } while (0)

#define MVP_MPIDI_Device_StreamWaitEvent(_stream, _event, _flag)               \
    do {                                                                       \
        CUDA_CHECK(cudaStreamWaitEvent(_stream, _event, _flag));               \
    } while (0)

#define MVP_MPIDI_Device_EventDestroy(_event)                                  \
    do {                                                                       \
...
#define MVP_MPIDI_Device_IpcGetMemHandle(_memhandle_out, _base)                \
    do {                                                                       \
        CUDA_CHECK(cudaIpcGetMemHandle(_memhandle_out, _base));                \
    } while (0)

#define MVP_MPIDI_Device_IpcOpenMemHandle(_base_out, _memhandle)               \
    do {                                                                       \
        CUDA_CHECK(cudaIpcOpenMemHandle(_base_out, _memhandle,                 \
                                        cudaIpcMemLazyEnablePeerAccess));      \
    } while (0)

#define MVP_MPIDI_Device_IpcOpenEventHandle(_event, _handle)                   \
    do {                                                                       \
        CUDA_CHECK(cudaIpcOpenEventHandle(_event, _handle));                   \
    } while (0)

#define MVP_MPIDI_Device_IpcGetEventHandle(_handle, _event)                    \
    do {                                                                       \
        CUDA_CHECK(cudaIpcGetEventHandle(_handle, _event));                    \
    } while (0)

#define MVP_MPIDI_Device_IpcCloseMemHandle(_base)                              \
    do {                                                                       \
...
#define DEVICE_IPC_RECV_IN_PROGRESS(c, s)                                      \
    {                                                                          \
        ...
        if (NULL == (c)->mrail.device_ipc_sreq_tail) {                         \
            ...
            ((MPIR_Request *)(c)->mrail.device_ipc_sreq_tail)                  \
                ->mrail.next_inflow = (void *)(s);                             \
        }                                                                      \
...
#define DEVICE_IPC_RECV_DONE(c)                                                \
    {                                                                          \
        ...
        (c)->mrail.device_ipc_sreq_head =                                      \
            ((MPIR_Request *)(c)->mrail.device_ipc_sreq_head)                  \
                ->mrail.next_inflow;                                           \
...
extern int mvp_device_use_ipc_stage_buffer;
extern size_t mvp_device_ipc_stage_buffer_limit;
extern int cudaipc_sync_limit;
#endif
#endif
#if defined(USE_GPU_KERNEL)
void pack_subarray(void *dst, void *src, int dim, int nx, int ny, int nz,
                   int sub_nx, int sub_ny, int sub_nz, int h_x, int h_y,
                   int h_z, int sub_order, int el_size, cudaStream_t stream);
void unpack_subarray(void *dst, void *src, int dim, int nx, int ny, int nz,
                     int sub_nx, int sub_ny, int sub_nz, int h_x, int h_y,
                     int h_z, int sub_order, int el_size, cudaStream_t stream);
void pack_unpack_vector_kernel(void *dst, int dpitch, void *src, int spitch,
