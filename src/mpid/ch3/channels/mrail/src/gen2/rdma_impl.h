...
#define MVP_MAX_GID_PATH_LEN          256
#define MVP_HCA_BOARD_ID_PATH         "/sys/class/infiniband/%s/board_id"
#define MVP_ROCKPORT_FW_BOARD_ID      "RCP0000000001"
#define MVP_MAX_HCA_BOARD_ID_PATH_LEN 128
#define MVP_MAX_HCA_BOARD_ID_LEN      64
...
typedef struct mvp_MPIDI_CH3I_RDMA_Process_t {
    ...
    mvp_ud_ctx_t *ud_rails[MAX_NUM_HCAS];
...
typedef struct mvp_process_init_info {
    int hostid;
    uint32_t ud_cm_qpn;
...
extern win_elem_t *mvp_win_list;

extern int mvp_get_verbs_ips_dev_names(
    int *num_interfaces,
    ip_address_enabled_devices_t *mvp_get_verbs_ips_dev_names);
...
#define PREPOST_VBUF_RECV(_c, _subrail)                                        \
    {                                                                          \
        vbuf *__v = get_vbuf_by_offset(MVP_RECV_VBUF_POOL_OFFSET);             \
...
#define XRC_FILL_SRQN_FIX_CONN(_v, _vc, _rail)                                 \
    do {                                                                       \
        if (unlikely(USE_XRC && VC_XST_ISUNSET((_vc), XF_DPM_INI))) {          \
            int hca_index = _rail / (rdma_num_ports * rdma_num_qp_per_port);   \
            (_v)->desc.u.sr.xrc_remote_srq_num =                               \
                (_vc)->ch.xrc_srqn[hca_index];                                 \
            PRINT_DEBUG(DEBUG_XRC_verbose > 1,                                 \
                        "Msg for %d. Fixed SRQN: %d (WQE: %d) (%s:%d)\n",      \
                        (_vc)->pg_rank, (_v)->desc.u.sr.xrc_remote_srq_num,    \
                        (_vc)->mrail.rails[(_rail)].send_wqes_avail, __FILE__, \
                        __LINE__);                                             \
            if (VC_XST_ISSET((_vc), XF_INDIRECT_CONN)) {                       \
                PRINT_DEBUG(DEBUG_XRC_verbose > 1,                             \
                            "Switched vc from %d to %d\n", (_vc)->pg_rank,     \
...
#define IBV_POST_SR(_v, _c, _rail, err_string)                                 \
    {                                                                          \
        {                                                                      \
            ...
            PRINT_DEBUG(                                                       \
                DEBUG_XRC_verbose > 1,                                         \
                "POST_SR: to %d (qpn: %d) (state: %d %d %d) (%s:%d)\n",        \
    ...
    {                                                                          \
        {                                                                      \
            ...
            __ret = ibv_post_send((_c)->mrail.rails[(_rail)].qp_hndl,          \
                                  &((_v)->desc.u.sr), &((_v)->desc.y.bad_sr)); \
            if (unlikely(__ret)) {                                             \
                printf("[%d => %d]: %s(%s): ret=%d, errno=%d: failed while "   \
                       "avail wqe is %d, "                                     \
                       "rail %d\n",                                            \
                       MPIDI_Process.my_pg_rank, _c->pg_rank, __func__,        \
                       err_string, __ret, errno,                               \
                       (_c)->mrail.rails[(_rail)].send_wqes_avail, (_rail));   \
...
#define IBV_POST_RR(_c, _vbuf, _rail)                                          \
    {                                                                          \
        ...
        _vbuf->vc = (void *)_c;                                                \
        __ret =                                                                \
            ibv_post_recv(_c->mrail.rails[(_rail)].qp_hndl,                    \
                          &((_vbuf)->desc.u.rr), &((_vbuf)->desc.y.bad_rr));   \
        if (unlikely(__ret)) {                                                 \
            ibv_va_error_abort(IBV_RETURN_ERR, "ibv_post_recv err with %d",    \
                               __ret);                                         \
        }                                                                      \
    }
...
#ifdef _ENABLE_UD_
#define MVP_HYBRID_SET_RC_CONN_INITIATED(vc)                                   \
    do {                                                                       \
...
int rdma_ring_boot_exchange(struct mvp_MPIDI_CH3I_RDMA_Process_t *proc,
                            MPIDI_PG_t *pg, int pg_size,
                            struct process_init_info *);
int rdma_setup_startup_ring(struct mvp_MPIDI_CH3I_RDMA_Process_t *, int pg_rank,
                            int pg_size);
...
int rdma_ring_based_allgather(void *sbuf, int data_size, int proc_rank,
                              void *rbuf, int job_size,
...
int rdma_find_network_type(struct ibv_device **dev_list, int num_devices,
                           ...
                           int *num_usable_hcas,
                           int *num_usable_hcas_on_my_sock,
...
int rdma_iba_hca_init(struct mvp_MPIDI_CH3I_RDMA_Process_t *proc, int pg_rank,
                      MPIDI_PG_t *pg, struct process_init_info *);
...
int rdma_iba_enable_connections(struct mvp_MPIDI_CH3I_RDMA_Process_t *proc,
                                ...
                                struct process_init_info *);
void rdma_param_handle_heterogeneity(mvp_arch_hca_type hca_type[], int pg_size);
...
#ifdef _ENABLE_UD_
int post_ud_send(MPIDI_VC_t *vc, vbuf *v, int rail, mvp_ud_ctx_t *);
...
int mvp_ud_get_remote_info(MPIDI_PG_t *pg, int pg_rank, int pg_size);
...
void MRAILI_RDMA_Put(MPIDI_VC_t *vc, vbuf *v, char *local_addr, uint32_t lkey,
                     char *remote_addr, uint32_t rkey, int nbytes, int subrail);
...
int MPIDI_CH3I_MRAILI_rput_complete(MPIDI_VC_t *, struct iovec *, int,
                                    int *num_bytes_ptr, vbuf **, int rail);
int MPIDI_CH3I_MRAILI_rget_finish(MPIDI_VC_t *, struct iovec *, int,
                                  int *num_bytes_ptr, vbuf **, int rail);
...
int init_MVP_collops(MPIR_Comm *comm);
