...
mvp_device_event_t *free_cudaipc_event_list_head = NULL;
mvp_device_event_t *free_cuda_event_list_head = NULL;
mvp_device_event_t *busy_cuda_event_list_head = NULL;
mvp_device_event_t *busy_cuda_event_list_tail = NULL;
...
{
    if (mvp_device_nonblocking_streams) {
        CUDA_CHECK(
            cudaStreamCreateWithFlags(&stream_d2h, cudaStreamNonBlocking));
        CUDA_CHECK(
            cudaStreamCreateWithFlags(&stream_h2d, cudaStreamNonBlocking));
        CUDA_CHECK(
            cudaStreamCreateWithFlags(&stream_kernel, cudaStreamNonBlocking));
...
void allocate_cuda_events()
{
    int i;
    cudaError_t result;
    mvp_device_event_t *curr;
    cuda_event_region =
        (void *)MPL_malloc(sizeof(mvp_device_event_t) * mvp_device_event_count);
    ...
    curr = (mvp_device_event_t *)cuda_event_region;
    for (i = 1; i <= mvp_device_event_count; i++) {
        curr->next = (mvp_device_event_t *)((size_t)cuda_event_region +
                                            i * sizeof(mvp_device_event_t));
        result =
            cudaEventCreateWithFlags(&(curr->event), cudaEventDisableTiming
...
{
    mvp_device_event_t *curr_event = free_cuda_event_list_head;
...
int allocate_cuda_event(mvp_device_event_t **cuda_event)
{
    cudaError_t result;
    *cuda_event = (mvp_device_event_t *)MPL_malloc(sizeof(mvp_device_event_t));
    result = cudaEventCreateWithFlags(&((*cuda_event)->event),
                                      cudaEventDisableTiming
#if defined(HAVE_CUDA_IPC)
                                          | cudaEventInterprocess
#endif
...
void deallocate_cuda_event(mvp_device_event_t **cuda_event)
{
...
#if defined(HAVE_CUDA_IPC)
mvp_device_event_t *get_free_cudaipc_event()
{
    mvp_device_event_t *curr_event;
...
void release_cudaipc_event(mvp_device_event_t *event)
{
...
void process_cuda_event_op(mvp_device_event_t *event)
{
    ...
    if (event->op_type == SEND) {
        ...
        is_finish =
            (req->mrail.pipeline_nm == req->mrail.num_device_blocks) ? 1 : 0;
        ...
        if (req->mrail.device_transfer_mode == DEVICE_TO_DEVICE) {
            if (size <= rdma_large_msg_rail_sharing_threshold) {
                ...
                MRAILI_RDMA_Put(
                    vc, v, (char *)(v->buffer),
                    v->region->mem_handle[vc->mrail.rails[rail].hca_index]
                        ->lkey,
                    (char *)(req->mrail.device_remote_addr
                                 [req->mrail.num_remote_device_done]),
                    req->mrail
                        .device_remote_rkey[req->mrail.num_remote_device_done]
                                           [vc->mrail.rails[rail].hca_index],
                    size, rail);
            } else {
                ...
                for (rail = 0; rail < rdma_num_rails; rail++) {
                    ...
                    stripe_size = (rail < rdma_num_rails - 1) ? stripe_avg :
                                                                (size - offset);
                    ...
                    MRAILI_RDMA_Put(
                        vc, v, (char *)(orig_vbuf->buffer) + offset,
                        orig_vbuf->region
                            ->mem_handle[vc->mrail.rails[rail].hca_index]
                            ->lkey,
                        (char *)(req->mrail.device_remote_addr
                                     [req->mrail.num_remote_device_done]) +
                            offset,
                        req->mrail.device_remote_rkey
                            [req->mrail.num_remote_device_done]
                            [vc->mrail.rails[rail].hca_index],
                        stripe_size, rail);
                }
            ...
            for (rail = 0; rail < rdma_num_rails; rail++) {
                MRAILI_RDMA_Put_finish_device(
                    vc, req, rail, is_pipeline, is_finish,
                    mvp_device_stage_block_size * displacement);
            }
            req->mrail.num_remote_device_done++;
            ...
            if (size <= rdma_large_msg_rail_sharing_threshold) {
                ...
                MRAILI_RDMA_Put(
                    ...
                    (char *)(req->mrail.remote_addr) +
                        displacement * mvp_device_stage_block_size,
                    req->mrail.rkey[vc->mrail.rails[rail].hca_index], size,
                    rail);
            } else {
                stripe_avg = size / rdma_num_rails;
                orig_vbuf = cuda_vbuf;
                ...
                for (rail = 0; rail < rdma_num_rails; rail++) {
                    ...
                    MRAILI_RDMA_Put(
                        ...
                        orig_vbuf->region
                            ...
                            ->lkey,
                        (char *)(req->mrail.remote_addr) +
                            displacement * mvp_device_stage_block_size + offset,
        ...
        PRINT_DEBUG(DEBUG_CUDA_verbose > 1,
                    ...
                    displacement, req->mrail.num_remote_device_done, is_finish,
                    mvp_device_stage_block_size * displacement, event);
        ...
        while (cuda_vbuf) {
            PRINT_DEBUG(DEBUG_CUDA_verbose > 2,
                        "CUDA copy block: "
                        "buf:%p\n",
                        cuda_vbuf->buffer);
        ...
        int complete;
        if (req->mrail.rndv_buf_alloc == 1 && req->mrail.rndv_buf != NULL) {
            /* a temporary host rndv buffer would have been allocated only when
               the sender buffers is noncontiguous and is in the host memory */
            MPIR_Assert(req->mrail.device_transfer_mode == HOST_TO_DEVICE);
            MVP_MPIDI_Free_Device_Pinned_Host(req->mrail.rndv_buf);
...
void progress_cuda_events()
{
    ...
    mvp_device_event_t *curr_event = busy_cuda_event_list_head;
    mvp_device_event_t *next_event;
        ...
        if (cudaSuccess == result || curr_event->is_query_done) {
            ...
            if ((SEND == curr_event->op_type) &&
                ((1 != ((MPIR_Request *)curr_event->req)->mrail.cts_received) ||
                 !((MPIR_Request *)curr_event->req)
                      ->mrail.num_remote_device_pending)) {
                curr_event = curr_event->next;
            } else {
...
mvp_device_event_t *get_device_event()
{
    mvp_device_event_t *curr_event;

    if (NULL == free_cuda_event_list_head) {
        if (NULL != busy_cuda_event_list_head) {
            /* if there are no more free events in the list, allocate a
             * temporary one */
            mvp_device_event_t *dedicate_cuda_event = NULL;
            ...
            if (NULL != dedicate_cuda_event) {
                ...
                CUDA_LIST_ADD(dedicate_cuda_event, busy_cuda_event_list_head,
