...
int MPIDI_CH3I_MRAIL_Prepare_rndv(MPIDI_VC_t *vc, MPIR_Request *req)
{
    ...
    if (IS_VC_SMP(vc)) {
        ...
        if (USE_XRC && !(vc->mrail.state & MRAILI_RC_CONNECTED) &&
            (req->mrail.protocol == MRAILI_PROTOCOL_RPUT ||
             req->mrail.protocol == MRAILI_PROTOCOL_RGET)) {
            req->mrail.protocol = MRAILI_PROTOCOL_R3;
            return 0;
        }
#endif
    ...
    else if (req->mrail.protocol != MRAILI_PROTOCOL_R3) {
        req->mrail.protocol = MVP_RNDV_PROTOCOL;
    }

    /* Step 1: ready for user space (user buffer or pack) */
    if (1 == req->dev.iov_count &&
        ...
        req->mrail.rndv_buf_sz = req->dev.msgsize;
        req->mrail.rndv_buf =
            MPL_malloc(req->mrail.rndv_buf_sz, MPL_MEM_BUFFER);
...
#ifdef _ENABLE_CUDA_
    if (mvp_enable_device && req->mrail.device_transfer_mode != NONE) {
        if ((MRAILI_PROTOCOL_RPUT == req->mrail.protocol) ||
            (MRAILI_PROTOCOL_RGET == req->mrail.protocol)) {
            return 1;
        } else {
            return 0;
        }
    ...
    if (MVP_USE_UD_HYBRID &&
        (req->mrail.rndv_buf_sz < rdma_ud_zcopy_threshold ||
    ...
    if (
#ifdef _ENABLE_CUDA_
        (!mvp_enable_device || !SMP_INIT || vc->smp.local_nodes == -1) &&
#endif
        (MRAILI_PROTOCOL_RPUT == req->mrail.protocol ||
         MRAILI_PROTOCOL_RGET == req->mrail.protocol ||
         MRAILI_PROTOCOL_UD_ZCOPY == req->mrail.protocol)) {
        if (IS_VC_SMP(vc)) {
            PRINT_DEBUG(DEBUG_RNDV_verbose > 1,
                        "SMP vc, not registering. rank: %d, buf size: %ld, "
                        "addr: %p, protocol: %d\n",
                        vc->pg_rank, req->mrail.rndv_buf_sz,
                        req->mrail.rndv_buf, req->mrail.protocol);
...
#ifdef _ENABLE_UD_
                if (MRAILI_PROTOCOL_UD_ZCOPY == req->mrail.protocol) {
...
int MPIDI_CH3I_MRAIL_Prepare_rndv_transfer(MPIR_Request *sreq,
                                           /* contains local info */
                                           MPIDI_CH3I_MRAILI_Rndv_info_t *rndv)
{
    int hca_index;

    if (rndv->protocol == MRAILI_PROTOCOL_R3) {
...
#ifdef _ENABLE_UD_
        int hca_num = 0;
        if (rndv->protocol == MRAILI_PROTOCOL_UD_ZCOPY) {
...
#ifdef _ENABLE_CUDA_
        int i;
        if (mvp_enable_device && sreq->mrail.device_transfer_mode != NONE) {
            sreq->mrail.cts_received = 1;
            sreq->mrail.num_send_device_copy = 0;
            if (rndv->device_transfer_mode == NONE ||
                rndv->device_transfer_mode == DEVICE_TO_HOST) {
            ...
            if (sreq->mrail.device_transfer_mode == DEVICE_TO_DEVICE ||
                ...
                sreq->mrail.num_remote_device_pending = ROUNDUP(
                    sreq->mrail.rndv_buf_sz, mvp_device_stage_block_size);
        ...
        DEBUG_PRINT("[add rndv list] addr %p, key %p\n",
                    sreq->mrail.remote_addr, sreq->mrail.rkey[0]);
...
void MRAILI_RDMA_Put_finish(MPIDI_VC_t *vc, MPIR_Request *sreq, int rail)
{
    ...
    if (IS_VC_SMP(vc)) {
        ...
        mpi_errno = MPIDI_CH3_SMP_iStartMsg(
            vc, &rput_pkt, sizeof(MPIDI_CH3_Pkt_rput_finish_t), &new_req);
        if (mpi_errno != MPI_SUCCESS) {
            ibv_error_abort(
                IBV_STATUS_ERR,
...
void MRAILI_RDMA_Get_finish(MPIDI_VC_t *vc, MPIR_Request *rreq, int rail)
{
    ...
    if (IS_VC_SMP(vc)) {
        ...
        mpi_errno = MPIDI_CH3_SMP_iStartMsg(
            vc, &rget_pkt, sizeof(MPIDI_CH3_Pkt_rget_finish_t), &new_req);
        if (mpi_errno != MPI_SUCCESS) {
            ibv_error_abort(
                IBV_STATUS_ERR,
        ...
        iov.iov_len = sizeof(MPIDI_CH3_Pkt_rget_finish_t);
...
void MPIDI_CH3I_MRAILI_Rendezvous_rget_push(MPIDI_VC_t *vc, MPIR_Request *rreq)
{
    vbuf *v;
    int queued = 0;
    ...
    if (rreq->mrail.rndv_buf_sz > 0) {
        MPIR_Assert(rreq->mrail.d_entry != NULL);
        MPIR_Assert(rreq->mrail.remote_addr != NULL);
    }
#endif /* defined(DEBUG) */
    ...
    if (mvp_MPIDI_CH3I_RDMA_Process.has_hsam &&
        ...
        MPIR_Memset(mapped, 0, rdma_num_rails * sizeof(int));
        MPIR_Memset(actual_index, 0, rdma_num_rails * sizeof(int));
        ...
        for (rail = 0; rail < rdma_num_rails; rail++) {
            rreq->mrail.initial_weight[rail] = vc->mrail.rails[rail].s_weight;
            rreq->mrail.stripe_finish_time[rail] = 0;
        }
    }
#endif /*_ENABLE_HSAM_*/

    while (rreq->mrail.rndv_buf_off < rreq->mrail.rndv_buf_sz) {
        ...
        DEBUG_PRINT("[buffer content]: %02x,%02x,%02x, "
                    ...
                    ((char *)rreq->mrail.rndv_buf)[2], rreq->mrail.rndv_buf_off,
                    rreq->mrail.remote_addr);
        ...
        if (nbytes <= rdma_large_msg_rail_sharing_threshold) {
            ...
            MRAILI_RDMA_Get(
                vc, v,
                (char *)(rreq->mrail.rndv_buf) + rreq->mrail.rndv_buf_off,
                ((dreg_entry *)rreq->mrail.d_entry)
                    ->memhandle[vc->mrail.rails[rail].hca_index]
                    ->lkey,
                (char *)(rreq->mrail.remote_addr) + rreq->mrail.rndv_buf_off,
                rreq->mrail.rkey[vc->mrail.rails[rail].hca_index], nbytes,
                rail);

            rreq->mrail.num_rdma_read_completions++;

        } else if (!mvp_MPIDI_CH3I_RDMA_Process.has_hsam) {
            ...
            for (rail = 0; rail < rdma_num_rails - 1; rail++) {
                ...
                MRAILI_RDMA_Get(
                    ...
                    rreq->mrail.rkey[vc->mrail.rails[rail].hca_index], inc,
                    rail);
                rreq->mrail.num_rdma_read_completions++;
            ...
            for (count_rail = 0;
                 count_rail < ((rdma_num_rails / stripe_factor) - 1);
                 count_rail++) {
                ...
                MRAILI_RDMA_Get(
                    ...
                    (char *)(rreq->mrail.rndv_buf) + rreq->mrail.rndv_buf_off +
                        disp,
                    ((dreg_entry *)rreq->mrail.d_entry)
                        ->memhandle[vc->mrail.rails[mapped[count_rail]]
                                        .hca_index]
                        ->lkey,
                    (char *)(rreq->mrail.remote_addr) +
                        rreq->mrail.rndv_buf_off + disp,
                    rreq->mrail
                        .rkey[vc->mrail.rails[mapped[count_rail]].hca_index],
            ...
            MRAILI_RDMA_Get(
                ...
                (char *)(rreq->mrail.rndv_buf) + rreq->mrail.rndv_buf_off +
                    disp,
                ((dreg_entry *)rreq->mrail.d_entry)
                    ->memhandle[vc->mrail.rails[mapped[count_rail]].hca_index]
                    ->lkey,
                (char *)(rreq->mrail.remote_addr) + rreq->mrail.rndv_buf_off +
                    disp,
                rreq->mrail.rkey[vc->mrail.rails[mapped[count_rail]].hca_index],
...
void MPIDI_CH3I_MRAILI_Rendezvous_rput_push(MPIDI_VC_t *vc, MPIR_Request *sreq)
{
    vbuf *v;
    int rail;
    intptr_t nbytes, inc;
    ...
    if (sreq->mrail.rndv_buf_sz > 0) {
        MPIR_Assert(sreq->mrail.d_entry != NULL);
        MPIR_Assert(sreq->mrail.remote_addr != NULL);
    }
#endif /* defined(DEBUG) */
    ...
    if (mvp_MPIDI_CH3I_RDMA_Process.has_hsam &&
        ...
        for (rail = 0; rail < rdma_num_rails; rail++) {
            sreq->mrail.initial_weight[rail] = vc->mrail.rails[rail].s_weight;
    ...
    while (sreq->mrail.rndv_buf_off < sreq->mrail.rndv_buf_sz) {
        ...
        DEBUG_PRINT(
            "[buffer content]: %02x,%02x,%02x, offset %d, remote buf %p\n",
            ((char *)sreq->mrail.rndv_buf)[0],
            ((char *)sreq->mrail.rndv_buf)[1],
            ((char *)sreq->mrail.rndv_buf)[2], sreq->mrail.rndv_buf_off,
        ...
        if (nbytes <= rdma_large_msg_rail_sharing_threshold) {
            ...
            MRAILI_RDMA_Put(
                vc, v,
                (char *)(sreq->mrail.rndv_buf) + sreq->mrail.rndv_buf_off,
                ((dreg_entry *)sreq->mrail.d_entry)
                    ->memhandle[vc->mrail.rails[rail].hca_index]
                    ->lkey,
                (char *)(sreq->mrail.remote_addr) + sreq->mrail.rndv_buf_off,
                sreq->mrail.rkey[vc->mrail.rails[rail].hca_index], nbytes,
                rail);
            ...
            for (rail = 0; rail < rdma_num_rails - 1; rail++) {
                ...
                MRAILI_RDMA_Put(
                    ...
                    sreq->mrail.rkey[vc->mrail.rails[rail].hca_index], inc,
                    rail);
                /* Send the finish message immediately after the data */
            }
            GET_VBUF_BY_OFFSET_WITHOUT_LOCK(v, MVP_SMALL_DATA_VBUF_POOL_OFFSET);
            ...
            MRAILI_RDMA_Put(
                ...
                (char *)(sreq->mrail.remote_addr) + sreq->mrail.rndv_buf_off +
                    disp,
                sreq->mrail.rkey[vc->mrail.rails[mapped[count_rail]].hca_index],
                nbytes - disp, mapped[count_rail]);
        }
#endif /*_ENABLE_HSAM_*/
    ...
    if (sreq->mrail.rndv_buf_off != sreq->mrail.rndv_buf_sz) {
        DEBUG_PRINT("%s: [%d -> %d]: rndv_buf_off %d != rndv_buf_sz %d putting "
                    "vc back in flowlist\n",
...
int MPIDI_CH3I_MRAILI_Rendezvous_r3_ack_send(MPIDI_VC_t *vc)
{
    ...
    iov.iov_base = &r3_ack;
    iov.iov_len = sizeof(MPIDI_CH3_Pkt_rndv_r3_ack_t);
...
int MPIDI_CH3I_MRAIL_Finish_request(MPIR_Request *rreq)
{
    PRINT_DEBUG(
        DEBUG_RNDV_verbose > 1,
        "rreq: %p, protocol: %d, local_complete: %d, remote_complete: %d\n",
        rreq, rreq->mrail.protocol, rreq->mrail.local_complete,
        rreq->mrail.remote_complete);

    MPIR_Assert(rreq->mrail.local_complete == UINT32_MAX ||
                rreq->mrail.local_complete <= rdma_num_rails);
    MPIR_Assert(rreq->mrail.remote_complete == UINT32_MAX ||
                rreq->mrail.remote_complete <= rdma_num_rails);

    switch (rreq->mrail.protocol) {
        case MRAILI_PROTOCOL_RGET:
        case MRAILI_PROTOCOL_RPUT:
            return (rreq->mrail.local_complete == UINT32_MAX ||
...
void get_sorted_index(MPIDI_VC_t *vc, int *b)
{
    ...
    taken = (int *)MPL_malloc(sizeof(int) * rdma_num_rails, MPL_MEM_OTHER);

    /* Sanity */
    MPIR_Memset(taken, 0, sizeof(int) * rdma_num_rails);

    /* Sort the array */
    for (i = 0; i < rdma_num_rails; i++) {
        for (j = 0; j < rdma_num_rails; j++) {
            if ((vc->mrail.rails[j].s_weight >= max) && (taken[j] != -1)) {
        ...
        MPIR_Assert((index >= 0) && (index < rdma_num_rails));
        max = -1;
    }
...
#ifdef _ENABLE_HSAM_
void adjust_weights(MPIDI_VC_t *vc, double start_time, double *finish_time,
                    double *init_weight)
{
    int i;
    ...
    MPIR_Memset(rail_used, 0, sizeof(int) * MAX_NUM_SUBRAILS);

    for (i = 0; i < rdma_num_rails; i++) {
        /* This rail was used at all */
        if (finish_time[i] > 0) {
            finish_time[i] -= start_time;
            MPIR_Assert(finish_time[i] > 0);
            finish_time[i] /= 100;
    ...
    for (i = 0; i < rdma_num_rails; i++) {
        ...
        if (rail_used[i]) {
            ...
            MPIR_Assert(vc->mrail.rails[i].s_weight > 0);
        }
...
void get_wall_time(double *t)
{
    ...
    *t = (double)(tv.tv_sec - sec_base) * 1.0 + (double)tv.tv_usec * 1.0e-6;
}
