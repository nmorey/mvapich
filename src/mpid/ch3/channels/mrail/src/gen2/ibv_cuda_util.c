...
cvars:
    ...
    - name        : MV2_CUDA_KERNEL_VECTOR_TIDBLK_SIZE
      category    : CH3
      type        : int
      default     : 1024
      class       : none
    ...
    - name        : MV2_CUDA_KERNEL_VECTOR_YSIZE
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_KERNEL_SUBARR_TIDBLK_SIZE
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_KERNEL_SUBARR_XDIM
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_KERNEL_SUBARR_YDIM
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_KERNEL_SUBARR_ZDIM
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_ALLTOALL_DYNAMIC
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_REGISTER_NAIVE_BUF
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_GATHER_NAIVE_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_SCATTER_NAIVE_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_ALLTOALL_NAIVE_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_ALLTOALLV_NAIVE_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_ALLGATHER_NAIVE_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_ALLGATHERV_NAIVE_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_BCAST_NAIVE_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_GATHERV_NAIVE_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_SCATTERV_NAIVE_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_ALLGATHER_RD_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_ENABLE_IPC_CACHE
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_IPC_MAX_CACHE_ENTRIES
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_IPC_NUM_STAGE_BUFFERS
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_IPC_STAGE_BUF_SIZE
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_IPC_BUFFERED_LIMIT
      category    : CH3
      type        : int
      default     : -1
      class       : none
    ...
    - name        : MV2_CUDA_NONBLOCKING_STREAMS
      category    : CH3
      type        : int
      default     : 1
      class       : none
...
void MV2_MPII_IOV_pack_cuda(void *buf, struct iovec *iov, int n_iov,
                int position, cudaStream_t stream)
{
    ...
    for (i = 0; i < n_iov; i++) {
        MV2_MPIDI_Memcpy_Device_Async(ptr, iov[i].iov_base, iov[i].iov_len,
...
void MV2_MPII_IOV_unpack_cuda(void *buf, struct iovec *iov, int n_iov,
                int position, int *bytes_unpacked, cudaStream_t stream)
{
    ...
    for (i = 0; i < n_iov; i++) {
        MV2_MPIDI_Memcpy_Device_Async(iov[i].iov_base, ptr, iov[i].iov_len,
                                cudaMemcpyDefault, stream);
        ptr = (char *)ptr + iov[i].iov_len;
...
void vector_pack_cudabuf(void *buf, struct iovec *iov, int size, cudaStream_t stream)
{
    cudaError_t cerr = cudaSuccess;
    cerr = cudaMemcpy2DAsync(buf,
                iov[0].iov_len,
                iov[0].iov_base,
                (size_t)(iov[1].iov_base) - (size_t)(iov[0].iov_base),
                iov[0].iov_len,
                size / iov[0].iov_len,
...
void vector_unpack_cudabuf(void *buf, struct iovec *iov, int size, cudaStream_t stream)
{
    cudaError_t cerr = cudaSuccess;
    cerr = cudaMemcpy2DAsync(iov[0].iov_base,
                (size_t)(iov[1].iov_base) - (size_t)(iov[0].iov_base),
                buf,
                iov[0].iov_len,
                iov[0].iov_len,
...
int hindexed_pack_cudabuf(void *dst, struct iovec *iov, MPIR_Datatype *dtp, int size, cudaStream_t stream)
{
            ...
            if (HANDLE_GET_KIND(sub_array_of_types[0]) != HANDLE_KIND_BUILTIN) {
                ...
                if (sub_dtp->contents->combiner == MPI_COMBINER_CONTIGUOUS) {
                    ...
                    if (HANDLE_GET_KIND(cont_array_of_types[0]) == HANDLE_KIND_BUILTIN) {
                        builtin_element_size = sub_dtp->n_builtin_elements * MPIR_Datatype_get_basic_size(cont_array_of_types[0]);
                    } else { return FAILURE_PACKUNPACK_OPT; }
                } else { return FAILURE_PACKUNPACK_OPT; }
            }
...
int hindexed_unpack_cudabuf(void *src, struct iovec *iov, MPIR_Datatype *dtp, int size, cudaStream_t stream)
{
    int i, builtin_element_size;
    int struct_sz = sizeof(MPIR_Datatype_contents);
    int types_sz  = dtp->contents->nr_types * sizeof(MPI_Datatype);
...
int MPIDI_CH3_ReqHandler_pack_device(MPIDI_VC_t * vc, MPIR_Request * rreq, int *complete)
{
...
int MPIDI_CH3_ReqHandler_pack_device_stream(
        ...
        MPIR_Request *req, int *complete ATTRIBUTE((unused)), void* stream)
{
    MPI_Aint last;
    int iov_n;
        ...
        if (req->dev.datatype_ptr->contents->combiner == MPI_COMBINER_VECTOR
            ...
            if (rdma_cuda_kernel_dt_opt) {
                pack_unpack_vector_kernel(req->dev.tmpbuf,
                                          ...
                                          iov[0].iov_len,
                                          req->dev.segment_size / iov[0].iov_len,
                                          stream_passed);
            } else
#endif
            {
            ...
            if ( subarr_dims < 4 ){
              ...
              MV2_MPII_IOV_pack_cuda(req->dev.tmpbuf, iov, iov_n, req->dev.segment_first, stream_passed);
            }
#endif
            ...
            if (return_hindexed_pack == SUCCESS_PACKUNPACK_OPT) {
                ...
                MV2_MPII_IOV_pack_cuda(req->dev.tmpbuf, iov, iov_n,
...
int MPIDI_CH3_ReqHandler_unpack_device(MPIDI_VC_t *vc, MPIR_Request *req, int *complete)
{
    return MPIDI_CH3_ReqHandler_unpack_device_stream(vc, req, complete, NULL);
}

int MPIDI_CH3_ReqHandler_unpack_device_stream(
        MPIDI_VC_t *vc ATTRIBUTE((unused)), MPIR_Request *req,
    ...
    do {
        ...
        if (req->dev.datatype_ptr->contents->combiner == MPI_COMBINER_VECTOR
...
#if defined(USE_GPU_KERNEL)
            if (rdma_cuda_kernel_dt_opt) {
                pack_unpack_vector_kernel(iov[0].iov_base,
                                          iov[1].iov_base - iov[0].iov_base,
                                          req->dev.tmpbuf,
                                          iov[0].iov_len,
                                          iov[0].iov_len,
            ...
            if ( subarr_dims < 4 ){
              ...
              MV2_MPII_IOV_unpack_cuda(req->dev.tmpbuf, iov, iov_n, req->dev.segment_first, &bytes_copied, stream_passed);
            }
#endif
            ...
            if (return_hindexed_unpack == SUCCESS_PACKUNPACK_OPT) {
                ...
                MV2_MPII_IOV_unpack_cuda(req->dev.tmpbuf, iov, iov_n,
...
void MPID_Segment_pack_device(DLOOP_Segment *segp, DLOOP_Offset first,
        DLOOP_Offset *lastp, MPIR_Datatype *dt_ptr, void *streambuf)
{
    int iov_n;
    ...
    if (!is_device_buffer(streambuf)) {
        MV2_MPIDI_Malloc_Device(tmpbuf, *lastp);
        device_pack_buf = 0;
    } else {
        tmpbuf = streambuf;
    }
    ...
    do {
        ...
        if (sbuf_isdev && dt_ptr->contents->combiner == MPI_COMBINER_VECTOR
            ...
            if (return_hindexed_pack == SUCCESS_PACKUNPACK_OPT) {
                ...
                MV2_MPII_IOV_pack_cuda((char *)tmpbuf, iov, iov_n,
    ...
    if (!device_pack_buf) {
        MV2_MPIDI_Memcpy_Device(streambuf, tmpbuf, *lastp,
                                    cudaMemcpyDeviceToHost);
        MV2_MPIDI_Free_Device(tmpbuf);
    }
}

void MPID_Segment_unpack_device(DLOOP_Segment *segp, DLOOP_Offset first,
        DLOOP_Offset *lastp, MPIR_Datatype *dt_ptr, void *inbuf)
{
    int iov_n;
    int bytes_unpacked;
    ...
    if (!is_device_buffer(inbuf)) {
        device_unpack_buf = 0;
        MV2_MPIDI_Malloc_Device(tmpbuf, *lastp);
        MV2_MPIDI_Memcpy_Device(tmpbuf, inbuf, *lastp, cudaMemcpyHostToDevice);
    } else {
        tmpbuf = inbuf;
    }
    ...
    do {
        ...
        if (rbuf_isdev && dt_ptr->contents->combiner == MPI_COMBINER_VECTOR
            ...
            if (return_hindexed_unpack == SUCCESS_PACKUNPACK_OPT) {
                ...
                MV2_MPII_IOV_unpack_cuda(tmpbuf, iov, iov_n, buff_off,
...
void cuda_get_user_parameters() {
    ...
    if (MV2_EAGER_CUDAHOST_REG != -1) {
        rdma_eager_devicehost_reg = MV2_EAGER_CUDAHOST_REG;
    }

    if (MV2_CUDA_VECTOR_OPT != -1) {
        rdma_cuda_vector_dt_opt = MV2_CUDA_VECTOR_OPT;
    }

#if defined(USE_GPU_KERNEL)
    if (MV2_CUDA_KERNEL_OPT != -1) {
        rdma_cuda_kernel_dt_opt = MV2_CUDA_KERNEL_OPT;
    }
#else
    {
    ...
    if (MV2_CUDA_KERNEL_VECTOR_TIDBLK_SIZE != -1) {
        rdma_cuda_vec_thread_blksz = MV2_CUDA_KERNEL_VECTOR_TIDBLK_SIZE;
    }

    if (MV2_CUDA_KERNEL_VECTOR_YSIZE != -1) {
        rdma_cuda_vec_thread_ysz = MV2_CUDA_KERNEL_VECTOR_YSIZE;
    }

    if (MV2_CUDA_KERNEL_SUBARR_TIDBLK_SIZE != -1) {
        rdma_cuda_subarr_thread_blksz = MV2_CUDA_KERNEL_SUBARR_TIDBLK_SIZE;
    }

    if (MV2_CUDA_KERNEL_SUBARR_XDIM != -1) {
        rdma_cuda_subarr_thread_xdim = MV2_CUDA_KERNEL_SUBARR_XDIM;
    }

    if (MV2_CUDA_KERNEL_SUBARR_YDIM != -1) {
        rdma_cuda_subarr_thread_ydim = MV2_CUDA_KERNEL_SUBARR_YDIM;
    }

    if (MV2_CUDA_KERNEL_SUBARR_ZDIM != -1) {
        rdma_cuda_subarr_thread_zdim = MV2_CUDA_KERNEL_SUBARR_ZDIM;
    }

    if (MV2_CUDA_NUM_EVENTS != -1) {
        mv2_device_event_count = MV2_CUDA_NUM_EVENTS;
    }

    if (MV2_CUDA_INIT_CONTEXT != -1) {
        mv2_device_init_context = MV2_CUDA_INIT_CONTEXT;
    }

    if (MV2_CHECK_CUDA_ATTRIBUTE != -1) {
        mv2_device_check_attribute = MV2_CHECK_CUDA_ATTRIBUTE;
    }

    if (MV2_CUDA_USE_NAIVE != -1) {
        mv2_device_coll_use_stage = MV2_CUDA_USE_NAIVE;
    }

    if (MV2_CUDA_ALLTOALL_DYNAMIC != -1) {
        mv2_device_alltoall_dynamic = MV2_CUDA_ALLTOALL_DYNAMIC;
    }

    if (MV2_CUDA_REGISTER_NAIVE_BUF != -1) {
        mv2_device_coll_register_stage_buf_threshold =
            MV2_CUDA_REGISTER_NAIVE_BUF;
    }

    if (MV2_CUDA_GATHER_NAIVE_LIMIT != -1) {
        mv2_device_gather_stage_limit = MV2_CUDA_GATHER_NAIVE_LIMIT;
    }

    if (MV2_CUDA_SCATTER_NAIVE_LIMIT != -1) {
        mv2_device_scatter_stage_limit = MV2_CUDA_SCATTER_NAIVE_LIMIT;
    }

    if (MV2_CUDA_ALLTOALL_NAIVE_LIMIT != -1) {
        mv2_device_alltoall_stage_limit = MV2_CUDA_ALLTOALL_NAIVE_LIMIT;
    }

    if (MV2_CUDA_ALLTOALLV_NAIVE_LIMIT != -1) {
        mv2_device_alltoallv_stage_limit = MV2_CUDA_ALLTOALLV_NAIVE_LIMIT;
    }

    if (MV2_CUDA_ALLGATHER_NAIVE_LIMIT != -1) {
        mv2_device_allgather_stage_limit = MV2_CUDA_ALLGATHER_NAIVE_LIMIT;
    }

    if (MV2_CUDA_ALLGATHERV_NAIVE_LIMIT != -1) {
        mv2_device_allgatherv_stage_limit = MV2_CUDA_ALLGATHERV_NAIVE_LIMIT;
    }

    if (MV2_CUDA_BCAST_NAIVE_LIMIT != -1) {
        mv2_device_bcast_stage_limit = MV2_CUDA_BCAST_NAIVE_LIMIT;
    }

    if (MV2_CUDA_GATHERV_NAIVE_LIMIT != -1) {
        mv2_device_gatherv_stage_limit = MV2_CUDA_GATHERV_NAIVE_LIMIT;
    }

    if (MV2_CUDA_SCATTERV_NAIVE_LIMIT != -1) {
        mv2_device_scatterv_stage_limit = MV2_CUDA_SCATTERV_NAIVE_LIMIT;
    }

    if (MV2_CUDA_ALLGATHER_RD_LIMIT != -1) {
        mv2_device_allgather_rd_limit = MV2_CUDA_ALLGATHER_RD_LIMIT;
    }

    if (MV2_CUDA_ALLGATHER_FGP != -1) {
        mv2_device_use_allgather_fgp = MV2_CUDA_ALLGATHER_FGP;
    }


#if defined(HAVE_CUDA_IPC)
    if (MV2_CUDA_IPC != -1) {
        mv2_device_use_ipc = MV2_CUDA_IPC;
    }

    if (MV2_CUDA_IPC_SHARE_GPU != -1) {
        rdma_enable_ipc_share_gpu = MV2_CUDA_IPC_SHARE_GPU;
    }

    if (MV2_CUDA_SMP_IPC != -1) {
        mv2_device_use_smp_eager_ipc = MV2_CUDA_SMP_IPC;
    }
    ...
    if (MV2_CUDA_ENABLE_IPC_CACHE != -1) {
        mv2_device_enable_ipc_cache = MV2_CUDA_ENABLE_IPC_CACHE;
    }

    if (MV2_CUDA_IPC_MAX_CACHE_ENTRIES != -1) {
        mv2_device_ipc_cache_max_entries = MV2_CUDA_IPC_MAX_CACHE_ENTRIES;
    }

    if (MV2_CUDA_IPC_NUM_STAGE_BUFFERS != -1) {
        cudaipc_num_stage_buffers = MV2_CUDA_IPC_NUM_STAGE_BUFFERS;
    }
    if (MV2_CUDA_IPC_STAGE_BUF_SIZE != -1) {
        cudaipc_stage_buffer_size = MV2_CUDA_IPC_STAGE_BUF_SIZE;
    }
    if (MV2_CUDA_IPC_BUFFERED != -1) {
        mv2_device_use_ipc_stage_buffer = MV2_CUDA_IPC_BUFFERED;
    }
    if (mv2_device_use_ipc_stage_buffer) {
        mv2_device_ipc_threshold = 0;
    }
    if (MV2_CUDA_IPC_THRESHOLD != -1) {
        mv2_device_ipc_threshold = MV2_CUDA_IPC_THRESHOLD;
    }
    if (MV2_CUDA_IPC_BUFFERED_LIMIT != -1) {
        mv2_device_ipc_stage_buffer_limit = MV2_CUDA_IPC_BUFFERED_LIMIT;
    }
    if (MV2_CUDA_IPC_SYNC_LIMIT != -1) {
        cudaipc_sync_limit = MV2_CUDA_IPC_SYNC_LIMIT;
    }
    MPIR_Assert(mv2_device_ipc_stage_buffer_limit >= mv2_device_ipc_threshold);

    if (MV2_CUDA_DYNAMIC_INIT != -1) {
        mv2_device_dynamic_init = MV2_CUDA_DYNAMIC_INIT;
    }

    if (MV2_CUDA_NONBLOCKING_STREAMS != -1) {
        mv2_device_nonblocking_streams = MV2_CUDA_NONBLOCKING_STREAMS;
    }
...
void device_init (MPIDI_PG_t * pg)
{
    ...
    deviceipc_num_local_procs =  MPIDI_MV2_Num_local_processes(pg);
    deviceipc_my_local_id = MPIDI_MV2_Get_local_process_id(pg);
