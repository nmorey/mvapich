...
extern MPIR_Request *create_eagercontig_request(
...
#define SMPI_BUF_POOL_PTR(destination, index)                                  \
    ((SEND_BUF_T *)((unsigned long)s_buffer_head[destination] +                \
                    (sizeof(SEND_BUF_T) + MVP_SMP_SEND_BUF_SIZE) * index))
...
static intptr_t *s_current_bytes = NULL;
static intptr_t *s_total_bytes = NULL;
...
void **smp_device_region_base;
void **smp_device_region_send;
void **smp_device_region_recv;
size_t *smp_device_region_head;
int smp_device_region_size;
int smp_device_eagersize;
int s_smp_device_handle_cache = 0;

deviceEvent_t *sr_event;
deviceEvent_t *sr_event_local;
deviceEvent_t *loop_event;
deviceEvent_t *loop_event_local;
#endif
...
static inline ssize_t process_vm_writev(
...
extern MPIR_Request *create_request(void *hdr, intptr_t hdr_sz, size_t nb);
#endif /*#if defined(_SMP_CMA_)*/

#if defined(_SMP_LIMIC_) || defined(_SMP_CMA_)
void MPIDI_CH3I_SMP_send_comp(void *c_header, MPIDI_VC_t *vc, intptr_t nb,
                              smp_dma_flag_t dma_flag,
...
extern int mvp_enable_shmem_collectives;
extern struct mvp_MPIDI_CH3I_RDMA_Process_t mvp_MPIDI_CH3I_RDMA_Process;

extern int MPIDI_MVP_Get_num_nodes();
extern int rdma_set_smp_parameters(struct mvp_MPIDI_CH3I_RDMA_Process_t *proc);
extern void MPIR_MVP_SHMEM_COLL_Cleanup();
                                      ...
                                      unsigned int my_id, unsigned int length)
{
    ...
    volatile void *ptr_flag =
        (volatile void *)((unsigned long)g_smpi_shmem->pool +
                          s_header_ptr_r[from_grank]);
    unsigned long header;
    header = s_header_ptr_r[from_grank];
    s_header_ptr_r[from_grank] =
        s_header_ptr_r[from_grank] + length + sizeof(int) * 2;
    READBAR();
                                   ...
                                   smp_ctrl_avail_flag_t num)
{
    ...
    if (avail[rank] < num * (len + sizeof(int) * 3)) {
        ...
        if (s_header_ptr_s[rank] + len + sizeof(int) * 3 >=
            ...
            s_tail_ptr_s[rank] = SMPI_SHARED_TAIL(g_smpi.my_local_id, rank);
            if (s_tail_ptr_s[rank] == SMPI_FIRST_S(g_smpi.my_local_id, rank)) {
                avail[rank] = SMPI_LAST_S(g_smpi.my_local_id, rank) -
                              s_header_ptr_s[rank];
                return 0;
            }
            s_header_ptr_s[rank] = SMPI_FIRST_S(g_smpi.my_local_id, rank);
            volatile void *ptr_flag;
            ptr_flag = *pptr_flag;
            *(volatile int *)ptr_flag = SMP_CBUF_END;
            WRITEBAR();
            ptr_flag =
                (volatile void *)((g_smpi_shmem->pool) + s_header_ptr_s[rank]);
            *pptr_flag = ptr_flag;
        } else {
            s_tail_ptr_s[rank] = SMPI_SHARED_TAIL(g_smpi.my_local_id, rank);
        }
        ...
        /* update avail */
        READBAR();
        avail[rank] = (s_tail_ptr_s[rank] >= s_header_ptr_s[rank] ?
...
static inline int MPIDI_CH3I_SMP_attach_shm_pool_inline()
{
    ...
    s_buffer_head = (SEND_BUF_T **)MPL_malloc(
        sizeof(SEND_BUF_T *) * g_smpi.num_local_nodes, MPL_MEM_ADDRESS);
    if (!s_buffer_head) {
        mpi_errno = MPIR_Err_create_code(MPI_SUCCESS, MPI_ERR_OTHER, __func__,
    ...
    for (i = 0; i < g_smpi.num_local_nodes; ++i) {
        s_buffer_head[i] =
            (SEND_BUF_T *)((unsigned long)g_smpi.send_buf_pool_ptr +
                           SMPI_ALIGN(
                               (sizeof(SEND_BUF_T) + MVP_SMP_SEND_BUF_SIZE) *
    ...
    s_sh_buf_pool.send_queue = (int *)MPL_malloc(
        sizeof(int) * g_smpi.num_local_nodes, MPL_MEM_ADDRESS);
    ...
    s_sh_buf_pool.tail = (int *)MPL_malloc(sizeof(int) * g_smpi.num_local_nodes,
                                           MPL_MEM_ADDRESS);
    ...
    if (mvp_enable_device && mvp_device_initialized) {
        ibv_device_register(g_smpi.send_buf_pool_ptr, g_size_pool);
    }
#endif
...
static inline int MPIDI_CH3I_SMP_Process_header(MPIDI_VC_t *vc,
                                                MPIDI_CH3_Pkt_t *pkt,
                                                int *index, void *limic_header,
                                                void *cma_header,
                                                int *use_limic, int *use_cma)
{
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_PROGRESS_HEADER);
    int mpi_errno = MPI_SUCCESS;

    if (pkt->type == MPIDI_CH3_PKT_EAGER_SEND_CONTIG) {
        intptr_t buflen = s_total_bytes[vc->smp.local_nodes] -
                          sizeof(MPIDI_CH3_Pkt_eager_send_t);
        if ((mpi_errno = MPIDI_CH3_PktHandler_EagerSend_Contig(
                 vc, pkt, ((char *)pkt + MPIDI_CH3U_PKT_SIZE(pkt)), &buflen,
        ...
        if (!vc->smp.recv_active) {
            ...
            smpi_complete_recv(vc->smp.local_nodes, g_smpi.my_local_id,
    ...
    if (MPIDI_CH3_PKT_SMP_DMA_COMP == pkt->type) {
        ...
        MPIR_Request *sreq = (MPIR_Request *)(lc_pkt->send_req_id);

        intptr_t nb = lc_pkt->nb;
        int complete = 0;
...
#endif
        {
            if (MPIDI_CH3I_Request_adjust_iov(sreq, nb)) {
                ...
                /* We have received the expected fin message as part of the
                 * CMA/LiMIC based message transfer */
                MVP_DEC_NUM_POSTED_RECV();
            }
    ...
    if (MPIDI_CH3_PKT_RNDV_R3_DATA == pkt->type) {
        MPIDI_CH3_Pkt_rndv_r3_data_t *pkt_header =
            (MPIDI_CH3_Pkt_rndv_r3_data_t *)pkt;
        ...
        if (pkt_header->csend_req_id) {
            *use_cma = 1;
            MPIR_Memcpy(c_header->remote, s_current_ptr[vc->smp.local_nodes],
                        sizeof(struct iovec));

            s_current_ptr[vc->smp.local_nodes] =
                (void *)((unsigned long)s_current_ptr[vc->smp.local_nodes] +
                         sizeof(struct iovec));
            ...
            s_current_ptr[vc->smp.local_nodes] =
                (void *)((unsigned long)s_current_ptr[vc->smp.local_nodes] +
                         sizeof(pid_t));
            c_header->total_bytes =
                *((intptr_t *)s_current_ptr[vc->smp.local_nodes]);
            c_header->csend_req_id = (MPIR_Request *)(pkt_header->csend_req_id);

            s_current_ptr[vc->smp.local_nodes] =
                (void *)((unsigned long)s_current_ptr[vc->smp.local_nodes] +
                         sizeof(intptr_t));
            s_current_bytes[vc->smp.local_nodes] =
                s_current_bytes[vc->smp.local_nodes] - sizeof(struct iovec) -
                sizeof(intptr_t) - sizeof(pid_t);
        } else {
#endif /*#if defined(_SMP_CMA_)*/
            ...
            if (pkt_header->send_req_id) {
                *use_limic = 1;
                MPIR_Memcpy(&(l_header->lu), s_current_ptr[vc->smp.local_nodes],
                            sizeof(limic_user));
                ...
                l_header->total_bytes =
                    *((intptr_t *)s_current_ptr[vc->smp.local_nodes]);
                l_header->send_req_id =
                    (MPIR_Request *)(pkt_header->send_req_id);
                ...
                s_current_bytes[vc->smp.local_nodes] =
                    s_current_bytes[vc->smp.local_nodes] -
                    sizeof(struct limic_user) - sizeof(intptr_t);
            } else {
#endif /*#if defined(_SMP_LIMIC_)*/
        ...
        MPIR_Request_get_ptr(
            ((MPIDI_CH3_Pkt_rndv_r3_data_t *)pkt)->receiver_req_id, rreq);
        ...
        PRINT_DEBUG(DEBUG_RNDV_verbose > 1, "RPUT FINISH received from: %d\n",
                    vc->pg_rank);
        MPIDI_CH3_Rendezvous_rput_finish(vc,
                                         (MPIDI_CH3_Pkt_rput_finish_t *)pkt);
        goto fn_exit;
    } else if (pkt->type == MPIDI_CH3_PKT_RGET_FINISH) {
        PRINT_DEBUG(DEBUG_RNDV_verbose > 1, "RGET FINISH received from: %d\n",
                    vc->pg_rank);
        MPIDI_CH3_Rendezvous_rget_send_finish(
            vc, (MPIDI_CH3_Pkt_rget_finish_t *)pkt);
        goto fn_exit;
    }
#if defined(CKPT)
    /*
     ...
     */
        ...
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "%s (pid %ld):[%d <= %d]: get CM_SUSPEND vcstate=%d, "
                    "send=%d,recv=%d\n",
                    __func__, pthread_self(), MPIDI_Process.my_pg_rank,
                    vc->pg_rank, vc->ch.state, vc->mrail.suspended_rails_send,
                    vc->mrail.suspended_rails_recv);
     ...
     */
        ...
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "(pid %ld):CM_REACT channel from %d to %d. vcstate=%d\n",
                    pthread_self(), MPIDI_Process.my_pg_rank, vc->pg_rank,
                    vc->ch.state);
...
fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMP_PROGRESS_HEADER);
    return mpi_errno;
...
{
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_WRITE_PROGRESS);
    int mpi_errno = MPI_SUCCESS;
    int nb;
    int i = 0;
    MPIDI_VC_t *vc;
    ...
    MPIR_T_PVAR_COUNTER_INC(MVP, mvp_smp_write_progress_poll, 1);
    ...
    for (i = 0; i < g_smpi.num_local_nodes; ++i) {
        ...
        while (vc->smp.send_active != NULL) {
            MPIR_Request *req = vc->smp.send_active;
            PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                        "smp send active, vc->rank: %d, req: %p, type: %d, "
                        "ch.reqtype: %d\n",
                        vc->pg_rank, req, MPIDI_Request_get_type(req),
                        req->ch.reqtype);
...
#if defined(_ENABLE_CUDA_)
            if (mvp_enable_device && MVP_CUDA_SMP_PIPELINE) {
                /*as it is all data, we check the first iov to determine if the
                 * buffer is on device*/
                iov_isdev = is_device_buffer(
                    (void *)req->dev.iov[req->dev.iov_offset].iov_base);
            }
#endif
                    ...
                    if (iov_isdev) {
                        int is_cont =
                            (vc->smp.send_current_pkt_type == SMP_RNDV_MSG) ?
                                0 :
                                1;
                        mpi_errno = MPIDI_CH3I_SMP_writev_rndv_data_device(
                            vc, req, req->dev.iov + req->dev.iov_offset,
                            req->dev.iov_count - req->dev.iov_offset, &nb,
                            is_cont);
                    } else
#endif
...
#if defined(_ENABLE_CUDA_)
                    if (iov_isdev &&
                        (req->ch.reqtype == REQUEST_RNDV_R3_DATA)) {
                        /* dequeue the request so that the next one is processed
                         * the actual request is completed when the copy
                         * completes */
...
#endif
                    {
                        if ((mpi_errno = MPIDI_CH3U_Handle_send_req(
...
fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMP_WRITE_PROGRESS);
    return mpi_errno;
...
{
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_READ_PROGRESS);
    int mpi_errno = MPI_SUCCESS;
    MPIDI_VC_t *vc = NULL;
    ...
    if (MVP_SMP_PRIORITY_POLLING) {
        ...
        while (ptr) {
...
#if defined(_ENABLE_CUDA_) && defined(HAVE_CUDA_IPC)
            if (mvp_enable_device && vc->smp.local_rank == -1) {
                continue;
            }
#endif
            if (!vc->smp.recv_active) {
                ...
                if (pkt_head) {
                    ...
                    PRINT_DEBUG(
                        DEBUG_RNDV_verbose > 1,
                        "vc rank: %d, pkt_head: %p, limic: %d, cma: %d\n",
                        vc->pg_rank, pkt_head, use_limic, use_cma);
                    MPIR_T_PVAR_COUNTER_INC(
                        MVP, mvp_smp_read_progress_poll_success, 1);
                    mpi_errno = MPIDI_CH3I_SMP_Process_header(
                        vc, pkt_head, &index, &l_header, &c_header, &use_limic,
                        &use_cma);
            ...
            if (vc->smp.recv_active) {
                struct MPIR_Request *req ATTRIBUTE((unused)) =
                    vc->smp.recv_active;
                PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                            "smp recv active, rank: %d, req: %p, type: %d, "
                            "ch.reqtype: %d, pkt_type: %d\n",
                            vc->pg_rank, req, MPIDI_Request_get_type(req),
                            req->ch.reqtype, vc->smp.recv_current_pkt_type);
                poll_flag = 1;
#if defined(_ENABLE_CUDA_)
                if (mvp_enable_device && MVP_CUDA_SMP_PIPELINE) {
                    /*as it is all data, we check the first iov to determine if
                     * the buffer is on device*/
                    iov_isdev = is_device_buffer(
                        (void *)vc->smp.recv_active->dev
                            .iov[vc->smp.recv_active->dev.iov_offset]
                            .iov_base);
                }
#endif
...
#if defined(_ENABLE_CUDA_)
                        if (iov_isdev) {
                            mpi_errno = MPIDI_CH3I_SMP_readv_rndv_device(
                                vc, vc->smp.recv_active,
                                &vc->smp.recv_active->dev
                                     .iov[vc->smp.recv_active->dev.iov_offset],
                    ...
                    case SMP_RNDV_MSG_CONT:
...
#endif
                        {
                            ...
                            if (vc->smp.use_limic == 1)
                                ...
                                dma_flag = SMP_DMA_CMA;
                            mpi_errno = MPIDI_CH3I_SMP_readv_rndv_cont(
                                vc,
                                &vc->smp.recv_active->dev
                                     .iov[vc->smp.recv_active->dev.iov_offset],
                                vc->smp.recv_active->dev.iov_count -
                                    vc->smp.recv_active->dev.iov_offset,
                                index, &vc->smp.current_l_header,
                                &vc->smp.current_c_header, &nb, dma_flag);
                        }
                        break;
                    default:
                        mpi_errno = MPIDI_CH3I_SMP_readv(
                            vc,
                            &vc->smp.recv_active->dev
                                 .iov[vc->smp.recv_active->dev.iov_offset],
                            vc->smp.recv_active->dev.iov_count -
                ...
                PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                            "request to fill: iovlen %ld, iov[0].len %ld, [1] "
                            "%ld, nb %ld\n",
                            vc->smp.recv_active->dev.iov_count -
                                vc->smp.recv_active->dev.iov_offset,
                            vc->smp.recv_active->dev
                                ...
                                .iov_len,
                            vc->smp.recv_active->dev
                                .iov[vc->smp.recv_active->dev.iov_offset + 1]
                                .iov_len,
                            nb);

                if (nb > 0) {
                    PRINT_DEBUG(DEBUG_RNDV_verbose > 1,
                                "Read %lu bytes from rank: %d, req: %p, cma: "
                                "%d, limic: %d\n",
                                nb, vc->pg_rank, vc->smp.recv_active, use_cma,
                                use_limic);
                    if (MPIDI_CH3I_Request_adjust_iov(vc->smp.recv_active,
                                                      nb)) {
#if defined(_ENABLE_CUDA_)
                        if (iov_isdev &&
                            (vc->smp.recv_current_pkt_type == SMP_RNDV_MSG ||
                             vc->smp.recv_current_pkt_type ==
                                 SMP_RNDV_MSG_CONT)) {
                            /*set recv_active to NULL so that next request is
                             * picked up*/
                        ...
                        {
                            PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                                        "adjust iov finished, handle req\n");
                            mpi_errno = MPIDI_CH3U_Handle_recv_req(
                                vc, vc->smp.recv_active, &complete);
                            PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                                        "finished handle req, complete %d\n",
                                        complete);

                            if (mpi_errno)
                                MPIR_ERR_POP(mpi_errno);

                            while (vc->smp.recv_current_pkt_type ==
                                       SMP_EAGER_MSG &&
                                   !complete) {
                                ...
                                if (!MPIDI_CH3I_Request_adjust_iov(
                                        vc->smp.recv_active, nb)) {
                                    goto fn_exit;
                                }
                                mpi_errno = MPIDI_CH3U_Handle_recv_req(
                                    vc, vc->smp.recv_active, &complete);
                            ...
                            if (complete) {
                                ...
                                if (vc->smp.recv_current_pkt_type ==
                                    SMP_RNDV_MSG) {
                                    if (use_limic)
                                        MPIDI_CH3I_SMP_send_comp(
                                            &l_header, vc, nb, SMP_DMA_LIMIC,
                                            NO_FALLBACK);
                                    else if (use_cma)
                                        MPIDI_CH3I_SMP_send_comp(
                                            &c_header, vc, nb, SMP_DMA_CMA,
                                            NO_FALLBACK);
                                } else if (vc->smp.recv_current_pkt_type ==
                                           SMP_RNDV_MSG_CONT) {
                                    if (vc->smp.use_limic) {
                                        vc->smp.current_nb += nb;
                                        MPIDI_CH3I_SMP_send_comp(
                                            &vc->smp.current_l_header, vc,
                                            vc->smp.current_nb, SMP_DMA_LIMIC,
                                            NO_FALLBACK);
                                    } else if (vc->smp.use_cma) {
                                        vc->smp.current_cnb += nb;
                                        MPIDI_CH3I_SMP_send_comp(
                                            &vc->smp.current_c_header, vc,
                                            vc->smp.current_cnb, SMP_DMA_CMA,
                                            NO_FALLBACK);
                                    }
                                }
#endif
                ...
                MPIR_T_PVAR_COUNTER_INC(MVP, mvp_smp_read_progress_poll_success,
                                        1);
            }
...
fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMP_READ_PROGRESS);
    return mpi_errno;
...
void MPIDI_CH3I_set_smp_only()
{
    ...
    if (MVP_USE_PT2PT_SHMEM != -1) {
        rdma_use_smp = !!MVP_USE_PT2PT_SHMEM;
    }

    if (MVP_USE_SHARED_MEM != -1) {
        mvp_use_smp = !!MVP_USE_SHARED_MEM;
        if (!mvp_use_smp) {
            rdma_use_smp = 0;
        }
    }

    if (MVP_USE_BLOCKING != -1) {
        rdma_use_blocking = !!MVP_USE_BLOCKING;
    }
...
int MPIDI_CH3I_CM_SHMEM_Sync(volatile int *volatile bar_array, int my_local_id,
                             int num_local_procs)
{
    int mpi_errno = MPI_SUCCESS;
...
int MPIDI_CH3I_CM_Unlink_region(MPIDI_PG_t *pg)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPIDI_CH3I_CM_UNLINK_REGION);
    ...
    if (pg->ch.mrail->cm_shmem_file != NULL) {
        MPL_free(pg->ch.mrail->cm_shmem_file);
        pg->ch.mrail->cm_shmem_file = NULL;
    }

    MPIR_FUNC_VERBOSE_EXIT(MPIDI_CH3I_CM_UNLINK_REGION);
    return mpi_errno;
}
...
int MPIDI_CH3I_CM_Destroy_region(MPIDI_PG_t *pg)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPIDI_CH3I_CM_DESTROY_REGION);
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPIDI_CH3I_CM_DESTROY_REGION);
    return mpi_errno;
}
...
int MPIDI_CH3I_CM_Create_region(MPIDI_PG_t *pg)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPIDI_CH3I_CM_CREATE_REGION);

    /* Identify size of shared mem file size for storing UD CM info */
    pid_len = sizeof(int) * pg->ch.num_local_processes;
    pid_len =
        (pid_len + SMPI_CACHE_LINE_SIZE) - (pid_len % SMPI_CACHE_LINE_SIZE);
    ud_cm_len = sizeof(MPIDI_CH3I_MRAIL_UD_CM_t) * pg->size;
    ud_cm_len =
        (ud_cm_len + SMPI_CACHE_LINE_SIZE) - (ud_cm_len % SMPI_CACHE_LINE_SIZE);
#ifdef _ENABLE_UD_
    if (MVP_USE_UD_HYBRID) {
        ud_len = (sizeof(mvp_ud_exch_info_t) * pg->size * rdma_num_hcas);
        ud_len =
            (ud_len + SMPI_CACHE_LINE_SIZE) - (ud_len % SMPI_CACHE_LINE_SIZE);
    }
#endif /* _ENABLE_UD_ */
    ...
    mpi_errno = MPIR_MVP_SHMEM_Helper_fn(
        pg->ch.local_process_id, &pg->ch.mrail->cm_shmem_file, "cm_shmem",
        &pg->ch.mrail->cm_shmem_fd, cm_shmem_file_size);
    if (mpi_errno != MPI_SUCCESS) {
        MPIR_ERR_POP(mpi_errno);
    }

    /* Mmap UD_CM SHMEM region */
    pg->ch.mrail->cm_shmem_mmap_ptr =
        mmap(0, cm_shmem_file_size, (PROT_READ | PROT_WRITE), (MAP_SHARED),
    ...
    pg->ch.mrail->cm_shmem.pid =
        (int *)((char *)pg->ch.mrail->cm_shmem_mmap_ptr);
    pg->ch.mrail->cm_shmem.ud_cm =
        (MPIDI_CH3I_MRAIL_UD_CM_t *)((char *)pg->ch.mrail->cm_shmem_mmap_ptr +
                                     pid_len);
#ifdef _ENABLE_UD_
    if (MVP_USE_UD_HYBRID) {
        int i = 0, j = 0;
        MPIR_Assert(rdma_num_hcas > 0);
        char *base_addr =
            (char *)pg->ch.mrail->cm_shmem_mmap_ptr + pid_len + ud_cm_len;
        for (i = 0; i < pg->size; i++) {
            pg->ch.mrail->cm_shmem.remote_ud_info[i] =
                (mvp_ud_exch_info_t *)((char *)base_addr +
                                       i * rdma_num_hcas *
                                           sizeof(mvp_ud_exch_info_t));
        ...
        PRINT_DEBUG(DEBUG_CM_verbose > 1,
                    "cm_shmem.pid = %p, cm_shmem.ud_cm = %p,"
    ...
    if (pg->ch.local_process_id == 0) {
        MPIR_Memset(pg->ch.mrail->cm_shmem.ud_cm, 0, ud_cm_len);
    }
...
fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPIDI_CH3I_CM_CREATE_REGION);
    return mpi_errno;
...
{
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_INIT);
    int mpi_errno = MPI_SUCCESS;
    unsigned int i;
    ...
    rdma_set_smp_parameters(&mvp_MPIDI_CH3I_RDMA_Process);
    ...
    sh_size = sizeof(struct shared_mem) + pid_len + SMPI_ALIGN(st_len) +
...
#if defined(_ENABLE_CUDA_) && defined(HAVE_CUDA_IPC)
    cu_ipc_len = sizeof(smpi_device_ipc_attr) * g_smpi.num_local_nodes *
                 (g_smpi.num_local_nodes - 1);
    ...
    g_size_pool = SMPI_ALIGN((sizeof(SEND_BUF_T) + MVP_SMP_SEND_BUF_SIZE) *
                                 MVP_SMP_NUM_SEND_BUFFER +
                             pagesize) *
    ...
    MPIR_T_PVAR_LEVEL_INC(
        ...
        SMPI_ALIGN((sizeof(SEND_BUF_T) + MVP_SMP_SEND_BUF_SIZE) *
                       MVP_SMP_NUM_SEND_BUFFER +
                   pagesize));
    ...
    /* Call helper function to create shmem region */
    mpi_errno = MPIR_MVP_SHMEM_Helper_fn(g_smpi.my_local_id, &shmem_file,
                                         "ib_shmem", &g_smpi.fd, g_size_shmem);
    MPIR_ERR_CHECK(mpi_errno);

    /* Call helper function to create shmem region */
    mpi_errno =
        MPIR_MVP_SHMEM_Helper_fn(g_smpi.my_local_id, &pool_file, "ib_pool",
                                 &g_smpi.fd_pool, g_size_pool);
    MPIR_ERR_CHECK(mpi_errno);

    g_smpi_shmem =
        (struct shared_mem *)MPL_malloc(sizeof(struct shared_mem), MPL_MEM_SHM);
    if (!g_smpi_shmem) {
        mpi_errno = MPIR_Err_create_code(MPI_SUCCESS, MPI_ERR_OTHER, __func__,
                                         __LINE__, MPI_ERR_OTHER, "**nomem", 0);
        MPIR_ERR_POP(mpi_errno);
    ...
    g_smpi.mmap_ptr = mmap(0, g_size_shmem, (PROT_READ | PROT_WRITE),
                           (MAP_SHARED), g_smpi.fd, 0);
    ...
    if (((long)shmem & (SMPI_CACHE_LINE_SIZE - 1)) != 0) {
        ...
        mpi_errno = MPIR_Err_create_code(
            MPI_SUCCESS, MPI_ERR_OTHER, __func__, __LINE__, MPI_ERR_OTHER,
            "**nomem", "%s", "error in shifting mapped shmem");
        MPIR_ERR_POP(mpi_errno);
    }
    ...
    g_smpi_shmem->rqueues_limits_s = (smpi_rq_limit *)MPL_malloc(
        sizeof(smpi_rq_limit) * g_smpi.num_local_nodes, MPL_MEM_SHM);
    g_smpi_shmem->rqueues_limits_r = (smpi_rq_limit *)MPL_malloc(
        sizeof(smpi_rq_limit) * g_smpi.num_local_nodes, MPL_MEM_SHM);
    g_smpi_shmem->shared_tails = (smpi_shared_tails **)MPL_malloc(
        sizeof(smpi_shared_tails *) * g_smpi.num_local_nodes, MPL_MEM_SHM);

#if defined(_ENABLE_CUDA_) && defined(HAVE_CUDA_IPC)

    g_smpi_shmem->cu_attrbs = (smpi_device_ipc_attr **)MPL_malloc(
        sizeof(smpi_device_ipc_attr *) * g_smpi.num_local_nodes, MPL_MEM_OTHER);
#endif
    ...
    if (g_smpi.num_local_nodes > 1) {
        g_smpi_shmem->shared_tails[0] =
            (smpi_shared_tails *)((char *)shmem + pid_len +
                                  SMPI_CACHE_LINE_SIZE);

        for (i = 1; i < g_smpi.num_local_nodes; ++i) {
            g_smpi_shmem->shared_tails[i] =
                (smpi_shared_tails *)(g_smpi_shmem->shared_tails[i - 1] +
                                      g_smpi.num_local_nodes);
        }

        g_smpi_shmem->pool =
            (char *)((char *)g_smpi_shmem->shared_tails[1] +
                     SMPI_ALIGN(st_len) + SMPI_CACHE_LINE_SIZE);

#if defined(_ENABLE_CUDA_) && defined(HAVE_CUDA_IPC)

        g_smpi_shmem->cu_attrbs[0] =
            (smpi_device_ipc_attr *)((char *)shmem + cu_ipc_offset);

        for (i = 1; i < g_smpi.num_local_nodes; ++i) {
            g_smpi_shmem->cu_attrbs[i] =
                (smpi_device_ipc_attr *)(g_smpi_shmem->cu_attrbs[i - 1] +
                                         g_smpi.num_local_nodes);
        }
    ...
    for (i = 0; i < g_smpi.num_local_nodes; ++i) {
        if (i == g_smpi.my_local_id)
            continue;
        g_smpi_shmem->rqueues_limits_s[i].first = SMPI_ALIGN(
            pagesize + (pagesize + MVP_SMP_QUEUE_LENGTH) *
                           (i * (g_smpi.num_local_nodes - 1) +
                            (g_smpi.my_local_id > i ? (g_smpi.my_local_id - 1) :
                                                      g_smpi.my_local_id)));
        g_smpi_shmem->rqueues_limits_r[i].first = SMPI_ALIGN(
            pagesize + (pagesize + MVP_SMP_QUEUE_LENGTH) *
...
#if defined(_SMP_CMA_)
    if (MVP_SMP_USE_CMA && g_smpi.num_local_nodes > 1) {
        g_smpi_shmem->cma_test_buffer =
            (volatile char **)((char *)shmem + cma_test_buffer_offset);
            ...
            case 0:
                ...
                if (-1 != (intptr_t)*g_smpi_shmem->cma_test_buffer) {
                    ...
                    mpi_errno = MPIR_Err_create_code(
                        ...
                        "*g_smpi_shmem->cma_test_buffer");
                    PRINT_ERROR("CMA is not available. Set MVP_SMP_USE_CMA=0 "
                                "to disable CMA.\n");
                    MPIR_ERR_POP(mpi_errno);
                }
                break;
                ...
                *g_smpi_shmem->cma_test_buffer =
                ...
                if (!*g_smpi_shmem->cma_test_buffer) {
                    *g_smpi_shmem->cma_test_buffer = (char *)-1;
                    ...
                    while (!**g_smpi_shmem->cma_test_buffer)
                        ;
                    MPL_free(*g_smpi_shmem->cma_test_buffer);
                }
    ...
    s_current_ptr = (void **)MPL_malloc(sizeof(void *) * g_smpi.num_local_nodes,
                                        MPL_MEM_OTHER);
    if (!s_current_ptr) {
    ...
    s_current_bytes = (intptr_t *)MPL_malloc(
        sizeof(intptr_t) * g_smpi.num_local_nodes, MPL_MEM_OTHER);
    if (!s_current_bytes) {
        MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_OTHER, "**nomem",
    ...
    s_header_ptr_s = (size_t *)MPL_malloc(
        sizeof(size_t) * g_smpi.num_local_nodes, MPL_MEM_OTHER);
    ...
    s_tail_ptr_s = (size_t *)MPL_malloc(sizeof(size_t) * g_smpi.num_local_nodes,
                                        MPL_MEM_OTHER);
    ...
    avail = (size_t *)MPL_malloc(sizeof(size_t) * g_smpi.num_local_nodes,
                                 MPL_MEM_OTHER);
    ...
    if (MVP_SMP_USE_LIMIC2) {
        ...
        mvp_MPIDI_CH3I_RDMA_Process.g_smp_can_fallback = 1;
    }
#endif /*#if defined(_SMP_LIMIC_)*/

    polling_set_p = (POLLING_ELEMENT_T *)MPL_malloc(
        sizeof(POLLING_ELEMENT_T) * g_smpi.num_local_nodes, MPL_MEM_OTHER);
    polling_set_c = (POLLING_ELEMENT_T *)MPL_malloc(
        sizeof(POLLING_ELEMENT_T) * g_smpi.num_local_nodes, MPL_MEM_OTHER);
    ...
    polling_counters =
        (int *)MPL_malloc(sizeof(int) * g_smpi.num_local_nodes, MPL_MEM_OTHER);
    for (i = 0; i < g_smpi.num_local_nodes; ++i) {
...
fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMP_INIT);
    return mpi_errno;
...
#if defined(_ENABLE_CUDA_) && defined(HAVE_CUDA_IPC)
void MPIDI_CH3I_SMP_device_ipc_init(MPIR_Comm *comm_ptr)
{
    int err = MPI_SUCCESS;
    MPIR_Errflag_t errflag = MPIR_ERR_NONE;
    int i, total_region_size;
    deviceIpcMemHandle_t *handle_exchange;
    deviceIpcMemHandle_t *smp_cuda_memhandle;
    deviceIpcEventHandle_t *sr_eventhandle;
    deviceIpcEventHandle_t *sr_eventhandle_local;
    deviceIpcEventHandle_t *loop_eventhandle;
    deviceIpcEventHandle_t *loop_eventhandle_local;
    MPIDI_VC_t *vc = NULL;

    MPIR_Barrier_impl(comm_ptr, &errflag);

    total_region_size = smp_device_region_size * g_smpi.num_local_nodes;

    smp_device_region_base = (void **)MPL_malloc(
        sizeof(void *) * g_smpi.num_local_nodes, MPL_MEM_OTHER);
    smp_device_region_send = (void **)MPL_malloc(
        sizeof(void *) * g_smpi.num_local_nodes, MPL_MEM_OTHER);
    smp_device_region_recv = (void **)MPL_malloc(
        sizeof(void *) * g_smpi.num_local_nodes, MPL_MEM_OTHER);

    MVP_MPIDI_Malloc_Device(smp_device_region_base[g_smpi.my_local_id],
                            total_region_size);

    smp_cuda_memhandle = (deviceIpcMemHandle_t *)MPL_malloc(
        sizeof(deviceIpcMemHandle_t) * g_smpi.num_local_nodes, MPL_MEM_OTHER);
    MVP_MPIDI_Device_IpcGetMemHandle(
        &smp_cuda_memhandle[g_smpi.my_local_id],
        smp_device_region_base[g_smpi.my_local_id]);

    handle_exchange = (deviceIpcMemHandle_t *)MPL_malloc(
        sizeof(deviceIpcMemHandle_t) * comm_ptr->local_size, MPL_MEM_OTHER);
    MPIR_Memcpy(&handle_exchange[comm_ptr->rank],
                &smp_cuda_memhandle[g_smpi.my_local_id],
                sizeof(deviceIpcMemHandle_t));

    /*Optimization needed - Try to combine all allgathers and alltoalls*/

    err = MPIR_Allgather_impl(
        MPI_IN_PLACE, 0, MPI_BYTE, (void *)handle_exchange,
        sizeof(deviceIpcMemHandle_t), MPI_BYTE, comm_ptr, &errflag);

    if (err != MPI_SUCCESS) {
        smp_error_abort(SMP_EXIT_ERR,
                        "handle exchange using Allgather failed \n");
    }

    int g_rank;
    smpi_device_ipc_attr *ref;
    ...
    for (i = 0; i < g_smpi.num_local_nodes; i++) {
        ...
        MPIR_Memcpy(&smp_cuda_memhandle[i], &handle_exchange[g_rank],
                    sizeof(deviceIpcMemHandle_t));
        MVP_MPIDI_Device_IpcOpenMemHandle(&smp_device_region_base[i],
                                          smp_cuda_memhandle[i]);

        smp_device_region_send[i] =
            (void *)((unsigned long)smp_device_region_base[g_smpi.my_local_id] +
                     i * smp_device_region_size);
        smp_device_region_recv[i] =
            (void *)((unsigned long)smp_device_region_base[i] +
                     g_smpi.my_local_id * smp_device_region_size);
    }

    sr_event_local = (deviceEvent_t *)MPL_malloc(
        sizeof(deviceEvent_t) * comm_ptr->local_size, MPL_MEM_OTHER);
    loop_event_local = (deviceEvent_t *)MPL_malloc(
        sizeof(deviceEvent_t) * comm_ptr->local_size, MPL_MEM_OTHER);
    sr_eventhandle_local = (deviceIpcEventHandle_t *)MPL_malloc(
        sizeof(deviceIpcEventHandle_t) * comm_ptr->local_size, MPL_MEM_OTHER);
    loop_eventhandle_local = (deviceIpcEventHandle_t *)MPL_malloc(
        sizeof(deviceIpcEventHandle_t) * comm_ptr->local_size, MPL_MEM_OTHER);
    ...
    for (i = 0; i < g_smpi.num_local_nodes; i++) {
        ...
        MVP_MPIDI_DeviceEventCreateWithFlags(
            &sr_event_local[g_rank], MVP_DEVICE_FLAG(EventInterprocess) |
                                         MVP_DEVICE_FLAG(EventDisableTiming));
        MVP_MPIDI_Device_EventRecord(sr_event_local[g_rank], 0);
        MVP_MPIDI_Device_IpcGetEventHandle(&sr_eventhandle_local[g_rank],
                                           sr_event_local[g_rank]);
        MVP_MPIDI_DeviceEventCreateWithFlags(
            &loop_event_local[g_rank], MVP_DEVICE_FLAG(EventInterprocess) |
                                           MVP_DEVICE_FLAG(EventDisableTiming));
        MVP_MPIDI_Device_EventRecord(loop_event_local[g_rank], 0);
        MVP_MPIDI_Device_IpcGetEventHandle(&loop_eventhandle_local[g_rank],
                                           loop_event_local[g_rank]);
    }

    sr_event = (deviceEvent_t *)MPL_malloc(
        sizeof(deviceEvent_t) * comm_ptr->local_size, MPL_MEM_OTHER);
    loop_event = (deviceEvent_t *)MPL_malloc(
        sizeof(deviceEvent_t) * comm_ptr->local_size, MPL_MEM_OTHER);

    sr_eventhandle = (deviceIpcEventHandle_t *)MPL_malloc(
        sizeof(deviceIpcEventHandle_t) * comm_ptr->local_size, MPL_MEM_OTHER);
    loop_eventhandle = (deviceIpcEventHandle_t *)MPL_malloc(
        sizeof(deviceIpcEventHandle_t) * comm_ptr->local_size, MPL_MEM_OTHER);

    err = MPIR_Alltoall_impl(
        (void *)sr_eventhandle_local, sizeof(deviceIpcEventHandle_t), MPI_BYTE,
        (void *)sr_eventhandle, sizeof(deviceIpcEventHandle_t), MPI_BYTE,
        comm_ptr, &errflag);

    if (err != MPI_SUCCESS) {
        smp_error_abort(SMP_EXIT_ERR,
                        "sr eventhandle exchange using Allgather failed \n");
    }
    ...
    for (i = 0; i < g_smpi.num_local_nodes; i++) {
        ...
        MVP_MPIDI_Device_IpcOpenEventHandle(&sr_event[g_rank],
                                            sr_eventhandle[g_rank]);
        MVP_MPIDI_Device_EventRecord(sr_event[g_rank], 0);
    }

    err = MPIR_Alltoall_impl(
        (void *)loop_eventhandle_local, sizeof(deviceIpcEventHandle_t),
        MPI_BYTE, (void *)loop_eventhandle, sizeof(deviceIpcEventHandle_t),
        MPI_BYTE, comm_ptr, &errflag);

    if (err != MPI_SUCCESS) {
        smp_error_abort(SMP_EXIT_ERR,
                        "loop eventhandle exchange using Allgather failed \n");
    }
    ...
    for (i = 0; i < g_smpi.num_local_nodes; i++) {
        ...
        MVP_MPIDI_Device_IpcOpenEventHandle(&loop_event[g_rank],
                                            loop_eventhandle[g_rank]);
        MVP_MPIDI_Device_EventRecord(loop_event[g_rank], 0);
    }

    if (handle_exchange != NULL) {
        MPL_free(handle_exchange);
    }
    if (smp_cuda_memhandle) {
        MPL_free(smp_cuda_memhandle);
    }
    if (sr_eventhandle_local) {
        MPL_free(sr_eventhandle_local);
    }
    if (sr_eventhandle) {
        MPL_free(sr_eventhandle);
    }
    if (loop_eventhandle_local) {
        MPL_free(loop_eventhandle_local);
    }
    if (loop_eventhandle) {
        MPL_free(loop_eventhandle);
    }
}
#endif
...
int MPIDI_CH3I_SMP_COLL_finalize()
{
    ...
    int local_num_processes = MPIDI_MVP_Num_local_processes(pg);
    int local_id = MPIDI_MVP_Get_local_process_id(pg);
    if (mvp_enable_shmem_collectives || finalize_coll_comm == 1) {
        /* Freeing up shared memory collective resources*/
        mpi_errno = MPIR_MVP_SHMEM_COLL_finalize(local_id, local_num_processes);
    }
...
{
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_FINALIZE);
    ...
    if (g_smpi.l2g_rank) {
        MPL_free(g_smpi.l2g_rank);
    }

    if (g_smpi_shmem) {
        if (g_smpi_shmem->rqueues_limits_s != NULL) {
            MPL_free(g_smpi_shmem->rqueues_limits_s);
        }
        if (g_smpi_shmem->rqueues_limits_r != NULL) {
            MPL_free(g_smpi_shmem->rqueues_limits_r);
        }
        if (g_smpi_shmem->shared_tails != NULL) {
            MPL_free(g_smpi_shmem->shared_tails);
        }
#if defined(_ENABLE_CUDA_) && defined(HAVE_CUDA_IPC)
        if (g_smpi_shmem->cu_attrbs) {
            MPL_free(g_smpi_shmem->cu_attrbs);
        }
#endif
        if (g_smpi_shmem != NULL) {
            MPL_free(g_smpi_shmem);
        }
    }
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMP_FINALIZE);
    return MPI_SUCCESS;
}
...
void MPIDI_CH3I_CUDA_SMP_device_init(MPIDI_PG_t *pg)
{
    ...
    if (mvp_device_use_smp_eager_ipc) {
        MPIDI_CH3I_SMP_device_ipc_init(MPIR_Process.comm_world);
    }
#endif
}
...
void MPIDI_CH3I_CUDA_SMP_device_finalize(MPIDI_PG_t *pg)
{
    ...
    if (mvp_shmem_pool_init) {
        ibv_device_unregister(g_smpi.send_buf_pool_ptr);
    }

#if defined(HAVE_CUDA_IPC)
    if (SMP_INIT && mvp_device_use_smp_eager_ipc) {
        for (i = 0; i < g_smpi.num_local_nodes; i++) {
            ...
            MVP_MPIDI_Device_EventDestroy(sr_event_local[i]);
            MVP_MPIDI_Device_EventDestroy(loop_event_local[i]);
            MVP_MPIDI_Device_EventDestroy(sr_event[i]);
            MVP_MPIDI_Device_EventDestroy(loop_event[i]);
            MVP_MPIDI_Device_IpcCloseMemHandle(smp_device_region_base[i]);
        }

        if (smp_device_region_base) {
            MPL_free(smp_device_region_base);
        }
        if (smp_device_region_send) {
            MPL_free(smp_device_region_send);
        }
        if (smp_device_region_recv) {
            MPL_free(smp_device_region_recv);
        }
        if (sr_event_local) {
            MPL_free(sr_event_local);
        }
        if (sr_event) {
            MPL_free(sr_event);
        }
...
void MPIDI_CH3I_SMP_writev_rndv_header(MPIDI_VC_t *vc, const struct iovec *iov,
                                       const int n, int *num_bytes_ptr)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_WRITEV_RNDV_HEADER);
    ...
    pkt_header = (MPIDI_CH3_Pkt_rndv_r3_data_t *)(iov[0].iov_base);
    *num_bytes_ptr = 0;
#if defined(_SMP_CMA_)
    creq = pkt_header->csend_req_id;
    /* creq_req_id is set to NULL for non-contig data, then fall back to shared
     * memory instead of using limic; or else, continue data transfer by limic
     */
    if (MVP_SMP_USE_CMA && creq) {
        assert(creq->dev.iov_count == 1);

        /* The last sizeof(intptr_t) is the total num of data bytes */
        pkt_len = iov[0].iov_len + sizeof(struct iovec) * creq->dev.iov_count +
                  sizeof(pid_t) + sizeof(intptr_t);

        /* check if avail is less than data size */
        if (!smpi_check_avail(vc->smp.local_nodes, pkt_len,
        ...
        *num_bytes_ptr = iov[0].iov_len;
        smpi_complete_send(vc->smp.local_nodes, pkt_len, pkt_len, ptr, ptr_head,
                           ptr_flag);
        ...
        if (MVP_SMP_USE_LIMIC2 && sreq) {
            ...
            for (i = 0; i < sreq->dev.iov_count; ++i) {
                err = limic_tx_init(limic_fd, sreq->dev.iov[i].iov_base,
                                    sreq->dev.iov[i].iov_len, &lu);
                if (!err) {
                    MPIR_ERR_SETFATALANDJUMP1(
                        err, MPI_ERR_OTHER, "**fail", "**fail %s",
                        "LiMIC: (MPIDI_CH3I_SMP_writev_rndv_header) "
                        "limic_tx_init fail");
                }
                total_bytes += sreq->dev.iov[i].iov_len;

                /* copy the limic_user information to the shared memory
                   and move the shared memory pointer properly
                 */
                MPIR_Memcpy((void *)ptr, &lu, sizeof(limic_user));
                ptr =
                    (volatile void *)((unsigned long)ptr + sizeof(limic_user));
            }

            *((volatile intptr_t *)ptr) = total_bytes;
            ptr = (volatile void *)((unsigned long)ptr + sizeof(intptr_t));

            *num_bytes_ptr = iov[0].iov_len;

            smpi_complete_send(vc->smp.local_nodes, pkt_len, pkt_len, ptr,
                               ptr_head, ptr_flag);
            ...
            /* check if avail is less than data size */
            if (!smpi_check_avail(vc->smp.local_nodes, len,
                                  (volatile void **)&ptr_flag, TWO_FREE))
                return;
            ...
            i = 0;
            for (i = 0; i < n; i++) {
                MPIR_Memcpy((void *)ptr,
                            (void *)((unsigned long)iov[i].iov_base),
                            iov[i].iov_len);
            ...
            smpi_complete_send(vc->smp.local_nodes, len, len, ptr, ptr_head,
                               ptr_flag);
...
fn_exit:
    PRINT_DEBUG(DEBUG_SHM_verbose > 1, "writev_rndv_header returns bytes %d\n",
                *num_bytes_ptr);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_WRITEV_RNDV_HEADER);
    return;
...
#if defined(_ENABLE_CUDA_)
void smp_device_send_copy_complete(MPIDI_VC_t *vc, MPIR_Request *req,
                                   void *ptr_flag)
{
    int mpi_errno = MPI_SUCCESS;
    int complete = 0;
    ...
    READBAR();
    MPIR_Assert(*((volatile int *)ptr_flag) == SMP_CBUF_PENDING);
    *((volatile int *)ptr_flag) = SMP_CBUF_BUSY;
    WRITEBAR();
    ...
    if (req != NULL) {
        ...
        MPIR_Assert(complete == 1);
    }
}

void smp_device_send_copy_enqueue(MPIDI_VC_t *vc, MPIR_Request *req,
                                  SEND_BUF_T *send_buf, void *user_buf,
                                  size_t size, int is_last,
                                  volatile void *ptr_flag)
{
    mvp_device_event_t *device_event;

    *((volatile int *)ptr_flag) = SMP_CBUF_PENDING;
    WRITEBAR();

    send_buf->len = size;
    MVP_MPIDI_Memcpy_Device_Async(send_buf->buf, user_buf, size,
                                  deviceMemcpyDeviceToHost, stream_d2h);

    device_event = get_device_event();
    device_event->op_type = SMP_SEND;
    device_event->vc = vc;
    device_event->smp_ptr = (void *)ptr_flag;
    device_event->size = size;
    if (is_last) {
        device_event->req = req;
    } else {
        device_event->req = NULL;
    }

    MVP_MPIDI_Device_EventRecord(device_event->event, stream_d2h);
}

int MPIDI_CH3I_SMP_writev_rndv_data_device(MPIDI_VC_t *vc, MPIR_Request *req,
                                           const struct iovec *iov, const int n,
                                           int *num_bytes_ptr, int is_cont)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_WRITE_RNDV_DATA_CUDA);
    ...
    *num_bytes_ptr = 0;
    ...
    do {
        ...
        if (!smpi_check_avail(vc->smp.local_nodes, length,
                              (volatile void **)&ptr_flag, ONE_FREE)) {
            return 0;
        }
        ...
        pkt_avail = MVP_SMP_SEND_BUF_SIZE;
        pkt_len = 0;
        ...
        for (; i < n;) {
            if (pkt_avail >= (iov[i].iov_len - offset)) {
                if (i == n - 1)
                    is_last = 1;
                smp_device_send_copy_enqueue(
                    vc, req, send_buf,
                    (void *)((unsigned long)iov[i].iov_base + offset),
                    iov[i].iov_len - offset, is_last, ptr_flag);
                pkt_len += (iov[i].iov_len - offset);
...
fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_WRITE_RNDV_DATA_CUDA);
    return mpi_errno;
...
void MPIDI_CH3I_SMP_writev_rndv_data_cont(MPIDI_VC_t *vc, MPIR_Request *req,
                                          const struct iovec *iov, const int n,
                                          int *num_bytes_ptr)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_WRITEV_RNDV_DATA_CONT);
    *num_bytes_ptr = 0;
...
#if defined(_ENABLE_CUDA_)
    if (mvp_enable_device && MVP_CUDA_SMP_PIPELINE &&
        req->mrail.device_transfer_mode == NONE) {
        pkt_avail = MVP_SMP_SEND_BUF_SIZE;
    } else
#endif
    {
    ...
    if (mvp_enable_device) {
        /*as it is all data, we check the first iov to determine if the buffer
         * is on device*/
        iov_isdev = is_device_buffer((void *)iov[0].iov_base);
    }
#endif
    ...
    *num_bytes_ptr = 0;
    do {
        ...
        for (; i < n;) {
            PRINT_DEBUG(
                DEBUG_SHM_verbose > 1,
                "i %d, iov[i].len %ld, (len-offset) %ld, pkt_avail %d\n", i,
                iov[i].iov_len, (iov[i].iov_len - offset), pkt_avail);

            if (has_sent >= MVP_SMP_BATCH_SIZE)
                break;
            ++has_sent;
            ...
            if (pkt_avail >= (iov[i].iov_len - offset)) {
                if (offset != 0) {
                    ...
                    if (iov_isdev) {
                        MVP_MPIDI_Memcpy_Device(
                            &send_buf->buf,
                            (void *)((unsigned long)iov[i].iov_base + offset),
                            iov[i].iov_len - offset, deviceMemcpyDeviceToHost);
                    } else
#endif
                    {
                        MPIR_Memcpy(
                    ...
                    send_buf->busy = 1;
                    send_buf->len = iov[i].iov_len - offset;
                    send_buf->has_next = 1;
...
#if defined(_ENABLE_CUDA_)
                    if (iov_isdev) {
                        MVP_MPIDI_Memcpy_Device(&send_buf->buf, iov[i].iov_base,
                                                iov[i].iov_len,
                                                deviceMemcpyDeviceToHost);
                    } else
#endif
                    {
                        MPIR_Memcpy(&send_buf->buf, iov[i].iov_base,
                                    iov[i].iov_len);
                    }
                ...
                if (iov_isdev) {
                    MVP_MPIDI_Memcpy_Device(
                        &send_buf->buf,
                        (void *)((unsigned long)iov[i].iov_base + offset),
                        pkt_avail, deviceMemcpyDeviceToHost);
                } else
#endif
                {
                    MPIR_Memcpy(
                ...
                MPIR_T_PVAR_COUNTER_INC(MVP, mvp_smp_rndv_sent, pkt_avail);
        ...
        *num_bytes_ptr += pkt_len;
        ...
        if (i == n || has_sent >= MVP_SMP_BATCH_SIZE) {
            break;
        }
    ...
    *((volatile int *)ptr) = first_index;
    ptr = (volatile void *)((unsigned long)ptr + sizeof(int));
    /* update(header) */
    smpi_complete_send(vc->smp.local_nodes, len, *num_bytes_ptr, ptr, ptr_head,
                       ptr_flag);

fn_exit:
    PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                "writev_rndv_data_cont returns bytes %d\n", *num_bytes_ptr);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_WRITEV_RNDV_DATA_CONT);
}

int MPIDI_CH3I_SMP_writev_rndv_data(MPIDI_VC_t *vc, MPIR_Request *req,
                                    const struct iovec *iov, const int n,
                                    int *num_bytes_ptr)
{
    volatile void *ptr_head, *ptr_flag, *ptr;
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_WRITE_RNDV_DATA);
...
fn_exit:
    PRINT_DEBUG(DEBUG_SHM_verbose > 1, "writev_rndv_data returns bytes %d\n",
                *num_bytes_ptr);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_WRITE_RNDV_DATA);
    return mpi_errno;
...
void MPIDI_CH3I_SMP_writev(MPIDI_VC_t *vc, const struct iovec *iov, const int n,
                           int *num_bytes_ptr)
{
    int pkt_len = 0;
    volatile void *ptr_head, *ptr, *ptr_flag;
    ...
    int buf_isdev = 0;
    int contig_avail = 0;
    void *device_region_ptr = NULL;
    smpi_device_ipc_attr *my_base;
    int cur_t, cur_h;
#endif
#endif

    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_WRITEV);
    ...
    ** the former while iov[1] refers to latter*/
    ...
    buf_isdev = is_device_buffer((void *)iov[1].iov_base);

    if (mvp_enable_device && mvp_device_use_smp_eager_ipc &&
        vc->smp.can_access_peer == MVP_DEVICE_IPC_ENABLED) {
        if (buf_isdev && n == 2) {
            /*first iov is the header*/
            data_sz = len - iov[0].iov_len;
            len = iov[0].iov_len;

            rem_rank = vc->smp.local_nodes;
            my_base = g_smpi_shmem->cu_attrbs[g_smpi.my_local_id];
            cur_h = ((smpi_device_ipc_attr *)my_base + rem_rank)->device_head;
            cur_t = ((smpi_device_ipc_attr *)my_base + rem_rank)->device_tail;

            if (cur_t > cur_h) {
                contig_avail = cur_t - cur_h;
            } else {
                contig_avail = smp_device_region_size - cur_h;
                if (contig_avail < data_sz) {
                    cur_h = 0;
                    contig_avail = cur_t;
                    MVP_MPIDI_Device_StreamWaitEvent(
                        0, loop_event_local[vc->pg_rank], 0);
                }
            }
    ...
    for (i = 0; i < n; i++) {
#if defined(_ENABLE_CUDA_)
        if (mvp_enable_device) {
            iov_isdev = is_device_buffer((void *)iov[i].iov_base);
        }
        ...
        if (iov_isdev) {
            ...
            if (mvp_device_use_smp_eager_ipc &&
                ...
                device_region_ptr =
                    (void *)((unsigned long)
                                 smp_device_region_send[vc->smp.local_nodes] +
                             cur_h);
                MVP_MPIDI_Memcpy_Device(
                    device_region_ptr, (void *)((unsigned long)iov[i].iov_base),
                    iov[i].iov_len, deviceMemcpyDefault);
                MVP_MPIDI_Device_EventRecord(sr_event[vc->pg_rank], 0);
                if (0 == cur_h) {
                    ((smpi_device_ipc_attr *)my_base + rem_rank)->device_head =
                        iov[i].iov_len;
                } else {
                    ((smpi_device_ipc_attr *)my_base + rem_rank)->device_head +=
                        iov[i].iov_len;
            ...
            {
                MVP_MPIDI_Memcpy_Device(
                    (void *)ptr, (void *)((unsigned long)iov[i].iov_base),
                    iov[i].iov_len, deviceMemcpyDeviceToHost);
            }
        } else
#endif
        {
        ...
        if (mvp_device_use_smp_eager_ipc && iov_isdev &&
            vc->smp.can_access_peer == MVP_DEVICE_IPC_ENABLED) {
            /*do nothing*/
        } else
#endif
    ...
    *num_bytes_ptr += pkt_len;
    ...
    PRINT_DEBUG(DEBUG_SHM_verbose > 1, "writev returns bytes %d\n",
                *num_bytes_ptr);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMP_WRITEV);
}

int mvp_smp_fast_write_contig(MPIDI_VC_t *vc, const void *buf, intptr_t data_sz,
                              int rank, int tag, MPIR_Comm *comm,
                              int context_offset, MPIR_Request **sreq_p)
{
    int len = 0;
    ...
    if (likely(smpi_check_avail(vc->smp.local_nodes, len,
            ...
            if (mvp_device_use_smp_eager_ipc &&
                ...
                device_region_ptr =
                    (void *)((unsigned long)
                                 smp_device_region_send[vc->smp.local_nodes] +
                             cur_h);
                MVP_MPIDI_Memcpy_Device(device_region_ptr, buf, data_sz,
                                        deviceMemcpyDefault);
...
#endif
            {
                MVP_MPIDI_Memcpy_Device((void *)ptr, buf, data_sz,
                                        deviceMemcpyDeviceToHost);
        ...
        *sreq_p = sreq;
        PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                    "smp buffer not available, dst: %d, request enqueued: %p, "
                    "type: %d, ch.reqtype: %d\n",
                                 ...
                                 int *num_bytes_ptr)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_WRITE_CONTIG);
    *num_bytes_ptr = 0;
    len = data_sz + sizeof(MPIDI_CH3_Pkt_eager_send_t);
    ...
    *num_bytes_ptr += sizeof(MPIDI_CH3_Pkt_eager_send_t);
    ...
    if (buf_isdev) {
        ...
        if (mvp_device_use_smp_eager_ipc &&
            ...
            cur_h += data_sz;
            ((smpi_device_ipc_attr *)my_base + rem_rank)->device_head = cur_h;
...
#if defined(_ENABLE_CUDA_)
void smp_device_recv_copy_complete(MPIDI_VC_t *vc, MPIR_Request *req,
                                   void *recv_buf_ptr)
{
    int mpi_errno = MPI_SUCCESS;
    int complete = 0;
...
void smp_device_recv_copy_enqueue(MPIDI_VC_t *vc, MPIR_Request *req,
                                  SEND_BUF_T *recv_buf, int recv_offset,
                                  void *user_buf, size_t size, int is_last)
{
    mvp_device_event_t *device_event;

    MVP_MPIDI_Memcpy_Device_Async(user_buf,
                                  (void *)((char *)recv_buf->buf + recv_offset),
                                  size, deviceMemcpyHostToDevice, stream_h2d);

    device_event = get_device_event();
    device_event->op_type = SMP_RECV;
    device_event->vc = vc;
    device_event->smp_ptr = (void *)recv_buf;
    device_event->size = size;
    if (is_last) {
        device_event->req = req;
    } else {
    ...
    MVP_MPIDI_Device_EventRecord(device_event->event, stream_h2d);
}

int MPIDI_CH3I_SMP_readv_rndv_device(MPIDI_VC_t *recv_vc_ptr, MPIR_Request *req,
                                     const struct iovec *iov,
                                     const int iov_count, int index,
                                     size_t *num_bytes_ptr, int is_cont)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_READV_RNDV_CUDA);
    ...
    *num_bytes_ptr = 0;
    ...
    if (!is_cont) {
        ...
        MPIR_Assert(0 == s_current_bytes[recv_vc_ptr->smp.local_nodes]);

        READBAR();
        s_current_ptr[recv_vc_ptr->smp.local_nodes] = NULL;

        smpi_complete_recv(recv_vc_ptr->smp.local_nodes, g_smpi.my_local_id,
    ...
    if (s_current_bytes[recv_vc_ptr->smp.local_nodes] == 0) {
        MPIR_Assert(s_total_bytes[recv_vc_ptr->smp.local_nodes] == 0);
        ...
        s_total_bytes[recv_vc_ptr->smp.local_nodes] =
            *(int *)((unsigned long)ptr + sizeof(int));
    ...
    if (current_index != -1) {
        recv_buf =
            SMPI_BUF_POOL_PTR(recv_vc_ptr->smp.local_nodes, current_index);

        MPIR_Assert(*((volatile int *)&recv_buf->busy) == 1);

        msglen = recv_buf->len - recv_offset;
        current_buf = (void *)((unsigned long)&recv_buf->buf + recv_offset);
        iov_len = iov[0].iov_len;

        for (; iov_off < iov_count &&
               s_current_bytes[recv_vc_ptr->smp.local_nodes] > 0 &&
               current_index != -1;) {
            if (msglen > iov_len) {
                smp_device_recv_copy_enqueue(
                    recv_vc_ptr, req, recv_buf, recv_offset,
                    (void *)((unsigned long)iov[iov_off].iov_base + buf_off),
                    iov_len, is_last);

                current_buf = (void *)((unsigned long)current_buf + iov_len);
                msglen -= iov_len;
                ...
                if (s_current_bytes[recv_vc_ptr->smp.local_nodes] <= 0) {
                    MPIR_Assert(s_current_bytes[recv_vc_ptr->smp.local_nodes] ==
                                0);
                    break;
                }
                ...
                recv_buf = SMPI_BUF_POOL_PTR(recv_vc_ptr->smp.local_nodes,
                                             current_index);
        ...
        *num_bytes_ptr += received_bytes;
        ...
        received_bytes = 0;
        if (iov_off == iov_count) {
            MPIR_Assert(s_current_ptr[recv_vc_ptr->smp.local_nodes] == 0);
            goto fn_exit;
        }
    }

fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_READV_RNDV_CUDA);
    return mpi_errno;
...
#if defined(_SMP_CMA_)
int MPIDI_CH3I_SMP_do_cma_read(const struct iovec *iov, const int iovlen,
                               void *cma_header, size_t *num_bytes_ptr)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_DO_CMA_READ);
...
fn_exit:
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_DO_CMA_READ);
    return mpi_errno;
...
#if defined(_SMP_LIMIC_)
int MPIDI_CH3I_SMP_do_limic_read(MPIDI_VC_t *recv_vc_ptr,
                                 const struct iovec *iov, const int iovlen,
                                 void *limic_header, size_t *num_bytes_ptr)
{
    int mpi_errno = MPI_SUCCESS;
    int iov_off = 0, buf_off = 0;
    size_t received_bytes = 0;
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_DO_LIMIC_READ);
    ...
    for (; total_bytes > 0 && iov_off < iovlen;) {
        if (msglen == iov_len) {
            ...
            if (mvp_MPIDI_CH3I_RDMA_Process.g_smp_can_fallback && !err) {
                MPIDI_CH3I_SMP_send_comp(l_header, recv_vc_ptr, received_bytes,
                                         SMP_DMA_LIMIC, FALLBACK);
                *num_bytes_ptr = 0;
...
fn_exit:
    PRINT_DEBUG(DEBUG_SHM_verbose > 1, "return with nb %ld\n", *num_bytes_ptr);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_DO_LIMIC_READ);
    return mpi_errno;
}
#endif

int MPIDI_CH3I_SMP_readv_rndv_cont(MPIDI_VC_t *recv_vc_ptr,
                                   const struct iovec *iov, const int iovlen,
                                   int index, void *limic_header,
                                   void *cma_header, size_t *num_bytes_ptr,
                                   smp_dma_flag_t dma_flag)
{
    ...
    /* all variable must be declared before the state declarations */
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_READV_RNDV_CONT);
    ...
    *num_bytes_ptr = 0;
    ...
    if (dma_flag == SMP_DMA_CMA) {
        mpi_errno =
            MPIDI_CH3I_SMP_do_cma_read(iov, iovlen, cma_header, num_bytes_ptr);
        if (mpi_errno) {
            MPIR_ERR_SETFATALANDJUMP1(
                mpi_errno, MPI_ERR_OTHER, "**fail", "**fail %s",
        ...
        if (dma_flag == SMP_DMA_LIMIC) {
            ...
            if (s_current_bytes[recv_vc_ptr->smp.local_nodes] == 0) {
                ...
                READBAR();
                if (*ptr_flag == SMP_CBUF_END) {
                    s_header_ptr_r[recv_vc_ptr->smp.local_nodes] = SMPI_FIRST_R(
                        recv_vc_ptr->smp.local_nodes, g_smpi.my_local_id);
                    ptr =
                ...
                MPIR_T_PVAR_LEVEL_DEC(
                    MVP, mvp_smp_rndv_avail_buffer,
                    s_current_bytes[recv_vc_ptr->smp.local_nodes]);

                rndv_buffer_max_usage =
                    (rndv_buffer_max_usage >
                     s_current_bytes[recv_vc_ptr->smp.local_nodes]) ?
                        rndv_buffer_max_usage :
                        s_current_bytes[recv_vc_ptr->smp.local_nodes];
                MPIR_T_PVAR_LEVEL_SET(MVP, mvp_smp_rndv_buffer_max_use,
                                      rndv_buffer_max_usage);

                PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                            "current byte %ld, total bytes %ld, iovlen %d, "
                            "iov[0].len %ld\n",
                            s_current_bytes[recv_vc_ptr->smp.local_nodes],
            ...
            if (current_index != -1) {
                ...
                PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                            "iov_off %d, current bytes %ld, iov len %ld\n",
                            iov_off,
                            s_current_bytes[recv_vc_ptr->smp.local_nodes],
                            iov[iov_off].iov_len);
                ...
                msglen = recv_buf->len - recv_offset;
                current_buf =
                    (void *)((unsigned long)&recv_buf->buf + recv_offset);
                iov_len = iov[0].iov_len;
                ...
                for (; iov_off < iovlen &&
                    ...
                    if (msglen > iov_len) {
                        ...
                        if (iov_isdev) {
                            MVP_MPIDI_Memcpy_Device(
                                (void *)((unsigned long)iov[iov_off].iov_base +
                                         buf_off),
                        ...
                        MPIR_T_PVAR_COUNTER_INC(MVP, mvp_smp_rndv_received,
                                                iov_len);
                        MPIR_T_PVAR_LEVEL_INC(MVP, mvp_smp_rndv_avail_buffer,
                                              iov_len);

                        READBAR();
                        current_buf =
                            (void *)((unsigned long)current_buf + iov_len);
                        msglen -= iov_len;
                        s_current_bytes[recv_vc_ptr->smp.local_nodes] -=
                            iov_len;
                        received_bytes += iov_len;
                        ...
                        if (iov_off >= iovlen) {
                            recv_vc_ptr->smp.read_index = current_index;
                            recv_vc_ptr->smp.read_off =
                                (unsigned long)current_buf -
                                (unsigned long)&recv_buf->buf;
                            break;
                        }
                        ...
                        else if (s_current_bytes[recv_vc_ptr->smp.local_nodes] <
                                 0) {
                            MPIR_ERR_SETFATALANDJUMP1(
                                mpi_errno, MPI_ERR_OTHER, "**fail", "**fail %s",
                                "s_current_bytes[recv_vc_ptr->smp.local_nodes] "
                        ...
                        recv_buf->busy = 0;
                        recv_buf =
                            SMPI_BUF_POOL_PTR(destination, current_index);
                        MPIR_Assert(recv_buf->busy == 1);
                        msglen = recv_buf->len;
                        current_buf = (void *)&recv_buf->buf;
                ...
                *num_bytes_ptr += received_bytes;
                PRINT_DEBUG(
                    DEBUG_SHM_verbose > 1,
                    "current bytes %ld, num_bytes %ld, iov_off %d, iovlen %d\n",
                    s_current_bytes[recv_vc_ptr->smp.local_nodes],
                    *num_bytes_ptr, iov_off, iovlen);
...
fn_exit:
    PRINT_DEBUG(DEBUG_SHM_verbose > 1, "return with nb %ld\n", *num_bytes_ptr);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_READV_RNDV_CONT);
    return mpi_errno;
...
int MPIDI_CH3I_SMP_readv_rndv(MPIDI_VC_t *recv_vc_ptr, const struct iovec *iov,
                              ...
                              smp_dma_flag_t dma_flag)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_READ_RNDV);
        ...
        if (dma_flag == SMP_DMA_LIMIC) {
            ...
            if (*ptr_flag == SMP_CBUF_END) {
                s_header_ptr_r[recv_vc_ptr->smp.local_nodes] = SMPI_FIRST_R(
                    recv_vc_ptr->smp.local_nodes, g_smpi.my_local_id);
                ptr = (volatile void
                           *)((unsigned long)g_smpi_shmem->pool +
                              s_header_ptr_r[recv_vc_ptr->smp.local_nodes]);
                ptr_flag = (volatile int *)ptr;
                READBAR();
                while (*ptr_flag == SMP_CBUF_FREE ||
            ...
            if (current_index != -1) {
                /** last smp packet has not been drained up yet **/
                PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                            "iov_off %ld, current bytes %ld, iov len %ld\n",
                            iov_off,
                            s_current_bytes[recv_vc_ptr->smp.local_nodes],
                            iov[iov_off].iov_len);
                ...
                *num_bytes_ptr += received_bytes;
                PRINT_DEBUG(DEBUG_SHM_verbose > 1,
                            "current bytes %ld, num_bytes %ld, iov_off %ld, "
                            "iovlen %d\n",
                            s_current_bytes[recv_vc_ptr->smp.local_nodes],
                            *num_bytes_ptr, iov_off, iovlen);
...
fn_exit:
    PRINT_DEBUG(DEBUG_SHM_verbose > 1, "return with nb %ld\n", *num_bytes_ptr);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMP_READ_RNDV);
    return mpi_errno;
...
int MPIDI_CH3I_SMP_readv(MPIDI_VC_t *recv_vc_ptr, const struct iovec *iov,
                         const int iovlen, size_t *num_bytes_ptr)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_READV);

    *num_bytes_ptr = 0;

    if (s_current_ptr[recv_vc_ptr->smp.local_nodes] != NULL) {
        MPIR_T_PVAR_LEVEL_DEC(MVP, mvp_smp_eager_avail_buffer,
                              s_current_bytes[recv_vc_ptr->smp.local_nodes]);

        eager_buffer_max_usage =
            (eager_buffer_max_usage >
             s_current_bytes[recv_vc_ptr->smp.local_nodes]) ?
                eager_buffer_max_usage :
                s_current_bytes[recv_vc_ptr->smp.local_nodes];
        MPIR_T_PVAR_LEVEL_SET(MVP, mvp_smp_eager_buffer_max_use,
                              eager_buffer_max_usage);
...
#if defined(_ENABLE_CUDA_)
            if (mvp_enable_device) {
                iov_isdev = is_device_buffer((void *)iov[iov_off].iov_base);
            }
#endif
            if (s_current_bytes[recv_vc_ptr->smp.local_nodes] >=
                iov[iov_off].iov_len) {
                ...
                if (iov_isdev) {
                    MVP_MPIDI_Memcpy_Device(
                        (void *)iov[iov_off].iov_base,
                        (void *)s_current_ptr[recv_vc_ptr->smp.local_nodes],
                        iov[iov_off].iov_len, deviceMemcpyHostToDevice);
                } else
#endif
                {
                ...
                MPIR_T_PVAR_COUNTER_INC(
                    MVP, mvp_smp_eager_received,
                    s_current_bytes[recv_vc_ptr->smp.local_nodes]);
                MPIR_T_PVAR_LEVEL_INC(
                    MVP, mvp_smp_eager_avail_buffer,
                    s_current_bytes[recv_vc_ptr->smp.local_nodes]);
                ...
                s_current_ptr[recv_vc_ptr->smp.local_nodes] =
                    (void *)((unsigned long)
                                 s_current_ptr[recv_vc_ptr->smp.local_nodes] +
                             s_current_bytes[recv_vc_ptr->smp.local_nodes]);
                received_bytes += s_current_bytes[recv_vc_ptr->smp.local_nodes];
...
fn_exit:
    PRINT_DEBUG(DEBUG_SHM_verbose > 1, "return with nb %ld\n", *num_bytes_ptr);
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMP_READV);
    return mpi_errno;
}

int MPIDI_CH3I_SMP_pull_header(MPIDI_VC_t *vc, MPIDI_CH3_Pkt_t **pkt_head)
{
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_PULL_HEADER);
    ...
    if (*ptr_flag == SMP_CBUF_FREE || *ptr_flag == SMP_CBUF_PENDING) {
        ...
        *pkt_head = (void *)((unsigned long)ptr + sizeof(int) * 2);
        ...
        s_current_ptr[vc->smp.local_nodes] =
            (void *)((unsigned long)*pkt_head + MPIDI_CH3U_PKT_SIZE(*pkt_head));
    }

fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMP_PULL_HEADER);
    return MPI_SUCCESS;
}
...
static int smpi_exchange_info(MPIDI_PG_t *pg)
{
    ...
    MPIDI_VC_t *vc = NULL;
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMPI_EXCHANGE_INFO);

    UPMI_GET_RANK(&pg_rank);
    UPMI_GET_SIZE(&pg_size);

    g_smpi.num_local_nodes = MPIDI_MVP_Num_local_processes(pg);
    if (g_smpi.num_local_nodes >= MVP_SHMEM_PRIORTY_THRESHOLD)
    ...
    g_smpi.l2g_rank = (unsigned int *)MPL_malloc(
        g_smpi.num_local_nodes * sizeof(int), MPL_MEM_OTHER);
...
#pragma error_messages(off, E_STATEMENT_NOT_REACHED)
        ...
        MPL_error_printf("malloc: in ib_rank_lid_table for SMP");
#if defined(__SUNPRO_C) || defined(__SUNPRO_CC)
...
fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMPI_EXCHANGE_INFO);
    return mpi_errno;
...
/*----------------------------------------------------------*/
static inline SEND_BUF_T *get_buf_from_pool()
{
    SEND_BUF_T *ptr;
...
static inline void adjust_lu_info(struct limic_user *lu, int old_len)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_ADJUST_LU_INFO);
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_ADJUST_LU_INFO);
}

#endif /* _SMP_LIMIC_ */

#if defined(_SMP_LIMIC_) || defined(_SMP_CMA_)
void MPIDI_CH3I_SMP_send_comp(void *header, MPIDI_VC_t *vc, intptr_t nb,
                              smp_dma_flag_t dma_flag,
                              smp_fallback_flag_t fallback)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SMP_SEND_COMP);
    ...
    /* check if avail is less than data size */
    if (!smpi_check_avail(vc->smp.local_nodes, pkt_sz,
                          (volatile void **)&ptr_flag, ONE_FREE)) {
        /* queue the message */
        creq = create_request(&pkt, pkt_sz, 0);
        int inuse = 0;
    ...
    smpi_complete_send(vc->smp.local_nodes, pkt_sz, pkt_sz, ptr, ptr_head,
                       ptr_flag);

    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SMP_SEND_COMP);
    return;
}
...
int MPIDI_CH3I_SMP_do_cma_get(MPIDI_VC_t *vc, const void *src, void *dst,
                              ssize_t len)
{
    pid_t pid;
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPIDI_CH3I_SMP_DO_CMA_GET);
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPIDI_CH3I_SMP_DO_CMA_GET);
fn_fail:
    return mpi_errno;
}

int MPIDI_CH3I_SMP_do_cma_put(MPIDI_VC_t *vc, const void *src, void *dst,
                              ssize_t len)
{
    pid_t pid;
    ssize_t nbytes, total = 0;
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPIDI_CH3I_SMP_DO_CMA_PUT);
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPIDI_CH3I_SMP_DO_CMA_PUT);
fn_fail:
    return mpi_errno;
}
