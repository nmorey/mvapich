...
#define MPIDI_CH3_ROUND_UP_PAGESIZE(x)                                         \
    ((((MPI_Aint)x) + (~MPIDI_CH3_PAGESIZE_MASK)) & MPIDI_CH3_PAGESIZE_MASK)

extern MPIR_T_pvar_timer_t PVAR_TIMER_rma_wincreate_allgather;

MPIDI_SHM_Wins_list_t shm_wins_list;

static int MPIDI_CH3I_Win_init(MPI_Aint size, int disp_unit, int create_flavor,
                               int model, MPIR_Info *info, MPIR_Comm *comm_ptr,
                               MPIR_Win **win_ptr);

static int MPIDI_CH3I_Win_allocate_shm(MPI_Aint size, int disp_unit,
                                       MPIR_Info *info, MPIR_Comm *comm_ptr,
                                       void *base_ptr, MPIR_Win **win_ptr);

static int MPIDI_CH3I_Win_detect_shm(MPIR_Win **win_ptr);

static int MPIDI_CH3I_Win_gather_info(void *base, MPI_Aint size, int disp_unit,
                                      MPIR_Info *info, MPIR_Comm *comm_ptr,
                                      MPIR_Win **win_ptr);
...
int MPIDI_CH3_SHM_Win_shared_query(MPIR_Win *win_ptr, int target_rank,
                                   ...
                                   void *baseptr)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3_WIN_SHARED_QUERY);
        ...
        *((void **)baseptr) = NULL;
        ...
        for (i = 0; i < comm_size; i++) {
            if (win_ptr->basic_info_table[i].size > 0) {
                ...
                if (FALSE == win_ptr->shm_allocated || !SMP_INIT ||
                    (SMP_INIT && comm_size <= 1)) {
                    MPIR_Assert(local_i >= 0 &&
                                local_i < win_ptr->comm_ptr->local_size);
                    *(void **)baseptr =
                        win_ptr->basic_info_table[local_i].base_addr;
                } else {
                    MPIR_Assert(local_i >= 0 &&
                                local_i <
                                    win_ptr->comm_ptr->node_comm->local_size);
        ...
        if (FALSE == win_ptr->shm_allocated || !SMP_INIT ||
            ...
            *(void **)baseptr =
                win_ptr->basic_info_table[local_target_rank].base_addr;
        } else {
            MPIR_Assert(local_target_rank >= 0 &&
                        local_target_rank <
                            win_ptr->comm_ptr->node_comm->local_size);
...
fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3_WIN_SHARED_QUERY);
    return mpi_errno;
...
int MPIDI_CH3_SHM_Win_free(MPIR_Win **win_ptr)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3_SHM_WIN_FREE);

    if (!SMP_INIT || (SMP_INIT && (*win_ptr)->comm_ptr->local_size <= 1) ||
    ...
    if ((*win_ptr)->shm_allocated) {
        ...
        if (((*win_ptr)->create_flavor == MPI_WIN_FLAVOR_SHARED ||
            ...
            mpi_errno = MPL_shm_seg_detach((*win_ptr)->shm_segment_handle,
                                           (void **)&(*win_ptr)->shm_base_addr,
    ...
    if (((*win_ptr)->create_flavor == MPI_WIN_FLAVOR_SHARED ||
        ...
        /* detach from shared memory segment */
        mpi_errno = MPL_shm_seg_detach((*win_ptr)->shm_mutex_segment_handle,
                                       (void **)&(*win_ptr)->shm_mutex,
    ...
    if ((*win_ptr)->info_shm_base_addr != NULL) {
        mpi_errno = MPL_shm_seg_detach((*win_ptr)->info_shm_segment_handle,
                                       (void **)&(*win_ptr)->info_shm_base_addr,
...
fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3_SHM_WIN_FREE);
    return mpi_errno;
...
int MPIDI_CH3_Win_fns_init(MPIDI_CH3U_Win_fns_t *win_fns)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3_WIN_FNS_INIT);
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3_WIN_FNS_INIT);
...
int MPIDI_CH3_Win_hooks_init(MPIDI_CH3U_Win_hooks_t *win_hooks)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3_WIN_HOOKS_INIT);

    win_hooks->win_init = MPIDI_CH3I_Win_init;
    win_hooks->win_free = MPIDI_CH3_SHM_Win_free;

    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3_WIN_HOOKS_INIT);
                               ...
                               MPIR_Win **win_ptr)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_WIN_INIT);
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_WIN_INIT);
    return mpi_errno;
}
                                     ...
                                     MPI_Aint **base_shm_offs_ptr)
{
    ...
    MPIR_CHKLMEM_DECL(2);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_SHM_WINS_MATCH);
    ...
    *matched_win = NULL;
    ...
    MPIR_CHKLMEM_MALLOC(node_ranks, int *, node_size * sizeof(int), mpi_errno,
                        "node_ranks", MPL_MEM_OTHER);
    MPIR_CHKLMEM_MALLOC(node_ranks_in_shm_node, int *, node_size * sizeof(int),
                        mpi_errno, "node_ranks_in_shm_comm", MPL_MEM_OTHER);
    ...
    while (elem != NULL) {
        ...
        /* Compare node_comm.
         *
         * Only support shm if new node_comm is equal to or a subset of shm
         * node_comm. Shm node_comm == a subset of node_comm is not supported,
         * because it means some processes of node_comm cannot be shared, but
         * RMA operation simply checks the node_id of a target process for
         * distinguishing shm target.  */
        ...
        mpi_errno =
            MPIR_Comm_group_impl(shm_win->comm_ptr, &shm_node_group_ptr);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);

        mpi_errno = MPIR_Group_translate_ranks_impl(
            node_group_ptr, node_size, node_ranks, shm_node_group_ptr,
         ...
         * this call. */
        base_shm_offs[node_rank] =
            (MPI_Aint)((*win_ptr)->base) - (MPI_Aint)(shm_win->shm_base_addr);
        mpi_errno = MPIR_Allgather_impl(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,
                                        base_shm_offs, 1, MPI_AINT,
        ...
        for (i = 0; i < comm_size; ++i) {
            ...
            if (i_node_rank >= 0) {
                ...
                if (base_shm_offs[i_node_rank] < 0 ||
                    base_shm_offs[i_node_rank] +
                            (*win_ptr)->basic_info_table[i].size >
...
fn_exit:
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_SHM_WINS_MATCH);
    return mpi_errno;
...
static int MPIDI_CH3I_Win_detect_shm(MPIR_Win **win_ptr)
{
    ...
    MPIR_CHKPMEM_DECL(1);
    MPIR_CHKLMEM_DECL(1);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_WIN_DETECT_SHM);
    ...
    MPIR_CHKLMEM_MALLOC(base_shm_offs, MPI_Aint *, node_size * sizeof(MPI_Aint),
                        mpi_errno, "base_shm_offs", MPL_MEM_SHM);
    ...
    mpi_errno =
        MPIDI_CH3I_SHM_Wins_match(win_ptr, &shm_win_ptr, &base_shm_offs);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
    if (shm_win_ptr == NULL)
        goto fn_exit;
    ...
    MPIR_CHKPMEM_MALLOC((*win_ptr)->shm_base_addrs, void **,
                        node_size * sizeof(void *), mpi_errno,
                        "(*win_ptr)->shm_base_addrs", MPL_MEM_SHM);
...
fn_exit:
    MPIR_CHKLMEM_FREEALL();
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_WIN_DETECT_SHM);
    return mpi_errno;
    /* --BEGIN ERROR HANDLING-- */
fn_fail:
    MPIR_CHKPMEM_REAP();
                                      ...
                                      MPIR_Win **win_ptr)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_WIN_GATHER_INFO);

    comm_size = (*win_ptr)->comm_ptr->local_size;
    comm_rank = (*win_ptr)->comm_ptr->rank;

    if (!SMP_INIT || (SMP_INIT && comm_size <= 1)) {
        mpi_errno = MPIDI_CH3U_Win_gather_info(base, size, disp_unit, info,
                                               comm_ptr, win_ptr);
        goto fn_exit;
    }

    if (!mvp_enable_shmem_collectives && (*win_ptr)->shm_coll_comm_ref == -1) {
        /* Shared memory for collectives */
        mpi_errno = MPIR_MVP_SHMEM_COLL_init(g_smpi.my_local_id);
        if (mpi_errno) {
            MPIR_ERR_POP(mpi_errno);
        }
        ...
        mpi_errno = MPIR_MVP_SHMEM_COLL_Mmap(g_smpi.my_local_id);
        if (mpi_errno) {
            MPIR_ERR_POP(mpi_errno);
        }
    ...
    if ((*win_ptr)->node_comm_ptr == NULL) {
        ...
        MPIR_Comm_get_ptr(shmem_comm, node_comm_ptr);
    ...
    mvp_init_rank_for_barrier(win_ptr);
    (*win_ptr)->info_shm_segment_len =
    ...
    if (node_rank == 0) {
        ...
        mpi_errno = MPL_shm_seg_create_and_attach(
            (*win_ptr)->info_shm_segment_handle,
            (*win_ptr)->info_shm_segment_len,
            (void **)&(*win_ptr)->info_shm_base_addr, 0);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);

        /* serialize handle and broadcast it to the other processes in win */
        mpi_errno = MPL_shm_hnd_get_serialized_by_ref(
            (*win_ptr)->info_shm_segment_handle, &serialized_hnd_ptr);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);

        mpi_errno = MPIR_Bcast_impl(serialized_hnd_ptr, MPL_SHM_GHND_SZ,
                                    MPI_CHAR, 0, node_comm_ptr, &errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
        MPIR_ERR_CHKANDJUMP(errflag, mpi_errno, MPI_ERR_OTHER, "**coll_fail");
        ...
        /* unlink shared memory region so it gets deleted when all processes
         * exit */
        ...
        mpi_errno = MPIR_Bcast_impl(serialized_hnd, MPL_SHM_GHND_SZ, MPI_CHAR,
                                    0, node_comm_ptr, &errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
        MPIR_ERR_CHKANDJUMP(errflag, mpi_errno, MPI_ERR_OTHER, "**coll_fail");

        mpi_errno =
            MPL_shm_hnd_deserialize((*win_ptr)->info_shm_segment_handle,
    ...
    (*win_ptr)->basic_info_table =
        (MPIDI_Win_basic_info_t *)((*win_ptr)->info_shm_base_addr);
    ...
    mpi_errno =
        MPIR_Allgather_impl(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, tmp_buf, 4,
                            MPI_AINT, (*win_ptr)->comm_ptr, &errflag);
    if (mpi_errno != MPI_SUCCESS)
        MPIR_ERR_POP(mpi_errno);

    if (node_rank == 0) {
        /* only node_rank == 0 writes results to basic_info_table on shared
         * memory region. */
        k = 0;
        for (i = 0; i < comm_size; i++) {
            (*win_ptr)->basic_info_table[i].base_addr =
                MPIR_Aint_to_ptr(tmp_buf[k++]);
    ...
    /* Make sure that all local processes see the results written by node_rank
     * == 0 */
    mpi_errno = MPIR_Barrier_impl(node_comm_ptr, &errflag);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
    ...
    MPIDI_CH3I_RDMA_win_create(base, size, comm_size, comm_rank, win_ptr,
                               comm_ptr);
#endif

#if !defined(_OSU_MVAPICH_)
    /* In default MVAPICH, communicator will be shared among RMA windows if
     * applicable (see function `win_init`), so no need to free 2-level comm
     * here  */
    if ((*win_ptr)->comm_ptr->dev.ch.shmem_coll_ok == 1 &&
        node_comm_ptr != NULL) {
...
fn_exit:
    MPIR_CHKLMEM_FREEALL();
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_WIN_GATHER_INFO);
    return mpi_errno;
    /* --BEGIN ERROR HANDLING-- */
fn_fail:
    goto fn_exit;
                                       ...
                                       void *base_ptr, MPIR_Win **win_ptr)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_CH3I_WIN_ALLOCATE_SHM);

    comm_size = (*win_ptr)->comm_ptr->local_size;

    if (!SMP_INIT || (SMP_INIT && comm_size <= 1)) {
        mpi_errno = MPIDI_CH3U_Win_allocate_no_shm(size, disp_unit, info,
     ...
     * If node_comm == NULL, this process is the only one on this node,
     * therefore we use comm_self as node comm. */
    ...
    if (likely(!(*win_ptr)->node_comm_ptr)) {
        ...
        if (node_comm_ptr == NULL) {
            ...
            PRINT_INFO(
                (!shown_warning && (*win_ptr)->comm_ptr->rank == 0),
                "\n\t[WARNING] Shared memory window cannot be created, "
                "for better performance, please consider increasing the value "
                "of MVP_SHMEM_COLL_NUM_COMM (current value %d)\n\n",
    ...
    MPIR_CHKLMEM_MALLOC(node_sizes, MPI_Aint *, node_size * sizeof(MPI_Aint),
                        mpi_errno, "node_sizes", MPL_MEM_OTHER);
    ...
    for (i = 0; i < node_size; i++) {
        if (noncontig)
            ...
            (*win_ptr)->shm_segment_len +=
                MPIDI_CH3_ROUND_UP_PAGESIZE(node_sizes[i]);
        else
    ...
    if ((*win_ptr)->shm_segment_len == 0) {
        ...
        if (node_rank == 0) {
            ...
            /* create shared memory region for all processes in win and map */
            mpi_errno = MPL_shm_seg_create_and_attach(
                (*win_ptr)->shm_segment_handle, (*win_ptr)->shm_segment_len,
                (void **)&(*win_ptr)->shm_base_addr, 0);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
             ...
             */
            mpi_errno = MPL_shm_hnd_get_serialized_by_ref(
                (*win_ptr)->shm_segment_handle, &serialized_hnd_ptr);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
            ...
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
            MPIR_ERR_CHKANDJUMP(errflag, mpi_errno, MPI_ERR_OTHER,
                                "**coll_fail");
            ...
            /* get serialized handle from rank 0 and deserialize it */
            mpi_errno = MPIR_Bcast_impl(serialized_hnd, MPL_SHM_GHND_SZ,
                                        MPI_CHAR, 0, node_comm_ptr, &errflag);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
            MPIR_ERR_CHKANDJUMP(errflag, mpi_errno, MPI_ERR_OTHER,
            ...
            mpi_errno =
                MPL_shm_hnd_deserialize((*win_ptr)->shm_segment_handle,
                                        serialized_hnd, strlen(serialized_hnd));
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);

            /* attach to shared memory region created by rank 0 */
            mpi_errno = MPL_shm_seg_attach(
                (*win_ptr)->shm_segment_handle, (*win_ptr)->shm_segment_len,
                (void **)&(*win_ptr)->shm_base_addr, 0);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        ...
        if (node_rank == 0) {
             ...
             */
            mpi_errno = MPL_shm_hnd_get_serialized_by_ref(
                (*win_ptr)->shm_mutex_segment_handle, &serialized_hnd_ptr);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
             ...
             * exit */
            mpi_errno =
                MPL_shm_seg_remove((*win_ptr)->shm_mutex_segment_handle);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        } else {
            char serialized_hnd[MPL_SHM_GHND_SZ] = {0};
            ...
            mpi_errno =
                MPL_shm_hnd_deserialize((*win_ptr)->shm_mutex_segment_handle,
                                        serialized_hnd, strlen(serialized_hnd));
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);

            /* attach to shared memory region created by rank 0 */
            mpi_errno = MPL_shm_seg_attach((*win_ptr)->shm_mutex_segment_handle,
                                           sizeof(MPIDI_CH3I_SHM_MUTEX),
        ...
        /* compute the base addresses of each process within the shared memory
         * segment */
        {
            char *cur_base = NULL;
    ...
    *base_pp = (*win_ptr)->base;
    ...
    mpi_errno = MPIDI_CH3I_Win_gather_info((*base_pp), size, disp_unit, info,
                                           comm_ptr, win_ptr);
    if (mpi_errno != MPI_SUCCESS)
        MPIR_ERR_POP(mpi_errno);
...
fn_exit:
    MPIR_CHKLMEM_FREEALL();
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_CH3I_WIN_ALLOCATE_SHM);
    return mpi_errno;
    /* --BEGIN ERROR HANDLING-- */
fn_fail:
    MPIR_CHKPMEM_REAP();
