                        ...
                        int context_offset)
{
    ...
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MRAIL_RNDVSEND);
    ...
    MPL_DBG_MSGPKT(vc, tag, rts_pkt->match.parts.context_id, rank, data_sz,
                   "Rndv");
    ...
    if (dt_contig) {
        ...
        sreq->dev.iov[0].iov_base =
            (void *)((char *)sreq->dev.user_buf + dt_true_lb);
        ...
        if (IS_VC_SMP(vc) || MVP_RNDV_PROTOCOL_RPUT != MVP_RNDV_PROTOCOL) {
        ...
        if (mvp_enable_device &&
            ...
            MPIDI_CH3_ReqHandler_pack_device_stream(vc, sreq, &complete,
                                                    (void *)stream_d2h);
            sreq->dev.iov[0].iov_base = (void *)sreq->dev.tmpbuf;
            sreq->dev.iov[0].iov_len = sreq->dev.segment_size;
            sreq->dev.iov_count = 1;
            sreq->dev.OnDataAvail = 0;
            /* When processing datatype operations, we can end up with cases
             * where one IOV is on the host while the other is on the device.
             * This catches this situation. In particular, when we are using
             * MPI_BOTTOM based scheme in MPIR_Igather_binomial,
             * MPIR_Igather_binomial_MVP, MPIR_Gather_intra we can hit this
             * situation. */
            rts_pkt->rndv.device_transfer_mode =
...
fn_exit:
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MRAIL_RNDVSEND);
    return mpi_errno;
}
...
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MRAIL_RNDVRECV);
    ...
    PRINT_DEBUG(
        ...
        rreq->dev.match.parts.context_id, rreq->dev.user_buf,
...
fn_fail:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MRAIL_RNDVRECV);
    return mpi_errno;
}
