...
int MPIDI_CH3_Prepare_rndv_get(MPIDI_VC_t *vc, MPIR_Request *rreq)
{
    ...
    MPIR_Assert(MRAILI_PROTOCOL_RGET == rreq->mrail.protocol);
...
#ifdef _ENABLE_UD_
        case MRAILI_PROTOCOL_UD_ZCOPY: {
            int hca_num = 0;
            MPIDI_CH3I_MRAIL_Prepare_rndv_zcopy(vc, rreq);
            MPIDI_CH3I_MRAIL_SET_PKT_RNDV(cts_pkt, rreq);
            if (rreq->mrail.protocol == MRAILI_PROTOCOL_UD_ZCOPY) {
...
int MPIDI_CH3_Rndv_transfer(MPIDI_VC_t *vc, MPIR_Request *sreq,
                            MPIR_Request *rreq,
                            MPIDI_CH3_Pkt_rndv_clr_to_send_t *cts_pkt,
                            MPIDI_CH3_Pkt_rndv_req_to_send_t *rts_pkt)
{
    ...
    if (mvp_enable_device && sreq) {
         ...
         * require device_transfer_mode to be set so that we can choose not to
         * do CMA/LiMIC-based transfers if source or target is on device */
        if ((sreq->mrail.device_transfer_mode == NONE) &&
    ...
    switch (req->mrail.protocol) {
        ...
        case MRAILI_PROTOCOL_CUDAIPC:
            if (cts_pkt->rndv.protocol != MVP_RNDV_PROTOCOL_CUDAIPC) {
                sreq->mrail.protocol = cts_pkt->rndv.protocol;
                MPIR_Assert(sreq->mrail.protocol == MRAILI_PROTOCOL_R3);
            }
            rndv = (cts_pkt == NULL) ? NULL : &cts_pkt->rndv;
            sreq->mrail.partner_id = cts_pkt->receiver_req_id;
            break;
...
#ifdef _ENABLE_UD_
        case MRAILI_PROTOCOL_UD_ZCOPY:
            MPIDI_CH3I_MRAILI_Rendezvous_zcopy_push(
                vc, sreq, &(mvp_MPIDI_CH3I_RDMA_Process.zcopy_info));
            break;
#endif
#if defined(_ENABLE_CUDA_) && defined(HAVE_CUDA_IPC)
        case MRAILI_PROTOCOL_CUDAIPC:
...
#if defined(_SMP_CMA_)
static int MPIDI_CH3_CMA_Rendezvous_push(MPIDI_VC_t *vc, MPIR_Request *sreq)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    PRINT_DEBUG(DEBUG_RNDV_verbose > 2,
                ...
                "offset %lu, len: %lu\n",
                type, sreq->mrail.protocol, sreq->mrail.partner_id,
    ...
    MPIR_Assert(sreq->mrail.protocol == MRAILI_PROTOCOL_RPUT ||
                sreq->mrail.protocol == MRAILI_PROTOCOL_RGET);

    if (sreq->mrail.protocol == MRAILI_PROTOCOL_RPUT) {
    ...
    } else if (sreq->mrail.protocol == MRAILI_PROTOCOL_RGET) {
...
static int MPIDI_CH3_SMP_Rendezvous_push(MPIDI_VC_t *vc, MPIR_Request *sreq)
{
    int nb;
    int complete = 0;
    int seqnum;
    ...
    if (sreq->mrail.protocol != MRAILI_PROTOCOL_R3
#if defined(_ENABLE_CUDA_)
    ...
    if (MVP_SMP_USE_CMA &&
        (!MVP_SMP_USE_MAX_SWITCH ||
         (MVP_SMP_USE_MAX_SWITCH &&
          sreq->dev.iov[0].iov_len < MVP_SMP_CMA_MAX_SIZE)) &&
    ...
    if (MVP_SMP_USE_LIMIC2 &&
        (!MVP_SMP_USE_MAX_SWITCH ||
         (MVP_SMP_USE_MAX_SWITCH &&
          sreq->dev.iov[0].iov_len < MVP_SMP_LIMIC2_MAX_SIZE)) &&
        sreq->dev.OnDataAvail != MPIDI_CH3_ReqHandler_SendReloadIOV &&
        sreq->dev.iov_count == 1
#if defined(_ENABLE_CUDA_)
        && (sreq->mrail.device_transfer_mode == NONE)
    ...
    mpi_errno = MPIDI_CH3_iStartMsg(
        vc, &pkt_head, sizeof(MPIDI_CH3_Pkt_rndv_r3_data_t), &send_req);
    ...
    if (MPIDI_CH3I_SMP_SendQ_empty(vc)) {
        ...
        if (mvp_enable_device && MVP_CUDA_SMP_PIPELINE) {
            iov_isdev = is_device_buffer(
                (void *)sreq->dev.iov[sreq->dev.iov_offset].iov_base);
        }
#endif
            ...
            if (vc->smp.send_current_pkt_type == SMP_RNDV_MSG) {
                ...
                if (iov_isdev) {
                    mpi_errno = MPIDI_CH3I_SMP_writev_rndv_data_device(
                        vc, sreq, &sreq->dev.iov[sreq->dev.iov_offset],
                        sreq->dev.iov_count - sreq->dev.iov_offset, &nb,
                        0 /*is_cont*/);
                } else
#endif
                {
...
void MPIDI_CH3_Rendezvous_r3_push(MPIDI_VC_t *vc, MPIR_Request *sreq)
{
    ...
    do {
        do {
            /* We have introduced a mechanism to throttle VBUF use. This can
             * cause an infinite recursion in the R3 code path. Add this check
             * to handle that.
             */
            if ((vc->mrail.state & MRAILI_RC_CONNECTED) &&
                (rdma_vbuf_pools[MVP_LARGE_DATA_VBUF_POOL_OFFSET].free_head ==
                 NULL) &&
                (rdma_vbuf_pools[MVP_LARGE_DATA_VBUF_POOL_OFFSET]
                     .num_allocated >=
                 rdma_vbuf_pools[MVP_LARGE_DATA_VBUF_POOL_OFFSET]
                     .max_num_buf)) {
                ...
                break;
                     ...
                     (rdma_ud_vbuf_pools[MVP_SEND_UD_VBUF_POOL_OFFSET]
                          .free_head == NULL) &&
                     (rdma_ud_vbuf_pools[MVP_SEND_UD_VBUF_POOL_OFFSET]
                          .num_allocated >=
                      rdma_ud_vbuf_pools[MVP_SEND_UD_VBUF_POOL_OFFSET]
    ...
    DEBUG_PRINT("exit loop with complete %d, msg_buffered %d wait %d pending "
                "data:%d \n",
                complete, msg_buffered, wait_for_rndv_r3_ack,
                vc->ch.pending_r3_data);
    ...
    while (flowlist) {
...
#ifdef _ENABLE_XRC_
        if (USE_XRC &&
            VC_XSTS_ISUNSET(flowlist, XF_SMP_VC | XF_DPM_INI | XF_SEND_IDLE |
                                          XF_UD_CONNECTED)) {
        ...
        while (sreq != NULL) {
            ...
            if (flowlist->ch.rput_stop &&
                MRAILI_PROTOCOL_RPUT == sreq->mrail.protocol) {
            ...
            PRINT_DEBUG(DEBUG_RNDV_verbose > 1,
                        ...
                        sreq, sreq->mrail.nearly_complete,
                        sreq->mrail.local_complete,
...
int MPIDI_CH3_Rendezvouz_r3_recv_data(MPIDI_VC_t *vc, vbuf *buffer)
{
    ...
    MPIR_Request_get_ptr(
        ((MPIDI_CH3_Pkt_rndv_r3_data_t *)(buffer->pheader))->receiver_req_id,
        rreq);

    if (!(MRAILI_PROTOCOL_R3 == rreq->mrail.protocol ||
          MRAILI_PROTOCOL_RPUT == rreq->mrail.protocol)) {
        int rank;
        UPMI_GET_RANK(&rank);
        ...
        MPIR_Assert(MRAILI_PROTOCOL_R3 == rreq->mrail.protocol ||
                    MRAILI_PROTOCOL_RPUT == rreq->mrail.protocol);
    }
    ...
    if (MPIDI_CH3I_Request_adjust_iov(rreq, nb)) {
        ...
        if (TRUE == complete) {
            rreq->mrail.protocol = MRAILI_PROTOCOL_RENDEZVOUS_UNSPECIFIED;
        }
    }
fn_exit:
    if (mvp_MPIDI_CH3I_RDMA_Process.has_srq) {
                                          ...
                                          MPIDI_CH3_Pkt_rget_finish_t *rget_pkt)
{
    ...
    PRINT_DEBUG(
        ...
        sreq, sreq->mrail.protocol, sreq->mrail.local_complete,
        sreq->mrail.remote_complete);

#if defined(_ENABLE_CUDA_) && defined(HAVE_CUDA_IPC)
    if (mvp_enable_device && sreq->mrail.ipc_device_event) {
        MVP_MPIDI_Device_StreamWaitEvent(0, sreq->mrail.ipc_device_event->event,
                                         0);
...
int MPIDI_CH3_Rendezvous_rget_recv_finish(MPIDI_VC_t *vc, MPIR_Request *rreq)
{
    int mpi_errno = MPI_SUCCESS;
    int complete;
    ...
    if (rreq->mrail.rndv_buf_alloc == 1) {
        ...
        for (; iter < rreq->dev.iov_count; ++iter) {
            MPIR_Memcpy(rreq->dev.iov[iter].iov_base, (void *)buf,
                        rreq->dev.iov[iter].iov_len);
...
int MPIDI_CH3_Get_rndv_push(MPIDI_VC_t *vc, MPIDI_CH3_Pkt_t *resp_pkt,
                            MPIR_Request *req)
{
    ...
    if (resp_pkt->type == MPIDI_CH3_PKT_GET_RESP) {
        MPIDI_CH3_Pkt_get_resp_t *get_resp_pkt =
            (MPIDI_CH3_Pkt_get_resp_t *)resp_pkt;

        if (MRAILI_PROTOCOL_R3 == req->mrail.protocol) {
            ...
            get_resp_pkt->protocol = MRAILI_PROTOCOL_RPUT;
        ...
        MPIDI_CH3_Pkt_get_accum_resp_t *get_accum_resp_pkt =
            (MPIDI_CH3_Pkt_get_accum_resp_t *)resp_pkt;
        ...
        if (MRAILI_PROTOCOL_R3 == req->mrail.protocol) {
            ...
            get_accum_resp_pkt->protocol = MRAILI_PROTOCOL_RPUT;
...
int MPIDI_CH3_Get_rndv_recv(MPIDI_VC_t *vc, MPIR_Request *req)
{
    ...
    MPIR_Assert(req->mrail.protocol == MRAILI_PROTOCOL_RPUT);
    ...
    if (req->mrail.rndv_buf_alloc == 1) {
        ...
        for (iter = 0; iter < req->dev.iov_count; ++iter) {
            MPIR_Memcpy(req->dev.iov[iter].iov_base, (void *)buf,
                        req->dev.iov[iter].iov_len);
            buf += req->dev.iov[iter].iov_len;
        }

        while (req->dev.OnDataAvail ==
                   MPIDI_CH3_ReqHandler_UnpackSRBufReloadIOV ||
