...
#define PRINT_DEBUG_GID(gid)                                                   \
    do {                                                                       \
        PRINT_DEBUG(DEBUG_MCST_verbose > 2, GID_FMT, (gid)->raw[0],            \
                    (gid)->raw[1], (gid)->raw[2], (gid)->raw[3],               \
                    (gid)->raw[4], (gid)->raw[5], (gid)->raw[6],               \
                    (gid)->raw[7], (gid)->raw[8], (gid)->raw[9],               \
...
#define IBV_POST_MCAST_SEND(_v, _mcast_ctx)                                    \
    do {                                                                       \
        int ret;                                                               \
        ret = ibv_post_send(_mcast_ctx->ud_ctx->qp, &(_v->desc.u.sr),          \
                            &(_v->desc.y.bad_sr));                             \
        if (ret) {                                                             \
...
#if defined(RDMA_CM)
extern int mvp_rdma_cm_mcst_get_addr_info(char *dst, struct sockaddr *addr);
#endif /* #if defined(RDMA_CM) */

/* function return values */
enum { MCAST_SUCCESS, MCAST_FAILURE };

#define MAX_MCAST_FRAGMENT_SIZE                                                \
    (MRAIL_MAX_UD_SIZE - sizeof(MPIDI_CH3_Pkt_mcast_t))

#define MVP_DIFF(start, end) ((int)(start) - (end))
#define IS_MCAST_WINDOW_FULL(a, b)                                             \
    ((MVP_DIFF(a, b) >= mcast_window_size - 1) ? 1 : 0)
#define MVP_MCAST_MAX_COMMS (4096)

extern MPIR_Comm *comm_table[];

int mvp_mcast_init_bcast_info(bcast_info_t **bcast_info);
int mvp_setup_multicast(mcast_info_t *minfo, MPIR_Comm *);
int mvp_cleanup_multicast(mcast_info_t *minfo, MPIR_Comm *);
...
MPIR_Comm *mvp_mcast_find_comm(int comm_id);
mvp_ud_ctx_t *mvp_mcast_prepare_ud_ctx();

/* bitmask operation on char bit array */
#define MVP_CHAR_ISBITSET(x, i) ((x[(i) >> 3] & (1 << ((i)&7))) != 0)
#define MVP_CHAR_SETBIT(x, i)   (x[(i) >> 3] |= (1 << ((i)&7)))
#define MVP_CHAR_CLRBIT(x, i)   (x[(i) >> 3] &= (1 << ((i)&7)) ^ 0xFF);
...
#define MVP_SELECT_MCAST_BASED_BCAST(_comm_ptr, _size)                         \
    (mvp_bcast_indexed_table_ppn_conf[0] != -1 &&                              \
     _comm_ptr->dev.ch.is_mcast_ok && _size >= mcast_bcast_min_msg &&          \
