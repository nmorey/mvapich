...
enum { MSG_QUEUED_RECVWIN, MSG_IN_RECVWIN };
...
#define SEND_WINDOW_CHECK(_ud_vc, _v)                                          \
    {                                                                          \
        if ((_ud_vc)->send_window.count > rdma_default_ud_sendwin_size ||      \
            ...
            mvp_ud_ext_window_add(&(_ud_vc)->ext_window, v);                   \
            PRINT_DEBUG(DEBUG_UD_verbose > 1,                                  \
                        "msg(%p) queued to ext window size:%d\n", _v,          \
                        (_ud_vc)->ext_window.count);                           \
            return 0;                                                          \
        }                                                                      \
    }

static inline void mvp_ud_ext_sendq_queue(message_queue_t *q, vbuf *v)
{
    ...
    PRINT_DEBUG(DEBUG_UD_verbose > 1,
                "Queue msg to rank %d, queue len:%d seqnum:%d\n",
                ((MPIDI_VC_t *)(v->vc))->pg_rank, q->count, v->seqnum);
}

static inline void mvp_ud_ext_window_add(message_queue_t *q, vbuf *v)
{
...
static inline void mvp_ud_send_window_add(message_queue_t *q, vbuf *v)
{
...
static inline void mvp_ud_send_window_remove(message_queue_t *q, vbuf *v)
{
...
static inline void mvp_ud_unack_queue_add(message_queue_t *q, vbuf *v)
{
    ...
    if (q->head == NULL) {
        ...
        PRINT_DEBUG(DEBUG_UD_verbose > 1,
                    "Add head seqnum:%d to rank %d, retry : %d \n", v->seqnum,
                    ((MPIDI_VC_t *)(v->vc))->pg_rank, v->retry_count);
    } else {
        PRINT_DEBUG(
            DEBUG_UD_verbose > 1,
            "Add tail tail-seqnum: %d, seqnum:%d to rank %d, retry : %d \n",
            q->tail->seqnum, v->seqnum, ((MPIDI_VC_t *)(v->vc))->pg_rank,
            v->retry_count);
...
static inline void mvp_ud_unack_queue_remove(message_queue_t *q, vbuf *v)
{
    vbuf *next = v->unack_msg.next;
    vbuf *prev = v->unack_msg.prev;

    PRINT_DEBUG(DEBUG_UD_verbose > 1,
                "Remove seqnum:%d to rank %d, retry : %d \n", v->seqnum,
                ((MPIDI_VC_t *)(v->vc))->pg_rank, v->retry_count);
...
static inline void mvp_ud_track_send(mvp_ud_reliability_info_t *ud_vc,
                                     message_queue_t *unack_queue, vbuf *v)
{
    ...
    mvp_ud_send_window_add(&(ud_vc->send_window), v);
    /* Add vbuf to global unack queue */
    mvp_ud_unack_queue_add(unack_queue, v);
}

static inline void mvp_ud_unackq_traverse(message_queue_t *q)
{
    int r;
    int resend = 0;
    long delay;
    double timestamp = mvp_get_time_us();
    vbuf *cur = q->head;
    ...
    while (cur) {
        ...
        if ((delay > (rdma_ud_retry_timeout * r)) ||
            ...
            resend = mvp_ud_resend(cur);
            if (resend) {
                PRINT_DEBUG(DEBUG_UD_verbose > 1,
                            "Resent seqnum:%d retry : %d \n", cur->seqnum,
                            cur->retry_count);
                cur->timestamp = timestamp;
            } else {
                PRINT_DEBUG(DEBUG_UD_verbose > 1,
                            "Couldn't resend seqnum:%d retry : %d \n",
                            cur->seqnum, cur->retry_count);
            }
            timestamp = mvp_get_time_us();
        } else {
            PRINT_DEBUG(DEBUG_UD_verbose > 1,
                        "Skip seqnum:%d to rank %d, retry : %d \n", cur->seqnum,
                        ((MPIDI_VC_t *)(cur->vc))->pg_rank, cur->retry_count);
        }
        cur = cur->unack_msg.next;
    }
}

static inline int mvp_ud_recv_window_add(message_queue_t *q, vbuf *v,
                                         int recv_win_start)
{
    PRINT_DEBUG(DEBUG_UD_verbose > 1,
                "recv window add recv_win_start:%d rece'd seqnum:%d\n",
...
static inline void mvp_ud_recv_window_remove(message_queue_t *q)
{
