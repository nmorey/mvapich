...
int mvp_get_path_rec_sl(struct ibv_context *context_arg, struct ibv_pd *hca_pd,
                        ...
                        int network_is_3dtorus, int num_sa_retries)
{
    ...
    if (!cache) {
        ...
        cache->mr = ibv_ops.reg_mr(
            pd, cache->send_recv_buffer, sizeof(cache->send_recv_buffer),
        ...
        cache->rsge.addr = (uint64_t)(void *)(cache->send_recv_buffer +
                                              sizeof(struct ib_mad_sa));
    ...
    if (cache->sl_values[rem_lid] == 0x7F) {
        ...
        while (!got_sl_value) {
            ...
            while (!got_sl_value) {
                ...
                if (i > 0 && (wc.status == IBV_WC_SUCCESS) &&
                ...
                if (i == 0) {
                    ...
                    i = (i * 1000000) + get_sl_last_poll.tv_usec -
                        get_sl_last_sent.tv_usec;
                    if (i > 2000000) {
...
fn_fail:
    ...
    if (network_is_3dtorus) {
        fprintf(stderr,
                "Error: Failed to query Subnet Manager for correct "
...
int mvp_release_3d_torus_resources()
{
    int err = MPI_SUCCESS;
