...
#define CR_FTB_EVENT_INFO                                                      \
    {                                                                          \
        {"CR_FTB_CHECKPOINT", "info"}, {"CR_FTB_MIGRATE", "info"},             \
            {"CR_FTB_MIGRATE_PIIC", "info"}, {"CR_FTB_CKPT_DONE", "info"},     \
            {"CR_FTB_CKPT_FAIL", "info"}, {"CR_FTB_RSRT_DONE", "info"},        \
            {"CR_FTB_RSRT_FAIL", "info"}, {"CR_FTB_APP_CKPT_REQ", "info"},     \
            {"CR_FTB_CKPT_FINALIZE", "info"}, {"CR_FTB_MIGRATE_PIC", "info"},  \
            {"FTB_MIGRATE_TRIGGER", "info"}, {"MPI_PROCS_CKPTED", "info"},     \
            {"MPI_PROCS_CKPT_FAIL", "info"}, {"MPI_PROCS_RESTARTED", "info"},  \
...
extern int mvp_init_call_once;
extern int mvp_mmap_coll_once;
...
#define CR_ERR_ABORT(args...)                                                  \
    do {                                                                       \
        fprintf(stderr, "[Rank %d][%s:%d] ", MPIDI_Process.my_pg_rank,         \
                __FILE__, __LINE__);                                           \
        fprintf(stderr, args);                                                 \
        exit(-1);                                                              \
    } while (0)
...
int CR_MPDU_connect_MPD()
{
    ...
    if (err) {
        MPIR_ERR_SETANDJUMP2(err, MPI_ERR_OTHER, "**getaddrinfo",
                             "**getaddrinfo %s %d", strerror(err), err);
    }
    bcopy((void *)res->ai_addr, (void *)&sa, sizeof(sa));
    freeaddrinfo(res);

    if (using_mpirun_rsh) {
        snprintf(session_file, 32, "/tmp/cr.session.%s",
                 getenv("MVP_CKPT_SESSIONID"));
        ...
        /* Legacy Mode */
        sa.sin_port = htons(MPICR_MPD_port + MPICR_pg_rank +
                            restart_count * CR_RSRT_PORT_CHANGE);
    }

    if (setsockopt(MPICR_MPD_fd, IPPROTO_TCP, TCP_NODELAY, (char *)&optval,
                   sizeof(optval))) {
...
void CR_MPDU_Rsrt_succeed()
{
    ...
    set_event(&eprop, FTB_EVENT_NORMAL, MPICR_pg_rank);
    if (cr_ftb_mig_req)
        ret = FTB_Publish(ftb_handle, EVENT(MPI_PROCS_MIGRATED), &eprop,
                          &ehandle);
    else
        ret = FTB_Publish(ftb_handle, EVENT(MPI_PROCS_RESTARTED), &eprop,
                          &ehandle);
    if (ret != FTB_SUCCESS)
...
void CR_MPDU_Rsrt_fail()
{
    ...
    set_event(&eprop, FTB_EVENT_NORMAL, MPICR_pg_rank);
    if (cr_ftb_mig_req)
        ret = FTB_Publish(ftb_handle, EVENT(MPI_PROCS_MIGRATE_FAIL), &eprop,
                          &ehandle);
    else
        ret = FTB_Publish(ftb_handle, EVENT(MPI_PROCS_RESTART_FAIL), &eprop,
                          &ehandle);
    if (ret != FTB_SUCCESS)
...
MPICR_cr_state MPIDI_CH3I_CR_Get_state() { return MPICR_state; }
...
static int connect_to_migration_target(char *tgt)
{
    ...
    char lockfile[128];
    char syncfile[128];
    snprintf(lockfile, 127, "/tmp/cr-%s-mig", getenv("MVP_CKPT_SESSIONID"));
    snprintf(syncfile, 127, "/tmp/cr-%s-mig-begin",
             getenv("MVP_CKPT_SESSIONID"));
    ...
    wait_for_begin:
        ...
        while (ret != 0) {
            if (errno == ENOENT) { /* wait a while, then try again */
                ...
                PRINT_DEBUG(DEBUG_MIG_verbose > 1,
                            "%s: wait for sync-file %s: blocked...\n", __func__,
                            syncfile);
    ...
    char migmnt[128];
    int run = 1;
    snprintf(migmnt, 127, "/tmp/cr-%s/mig/", getenv("MVP_CKPT_SESSIONID"));
...
int CR_Thread_loop()
{
    ...
    while (1) {
        ...
        PRINT_DEBUG(DEBUG_CR_verbose,
                    "CR_Thread_loop:Got request from mpirun_rsh %s\n",
                    cr_msg_buf);
        CR_MPDU_parse_keyvals(cr_msg_buf);
...

        {
            ...
            if (cr_ftb_mig_req && use_aggre_mig)
                ...
                strcpy(valstr, getenv("MVP_CKPT_FILE"));
#else
            strcpy(valstr, getenv("MVP_CKPT_FILE"));
#endif
            ...
            if (cr_ftb_mig_req)
                PRINT_DEBUG(DEBUG_MIG_verbose > 1,
                            "CR_Thread_loop:Got migration request %s\n",
                            valstr);
            ...
            if (cr_ftb_ckpt_req)
#endif
                sprintf(cr_file, "%s.%d.%d", valstr, checkpoint_count,
                        MPICR_pg_rank);
            ...
            PRINT_DEBUG(DEBUG_CR_verbose > 2,
                        "locking CR MPICR_cs_lock: lock2=%d, unlock2=%d \n",
            ...
            if (MPICR_pg_rank == 0) {
                if (cr_ftb_ckpt_req)
                    ...
                    PRINT_DEBUG(DEBUG_MIG_verbose,
                                "Begin migration: source = %s, target = %s\n",
            ...
            PRINT_DEBUG(DEBUG_CR_verbose,
                        "cr_mig_src_host = %s, cr_mig_tgt_host = %s \n",
                        cr_mig_src_host, cr_mig_tgt_host);
            ...
            if (cr_ftb_mig_req && use_aggre_mig &&
                ...
                PRINT_DEBUG(DEBUG_MIG_verbose,
                            "At %s: before mig: connect to tgt %s\n",
                            cr_mig_src_host, cr_mig_tgt_host);
...
#ifdef ENABLE_SCR
            PRINT_DEBUG(DEBUG_CR_verbose,
                        "cr_request_checkpoint() with file '%s'\n", scr_file);
#else
            PRINT_DEBUG(DEBUG_CR_verbose,
                        "cr_request_checkpoint() with file '%s'\n", cr_file);
#endif
            ...
            PRINT_DEBUG(DEBUG_CR_verbose,
                        "cr_request_checkpoint() returned %d\n", ret);
            if (ret < 0) {
                CR_ERR("BLCR call cr_request_checkpoint() failed with error "
                       "%d: %s\n",
            ...
            if (ret == CR_POLL_CHKPT_ERR_POST && errno == CR_ERESTARTED) {
                ...
                // Wait for the CR_Callback thread to return from
                // cr_checkpoint()
            } else if (ret < 0) {
                CR_ERR(
                    "BLCR call cr_poll_checkpoint() failed with error %d: %s\n",
                    errno, cr_strerror(errno));
                CR_MPDU_Ckpt_fail();
                CR_ERR_ABORT("Checkpoint failed, aborting...\n");
            } else if (ret == 0) {
                // 0 means that the checkpoint is in progress
                // It should never happen because we don't specify any timeout
                // when calling cr_poll_checkpoint()
            ...
            /*
               PRINT_DEBUG(DEBUG_CR_verbose > 1,"checkpointing, wait for
               callback to finish\n");
               pthread_mutex_lock(&MPICR_cond_callback_lock);
               pthread_cond_wait(&MPICR_cond_callback,
               &MPICR_cond_callback_lock);
            ...
            if (!MPICR_is_restarting) {
                if (no_sync == 0) {
                    PRINT_DEBUG(DEBUG_CR_verbose > 1,
                                "Syncing checkpoint file to disk\n");
                ...
                if (ret < 0) {
                    PRINT_ERROR_ERRNO("Failed to close file '%s'", errno,
                                      cr_file);
                    CR_MPDU_Ckpt_fail();
                    CR_ERR_ABORT("Checkpoint failed, aborting...\n");
                }
            ...
            gethostname(my_hostname, 255);
            if (cr_ftb_mig_req &&
                strstr(my_hostname, cr_mig_src_host) ==
                    my_hostname) { // procs on src nodes, will exit soon
                PRINT_DEBUG(DEBUG_MIG_verbose > 1,
                            "host=%s:rank %d: Src node exit\n", my_hostname,
                            MPICR_pg_rank);
                PRINT_DEBUG(
                    DEBUG_MIG_verbose,
                    "Rank %d (pid=%d) on the source node is migrated -> exit\n",
            ...
            // Update error prefix with new hostname
            {
                ...
                snprintf(output_prefix, MAX_LENGTH, "%s:mpi_rank_%i", hostname,
                         MPICR_pg_rank);
            ...
            PRINT_DEBUG(DEBUG_MIG_verbose > 1,
                        "Rank %d (pid=%d) is restarted on the target node\n",
                        MPICR_pg_rank, getpid());
        skip_checkpoint:
#endif
            ...
            if (cr_ftb_mig_req) {
                PRINT_DEBUG(DEBUG_MIG_verbose > 1,
                            "Entering migration barrier\n");
                if (UPMI_BARRIER() != 0)
                    fprintf(stderr,
                            "At Migration: rank %d, UPMI_BARRIER failed\n",
                            MPICR_pg_rank);
            }
            /* end of phase 3 */

            if (cr_ftb_mig_req &&
                strstr(my_hostname, cr_mig_tgt_host) ==
                    my_hostname) { // this proc is migrated from src-node
                gethostname(my_hostname, 255);
                PRINT_DEBUG(
                    DEBUG_MIG_verbose > 1,
                    "\tHave mig [%d] to tgt-host=%s: Left migration-barrier.\n",
                    MPICR_pg_rank, my_hostname);
            }
#endif
            PRINT_DEBUG(DEBUG_MIG_verbose,
                        "Reactivating communication channels\n");
            ...
            if (MPICR_is_restarting) {
                ...
                if (MPICR_max_save_ckpts > 0 &&
                    MPICR_max_save_ckpts < checkpoint_count) {
                    /*remove the earlier checkpoints */
                    sprintf(cr_file, "%s.%d.%d", valstr,
                            checkpoint_count - MPICR_max_save_ckpts,
                            MPICR_pg_rank);
                    unlink(cr_file);
                }
            }
            ...
            if (MPIDI_Process.use_sync_ckpt) {
                ...
                pthread_cond_signal(&MVAPICH_sync_ckpt_cond);
...
#else
            PRINT_ERROR(
                "Internal Error: Unknown command: msg = '%s', cmd = '%s'\n",
                cr_msg_buf, valstr);
#endif
            abort();
        }
...
static int norsh_cr_callback(void *arg)
{
    ...
    if (rc < 0) { /* checkpoint failed */
        PRINT_DEBUG(DEBUG_CR_verbose, "Checkpoint failed: (%d) %s\n", rc,
                    cr_strerror(-rc));
        return -1;
    } else { /* process is continuing after checkpoint (or restarting from
                failure )*/
        PRINT_DEBUG(DEBUG_CR_verbose,
                    "Reactivating the communication channels\n");
        if (CR_IBU_Reactivate_channels()) {
            PRINT_DEBUG(DEBUG_CR_verbose,
                        "CR_IBU_Reactivate_channels failed!\n");
            return -1;
        }
    ...
    PRINT_DEBUG(DEBUG_CR_verbose > 2,
                "Unlocking the CH3 critical section lock\n");
    pthread_rwlock_unlock(&MPICR_cs_lock);
    wr_CR_lock_tid = 0;
    unlock2_count++;
...
#ifdef CR_FTB
        if (CR_FTB_Init(MPICR_pg_rank, getenv("MVP_CKPT_SESSIONID")) != 0) {
            fprintf(stderr, "%s: Rank[%d] CR_FTB_Init() Failed\n", __func__,
                    MPICR_pg_rank);
        } else {
            PRINT_DEBUG(DEBUG_CR_verbose > 1,
                        "%s: Rank[%d] CR_FTB_Init()done\n", __func__,
                        MPICR_pg_rank);
        }
#endif
...
void *CR_Thread_entry(void *arg)
{
    ...
    if (using_mpirun_rsh) {
        MPICR_callback_id =
            cr_register_callback(CR_Callback, NULL, CR_THREAD_CONTEXT);
        ...
        char *temp = getenv("MVP_CKPT_MPD_BASE_PORT");

        if (temp) {
            MPICR_MPD_port = atoi(temp);
        } else {
            CR_ERR_ABORT("MVP_CKPT_MPD_BASE_PORT is not set\n");
        }

        temp = getenv("MVP_CKPT_MAX_SAVE_CKPTS");

        if (temp) {
            MPICR_max_save_ckpts = atoi(temp);
            PRINT_DEBUG(DEBUG_CR_verbose > 1, "MVP_CKPT_MAX_SAVE_CKPTS  %s\n",
                        temp);
        } else {
        ...
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "Finish initialization, going to CR_Thread_loop\n");

        CR_Thread_loop();
    } else {
        MPICR_callback_id =
            cr_register_callback(norsh_cr_callback, NULL, CR_THREAD_CONTEXT);
        if (-1 == MPICR_callback_id) {
            CR_ERR_ABORT(
                "BLCR call cr_register_callback() failed with error %d: %s\n",
                errno, cr_strerror(errno));
        ...
        MPICR_is_initialized = 1;
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "Finished initialization for the non-mpirun_rsh case\n");
    }
...
int MPIDI_CH3I_CR_Init(MPIDI_PG_t *pg, int rank, int size)
{
    ...
    PRINT_DEBUG(
        DEBUG_CR_verbose > 1,
        "Creating a new thread for running cr controller: MPICR_cs_lock\n");
    ...
    PRINT_DEBUG(DEBUG_CR_verbose > 1,
                "Creating a new thread for running cr controller\n");
...
int MPIDI_CH3I_CR_Finalize()
{
    ...
    if (ftb_init_done) {
        ...
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "\tCR_Finalize: FTB_publish() MPICR_cs_lock\n");
        ret = FTB_Publish(ftb_handle, EVENT(CR_FTB_CKPT_FINALIZE), &eprop,
                          &ehandle);
        if (ret != FTB_SUCCESS)
        ...
        usleep(20000);
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "\tCR_Finalize: done FTB_publish() MPICR_cs_lock\n");
        FTB_Disconnect(ftb_handle);
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "\tCR_Finalize: done FTB_Disconnect() MPICR_cs_lock\n");
    }
    pthread_cancel(MPICR_child_thread);
    pthread_join(MPICR_child_thread, NULL);
...
void MPIDI_CH3I_CR_Handle_recv(MPIDI_VC_t *vc, MPIDI_CH3_Pkt_type_t msg_type,
                               vbuf *v)
{
    ...
    switch (msg_type) {
        case MPIDI_CH3_PKT_CR_REMOTE_UPDATE:
            PRINT_DEBUG(DEBUG_CR_verbose > 2,
                        "Received MPIDI_CH3_PKT_CR_REMOTE_UPDATE\n");
            ...
            while (sreq) {
                ...
                if (sreq->mrail.remote_addr == msg->recv_mem_addr) {
                    PRINT_DEBUG(DEBUG_CR_verbose > 2,
                                "Found a address match req: %p\n", sreq);

                    /*FIXME: Is address match enough? */
                    MPIR_Memcpy(sreq->mrail.rkey, msg->recv_buf_rkey,
                                sizeof(uint32_t) * MAX_NUM_HCAS);
                    found++;
                    if (vc->mrail.sreq_to_update <= 0) {
                        PRINT_DEBUG(DEBUG_CR_verbose > 2,
                                    "[%d <== %d]: %s: Warn: REM_UPDATE: "
                                    "sreq-to-up=%d...\n",
                                    MPICR_pg_rank, vc->pg_rank, __func__,
                                    vc->mrail.sreq_to_update);
                        /// a spurious REM_UPDATE msg, not a real bug...
                        // MPIR_Assert( vc->mrail.sreq_to_update > 0 );// don't
                        // need to crash the prog
                    } else
                        vc->mrail.sreq_to_update--; // has updated one sender'
                                                    // rndv(sreq)
                    if (vc->mrail.sreq_to_update == 0) {
                        // reset rput_stop at ibv_chann_man
                        // vc->ch.rput_stop = 0; // all pending sender rndv have
                        // been updated
                    }
            ...
            if (!found) {
                PRINT_DEBUG(
                    DEBUG_CR_verbose > 2,
                    "[%d <== %d]: %s: Warn: REM_UPDATE: no match found...\n",
                    MPICR_pg_rank, vc->pg_rank, __func__);
            }
...
int CR_IBU_Release_network()
{
    ...
    if (MPICM_Finalize_UD() != 0) {
        ...
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "CR_IBU_Release_network: MPICM_Finalize_UD succeed.\n");
    }
    ...
    PRINT_DEBUG(DEBUG_CR_verbose > 1,
                "CR_IBU_Release_network: CH3I_RDMA.has_srq\n");
    if (mvp_MPIDI_CH3I_RDMA_Process.has_srq) {
        for (i = 0; i < rdma_num_hcas; ++i) {
            ...
            if (ibv_ops.destroy_srq(mvp_MPIDI_CH3I_RDMA_Process.srq_hndl[i])) {
    ...
    while (curr) {
        if (curr->local_rank == 0) {
            PRINT_DEBUG(DEBUG_CR_verbose > 1,
                        "Freeing shmem region %p, reg = %d\n", curr,
                        curr->buffer_registered);
            if (curr->buffer_registered == 1) {
                mvp_shm_coll_dereg_buffer(curr->mem_handle);
            }
    ...
    PRINT_DEBUG(DEBUG_CR_verbose > 1,
                "CR_IBU_Release_network: deregister_all\n");
    dreg_deregister_all();
    ...
    if (SMP_INIT) {
        if (MPIDI_CH3I_SMP_finalize()) {
            fprintf(stderr,
                    "[%s:%d] MPIDI_CH3I_SMP_finalize() returned error\n",
                    __FILE__, __LINE__);
            return (-1);
        }
    }
...
int CR_IBU_Rebuild_network()
{
    ...
    union ibv_gid *gid_all =
        (union ibv_gid *)MPL_malloc(pg_size * sizeof(union ibv_gid));
    ...
    if (rdma_open_hca(&mvp_MPIDI_CH3I_RDMA_Process)) {
        MPL_error_printf("rdma_open_hca failed\n");
        return -1;
    }

    mpi_errno =
        rdma_iba_hca_init_noqp(&mvp_MPIDI_CH3I_RDMA_Process, pg_rank, pg_size);
    if (mpi_errno) {
    ...
    if (mvp_MPIDI_CH3I_RDMA_Process.has_srq) {
        ...
        for (i = 0; i < rdma_num_hcas; ++i) {
            pthread_mutex_init(
                &mvp_MPIDI_CH3I_RDMA_Process.srq_post_mutex_lock[i], 0);
            pthread_cond_init(&mvp_MPIDI_CH3I_RDMA_Process.srq_post_cond[i], 0);

            mvp_MPIDI_CH3I_RDMA_Process.srq_zero_post_counter[i] = 0;
            mvp_MPIDI_CH3I_RDMA_Process.posted_bufs[i] =
                mvp_post_srq_buffers(mvp_srq_fill_size, i);

            srq_attr.max_wr = mvp_srq_fill_size;
            srq_attr.max_sge = 1;
            srq_attr.srq_limit = mvp_srq_limit;

            if (ibv_ops.modify_srq(mvp_MPIDI_CH3I_RDMA_Process.srq_hndl[i],
            ...
            pthread_create(&mvp_MPIDI_CH3I_RDMA_Process.async_thread[i], NULL,
                           (void *)async_thread,
                           (void *)mvp_MPIDI_CH3I_RDMA_Process.nic_context[i]);
        }
    ...
    if (pg_size == 1) {
        ...
        lid_all[0] = mvp_MPIDI_CH3I_RDMA_Process.lids[0][0];
        gid_all[0] = mvp_MPIDI_CH3I_RDMA_Process.gids[0][0];
    } else if (pg_size > 1) {
        if (mvp_MPIDI_CH3I_RDMA_Process.has_ring_startup) {
            ...
            if (mpi_errno) {
                MPIR_ERR_SETANDJUMP2(mpi_errno, MPI_ERR_OTHER, "**getaddrinfo",
                                     "**getaddrinfo %s %d", strerror(mpi_errno),
                                     mpi_errno);
            }
            int hostid =
                (int)((struct sockaddr_in *)res->ai_addr)->sin_addr.s_addr;
            freeaddrinfo(res);
            self_info.hostid = hostid;

            memcpy(&self_info.lid, &mvp_MPIDI_CH3I_RDMA_Process.lids,
                   sizeof(uint16_t) * MAX_NUM_HCAS * MAX_NUM_PORTS);
            memcpy(&self_info.gid, &mvp_MPIDI_CH3I_RDMA_Process.gids,
                   sizeof(union ibv_gid) * MAX_NUM_HCAS * MAX_NUM_PORTS);
            self_info.qpn = ud_qpn_self;

            ud_addr_info_t *all_info =
                (ud_addr_info_t *)MPL_malloc(sizeof(ud_addr_info_t) * pg_size);
            /*will be freed in rdma_iba_bootstrap_cleanup */
            PRINT_DEBUG(DEBUG_CR_verbose > 2,
                        "Setting up ring-based exchange\n");
            rdma_setup_startup_ring(&mvp_MPIDI_CH3I_RDMA_Process, pg_rank,
                                    pg_size);

            PRINT_DEBUG(DEBUG_CR_verbose > 2,
                        "Ring-based all-gather exchange\n");
            mpi_errno = rdma_ring_based_allgather(&self_info, sizeof(self_info),
                                                  pg_rank, all_info, pg_size,
                                                  &mvp_MPIDI_CH3I_RDMA_Process);
            if (mpi_errno) {
                PRINT_DEBUG(DEBUG_CR_verbose > 2,
                            "rdma-ring-based-allgather failed, ret=%d...\n",
                            mpi_errno);
                MPIR_ERR_POP(mpi_errno);
            }
            for (i = 0; i < pg_size; ++i) {
            ...
            sprintf(mvp_pmi_key, "ud_info-%08d", pg_rank);
            if (!use_iboeth) {
                sprintf(mvp_pmi_val, "%08x:%08x",
                        mvp_MPIDI_CH3I_RDMA_Process.lids[0][0], ud_qpn_self);
            } else {
                sprintf(
                    mvp_pmi_val, "%08x:%08x:%016" PRIx64 ":%016" PRIx64,
                    mvp_MPIDI_CH3I_RDMA_Process.lids[0][0], ud_qpn_self,
                    mvp_MPIDI_CH3I_RDMA_Process.gids[0][0].global.subnet_prefix,
                    mvp_MPIDI_CH3I_RDMA_Process.gids[0][0].global.interface_id);
            }

            if (UPMI_KVS_PUT(pg->ch.kvs_name, mvp_pmi_key, mvp_pmi_val) != 0) {
            ...
            if (!using_mpirun_rsh) {
                ...
                char *envval;
                if ((envval = getenv("MVP_CKPT_SLURM_PMI_DELAY")) != NULL) {
            ...
            for (i = 0; i < pg_size; ++i) {
                if (pg_rank == i) {
                    lid_all[i] = mvp_MPIDI_CH3I_RDMA_Process.lids[0][0];
                    gid_all[i] = mvp_MPIDI_CH3I_RDMA_Process.gids[0][0];
                    ud_qpn_all[i] = ud_qpn_self;
                    continue;
                }

                sprintf(mvp_pmi_key, "ud_info-%08d", i);

                if (UPMI_KVS_GET(pg->ch.kvs_name, mvp_pmi_key, mvp_pmi_val,
                                 mvp_pmi_max_vallen) != 0) {
                    CR_ERR_ABORT("UPMI_KVS_GET failed\n");
                }

                if (!use_iboeth) {
                    sscanf(mvp_pmi_val, "%08hx:%08x", &(lid_all[i]),
                           &(ud_qpn_all[i]));
                } else {
                    sscanf(mvp_pmi_val, "%08hx:%08x:%016" SCNx64 ":%016" SCNx64,
                           &(lid_all[i]), &(ud_qpn_all[i]),
    ...
    for (i = 0; i < pg_size; ++i) {
        ...
        memcpy(&pg->ch.mrail->cm_shmem.ud_cm[i].cm_gid, &gid_all[i],
...
int CR_IBU_Prep_remote_update()
{
    ...
    while (temp != NULL) {
        if (temp->mrail.rndv_buf == last_mem_addr) {
            if (temp->mrail.d_entry->memhandle[0]->rkey != rkey[0]) {
                CR_ERR_ABORT("Same addr %p, different rkey %x, %x\n",
                             temp->mrail.rndv_buf,
                             temp->mrail.d_entry->memhandle[0]->rkey, rkey[0]);
            }
        ...
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "Found a new memory address %p registered\n",
                    temp->mrail.rndv_buf);
        ...
        i = 0;
        PRINT_DEBUG(DEBUG_CR_verbose > 2,
                    "Search msg log queue for CTS message with the address %p, "
                    "in vc %p \n",
        ...
        while (NULL != entry) {
            ...
            if (p->type == MPIDI_CH3_PKT_RNDV_CLR_TO_SEND) {
                ...
                if (cts_header->rndv.buf_addr == temp->mrail.rndv_buf) {
                    PRINT_DEBUG(DEBUG_CR_verbose > 2,
                                "Found a match in local pending cts\n");
                    MPIR_Memcpy(cts_header->rndv.rkey, rkey,
                                sizeof(uint32_t) * MAX_NUM_HCAS);
                }
            }
        ...
        if (!MPIDI_CH3I_CM_SendQ_empty(vc)) {
            ...
            while (req != NULL) {
                MPIDI_CH3_Pkt_t *upkt =
                    (MPIDI_CH3_Pkt_t *)&(req->dev.pending_pkt);
                ++i;
                ...
                if (upkt->type == MPIDI_CH3_PKT_RNDV_CLR_TO_SEND) {
                    ...
                    PRINT_DEBUG(DEBUG_CR_verbose > 2,
                                "CTS in cm_send_Q addr %p\n",
                                cts_header->rndv.buf_addr);

                    if (cts_header->rndv.buf_addr == temp->mrail.rndv_buf) {
                        PRINT_DEBUG(DEBUG_CR_verbose > 2,
                                    "Found a match in local cm queue\n");
                        MPIR_Memcpy(cts_header->rndv.rkey, rkey,
                                    sizeof(uint32_t) * MAX_NUM_HCAS);
                    }
                }
        ...
        if (!SMP_ONLY) {
            v = get_vbuf_by_offset(MVP_RECV_VBUF_POOL_OFFSET);
            p = (MPIDI_CH3I_MRAILI_Pkt_comm_header *)v->pheader;
        } else {
            p = (MPIDI_CH3I_MRAILI_Pkt_comm_header *)MPL_malloc(
                sizeof(MPIDI_CH3I_MRAILI_Pkt_comm_header));
        }

        p->type = MPIDI_CH3_PKT_CR_REMOTE_UPDATE;
        MPIR_Memcpy(v->buffer + sizeof(MPIDI_CH3I_MRAILI_Pkt_comm_header), &msg,
                    sizeof(msg));

        /* Push update packet to message log queue. */
        entry = (MPIDI_CH3I_CR_msg_log_queue_entry_t *)MPL_malloc(
            sizeof(MPIDI_CH3I_CR_msg_log_queue_entry_t));
...
void CR_record_rndv(MPIDI_VC_t **vc_vector)
{
    ...
    for (i = 0; i < MPICR_pg_size; i++) {
        ...
        if (n > 0 || vc->ch.sendq_head || vc->ch.cm_sendq_head ||
            vc->mrail.msg_log_queue_head) {
            PRINT_DEBUG(
                DEBUG_CR_verbose > 2,
                "%s: [%d vc_%d]: has  %d  rndv-send to update, ch.sendq_head = "
                "%p, ch.cm_sendq_head=%p, vc.mrail.msg_log_q_head=%p\n",
                __func__, MPICR_pg_rank, vc->pg_rank, n, vc->ch.sendq_head,
                vc->ch.cm_sendq_head, vc->mrail.msg_log_queue_head);
        }
    }
}
...
{
    int i = 0;
    MPIDI_VC_t **vc_vector =
        (MPIDI_VC_t **)MPL_malloc(sizeof(MPIDI_VC_t *) * MPICR_pg_size);
    MPIDI_VC_t *vc = NULL;
...
int CR_IBU_Reactivate_channels()
{
    ...
    mvp_init_call_once = 0;
    mvp_mmap_coll_once = 0;
    ...
    if (mvp_shmem_pool_init) {
        PRINT_DEBUG(DEBUG_CR_verbose > 2,
                    "Attempting to reattach SMP pool used for large-message "
                    "exchange before checkpoint.\n");
    ...
    if ((retval = CR_IBU_Prep_remote_update())) {
        PRINT_DEBUG(DEBUG_CR_verbose,
                    "CR_IBU_Prep_remote_update() returned %d\n", retval);
        return retval;
    }
    ...
    for (; i < MPICR_pg_size; ++i) {
        ...
        if (vc->ch.state == MPIDI_CH3I_VC_STATE_SUSPENDED ||
            ...
            if (vc->mrail.react_entry) { /* cm_thread has gotten REACT_DONE from
                                            peer */
                /* enquue the REACT_DONE to be sent to peer */
                PRINT_DEBUG(
                    DEBUG_CR_verbose > 2,
                    "%s: [%d => %d]: REACT_DONE came earlier, enq now...\n",
...
static int CR_FTB_Init(int rank, char *sessionid)
{
    ...
    strcpy(ftb_cinfo.event_space, "FTB.MPI.MVAPICH");

    snprintf(ftb_cinfo.client_name, FTB_MAX_CLIENT_NAME, "MVAPICH.%d", rank);
    ...
    /* Set PMI_PORT */
    snprintf(session_file, 64, "/tmp/cr.session.%s",
             getenv("MVP_CKPT_SESSIONID"));
    fd = open(session_file, O_RDWR);
    if (fd < 0)
        goto err_open;
...
err_declare_events:
    fprintf(stderr,
            "FTB_Declare_publishable_events() failed with %d"
            " for rank %d\n",
            ret, rank);
    ret = -2;
...
static void CR_FTB_Finalize()
{
    if (ftb_init_done) {
        ...
        FTB_Disconnect(ftb_handle);
        PRINT_DEBUG(DEBUG_CR_verbose > 1,
                    "\t %s:%d: [CR]rank =%d : Disconnected handle\n", __FILE__,
                    __LINE__, MPIDI_Process.my_pg_rank);
    }
}
...
static int CR_FTB_Callback(FTB_receive_event_t *revent, void *arg)
{
    ...
    if (!strcmp(revent->event_name, EVENT(CR_FTB_MIGRATE))) {
        if (use_aggre_mig)
            parse_aggre_migration_string(revent->event_payload, cr_mig_src_host,
                                         cr_mig_tgt_host);
        else
            get_src_tgt(revent->event_payload, cr_mig_src_host,
                        cr_mig_tgt_host);
...
int CR_MPDU_writeline(int fd, char *buf)
{
    ...
    if (size > MAX_CR_MSG_LEN) {
        ...
        fprintf(stderr,
                "write_line: message string doesn't end in newline: :%s:\n",
                buf);
    } else {
...
int CR_MPDU_parse_keyvals(char *st)
{
    ...
    while (1) {
        ...
        if (*p == ' ' || *p == '\n' || *p == '\0') {
            PRINT_ERROR("Internal error: Unexpected key delimiter at character "
                        "%d in '%s'\n",
                        (int)(p - st), st);
            return -1;
        }
