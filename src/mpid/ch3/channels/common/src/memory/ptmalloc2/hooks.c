...
void __malloc_check_init()
{
    ...
    s = getenv("MVP_ENABLE_MALLOC_CHECK");
    if (s && atoi(s) > 0) {
...
#endif
{
    ...
    p = mem2chunk(ptr);
    for (i = chunksize(p) -
             (chunk_is_mmapped(p) ? 2 * SIZE_SZ + 1 : SIZE_SZ + 1);
         i > sz; i -= 0xFF) {
        if (i - sz < 0x100) {
...
#endif
{
    ...
    if (!chunk_is_mmapped(p)) {
        ...
        if ((contig &&
            ...
            (!prev_inuse(p) &&
             (p->prev_size & MALLOC_ALIGN_MASK ||
              (contig && (char *)prev_chunk(p) < mp_.sbrk_base) ||
              next_chunk(prev_chunk(p)) != p)))
            return NULL;
        magic = MAGICBYTE(p);
        for (sz += SIZE_SZ - 1; (c = ((unsigned char *)p)[sz]) != magic;
             sz -= c) {
    ...
    if (chunk_is_mmapped(oldp)) {
...
#endif
        {
            ...
            if (oldsize - SIZE_SZ >= nb)
                ...
                if (newmem) {
                    MALLOC_COPY(BOUNDED_N(newmem, bytes + 1), oldmem,
                                oldsize - 2 * SIZE_SZ);
                    munmap_chunk(oldp);
                }
            }
...
#endif
{
    ...
    (void)mutex_lock(&main_arena.mutex);
    mem = (top_check() >= 0) ?
              _int_memalign(&main_arena, alignment, bytes + 1) :
              NULL;
    (void)mutex_unlock(&main_arena.mutex);
...
#else
memalign_starter(align, sz, caller)
size_t align, sz;
const Void_t *caller;
#endif
{
...
int public_sET_STATe(Void_t *msptr)
{
    ...
    for (i = 1; i < NBINS; i++) {
        ...
        if (ms->av[2 * i + 2] == 0) {
            ...
            if (i < NSMALLBINS ||
                (largebin_index(chunksize(ms->av[2 * i + 2])) == i &&
