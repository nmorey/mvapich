...
static __malloc_ptr_t(*save_malloc_hook)
    __MALLOC_P((size_t __size, __const __malloc_ptr_t));
#if !defined _LIBC || !defined USE_TLS || (defined SHARED && !USE___THREAD)
static __malloc_ptr_t(*save_memalign_hook)
    __MALLOC_P((size_t __align, size_t __size, __const __malloc_ptr_t));
#endif
static void(*save_free_hook)
    __MALLOC_P((__malloc_ptr_t __ptr, __const __malloc_ptr_t));
...
static void ptmalloc_unlock_all __MALLOC_P((void))
{
    mstate ar_ptr;
...
static void ptmalloc_unlock_all2 __MALLOC_P((void))
{
    mstate ar_ptr;
...
static void ptmalloc_init_minimal __MALLOC_P((void))
{
...
static
#endif
    void ptmalloc_init __MALLOC_P((void))
{
#if __STD_C
    const char *s;
#else
...
#endif
{
    ...
    if (p1 != MAP_FAILED) {
        ...
        /* Try to take the chance that an allocation of only HEAP_MAX_SIZE
           is already aligned. */
        p2 = (char *)MMAP(0, HEAP_MAX_SIZE, PROT_NONE,
                          MAP_PRIVATE | MAP_NORESERVE);
        if (p2 == MAP_FAILED)
            return 0;
