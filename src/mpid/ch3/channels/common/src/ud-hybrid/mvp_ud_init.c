...
struct ibv_qp *mvp_ud_create_qp(mvp_ud_qp_info_t *qp_info, int hca_index)
{
    ...
    if (mvp_ud_qp_transition(qp, hca_index)) {
        return NULL;
    }
...
int mvp_ud_qp_transition(struct ibv_qp *qp, int hca_index)
{
    ...
    attr.pkey_index = 0;
    attr.port_num = mvp_MPIDI_CH3I_RDMA_Process.ports[hca_index][0];
    ...
    if (ibv_ops.modify_qp(qp, &attr, IBV_QP_STATE | IBV_QP_SQ_PSN)) {
...
mvp_ud_ctx_t *mvp_ud_create_ctx(mvp_ud_qp_info_t *qp_info, int hca_index)
{
    ...
    ctx = MPL_malloc(sizeof(mvp_ud_ctx_t), MPL_MEM_OTHER);
    if (!ctx) {
        fprintf(stderr, "%s:no memory!\n", __func__);
        return NULL;
    }
    memset(ctx, 0, sizeof(mvp_ud_ctx_t));

    ctx->qp = mvp_ud_create_qp(qp_info, hca_index);
    if (!ctx->qp) {
        fprintf(stderr, "Error in creating UD QP\n");
        return NULL;
    }
