...
static inline vma_t *vma_new(unsigned long start, unsigned long end)
{
    ...
    if (NULL == vma) {
        vma = MPL_malloc(sizeof(vma_t), MPL_MEM_OTHER);
        MPIR_Memset(vma, 0x0, sizeof(vma_t));
    }
...
static inline void add_entry(vma_t *vma, dreg_entry *r)
{
    ...
    if (NULL == e) {
        e = MPL_malloc(sizeof(entry_t), MPL_MEM_OTHER);
        MPIR_Memset(e, 0x0, sizeof(entry_t));
    }
#else  /* !defined(DISABLE_PTMALLOC) */
    e = MPL_malloc(sizeof(entry_t), MPL_MEM_OTHER);
    MPIR_Memset(e, 0x0, sizeof(entry_t));
...
int dreg_init()
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_GEN2_DREG_INIT);
    MPIR_FUNC_TERSE_ENTER(MPID_GEN2_DREG_INIT);

    vma_db_init();
    dreg_free_list = MPL_malloc(
        (unsigned)(sizeof(dreg_entry) * rdma_ndreg_entries), MPL_MEM_OTHER);
    ...
    MPIR_Memset(dreg_free_list, 0, sizeof(dreg_entry) * rdma_ndreg_entries);
    ...
    delayed_buf_region = MPL_malloc(
        (unsigned)(sizeof(struct iovec) * rdma_ndreg_entries), MPL_MEM_BUFFER);
    ...
    MPIR_Memset(delayed_buf_region, 0,
                sizeof(struct iovec) * rdma_ndreg_entries);
    buf_reg_count = 0;

    deregister_mr_array = (dreg_entry **)MPL_malloc(
        sizeof(dreg_entry *) * rdma_ndreg_entries, MPL_MEM_OTHER);
    ...
    MPIR_Memset(deregister_mr_array, 0,
...
fn_exit:
    MPIR_FUNC_TERSE_EXIT(MPID_GEN2_DREG_INIT);
    return mpi_errno;
fn_fail:
    goto fn_exit;
}
...
int dreg_finalize()
{
    MPIR_FUNC_TERSE_STATE_DECL(MPID_GEN2_DREG_FINALIZE);
    MPIR_FUNC_TERSE_ENTER(MPID_GEN2_DREG_FINALIZE);
    ...
    if (dreg_all_list != NULL)
        MPL_free(dreg_all_list);

#if !defined(DISABLE_PTMALLOC)

    if (delayed_buf_region != NULL)
        MPL_free(delayed_buf_region);

    if (deregister_mr_array != NULL) {
        MPL_free(deregister_mr_array);
    }
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_GEN2_DREG_FINALIZE);
    return MPI_SUCCESS;
}
...
{
    int ret, count = 0;
    /* Instead of blocking while trying to acquire this lock, we "try" it. If
     * the lock is being held by another thread, we yield the CPU after looping
     * for a while */
...
void flush_dereg_mrs_external()
{
    unsigned long j, k;
    struct dreg_entry *d;
    /* Not adding these to the function stack becuase it
     * makes output extremely cluttered */
    /* MPIR_FUNC_VERBOSE_ENTER(MPID_GEN2_FLUSH_DREG_MRS_EXTERNALS); */
    ...
    for (j = 0; j < n_dereg_mr; j++) {
        ...
        PRINT_DEBUG(DEBUG_DREG_verbose,
                    "Trying to free dreg %p. ref_count = %d, valid = %d, "
                    "npages = %lu, pagenum = %lu\n",
                    d, d->refcount, d->is_valid, d->npages, d->pagenum);
        MPIR_Assert(d->is_valid == 0);
        MPIR_Assert(d->refcount == 0);
        ...
        PRINT_DEBUG(DEBUG_DREG_verbose,
                    "Adding dreg %p to free list. ref_count = %d, valid = %d, "
                    "npages = %lu, pagenum = %lu\n",
                    d, d->refcount, d->is_valid, d->npages, d->pagenum);
    ...
    unlock_dereg();
    unlock_dreg();
    /* MPIR_FUNC_VERBOSE_EXIT(MPID_GEN2_FLUSH_DREG_MRS_EXTERNALS); */
}
...
dreg_entry *dreg_register(void *buf, size_t len)
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_GEN2_DREG_REGISTER);
    MPIR_FUNC_TERSE_ENTER(MPID_GEN2_DREG_REGISTER);
    ...
    if (unlikely(buf_reg_count)) {
        for (i = 0; i < buf_reg_count; ++i) {
            if (buf >= delayed_buf_region[i].iov_base &&
                buf < (delayed_buf_region[i].iov_base +
                       delayed_buf_region[i].iov_len)) {
                PRINT_DEBUG(DEBUG_DREG_verbose,
                            "%p already in delayed_buf_region at loc %d (len = "
                            "%lu/%lu)\n",
                            buf, i, delayed_buf_region[i].iov_len, len);
                flush_delayed_dregs(delayed_buf_region[i].iov_base,
                                    delayed_buf_region[i].iov_len);
            }
        }
    }
    ...
    if (d != NULL) {
        ...
        PRINT_DEBUG(DEBUG_DREG_verbose,
                    "Dreg hit with dreg %p for buf %p, ref_count = %d, valid = "
                    "%d, npages = %lu, pagenum = %lu\n",
                    d, buf, d->refcount, d->is_valid, d->npages, d->pagenum);
        MPIR_T_PVAR_COUNTER_INC(MVP, mvp_reg_cache_hits, 1);
    } else {
        ++dreg_stat_cache_miss;
        MPIR_T_PVAR_COUNTER_INC(MVP, mvp_reg_cache_misses, 1);
        ...
        while ((d = dreg_new_entry(buf, len)) == NULL) {
            ...
            PRINT_INFO(!show_dreg_evict_warning,
                       "[Performance Impact Warning]: Entries are being "
                       "evicted from the InfiniBand registration cache."
                       " This can lead to degraded performance. Consider "
                       "increasing MVP_NDREG_ENTRIES_MAX "
                       "(current value: %d) and MVP_NDREG_ENTRIES (current "
                       "value: %d)\n",
        ...
        dreg_incr_refcount(d);
        PRINT_DEBUG(DEBUG_DREG_verbose,
                    "Dreg miss for %p. Dreg = %p ref_count = %d, valid = %d, "
                    "npages = %lu, pagenum = %lu\n",
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_GEN2_DREG_REGISTER);
    return d;
}
...
dreg_entry *dreg_get(void)
{
    ...
    if (d != NULL) {
        ...
        d->next = NULL;
        PRINT_DEBUG(DEBUG_DREG_verbose,
                    "Got Dreg %p from free list. ref_count = %d, valid = %d, "
                    "npages = %lu, pagenum = %lu\n",
                    d, d->refcount, d->is_valid, d->npages, d->pagenum);
    }
#if defined(DEBUG)
...
void dreg_decr_refcount(dreg_entry *d)
{
    ...
    PRINT_DEBUG(DEBUG_DREG_verbose,
                "Decrementing refcount for dreg %p. ref_count = %d, valid = "
                "%d, npages = %lu, pagenum = %lu\n",
                d, d->refcount, d->is_valid, d->npages, d->pagenum);
    if (d->refcount == 0) {
#ifdef NEMESIS_BUILD
...
#endif
        {
            ...
            dreg_remove(d);
            PRINT_DEBUG(DEBUG_DREG_verbose,
                        "Adding dreg %p to free list. ref_count = %d, valid = "
                        "%d, npages = %lu, pagenum = %lu\n",
                        d, d->refcount, d->is_valid, d->npages, d->pagenum);
            DREG_ADD_TO_FREE_LIST(d);
        }
...
void dreg_incr_refcount(dreg_entry *d)
{
    ...
    ++d->refcount;
    PRINT_DEBUG(DEBUG_DREG_verbose,
                "Incrementing refcount for dreg %p. ref_count = %d, valid = "
                "%d, npages = %lu, pagenum = %lu\n",
                d, d->refcount, d->is_valid, d->npages, d->pagenum);
}
...
int dreg_evict()
{
    ...
    MPIR_Assert(d->refcount == 0 || mvp_is_in_finalize);
#else
    MPIR_Assert(d->refcount == 0);
#endif

    PRINT_DEBUG(DEBUG_DREG_verbose,
                "Trying to evict dreg %p. ref_count = %d, valid = %d, npages = "
                "%lu, pagenum = %lu\n",
                d, d->refcount, d->is_valid, d->npages, d->pagenum);
#ifdef NEMESIS_BUILD
...
#endif
    {
        if (d->memhandle[hca_index]) {
            ...
            PRINT_DEBUG(DEBUG_DREG_verbose,
                        "Unregistering dreg %p: ref_count = %d, valid = %d, "
                        "npages = %lu, pagenum = %lu\n",
                        d, d->refcount, d->is_valid, d->npages, d->pagenum);
...
dreg_entry *dreg_new_entry(void *buf, size_t len)
{
    ...
    if (rdma_dreg_cache_limit != 0 && npages >= (int)rdma_dreg_cache_limit) {
        return NULL;
    }
    ...
    {
        d->memhandle[i] =
            register_memory((void *)pagebase_low_p, register_nbytes, i);
...
#ifdef NEMESIS_BUILD
    if (!g_is_dreg_initialized || !process_info.has_lazy_mem_unregister) {
#else
    if (!g_is_dreg_initialized ||
        !mvp_MPIDI_CH3I_RDMA_Process.has_lazy_mem_unregister) {
#endif
        return;
    }
...
find_buf:
    ...
    for (i = 0; i < npages; i++) {
        ...
        end = ((unsigned long)(((char *)addr) + DREG_PAGESIZE - 1)) >>
              DREG_PAGEBITS;
        ...
        while ((d = dreg_lookup(begin, end)) != NULL) {
            if (d->refcount != 0 || d->is_valid == 0) {
                ...
                PRINT_DEBUG(DEBUG_DREG_verbose,
                            "Not freeing dreg %p. ref_count = %d, valid = %d, "
                            "npages = %lu, pagenum = %lu\n",
                            d, d->refcount, d->is_valid, d->npages, d->pagenum);
                break;
            }
            ...
            d->is_valid = 0;
            PRINT_DEBUG(DEBUG_DREG_verbose,
                        "Trying to free associated dreg %p. ref_count = %d, "
...
void dreg_reregister_all()
{
    ...
    for (; i < (int)rdma_ndreg_entries; ++i) {
        ...
        if (d->is_valid) {
            ...
            {
                d->memhandle[j] =
                    register_memory(pagebase_low_p, register_nbytes, j);
                if (!d->memhandle[j]) {
                    printf("%d: reregister dentry %p, addr %p pagebase_low_p, "
                           "%lu register_nbytes\n",
                           MPIDI_Process.my_pg_rank, d, pagebase_low_p,
                           register_nbytes);
