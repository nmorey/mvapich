...
PRIVATE char *ckalloc(size)
unsigned int size;
{
    char *ptr = MPL_malloc((size_t)size, MPL_MEM_OTHER);
 ...
 */
PRIVATE AVLtree new_node(data, size) void *data;
unsigned int size;
{
    AVLtree root;
    ...
    root->data = (void *)ckalloc(size);
#endif /* !defined(DISABLE_PTMALLOC) */
    MPIR_Memcpy(root->data, data, size);
    root->bal = BALANCED;
 ...
 */
PRIVATE long avl_min(elt1, elt2, nd_typ) void *elt1, *elt2;
NODE nd_typ;
{
 ...
 */
PRIVATE long avl_max(elt1, elt2, nd_typ) void *elt1, *elt2;
NODE nd_typ;
{
        ...
        *data = (*rootp)->data; /* set return value in data */
        ...
        switch (nd_typ) { /* what kind of node are we removing? */
            ...
            case IS_TREE:
                 ...
                 * long.
                 */
                MPIR_Memcpy(scratch_space, successor_data, sizeof(void *));
                 ...
                 * was freed */
                MPIR_Memcpy((*rootp)->data, scratch_space, sizeof(void *));
 ...
 */
PRIVATE void avl_free(rootp, action, sibling_order, level) AVLtree *rootp;
void (*action)();
SIBLING_ORDER sibling_order;
long level;
{
    ...
    if (*rootp != NULL_TREE) {
        ...
        if ((*rootp)->subtree[dir1] != NULL_TREE) {
            avl_free(&((*rootp)->subtree[dir1]), action, sibling_order,
                     level + 1);
        }
        ...
        if ((*rootp)->subtree[dir2] != NULL_TREE) {
            avl_free(&((*rootp)->subtree[dir2]), action, sibling_order,
                     level + 1);
        }
 ...
 */
PUBLIC void avlwalk(tree, action, sibling_order) AVL_TREE tree;
void (*action)();
SIBLING_ORDER sibling_order;
{
    AVLdescriptor *avl_desc;
