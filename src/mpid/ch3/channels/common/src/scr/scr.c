...
static int scr_do_finalize =
    1; /* Hack to enable MVAPICH to call SCR_Finalize within MPI_Finalize */

/* writes entry to halt file to indicate that SCR should exit job at first
 * opportunity */
...
static int scr_bool_check_halt_and_decrement(int halt_cond, int decrement)
{
    ...
    if (scr_my_rank_world == 0) {
        ...
        int halt_seconds;
        if (scr_hash_util_get_int(scr_halt_hash, SCR_HALT_KEY_SECONDS,
                                  &halt_seconds) != SCR_SUCCESS) {
        ...
        if (halt_seconds > 0) {
            ...
            if (remaining >= 0 && remaining <= halt_seconds) {
                if (halt_cond == SCR_TEST_AND_HALT) {
                    scr_dbg(0,
                            "Job exiting: Reached time limit: (seconds "
                            "remaining = %ld) <= (SCR_HALT_SECONDS = %d).",
        ...
        char *reason;
        if (scr_hash_util_get_str(scr_halt_hash, SCR_HALT_KEY_EXIT_REASON,
                                  &reason) == SCR_SUCCESS) {
            if (strcmp(reason, "") != 0) {
                /* since reason points at the EXIT_REASON string in the halt
                 * hash, and since
                 * scr_halt() resets this value, we need to copy the current
                 * reason */
        ...
        if (scr_hash_util_get_int(scr_halt_hash, SCR_HALT_KEY_CHECKPOINTS,
                                  &checkpoints_left) == SCR_SUCCESS) {
        ...
        int exit_before;
        if (scr_hash_util_get_int(scr_halt_hash, SCR_HALT_KEY_EXIT_BEFORE,
                                  &exit_before) == SCR_SUCCESS) {
            if (now >= (exit_before - halt_seconds)) {
                if (halt_cond == SCR_TEST_AND_HALT) {
                    ...
                    char str_now[256];
                    char str_exit[256];
                    strftime(str_now, sizeof(str_now), "%c",
                             localtime(&time_now));
                    strftime(str_exit, sizeof(str_exit), "%c",
                             localtime(&time_exit));
                    scr_dbg(0,
                            "Job exiting: Current time (%s) is past "
                            "ExitBefore-HaltSeconds time (%s).",
                            str_now, str_exit);
        ...
        int exit_after;
        if (scr_hash_util_get_int(scr_halt_hash, SCR_HALT_KEY_EXIT_AFTER,
                                  &exit_after) == SCR_SUCCESS) {
            if (now >= exit_after) {
                if (halt_cond == SCR_TEST_AND_HALT) {
                    time_t time_now = (time_t)now;
                    ...
                    strftime(str_exit, sizeof(str_exit), "%c",
                             localtime(&time_exit));
                    scr_dbg(0,
                            "Job exiting: Current time (%s) is past ExitAfter "
                            "time (%s).",
                            str_now, str_exit);
    ...
    if (need_to_halt && halt_cond == SCR_TEST_AND_HALT) {
        ...
        if (scr_flush_async_in_progress) {
            /* there's an async flush ongoing, see which dataset is being
             * flushed */
            if (scr_flush_async_dataset_id == scr_dataset_id) {
                /* we're going to sync flush this same checkpoint below, so kill
                 * it */
                scr_flush_async_stop(scr_map);
            } else {
                /* the async flush is flushing a different checkpoint, so wait
                 * for it */
...
static int scr_check_flush(scr_filemap *map)
{
    ...
    if (scr_flush > 0) {
        ...
        if (scr_checkpoint_id > 0 && scr_checkpoint_id % scr_flush == 0) {
            /* need to flush this checkpoint, determine whether to use async or
             * sync flush */
            if (scr_flush_async) {
                /* check that we don't start an async flush if one is already in
                 * progress */
                if (scr_flush_async_in_progress) {
                    /* we need to flush the current checkpoint, however, another
                     * flush is ongoing, so wait for this other flush to
 ...
 * return the full path to the file which the caller should use to access the
 * file */
static int scr_route_file(const scr_reddesc *reddesc, int id, const char *file,
                          char *newfile, int n)
{
    ...
    if (strlen(file) >= SCR_MAX_FILENAME) {
        scr_abort(-1,
                  "file name (%s) is longer than SCR_MAX_FILENAME (%d) @ %s:%d",
...
static int scr_get_params()
{
    ...
    /* user may want to disable SCR at runtime, read env var to avoid reading
     * config files */
    ...
    /* check enabled parameter again, this time including settings from config
     * files */
    ...
    /* read username from SCR_USER_NAME, if not set, try to read from
     * environment */
    ...
    if (scr_username == NULL) {
        scr_abort(-1, "Failed to record username @ %s:%d", __FILE__, __LINE__);
    }
    ...
    if (scr_jobid == NULL) {
        scr_abort(-1, "Failed to record jobid @ %s:%d", __FILE__, __LINE__);
    }
    ...
    if ((value = scr_param_get("SCR_JOB_NAME")) != NULL) {
        ...
        if (scr_jobname == NULL) {
            scr_abort(
                -1, "Failed to allocate memory to record jobname (%s) @ %s:%d",
                value, __FILE__, __LINE__);
        }
    }

    /* read cluster name from SCR_CLUSTER_NAME, if not set, try to read from
     * environment */
    ...
    if (scr_clustername == NULL) {
        if (scr_my_rank_world == 0) {
            scr_warn("Failed to record cluster name @ %s:%d", __FILE__,
                     __LINE__);
        }
    }
    ...
    if (tmp != NULL) {
        ...
        tmp =
            scr_hash_set_kv(scr_storedesc_hash, SCR_CONFIG_KEY_STOREDESC, "0");
    ...
    if (scr_copy_type == SCR_COPY_SINGLE) {
        ...
        if (tmp != NULL) {
            ...
            scr_abort(-1, "Failed to define checkpoints @ %s:%d", __FILE__,
                      __LINE__);
        }
    }

    /* if job has fewer than SCR_HALT_SECONDS remaining after completing a
     * checkpoint, halt it */
    ...
    if ((value = scr_param_get("SCR_FLUSH_ASYNC_PERCENT")) != NULL) {
        if (scr_atod(value, &d) == SCR_SUCCESS) {
            ...
            scr_err(
                "Failed to read SCR_FLUSH_ASYNC_PERCENT successfully @ %s:%d",
                __FILE__, __LINE__);
        }
    }
    ...
    if ((value = scr_param_get("SCR_USE_CONTAINERS")) != NULL) {
        ...
        if (scr_flush_async && scr_use_containers) {
            scr_warn("Async flush does not yet support containers, disabling "
                     "containers @ %s:%d",
                     __FILE__, __LINE__);
    ...
    if ((value = scr_param_get("SCR_CHECKPOINT_OVERHEAD")) != NULL) {
        if (scr_atod(value, &d) == SCR_SUCCESS) {
            ...
            scr_err(
                "Failed to read SCR_CHECKPOINT_OVERHEAD successfully @ %s:%d",
                __FILE__, __LINE__);
        }
    }
    ...
    /* NOTE: We do this in between our existing calls to scr_param_init and
     * scr_param_finalize, since scr_log_init itself calls param_init to read
     * the db username and password from the config file, which in turn requires
     * a bcast.  However, only rank 0 calls scr_log_init(), so the bcast would
     * fail if scr_param_init really had to read the config file again. */
    if (scr_my_rank_world == 0 && scr_log_enable) {
        if (scr_log_init() != SCR_SUCCESS) {
            scr_warn(
                "Failed to initialize SCR logging, disabling logging @ %s:%d",
                __FILE__, __LINE__);
            scr_log_enable = 0;
        }
    }
...
int SCR_Init()
{
    ...
    if (dtcmp_rc != DTCMP_SUCCESS) {
        scr_abort(-1, "Failed to initialize DTCMP library @ %s:%d", __FILE__,
                  __LINE__);
    }
#endif /* HAVE_LIBDTCMP */

    /* NOTE: SCR_ENABLE can also be set in a config file, but to read a config
     * file, we must at least create scr_comm_world and call scr_get_params() */
    ...
    if (scr_my_hostname == NULL) {
        scr_err("Failed to get hostname @ %s:%d", __FILE__, __LINE__);
    ...
    if (scr_page_size <= 0) {
        scr_err("Call to getpagesize failed @ %s:%d", __FILE__, __LINE__);
        MPI_Abort(scr_comm_world, 0);
    }
    ...
    if (!scr_enabled) {
        ...
        if (dtcmp_rc != DTCMP_SUCCESS) {
            scr_abort(-1, "Failed to finalized DTCMP library @ %s:%d", __FILE__,
                      __LINE__);
        }
#endif /* HAVE_LIBDTCMP */
    ...
    if (scr_username == NULL || scr_jobid == NULL) {
        scr_abort(-1,
                  "Jobid or username is not set; set SCR_JOB_ID or "
                  "SCR_USER_NAME @ %s:%d",
                  __FILE__, __LINE__);
    }
    ...
    if (scr_reddescs_create() != SCR_SUCCESS) {
        if (scr_my_rank_world == 0) {
            scr_err(
                "Failed to prepare one or more redundancy descriptors @ %s:%d",
                __FILE__, __LINE__);
        }
    }
    ...
    for (i = 0; i < scr_nreddescs; i++) {
        /* check that we have at least one descriptor enabled with an interval
         * of one */
    ...
    if (!found_one) {
        if (scr_my_rank_world == 0) {
            scr_abort(-1,
                      "Failed to find an enabled redundancy descriptor with "
                      "interval 1 @ %s:%d",
                      __FILE__, __LINE__);
        }
    }

    /* if we're using containers we need two other communicators during each
     * flush, one contains all procs on each node, and one contains all procs
     * that have the same rank per node */
    ...
    if (groupdesc_node != NULL) {
        ...
        MPI_Comm_split(scr_comm_world, rank_node, scr_my_rank_world,
    ...
    if (scr_my_rank_world == 0 && scr_log_enable) {
        if (scr_username != NULL && scr_jobname != NULL) {
            ...
            if (scr_log_job(scr_username, scr_jobname, job_start) ==
                SCR_SUCCESS) {
                /* record the start time for this run */
                scr_log_run(job_start);
            } else {
                scr_err("Failed to log job for username %s and jobname %s, "
                        "disabling logging @ %s:%d",
                        scr_username, scr_jobname, __FILE__, __LINE__);
                scr_log_enable = 0;
            }
        } else {
            scr_err("Failed to read username or jobname from environment, "
                    "disabling logging @ %s:%d",
                    __FILE__, __LINE__);
            scr_log_enable = 0;
        }
    ...
    if (scr_prefix == NULL || strcmp(scr_prefix, "") == 0) {
        if (scr_my_rank_world == 0) {
            ...
            scr_abort(-1, "SCR_PREFIX must be set @ %s:%d" __FILE__, __LINE__);
        }
    ...
    /* TODO: should we check for access and required space in cache directory at
     * this point? */
        ...
        if (reddesc->enabled) {
            ...
            if (store != NULL) {
                ...
                if (scr_storedesc_dir_create(store, reddesc->directory) !=
                    SCR_SUCCESS) {
                    scr_abort(-1,
                              "Failed to create cache directory: %s @ %s:%d",
    ...
    /* TODO: should we check for access and required space in cache directore at
     * this point? */
    ...
    scr_path_append_strf(scr_map_file, "filemap_%d.scrinfo",
                         scr_storedesc_cntl->rank);
    ...
    /* TODO: continue draining a checkpoint if one is in progress from the
     * previous run,
    ...
    /* TODO: should we also record the list of nodes and / or MPI rank to node
     * mapping? */
    ...
    int num_nodes;
    MPI_Allreduce(&ranks_across, &num_nodes, 1, MPI_INT, MPI_MAX,
                  scr_comm_world);
    if (scr_my_rank_world == 0) {
    ...
    if (scr_halt_seconds > 0) {
        scr_hash_util_set_unsigned_long(scr_halt_hash, SCR_HALT_KEY_SECONDS,
                                        scr_halt_seconds);
    }
    ...
    /* now all processes are initialized (be careful when moving this line up or
     * down) */
    scr_initialized = 1;
    ...
    /* master on each node reads all filemaps and distributes them to other
     * ranks on the node, we take this step in case the number of ranks on this
     * node has changed since the last run */
    ...
    if (rc != SCR_SUCCESS && scr_distribute) {
        ...
        if (rc == SCR_SUCCESS) {
             ...
             * if the rebuild failed, we'll delete the flush file after purging
             * the cache below */
    ...
    /* start the clocks for measuring the compute time and time of last
     * checkpoint */
    if (scr_my_rank_world == 0) {
        ...
        if (scr_log_enable) {
            ...
            scr_log_event("COMPUTE STARTED", NULL, &compute_id,
                          &scr_timestamp_compute_start, NULL);
        }
    }
...
int SCR_Do_Finalize() { scr_do_finalize = 1; }

/* MVAPICH does not want SCR to finalize */
int SCR_Donot_Finalize() { scr_do_finalize = 0; }
...
int SCR_Finalize()
{
    ...
    if (scr_do_finalize) {
        ...
        if (!scr_initialized) {
            scr_abort(-1, "SCR has not been initialized @ %s:%d", __FILE__,
                      __LINE__);
            return SCR_FAILURE;
        }
        ...
        if (scr_my_rank_world == 0) {
            ...
            /* if we reach SCR_Finalize, assume that we should not restart the
             * job */
...
int SCR_Need_checkpoint(int *flag)
{
    ...
    *flag = 0;
    ...
    if (!*flag &&
        scr_bool_check_halt_and_decrement(SCR_TEST_BUT_DONT_HALT, 0)) {
        *flag = 1;
    }
    ...
    if (scr_my_rank_world == 0) {
        ...
        if (!*flag && scr_checkpoint_interval > 0 &&
            scr_need_checkpoint_count % scr_checkpoint_interval == 0) {
            *flag = 1;
        }

        /* if checkpoint seconds is set, check the time since the last
         * checkpoint */
        if (!*flag && scr_checkpoint_seconds > 0) {
            double now_seconds = MPI_Wtime();
            if ((int)(now_seconds - scr_time_checkpoint_end) >=
        ...
        if (!*flag && scr_checkpoint_overhead > 0) {
            ...
            if (scr_time_checkpoint_count == 0) {
                /* if we haven't taken a checkpoint, we need to take one in
                ...
                *flag = 1;
                ...
                double avg_cost = scr_time_checkpoint_total /
                                  (double)scr_time_checkpoint_count;
                double percent_cost =
                    avg_cost / (now - scr_time_checkpoint_end + avg_cost) *
                    100.0;
         ...
         */
        if (!*flag && scr_checkpoint_interval <= 0 &&
            scr_checkpoint_seconds <= 0 && scr_checkpoint_overhead <= 0) {
            *flag = 1;
        }
    }
...
int SCR_Start_checkpoint()
{
    ...
    /* bail out if user called Start_checkpoint twice without
     * Complete_checkpoint in between */
    if (scr_in_output) {
        scr_abort(-1,
                  "SCR_Complete_checkpoint must be called before "
                  "SCR_Start_checkpoint is called again @ %s:%d",
                  __FILE__, __LINE__);
        return SCR_FAILURE;
    }

    /* make sure everyone is ready to start before we delete any existing
     * checkpoints */
    MPI_Barrier(scr_comm_world);
    ...
    if (scr_my_rank_world == 0) {
        ...
        if (scr_log_enable) {
            ...
            scr_log_event("COMPUTE COMPLETED", NULL, &compute_id, &now,
                          &time_diff);
        }
    }
    ...
    scr_reddesc *reddesc = scr_reddesc_for_checkpoint(
        scr_checkpoint_id, scr_nreddescs, scr_reddescs);
    ...
    if (scr_my_rank_world == 0) {
        ...
        if (scr_log_enable) {
            ...
            scr_log_event("CHECKPOINT STARTED", reddesc->base,
                          &scr_checkpoint_id, &scr_timestamp_checkpoint_start,
                          NULL);
        }
    }
    ...
    /* run through each of our checkpoints and count how many we have in this
     * base */
    int nckpts_base = 0;
    for (i = 0; i < ndsets; i++) {
        ...
        /* get base for this checkpoint and increase count if it matches the
         * target base */
        base =
            scr_reddesc_base_from_filemap(scr_map, dsets[i], scr_my_rank_world);
        if (base != NULL) {
    ...
    /* run through and delete checkpoints from base until we make room for the
     * current one */
    int flushing = -1;
        ...
        if (base != NULL) {
            if (strcmp(base, reddesc->base) == 0) {
                if (!scr_bool_is_flushing(dsets[i])) {
                    /* this checkpoint is in our base, and it's not being
                     * flushed, so delete it */
                    scr_cache_delete(scr_map, dsets[i]);
                    nckpts_base--;
                } else if (flushing == -1) {
                    /* this checkpoint is in our base, but we're flushing it,
                     * don't delete it */
                    flushing = dsets[i];
                }
            }
    ...
    /* if we still don't have room and we're flushing, the checkpoint we need to
     * delete must be flushing, so wait for it to finish */
    if (nckpts_base >= size && flushing != -1) {
        /* TODO: we could increase the transfer bandwidth to reduce our wait
         * time */

        /* wait for this checkpoint to complete its flush */
        scr_flush_async_wait(scr_map);

        /* now checkpoint is no longer flushing, we can delete it and continue
         * on */
    ...
    if (scr_my_rank_world == 0) {
        ...
        scr_dataset_build_name(scr_dataset_id, dataset_time, dataset_name,
                               sizeof(dataset_name));
    ...
    scr_filemap_set_dataset(scr_map, scr_dataset_id, scr_my_rank_world,
                            dataset);
    ...
    scr_hash_util_set_int(flushdesc, SCR_SCAVENGE_KEY_PRESERVE,
                          scr_preserve_directories);
    scr_hash_util_set_int(flushdesc, SCR_SCAVENGE_KEY_CONTAINER,
                          scr_use_containers);
    scr_filemap_set_flushdesc(scr_map, scr_dataset_id, scr_my_rank_world,
                              flushdesc);
    scr_hash_delete(&flushdesc);

    /* store the redundancy descriptor in the filemap, so if we die before
     * completing
     * the checkpoint, we'll have a record of the new directory we're about to
     * create */
    scr_hash *my_desc_hash = scr_hash_new();
    scr_reddesc_store_to_hash(reddesc, my_desc_hash);
    scr_filemap_set_desc(scr_map, scr_dataset_id, scr_my_rank_world,
                         my_desc_hash);
...
/* given a filename, return the full path to the file which the user should
 * write to */
int SCR_Route_file(const char *file, char *newfile)
{
    ...
    if (scr_route_file(reddesc, scr_dataset_id, file, newfile, n) !=
        SCR_SUCCESS) {
        return SCR_FAILURE;
    }
     ...
     */
    if (scr_in_output) {
        /* TODO: to avoid duplicates, check that the file is not already in the
         * filemap, at the moment duplicates just overwrite each other, so
         * there's no harm */

        /* add the file to the filemap */
        scr_filemap_add_file(scr_map, scr_dataset_id, scr_my_rank_world,
                             newfile);

        /* read meta data for this file */
        scr_meta *meta = scr_meta_new();
        scr_filemap_get_meta(scr_map, scr_dataset_id, scr_my_rank_world,
                             newfile, meta);
        ...
        /* TODODSET: move the ranks field elsewhere, for now it's needed by
         * scr_index.c */
        ...
        if (!scr_path_is_absolute(path_abs)) {
            /* the path is not absolute, so prepend the current working
             * directory */
        ...
        scr_filemap_set_meta(scr_map, scr_dataset_id, scr_my_rank_world,
                             newfile, meta);
...
int SCR_Complete_checkpoint(int valid)
{
    ...
    if (!scr_in_output) {
        scr_abort(-1,
                  "SCR_Start_checkpoint must be called before "
                  "SCR_Complete_checkpoint @ %s:%d",
                  __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    ...
    scr_hash_elem *elem;
    for (elem =
             scr_filemap_first_file(scr_map, scr_dataset_id, scr_my_rank_world);
         elem != NULL; elem = scr_hash_elem_next(elem)) {
        /* get the filename */
        ...
        scr_meta *meta = scr_meta_new();
        scr_filemap_get_meta(scr_map, scr_dataset_id, scr_my_rank_world, file,
                             meta);
        scr_meta_set_filesize(meta, filesize);
        scr_meta_set_complete(meta, valid);
        scr_filemap_set_meta(scr_map, scr_dataset_id, scr_my_rank_world, file,
                             meta);
        scr_meta_delete(&meta);
    }

    /* we execute a sum as a logical allreduce to determine whether everyone is
     * valid we interpret the result to be true only if the sum adds up to the
     * number of processes */
    if (valid) {
        my_counts[2] = 1;
    }

    /* TODODSET: we may want to delay setting COMPLETE in the dataset until
     * after copy call? */

    /* store total number of files, total number of bytes, and complete flag in
     * dataset */
    unsigned long total_counts[3];
    MPI_Allreduce(my_counts, total_counts, 3, MPI_UNSIGNED_LONG, MPI_SUM,
                  scr_comm_world);
    scr_dataset *dataset = scr_dataset_new();
    scr_filemap_get_dataset(scr_map, scr_dataset_id, scr_my_rank_world,
                            dataset);
    ...
    /* TODO: PRESERVE preprocess info needed for flush/scavenge, e.g., container
     * offsets, list of directories to create, etc. we should also apply
     * redundancy to this info,
     * this could be done in flush, but it's hard to do in scavenge */
    char subdir[SCR_MAX_FILENAME];
    if (scr_flush_verify(scr_map, scr_dataset_id, subdir, sizeof(subdir)) !=
        SCR_SUCCESS) {
        scr_abort(-1, "Dataset cannot be flushed @ %s:%d", __FILE__, __LINE__);
    }
    ...
    if (scr_my_rank_world == 0) {
        ...
        if (cost < 0) {
            scr_err(
                "Checkpoint end time (%f) is less than start time (%f) @ %s:%d",
                scr_time_checkpoint_end, scr_time_checkpoint_start, __FILE__,
                __LINE__);
            cost = 0;
        }
        ...
        if (scr_log_enable) {
            ...
            double time_diff =
                scr_time_checkpoint_end - scr_time_checkpoint_start;
            time_t now = scr_log_seconds();
            scr_log_event("CHECKPOINT COMPLETED", reddesc->base,
                          &scr_checkpoint_id, &now, &time_diff);

            /* log the transfer details */
            char dir[SCR_MAX_FILENAME];
            scr_cache_dir_get(reddesc, scr_dataset_id, dir);
            scr_log_transfer(
                "CHECKPOINT", reddesc->base, dir, &scr_checkpoint_id,
    ...
    if (rc == SCR_SUCCESS) {
        /* check_flush may start an async flush, whereas check_halt will call
         * sync flush, so place check_flush after check_halt */
        scr_flush_file_location_set(scr_dataset_id,
                                    SCR_FLUSH_KEY_LOCATION_CACHE);
    ...
    /* if we have an async flush ongoing, take this chance to check whether it's
     * completed */
    if (scr_flush_async_in_progress) {
        double bytes = 0.0;
        if (scr_flush_async_test(scr_map, scr_flush_async_dataset_id, &bytes) ==
            SCR_SUCCESS) {
    ...
    /* unset the checkpoint flag to indicate we have exited the current
     * checkpoint */
    scr_in_output = 0;
