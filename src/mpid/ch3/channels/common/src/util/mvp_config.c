...
#ifndef MVP_SYSTEM_CONFIG
#define MVP_SYSTEM_CONFIG "/etc/mvapich.conf"
#endif
...
{
    char const *user_config = getenv("MVP_USER_CONFIG");
    char user_default[FILENAME_MAX];
    char const *ignore = getenv("MVP_IGNORE_USER_CONFIG");
    FILE *config_file;
    ...
    /*
     * Do not read user config if MVP_IGNORE_USER_CONFIG is set.
     */
    ...
    /*
     * Check if MVP_USER_CONFIG is set.  If not, use default user configuration
     * file.
     */
    if (user_config) {
        ...
        strcat(user_default, "/.mvapich.conf");
...
{
    int err = 0;
    char const *ignore = getenv("MVP_IGNORE_SYSTEM_CONFIG");
    FILE *config_file;
     ...
     * Initialize error information in case of error processing file
     */
    strcpy(config_error.filename, MVP_SYSTEM_CONFIG);
    config_error.lineno = 0;

    /*
     * Do not read system config if MVP_IGNORE_SYSTEM_CONFIG is set.
     */
    if (ignore ? atoi(ignore) : 0) {
        return 0;
    }
     ...
     * Open and process configuration file
     */
    if ((config_file = fopen(MVP_SYSTEM_CONFIG, "r"))) {
 ...
 *
 * Fills mvp_config structure with array of key/value pairs.  Each key only
