...
#define MVP_RDMA_CM_MIN_PORT_LIMIT 1024
#define MVP_RDMA_CM_MAX_PORT_LIMIT 65536

/*
=== BEGIN_MPI_T_MVP_CVAR_INFO_BLOCK ===

cvars:
    - name        : MVP_RDMA_CM_ARP_TIMEOUT
      category    : CH3
      type        : int
      default     : 2000
      class       : none
...
/* Obtain the information of local RNIC IP from the mvp.conf file */
                                ...
                                struct rdma_cm_event *event)
{
    ...
    switch (event->event) {
        case RDMA_CM_EVENT_ADDR_RESOLVED:
            ...
            do {
                ret = rdma_ops.resolve_route(cma_id,
                ...
                if (ret) {
                    ...
                    PRINT_DEBUG(
                        DEBUG_CM_verbose > 0,
                        "connect_attempts = %d, exp_factor=%d, ret = %d,"
                        "wait_time = %d ms\n",
        ...
        case RDMA_CM_EVENT_ROUTE_RESOLVED:
            ...
            PRINT_DEBUG(DEBUG_RDMACM_verbose,
                        "case RDMA_CM_EVENT_ROUTE_RESOLVED for %d\n", rank);
            ...
            if ((vc->ch.state != MPIDI_CH3I_VC_STATE_CONNECTING_CLI) &&
                ...
                PRINT_DEBUG(DEBUG_RDMACM_verbose,
                            "Switched into server mode for %d\n", rank);
                break;
            }
...
#endif
            {
                ...
                conn_param.responder_resources =
                    rdma_supported_max_rdma_dst_ops;
            }
            ...
            if (tmplen > RDMA_MAX_PRIVATE_LENGTH) {
                PRINT_ERROR("Length of private data too long. Requested: %d. "
                            "Supported: %d.",
                            tmplen, RDMA_MAX_PRIVATE_LENGTH);
                MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_OTHER, "**fail",
                                          "Cannot use RDMA CM on rank %d",
                                          pg_rank);
            }

            PRINT_DEBUG(DEBUG_RDMACM_verbose,
                        "allocating %d bytes for private_data\n", tmplen);
            ...
            do {
                ...
                offset = private_data_start_offset;
                PRINT_DEBUG(DEBUG_RDMACM_verbose,
                            "Sending connection request to [rank = %ld], "
                            " [rail = %ld] [vc = %lx]\n",
        ...
        case RDMA_CM_EVENT_CONNECT_REQUEST:
            ...
            PRINT_DEBUG(DEBUG_RDMACM_verbose,
                        "Passive side received connect request: [%d] :[%d]"
                        " [vc: %p], vc->pg_rank = %d\n",
            ...
            if (((vc->ch.state == MPIDI_CH3I_VC_STATE_CONNECTING_CLI) &&
                ...
                PRINT_DEBUG(DEBUG_RDMACM_verbose,
                            "Passive size rejecting connect request: "
            ...
            PRINT_DEBUG(DEBUG_CM_verbose > 0,
                        "Current state of %d is %s, moving to LOCAL_ACTIVE.\n",
            ...
            conn_param.private_data =
                MPL_malloc(conn_param.private_data_len, MPL_MEM_OTHER);
            ((uint64_t *)conn_param.private_data)[offset] = (uint64_t)vc;
        ...
        case RDMA_CM_EVENT_ESTABLISHED:
            ...
            rank = vc->pg_rank;
            PRINT_DEBUG(DEBUG_RDMACM_verbose,
                        "case RDMA_CM_EVENT_ESTABLISHED for rank %d\n", rank);
#ifdef _MULTI_SUBNET_SUPPORT_
            if (mvp_rdma_cm_multi_subnet_support) {
                /* Leave additional offset at the beginning of private_data if
                 * we are supporting routing */
...
#ifndef OFED_VERSION_1_1 /* OFED 1.2 */
            if (event->param.conn.private_data_len)
                vc->mrail.remote_vc_addr =
                    ((uint64_t *)event->param.conn
                         .private_data)[private_data_start_offset];
#else /* OFED 1.1 */
            if (event->private_data_len)
                vc->mrail.remote_vc_addr =
                    ((uint64_t *)
            ...
            if (rank < 0) { /* Overlapping connections */
                PRINT_DEBUG(DEBUG_RDMACM_verbose,
                            "Got event for overlapping connections? "
                            " removing...\n");
                break;
            }
            ...
            if (((vc->pg_rank == pg_rank) &&
                 (rdma_cm_connect_count[rank] == 2 * rdma_num_rails)) ||
                ((vc->pg_rank != pg_rank) &&
                 (rdma_cm_connect_count[rank] == rdma_num_rails))) {
                if (vc->ch.state == MPIDI_CH3I_VC_STATE_CONNECTING_CLI) {
                    ...
                    if (proc->use_iwarp_mode) {
                        ...
                        PRINT_DEBUG(DEBUG_RDMACM_verbose,
                                    "Connection Complete - Client: %d->%d\n",
                                    pg_rank, rank);
        ...
        default:
            PRINT_DEBUG(DEBUG_RDMACM_verbose,
                        "%s: Caught unhandled rdma cm event - %s\n",
...
{
    int mpi_errno = MPI_SUCCESS;
    int maxPort = MVP_RDMA_CM_MAX_PORT_LIMIT;
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_GET_BASE_LISTEN_PORT);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_GET_BASE_LISTEN_PORT);

    if (MVP_RDMA_CM_MAX_PORT != -1) {
        maxPort = MVP_RDMA_CM_MAX_PORT;

        if (maxPort > MVP_RDMA_CM_MAX_PORT_LIMIT ||
            maxPort < MVP_RDMA_CM_MIN_PORT_LIMIT) {
            MPIR_ERR_SETANDJUMP3(mpi_errno, MPI_ERR_OTHER, "**rdmacmmaxport",
                                 "**rdmacmmaxport %d %d %d", maxPort,
                                 MVP_RDMA_CM_MIN_PORT_LIMIT,
                                 MVP_RDMA_CM_MAX_PORT_LIMIT);
        }
    }

    int minPort = MVP_RDMA_CM_MIN_PORT_LIMIT;

    if (MVP_RDMA_CM_MIN_PORT != -1) {
        minPort = MVP_RDMA_CM_MIN_PORT;

        if (minPort > MVP_RDMA_CM_MAX_PORT_LIMIT ||
            minPort < MVP_RDMA_CM_MIN_PORT_LIMIT) {
            MPIR_ERR_SETANDJUMP3(mpi_errno, MPI_ERR_OTHER, "**rdmacmminport",
                                 "**rdmacmminport %d %d %d", minPort,
                                 MVP_RDMA_CM_MIN_PORT_LIMIT,
                                 MVP_RDMA_CM_MAX_PORT_LIMIT);
        }
    }
    ...
    if (maxPort - minPort < portRange) {
        MPIR_ERR_SETANDJUMP2(mpi_errno, MPI_ERR_OTHER, "**rdmacmportrange",
                             "**rdmacmportrange %d %d", maxPort - minPort,
                             portRange);
    }
    ...
    if (MVP_RDMA_CM_PORT != -1) {
        rdma_cm_default_port = MVP_RDMA_CM_PORT;
        ...
        if (rdma_cm_default_port == -1) {
            ...
            MPIR_ERR_SETANDJUMP1(mpi_errno, MPI_ERR_OTHER,
                                 "**rdmacminvalidport",
                                 "**rdmacminvalidport %d", MVP_RDMA_CM_PORT);
        }
    } else {
        srand(seed.tv_usec); /* Random seed for the port */
        rdma_cm_default_port = rand() % (maxPort - minPort + 1) + minPort;
...
static int bind_listen_port(int pg_rank, int pg_size)
{
    ...
    if (mvp_rdma_cm_multi_subnet_support) {
        ...
        if (!inet_ntop(AF_INET6, mvp_MPIDI_CH3I_RDMA_Process.gids[0][0].raw,
        ...
        rdma_base_listen_sid[pg_rank] =
            ((struct sockaddr_ib *)(&proc->cm_listen_id->route.addr.src_addr))
                ->sib_sid;
        rdma_ops.freeaddrinfo(rdma_addr);
    } else
#endif /*_MULTI_SUBNET_SUPPORT_*/
    {
        mpi_errno =
            get_base_listen_port(pg_rank, &rdma_base_listen_port[pg_rank]);
        ...
        while (ret) {
            if ((mpi_errno = get_base_listen_port(
                     pg_rank, &rdma_base_listen_port[pg_rank])) !=
                MPI_SUCCESS) {
                MPIR_ERR_POP(mpi_errno);
            }
            ...
            sin.sin_port = rdma_base_listen_port[pg_rank];
            ret =
                rdma_ops.bind_addr(proc->cm_listen_id, (struct sockaddr *)&sin);
            PRINT_DEBUG(DEBUG_RDMACM_verbose,
                        "[%d] Port bind failed - %d. retrying %d\n", pg_rank,
                        rdma_base_listen_port[pg_rank], count++);
            if (count > 1000) {
                MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_OTHER, "**fail",
                                          "rdma_bind_addr failed: %d\n", ret);
            }
        }
    }
                    ...
                    int pg_size)
{
...
#endif /*_MULTI_SUBNET_SUPPORT_*/
    {
        rdma_base_listen_port =
            (int *)MPL_malloc(pg_size * sizeof(int), MPL_MEM_OTHER);
    ...
    if (!rdma_cm_connect_count || !rdma_cm_accept_count ||
        !rdma_cm_iwarp_msg_count) {
        MPIR_ERR_SETFATALANDJUMP(mpi_errno, MPI_ERR_OTHER, "**nomem");
    }
    for (i = 0; i < pg_size; i++) {
    ...
    rdma_cm_arp_timeout = MVP_RDMA_CM_ARP_TIMEOUT;
    if (rdma_cm_arp_timeout < 0) {
        MPIR_ERR_SETFATALANDJUMP1(
            mpi_errno, MPI_ERR_OTHER, "**fail", "**fail %s",
            "Invalid rdma cm arp timeout value specified\n");
    }
        ...
        for (i = 0; i < rdma_num_hcas; i++) {
            for (j = 1; j <= rdma_num_ports; j++) {
                if (ibv_ops.query_gid(
                        ...
                        &mvp_MPIDI_CH3I_RDMA_Process.gids[i][j - 1])) {
                    MPIR_ERR_SETFATALANDJUMP1(
                        mpi_errno, MPI_ERR_OTHER, "**fail",
...
#endif /*_MULTI_SUBNET_SUPPORT_*/
    {
        ...
        if (num_interfaces < rdma_num_hcas * rdma_num_ports) {
            MPIR_ERR_SETANDJUMP1(mpi_errno, MPI_ERR_OTHER, "**fail",
                                 ...
                                 "specified in /etc/mvp.conf\n");
        }
    }

    /* Create the listen cm_id */
    ret = rdma_ops.create_id(proc->cm_channel, &proc->cm_listen_id, proc,
                             RDMA_PS_TCP);
    if (ret) {
        MPIR_ERR_SETANDJUMP1(mpi_errno, MPI_ERR_OTHER, "**fail",
                             "Could not create listen cm_id, "
...
int rdma_cm_connect_all(int pg_rank, MPIDI_PG_t *pg)
{
    ...
    if (!proc->use_rdma_cm_on_demand) {
            ...
            if (qp_required(vc, pg_rank, i)) {
                ...
                for (j = 0; j < rdma_num_hcas * rdma_num_ports; j++) {
                    for (k = 0; k < rdma_num_qp_per_port; k++) {
                        ...
                        mpi_errno = rdma_cm_connect_to_server(
                            vc, (i * max_num_ips + j), rail_index);
                        if (mpi_errno)
                            MPIR_ERR_POP(mpi_errno);
                    }
                }
        ...
        PRINT_DEBUG(DEBUG_RDMACM_verbose,
                    "RDMA CM based connection setup complete\n");
    }
    ...
    if (mvp_rdma_cm_multi_subnet_support) {
        ...
        for (i = 0; i < rdma_num_hcas; ++i) {
            for (j = 0; j < rdma_num_ports; ++j) {
                ...
                ret = rdma_ops.create_id(proc->cm_channel, &tmpcmid, proc,
                                         RDMA_PS_TCP);
                if (ret) {
                    MPIR_ERR_SETANDJUMP1(mpi_errno, MPI_ERR_OTHER, "**fail",
                                         "rdma_create_id error %d\n", ret);
                ...
                if (rdma_ops.getaddrinfo(rdma_cm_ipv6_addr, NULL, &hints,
                                         &rdma_addr)) {
                    MPIR_ERR_SETANDJUMP1(mpi_errno, MPI_ERR_OTHER, "**fail",
                                         "**fail %s",
                                         "Could not get transport independent "
                                         "address translation\n");
                }
                ((struct sockaddr_ib *)(rdma_addr->ai_dst_addr))->sib_sid =
                    ((struct sockaddr_ib *)(&tmpcmid->route.addr.dst_addr))
                        ->sib_sid;

                ret = rdma_ops.resolve_addr(
                    tmpcmid, NULL, rdma_addr->ai_dst_addr, rdma_cm_arp_timeout);
                rdma_ops.freeaddrinfo(rdma_addr);
                if (ret) {
                    MPIR_ERR_SETANDJUMP1(mpi_errno, MPI_ERR_OTHER, "**fail",
...
int rdma_cm_create_qp(MPIDI_VC_t *vc, int rail_index)
{
...

    {
        ...
        if (rdma_iwarp_use_multiple_cq &&
            MVP_IS_CHELSIO_IWARP_CARD(proc->hca_type) &&
    ...
    if (ret) {
        ibv_va_error_abort(
            ...
            proc->send_cq_hndl[hca_index], proc->recv_cq_hndl[hca_index]);
    }
    ...
    if (rdma_iwarp_use_multiple_cq &&
        ...
        vc->mrail.rails[rail_index].send_cq_hndl =
            proc->send_cq_hndl[hca_index];
        vc->mrail.rails[rail_index].recv_cq_hndl =
...
int rdma_cm_exchange_hostid(MPIDI_PG_t *pg, int pg_rank, int pg_size)
{
    ...
    memset(mvp_pmi_key, 0, mvp_pmi_max_keylen);
    MPL_snprintf(mvp_pmi_key, mvp_pmi_max_keylen, "HOST-%d", pg_rank);

    hostid_all[pg_rank] = gethostid();
    sprintf(mvp_pmi_val, "%d", hostid_all[pg_rank]);
    ...
    for (i = 0; i < pg_size; i++) {
        if (i != pg_rank) {
            MPL_snprintf(mvp_pmi_key, mvp_pmi_max_keylen, "HOST-%d", i);
            ...
            sscanf(mvp_pmi_val, "%d", &hostid_all[i]);
        }
    }
...
int rdma_cm_get_hostnames(int pg_rank, MPIDI_PG_t *pg)
{
...
#ifdef _MULTI_SUBNET_SUPPORT_
    if (mvp_rdma_cm_multi_subnet_support) {
        rdma_cm_host_gid_list = (union ibv_gid *)MPL_malloc(
            pg_size * max_num_ips * sizeof(union ibv_gid), MPL_MEM_OTHER);
    ...
    MPL_strncpy(mvp_pmi_key, rank, 16);
    MPL_strncpy(mvp_pmi_val, buffer, length);
    mpi_errno = UPMI_KVS_PUT(pg->ch.kvs_name, mvp_pmi_key, mvp_pmi_val);
    if (mpi_errno != UPMI_SUCCESS) {
        MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_OTHER, "**pmi_kvs_put",
                                  "**pmi_kvs_put %d", mpi_errno);
    ...
    for (i = 0; i < pg_size; i++) {
        if (i != pg_rank) {
            ...
            MPL_strncpy(buffer, mvp_pmi_val, length);
            ...
            for (j = 0; j < max_num_ips; j++) {
                ...
                if (mvp_rdma_cm_multi_subnet_support) {
                    sscanf(temp, "%016" SCNx64 ":%016" SCNx64,
                           &rdma_cm_host_gid_list[i * max_num_ips + j]
                                .global.subnet_prefix,
                           &rdma_cm_host_gid_list[i * max_num_ips + j]
                                .global.interface_id);
                } else
...
int rdma_cm_get_verbs_ip(int *num_interfaces)
{
    int index = 0;
    int mpi_errno = MPI_SUCCESS;
    char *value = getenv("MVP_IBA_HCA");

    *num_interfaces = num_ip_enabled_devices;
    rdma_cm_local_ips = MPL_malloc(
        (rdma_num_hcas * rdma_num_ports) * sizeof(int), MPL_MEM_OTHER);
    for (index = 0; index < (*num_interfaces, MPL_MEM_OTHER); index++) {
        if (value != NULL && ip_address_enabled_devices != NULL &&
            strstr(value, ip_address_enabled_devices[index].device_name) ==
                NULL) {
            continue;
        }
        ...
        int success =
            inet_pton(AF_INET, ip_address_enabled_devices[index].ip_address,
                      &rdma_cm_local_ips[index]);
        MPIR_ERR_CHKANDJUMP(success != 1, mpi_errno, MPI_ERR_OTHER,
                            "**inet_pton");
    }
...
int rdma_cm_get_local_ip(int *num_interfaces)
{
    ...
    value = getenv("MVP_RDMA_CM_CONF_FILE_PATH");

    if (value == NULL) {
        sprintf(fname, "/etc/mvp.conf");
    } else {
        strncpy(fname, value, strlen(value));
        sprintf(fname + strlen(value), "/mvp.conf");
    }
    ...
    if (NULL == fp_port) {
        ...
        while ((fscanf(fp_port, "%s\n", ip)) != EOF &&
               (i < (rdma_num_hcas * rdma_num_ports))) {
...
int rdma_cm_connect_to_server(MPIDI_VC_t *vc, int offset, int rail_index)
{
    ...
    mpi_errno = rdma_ops.create_id(proc->cm_channel,
                                   &(vc->mrail.rails[rail_index].cm_ids), vc,
                                   RDMA_PS_TCP);
    if (mpi_errno) {
        MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_OTHER, "**fail",
                                  "rdma_create_id error %d\n", mpi_errno);
...
#ifdef _MULTI_SUBNET_SUPPORT_
    if (mvp_rdma_cm_multi_subnet_support) {
        struct rdma_addrinfo *src_rdma_addr = NULL, *dst_rdma_addr = NULL,
                             hints;
        char rdma_cm_ipv6_src_addr[128], rdma_cm_ipv6_dst_addr[128];

        /* Address info for source */
        if (!inet_ntop(AF_INET6, proc->gids[0][0].raw, rdma_cm_ipv6_src_addr,
                       sizeof(rdma_cm_ipv6_src_addr))) {
            MPIR_ERR_SETFATALANDJUMP1(
                mpi_errno, MPI_ERR_OTHER, "**fail", "**fail %s",
                "Could not convert local GID to IPv6 address\n");
        }
        ...
        if (rdma_ops.getaddrinfo(rdma_cm_ipv6_src_addr, NULL, &hints,
                                 &src_rdma_addr)) {
            MPIR_ERR_SETFATALANDJUMP1(
                mpi_errno, MPI_ERR_OTHER, "**fail", "**fail %s",
                "Could not get transport independent address translation\n");
        ...
        if (rdma_ops.getaddrinfo(rdma_cm_ipv6_dst_addr, NULL, &hints,
                                 &dst_rdma_addr)) {
            MPIR_ERR_SETFATALANDJUMP1(
                mpi_errno, MPI_ERR_OTHER, "**fail", "**fail %s",
                "Could not get transport independent address translation\n");
        ...
        mpi_errno = rdma_ops.resolve_addr(
            vc->mrail.rails[rail_index].cm_ids, dst_rdma_addr->ai_src_addr,
            dst_rdma_addr->ai_dst_addr, rdma_cm_arp_timeout);
        rdma_ops.freeaddrinfo(src_rdma_addr);
        rdma_ops.freeaddrinfo(dst_rdma_addr);
        PRINT_DEBUG(DEBUG_RDMACM_verbose,
                    "Active connect initiated for %d"
                    " [ip: %s:%016lx] [rail %d]\n",
...
#endif /*_MULTI_SUBNET_SUPPORT_*/
    {
        ...
        mpi_errno =
            rdma_ops.resolve_addr(vc->mrail.rails[rail_index].cm_ids, NULL,
    ...
    for (; i < rdma_num_hcas; ++i) {
        ...
        if (rdma_use_blocking) {
            ...
            if (rdma_iwarp_use_multiple_cq &&
                ...
                if (!proc->send_cq_hndl[i]) {
                    ...
                    if (rdma_default_max_cq_size > RDMA_DEFAULT_IWARP_CQ_SIZE) {
                        ...
                        proc->send_cq_hndl[i] = ibv_ops.create_cq(
                            proc->nic_context[i], rdma_default_max_cq_size,
                            NULL, NULL, 0);
                        if (!proc->send_cq_hndl[i]) {
                            MPIR_ERR_SETFATALANDJUMP1(mpi_errno, MPI_ERR_OTHER,
                                                      "**fail", "**fail %s",
                ...
                if (!proc->cq_hndl[i]) {
                    /*Falling back to smaller cq size if creation failed*/
                    if ((rdma_default_max_cq_size >
                         RDMA_DEFAULT_IWARP_CQ_SIZE) &&
                        MVP_IS_CHELSIO_IWARP_CARD(proc->hca_type)) {
                        rdma_default_max_cq_size = RDMA_DEFAULT_IWARP_CQ_SIZE;
                        proc->send_cq_hndl[i] = ibv_ops.create_cq(
                            proc->nic_context[i], rdma_default_max_cq_size,
        ...
        PRINT_DEBUG(DEBUG_RDMACM_verbose,
                    "[%d][rail %d] proc->ptag %p, "
                    "proc->cq_hndl %p, proc->srq_hndl %p\n",
                    pg_rank, i, proc->ptag[i], proc->cq_hndl[i],
                    proc->srq_hndl[i]);
    }
...
int get_remote_rank(struct rdma_cm_id *cmid) { return -1; }
...
void ib_finalize_rdma_cm(int pg_rank, MPIDI_PG_t *pg)
{
...

    {
        ...
        for (i = 0; i < rdma_num_hcas; i++) {
            ...
            if (mvp_MPIDI_CH3I_RDMA_Process.has_srq) {
                ...
                pthread_cond_signal(
                    &mvp_MPIDI_CH3I_RDMA_Process.srq_post_cond[i]);
                ...
                if (mvp_MPIDI_CH3I_RDMA_Process.async_thread[i]) {
                    pthread_cancel(mvp_MPIDI_CH3I_RDMA_Process.async_thread[i]);
                    pthread_join(mvp_MPIDI_CH3I_RDMA_Process.async_thread[i],
                                 NULL);
                    mvp_MPIDI_CH3I_RDMA_Process.async_thread[i] = 0;
                }
                if (mvp_MPIDI_CH3I_RDMA_Process.srq_hndl[i]) {
                    ibv_ops.destroy_srq(
                        mvp_MPIDI_CH3I_RDMA_Process.srq_hndl[i]);
                    mvp_MPIDI_CH3I_RDMA_Process.srq_hndl[i] = NULL;
                }
            }
            if (rdma_use_blocking) {
                ibv_ops.destroy_comp_channel(
                    mvp_MPIDI_CH3I_RDMA_Process.comp_channel[i]);
                mvp_MPIDI_CH3I_RDMA_Process.comp_channel[i] = NULL;
            }
            deallocate_vbufs(i);
        }
    ...
    if (proc->cm_listen_id) {
        ...
        rdma_cm_finalized = 1;
        rdma_ops.destroy_event_channel(mvp_MPIDI_CH3I_RDMA_Process.cm_channel);
