...
static mvp_arch_type g_mvp_arch_type = MVP_ARCH_UNKWN;
...
mvp_arch_type mvp_get_arch_type()
{
    ...
    if ((value = getenv("MVP_FORCE_ARCH_TYPE")) != NULL) {
        mvp_arch_type val = atoi(value);
        PRINT_DEBUG(DEBUG_INIT_verbose, "Attempting to force ARCH %s\n",
                    mvp_get_arch_name(val));
        int retval = mvp_check_proc_arch(val, my_rank);
        if (retval) {
            ...
            if (g_mvp_arch_type >= MVP_ARCH_INTEL_START &&
                g_mvp_arch_type <= MVP_ARCH_INTEL_END) {
                g_mvp_cpu_family_type = MVP_CPU_FAMILY_INTEL;
            } else if (g_mvp_arch_type >= MVP_ARCH_AMD_START &&
                       g_mvp_arch_type <= MVP_ARCH_AMD_END) {
                g_mvp_cpu_family_type = MVP_CPU_FAMILY_AMD;
            } else if (g_mvp_arch_type >= MVP_ARCH_IBM_START &&
                       g_mvp_arch_type <= MVP_ARCH_IBM_END) {
    ...
    if (MVP_ARCH_UNKWN == g_mvp_arch_type) {
        FILE *fp;
...
int mvp_get_num_cpus()
{
    ...
    if (-1 == g_mvp_num_cpus) {
        g_mvp_arch_type = mvp_get_arch_type();
    }
