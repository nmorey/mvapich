...
int main(int argc, char *argv[])
{
    ...
    if (!myid)
        fprintf(
            stderr,
            "Reinitializing SCR to verify integrity of cached checkpoints.\n");
    SCR_Init();
    if (!myid)
        fprintf(stderr, "Checkpoints are healthy. Restart the application "
                        "using the \"cr_restart\" utility.\n");
