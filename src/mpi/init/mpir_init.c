...
#define MPIR_PPN_THRESHOLD_FOR_NETWORK_THROTTLE 16
                     ...
                     MPIR_Session ** p_session_ptr)
{
    ...
    /**********************************************************************/
    /* MVAPICH PVAR interface
    /**********************************************************************/
    MPIR_REGISTER_MVP_VARIABLES();
    ...
    if (need_init_builtin_comms) {
        ...
        /* Throttle logic for Alltoall */
        int ppn = MPIR_Process.comm_world->node_comm ?
                  MPIR_Process.comm_world->node_comm->local_size :
                            MPIR_Process.local_size;
        if (ppn <= MPIR_PPN_THRESHOLD_FOR_NETWORK_THROTTLE) {
            MPIR_CVAR_SOFT_SET(MPIR_CVAR_ALLTOALL_THROTTLE, 0)
        }
...
int MPII_Finalize(MPIR_Session * session_ptr)
{
    ...
    /* free local memory pool */
    MPIR_MVP_FREE_LMEMPOOL();
