...
#if defined(CHANNEL_MRAIL) || defined(CHANNEL_NEMESIS_IB)
/* TODO: this is CH3 specific right now. Should be either made channel
 * independent or moved to CH3. Should also be removed from MPICH file */
int MPIR_Comm_split_intra_bitonic(MPIR_Comm *comm_ptr, int color, int key, MPIR_Comm **newcomm_ptr)
{
    int i;
    int mpi_errno = MPI_SUCCESS;
    MPIR_CHKLMEM_DECL(1);
