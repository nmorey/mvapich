...
int free_limic_comm (MPIR_Comm* shmem_comm_ptr)
{
    ...
    MPIR_Comm_get_ptr(shmem_comm_ptr->dev.ch.intra_sock_comm, intra_sock_comm_ptr);
    MPIR_Comm_get_ptr(shmem_comm_ptr->dev.ch.intra_sock_leader_comm, intra_sock_leader_comm_ptr);
...
int free_intra_sock_comm (MPIR_Comm* comm_ptr)
{
    ...
    MPIR_Comm_get_ptr(comm_ptr->dev.ch.global_sock_leader_comm, global_sock_leader_comm_ptr);
...
int free_domain_comm_shmem(MPIR_Comm *comm_ptr) {
    ...
    if (comm_ptr->rank == 0 && comm_ptr->dev.ch.shmem_comm_rank >= 0) {
        lock_shmem_region();
        MPIR_MV2_SHMEM_Coll_Block_Clear_Status(comm_ptr->dev.ch.shmem_comm_rank);
        unlock_shmem_region();
    }
    comm_ptr->dev.ch.shmem_coll_ok = 0;
...
int free_2level_comm (MPIR_Comm* comm_ptr)
{
    ...
    if(local_rank == 0 && shmem_comm_ptr->dev.ch.shmem_comm_rank >= 0) {
        lock_shmem_region();
        MPIR_MV2_SHMEM_Coll_Block_Clear_Status(shmem_comm_ptr->dev.ch.shmem_comm_rank);
        unlock_shmem_region();
    }
...
int create_intra_node_multi_level_comm(MPIR_Comm* comm_ptr) {
    ...
    if(g_use_limic2_coll) {
        ...
        if (mv2_limic_comm_count <= mv2_max_limic_comms){
            ...
            int *intra_socket_map = MPL_malloc(sizeof(int)*my_local_size, MPL_MEM_COLL);
        ...
        if(ok_to_create_intra_sock_comm != 0) {
            ...
            intra_socket_leader_map = MPL_malloc(sizeof(int)*my_local_size, MPL_MEM_COLL);
            ...
            intra_sock_leader_group = MPL_malloc(sizeof(int) *
                    intra_socket_leader_cnt, MPL_MEM_COLL);
            ...
            if(intra_comm_rank == 0) {
                ...
                shmem_ptr->dev.ch.socket_size = MPL_malloc(sizeof(int)*
                        intra_leader_comm_size, MPL_MEM_COLL);
...
static inline int create_intra_node_topo_comm_at_level(
                                     MPIR_Comm *parent_comm_ptr, int in_color,
                                     MPI_Comm *comm, MPI_Comm *ldr_comm)
{
    int i = 0;
    int leader = 0;
    int grp_index = 0;
    ...
    mpi_errno = MPIR_Allgather_impl((void *)&in_color, 1, MPI_INT,
                                    (void *)color_array, 1, MPI_INT,
                                    parent_comm_ptr, &errflag);
    ...
    *comm = comm_ptr->handle;
    ...
    if (comm_rank == 0 && comm_size > 1) {
        lock_shmem_region();
        mv2_shmem_coll_blk_stat = MPIR_MV2_SHMEM_Coll_get_free_block();
    ...
    if (!output_flag) {
        ...
        if ((comm_rank == 0) && (mv2_shmem_coll_blk_stat >= 0)) {
            /* Relese the slot if it is acquired */
            lock_shmem_region();
            MPIR_MV2_SHMEM_Coll_Block_Clear_Status(mv2_shmem_coll_blk_stat);
            unlock_shmem_region();
        }
        comm_ptr->dev.ch.shmem_coll_ok = 0;
        ...
        mpi_errno = MPIR_Bcast_impl(&mv2_shmem_coll_blk_stat, 1,
                                    MPI_INT, 0, comm_ptr, &errflag);
        MPIR_ERR_CHECK(mpi_errno);

        comm_ptr->dev.ch.shmem_comm_rank = mv2_shmem_coll_blk_stat;
        PRINT_DEBUG(DEBUG_SHM_verbose,"parent rank: %d, in_color: %d, "
                    "local rank: %d, mv2_shmem_coll_blk_stat = %d\n",
                    parent_comm_ptr->rank, in_color, comm_ptr->rank,
                    mv2_shmem_coll_blk_stat);
        if (mv2_use_slot_shmem_coll && comm_ptr->local_size > 1) {
            comm_ptr->dev.ch.shmem_info =
                    mv2_shm_coll_init(mv2_shmem_coll_blk_stat, comm_ptr->rank,
    ...
    /* This is color of current process */
    mpi_errno = MPIR_Allgather_impl((void *)&leader, 1, MPI_INT,
                                    (void *)ldr_map, 1, MPI_INT,
                                    parent_comm_ptr, &errflag);
    MPIR_ERR_CHECK(mpi_errno);
    ...
    mpi_errno = MPIR_Group_incl_impl(comm_group, grp_index, ldr_group,
                                     &subgroup1);
    MPIR_ERR_CHECK(mpi_errno);

    mpi_errno = MPIR_Comm_create_intra(parent_comm_ptr, subgroup1,
                                       &ldr_comm_ptr);
    MPIR_ERR_CHECK(mpi_errno);
    ...
    if (ldr_comm_ptr != NULL) {
        ...
        *ldr_comm = ldr_comm_ptr->handle;
        ...
        PRINT_DEBUG(DEBUG_SHM_verbose, "Created leader_comm. rank: %d, "
...
static inline int create_intra_node_multi_level_topo_comm(MPIR_Comm *shmem_ptr)
{
    ...
    shmem_ptr->dev.ch.topo_comm =
            MPL_malloc(sizeof(MPI_Comm) * mv2_num_intra_node_comm_levels + 1,
                       MPL_MEM_COLL);
    shmem_ptr->dev.ch.topo_leader_comm =
            MPL_malloc(sizeof(MPI_Comm) * mv2_num_intra_node_comm_levels + 1,
                       MPL_MEM_COLL);
    ...
    for (i = 0; i <= mv2_num_intra_node_comm_levels; ++i) {
        ...
        color = mv2_intra_node_cluster_at_level[i];
        PRINT_DEBUG(DEBUG_SHM_verbose,"Calling create_intra_node_topo_comm_at_level for level %d\n", i);
        /* Create intra-node topo-aware communicators */
        mpi_errno = create_intra_node_topo_comm_at_level(
                                parent_comm_ptr, color,
                                &(shmem_ptr->dev.ch.topo_comm[i]),
        ...
        MPIR_Comm_get_ptr(shmem_ptr->dev.ch.topo_leader_comm[i], parent_comm_ptr);
    }
...
int create_intra_sock_comm(MPI_Comm comm)
{
    ...
    if (mv2_enable_socket_aware_collectives) {
        MPI_Group comm_group1;
        int is_uniform;
        int err = get_numa_bound_info(&socket_bound, &numSocketsNode, &numCoresSocket, &is_uniform);
        int output_err = 0;
        ...
        if (ok_to_create_intra_sock_comm) {
            ...
            global_socket_leader_map = MPL_malloc(sizeof(int)*global_size, MPL_MEM_COLL);
            ...
            global_sock_leader_group = MPL_malloc(sizeof(int) *
                    global_socket_leader_cnt, MPL_MEM_COLL);
...
int create_allgather_comm(MPIR_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (is_block) {
        int counter=0,j;
        comm_ptr->dev.ch.allgather_new_ranks = MPL_malloc(sizeof(int)*size, MPL_MEM_COLL);
        ...
        mpi_errno = MPIR_Group_incl_impl(comm_group, size,
                                         ...
                                         &allgather_group);
        MPIR_ERR_CHECK(mpi_errno);
        mpi_errno = MPIR_Comm_create_intra(comm_ptr, allgather_group,
                                           &allgather_comm);
        MPIR_ERR_CHECK(mpi_errno);
        if (allgather_comm) {
            ...
            mpi_errno = MPIR_Err_create_code(MPI_SUCCESS, MPI_ERR_OTHER,
                   __func__, __LINE__, MPI_ERR_OTHER, "**fail", "%s: %s",
                   "Failed to create allgather comm", strerror(errno));
            MPIR_ERR_POP(mpi_errno);
        }
    ...
    if (my_local_id == 0) {
        ...
        if (comm_ptr->dev.ch.is_uniform != 1) {
            ...
            int* displs = MPL_malloc(sizeof(int) * leader_comm_size, MPL_MEM_COLL);
            int* counts = MPL_malloc(sizeof(int) * leader_comm_size, MPL_MEM_COLL);
            if (!displs || !counts) {
            ...
            mpi_errno = MPIR_Allgatherv_impl(shmem_group, my_local_size,
                                             MPI_INT,
                                             comm_ptr->dev.ch.rank_list,
            ...
            mpi_errno = MPIR_Allgather_impl(shmem_group, my_local_size, MPI_INT,
                                            comm_ptr->dev.ch.rank_list,
                                            my_local_size, MPI_INT, leader_ptr,
                                            errflag);
        }
        MPIR_ERR_CHECK(mpi_errno);
    }
...
int create_sharp_comm(MPI_Comm comm, int size, int my_rank)
{
    ...
    if (comm == MPI_COMM_WORLD && mv2_enable_sharp_coll &&
        ...
        comm_ptr->dev.ch.sharp_coll_info =
            (sharp_info_t *)MPL_malloc(sizeof(sharp_info_t), MPL_MEM_COLL);
        sharp_coll_info = comm_ptr->dev.ch.sharp_coll_info;
        sharp_coll_info->sharp_comm_module = NULL;
        sharp_coll_info->sharp_conf = MPL_malloc(sizeof(sharp_conf_t));
        ...
        sharp_coll_info->sharp_comm_module = MPL_malloc(sizeof(coll_sharp_module_t));
        MPIR_Memset(sharp_coll_info->sharp_comm_module, 0, sizeof(coll_sharp_module_t), MPL_MEM_COLL);
...
int create_2level_comm(MPI_Comm comm, int size, int my_rank)
{
    ...
    */
    ...
    comm_ptr->dev.ch.coll_tmp_buf = MPL_malloc(mv2_coll_tmp_buf_size, MPL_MEM_COLL);
    ...
    mpi_errno = MPIR_Group_incl_impl(commgroup_ptr, shmem_size,
                                     shmem_group, &subgroup_ptr);
    MPIR_ERR_CHECK(mpi_errno);
    ...
    if(my_local_id == 0) {
        ...
        comm_ptr->dev.ch.node_sizes = MPL_malloc(sizeof(int) * leader_comm_size,
                                                 MPL_MEM_COLL);
        ...
        comm_ptr->dev.ch.node_disps = MPL_malloc(sizeof(int) * leader_comm_size,
                                                 MPL_MEM_COLL);
    ...
    /* To prevent Bcast taking the knomial_2level_bcast route */
    mpi_errno = MPIR_Bcast_impl(&mv2_shmem_coll_blk_stat, 1, MPI_INT, 0,
                                shmem_ptr, &errflag);
    MPIR_ERR_CHECK(mpi_errno);

    MPIR_Assert(mv2_shmem_coll_blk_stat >= 0);
    ...
    if (mv2_enable_topo_aware_collectives) {
        if (comm_ptr->dev.ch.shmem_coll_ok == 1 && mv2_use_slot_shmem_coll) {
            mpi_errno = create_intra_node_multi_level_topo_comm(shmem_ptr);
            if (mpi_errno == MPI_SUCCESS) {
                local_topo_comms_ok = 1;
            }
            mpi_errno = MPIR_Allreduce_impl(&local_topo_comms_ok, &global_topo_comms_ok, 1,
                    MPI_INT, MPI_LAND, comm_ptr, &errflag);
            if (mpi_errno) {
                MPIR_ERR_POP(mpi_errno);
            }
