...
extern MPIR_T_pvar_timer_t PVAR_TIMER_mv2_coll_timer_alltoall_cuda;

extern unsigned long long PVAR_COUNTER_mv2_coll_alltoall_cuda_intra_bytes_send;
extern unsigned long long PVAR_COUNTER_mv2_coll_alltoall_cuda_intra_bytes_recv;
extern unsigned long long PVAR_COUNTER_mv2_coll_alltoall_cuda_intra_count_send;
extern unsigned long long PVAR_COUNTER_mv2_coll_alltoall_cuda_intra_count_recv;
extern unsigned long long PVAR_COUNTER_mv2_coll_alltoall_cuda_bytes_send;
extern unsigned long long PVAR_COUNTER_mv2_coll_alltoall_cuda_bytes_recv;
extern unsigned long long PVAR_COUNTER_mv2_coll_alltoall_cuda_count_send;
extern unsigned long long PVAR_COUNTER_mv2_coll_alltoall_cuda_count_recv;
...
int MPIR_Alltoall_CUDA_cleanup () 
{
    ...
    if (send_events) {
        for(i = 0; i < send_events_count; i++) {
            MV2_MPID_Device_EventDestroy(send_events[i]);
        }
        MPL_free(send_events); 
    }
    if (recv_event) {
        MV2_MPID_Device_EventDestroy(*recv_event);
        MPL_free(recv_event); 
    }
    ...
    MPIR_Errflag_t *errflag )
{
    ...
    send_buf = (MPIDI_CH3U_COLL_SRBuf_element_t **) 
                    MPL_malloc(sizeof(MPIDI_CH3U_COLL_SRBuf_element_t *)*num_sbufs);
    ...
    recv_buf = (MPIDI_CH3U_COLL_SRBuf_element_t **) 
                    MPL_malloc(sizeof(MPIDI_CH3U_COLL_SRBuf_element_t *)*num_rbufs);
    ...
    if (send_events_count < num_sbufs) {
        ...
        send_events = (deviceEvent_t *) MPL_malloc(sizeof(deviceEvent_t) * num_sbufs);
        for (i=0; i<num_sbufs; i++) { 
    ...
    for (i = 0; i < comm_size; i+=sblock) {  
        ...
        if (begin <= end) {
            MV2_MPID_Memcpy_Device_Async(send_buf[sbufs_filled]->buf,
                    ((char *) sendbuf
            ...
            MV2_MPID_Memcpy_Device_Async(((char *)send_buf[sbufs_filled]->buf
                        + bytes_copied),
                    sendbuf,
        ...
        MV2_MPID_Device_EventRecord(send_events[sbufs_filled], stream_d2h);
        sbufs_filled++;
    }
    ...
    while (rbufs_filled < num_rbufs || bufs_send_initiated < num_sbufs) { 
        ...
        if (rbufs_filled < num_rbufs) { 
            ...
            if (flag && 
                ...
                if (begin <= end) {
                    MV2_MPID_Memcpy_Device_Async(((char *) recvbuf
                    ...
                    MV2_MPID_Memcpy_Device_Async(recvbuf,
    ...
    MV2_MPID_Device_EventRecord(*recv_event, stream_h2d);
    MV2_MPID_Device_EventSynchronize(*recv_event);
