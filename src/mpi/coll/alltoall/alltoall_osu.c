...
cvars:
   ...
   - name        : MV2_ALLTOALL_THROTTLE_FACTOR
     category    : COLLECTIVE
     type        : int
     default     : -1
     class       : none
...
int (*MV2_Alltoall_function) (const void *sendbuf, 
                              int sendcount, 
...
int MPIR_Alltoall_RD_MV2(const void *sendbuf,
                         int sendcount,
                         MPI_Datatype sendtype,
                         void *recvbuf,
                         int recvcount,
...
int MPIR_Alltoall_index_tuned_intra_MV2(const void *sendbuf,
                                        int sendcount,
                                        MPI_Datatype sendtype,
                                        void *recvbuf,
                                        int recvcount,
                                        ...
                                        MPIR_Errflag_t *errflag )
{
    ...
    if (comm_ptr->dev.ch.shmem_coll_ok == 1 && comm_ptr->dev.ch.is_uniform) {
        ...
        if (likely(mv2_enable_skip_tuning_table_search 
            ...
            if (comm_size * nbytes < mv2_alltoall_rd_max_msg_size 
