...
int MV2_set_allgatherv_tuning_table(int heterogeneity, struct coll_info *colls_arch_hca)
{
    ...
    if (MV2_IS_ARCH_HCA_TYPE(MV2_get_arch_hca_type(),
        ...
        mv2_allgatherv_thresholds_table = MPL_malloc(mv2_size_allgatherv_tuning_table *
                                                  sizeof (mv2_allgatherv_tuning_table), MPL_MEM_COLL);
        ...
        MPIR_Memcpy(mv2_allgatherv_thresholds_table, mv2_tmp_allgatherv_thresholds_table,
...
void MV2_cleanup_allgatherv_tuning_table()
{
    if (mv2_allgatherv_thresholds_table != NULL) {
        MPL_free(mv2_allgatherv_thresholds_table);
    }
...
int MV2_internode_Allgatherv_is_define(char *mv2_user_allgatherv_inter)
{
    ...
    if (nb_element == 1) {
        ...
        if (!(dup = MPL_strdup(mv2_user_allgatherv_inter))) {
    ...
    MPIR_Memcpy(mv2_allgatherv_thresholds_table, mv2_tmp_allgatherv_thresholds_table, sizeof
