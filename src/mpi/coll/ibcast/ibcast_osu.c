...
extern unsigned long long PVAR_COUTNER_mv2_coll_ibcast_sharp;
extern MPIR_T_pvar_timer_t PVAR_COUTNER_mv2_coll_timer_ibcast_sharp;
                                       ...
                                       int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (MV2_Ibcast_function == MPIR_Ibcast_intra_sched_scatter_recursive_doubling_allgather) {
        if (MPL_is_pof2(comm_size, NULL)) {
            ...
            mpi_errno = MPIR_Ibcast_intra_sched_scatter_ring_allgather(buffer,
                                           count, datatype, root, comm_ptr, s);
            MPIR_ERR_CHECK(mpi_errno);
        }
    }
    else {
...
int MPIR_Ibcast_intra_MV2(void *buffer, int count, MPI_Datatype datatype, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;
    int comm_size, is_homogeneous ATTRIBUTE((unused));
        ...
        if ((nbytes < MPIR_CVAR_BCAST_SHORT_MSG_SIZE) ||
        ...
        else
            {
                if ((nbytes < MPIR_CVAR_BCAST_LONG_MSG_SIZE) && (MPL_is_pof2(comm_size, NULL))) {
                    mpi_errno = MPIR_Ibcast_intra_sched_scatter_recursive_doubling_allgather(buffer, count, datatype, root, comm_ptr, s);
                    MPIR_ERR_CHECK(mpi_errno);
                }
                else {
...
int MPIR_Ibcast_MV2(void *buffer, int count, MPI_Datatype datatype, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;
