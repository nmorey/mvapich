                              ...
                              MPIR_Errflag_t *errflag)
{
    ...
    if(tmp_buf_size > 0) {
          MPIR_CHKLMEM_MALLOC(mcast_scatter_buf, void *, tmp_buf_size,
    ...
    if(rank == root && local_rank == 0 && sendtype_contig == 0) {
       ...
       * pack before calling mcst-bcast  */
        mpi_errno = MPIR_Localcopy((char *)sendbuf, sendcnt*comm_size, 
                                    sendtype, mcast_scatter_buf, 
    ...
    if(rank == root) {
       if( recvbuf != MPI_IN_PLACE)  {
          ...
          mpi_errno = MPIR_Localcopy((char *)sendbuf + copy_offset, 
                                    sendcnt, sendtype,
                                    recvbuf, recvcnt, recvtype);
       }
    } else {
       copy_offset =  recvtype_extent * recvcnt * rank;
       mpi_errno = MPIR_Localcopy((char *) mcast_scatter_buf + copy_offset, 
                                  nbytes, MPI_BYTE,
                                  recvbuf, recvcnt, recvtype);
    }
