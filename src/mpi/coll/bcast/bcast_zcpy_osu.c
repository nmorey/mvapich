...
int MPIR_Knomial_Bcast_inter_node_trace_MV2(int root, 
                                            ...
                                            MPIR_Comm *comm_ptr)
{
    ...
    if(send_iter > 0) {
        knomial_bcast_dst_array = MPL_malloc(sizeof(int)*send_iter, 
                                             MPL_MEM_COLL);
    }
                         ...
                         MPIR_Errflag_t *errflag)
{
    ...
    MPIR_Assert(mv2_enable_zcpy_bcast==1 && mv2_use_slot_shmem_coll==1);
    if (count == 0) {
        goto fn_exit;
    }
    ...
    if (mv2_use_slot_shmem_coll && mv2_use_slot_shmem_bcast) {
        ...
        for (pos = 0; pos < nbytes; pos += mv2_shm_slot_len) {
            ...
            mpi_errno = mv2_shm_zcpy_bcast(shmem_commptr->dev.ch.shmem_info, 
                                           buf, len, root,
                             ...
                             MPIR_Errflag_t *errflag)
{
    ...
    if (local_rank == 0) {
        ...
        if((shmem_info)->bcast_knomial_factor != zcpy_knomial_factor) {
             MPIR_Knomial_Bcast_inter_node_trace_MV2(leader_root, 
                                                zcpy_knomial_factor,
                                                &src, &expected_send_count,
                                                &expected_recv_count, 
                                                &dst_array, leader_commptr);
    ...
    * that we have already set up */
    ...
    if(rank == 0 && rank != root) {
        ...
        mpi_errno = MPIC_Irecv((char *) tmp_buf +
                              bcast_curr_count, bcast_segment_count,
                              MPI_BYTE, root, MPIR_BCAST_TAG, 
                              comm_ptr, &prev_request);
        MPIR_ERR_CHECK(mpi_errno);
    }
    ...
    while (bcast_curr_count < nbytes) {
        ...
        if(rank == root && rank != 0) {
            ...
            bcast_next_segment_count = MIN(rem_count - bcast_segment_count, 
                                           bcast_segment_count);
