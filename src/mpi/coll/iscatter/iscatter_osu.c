...
int (*MV2_Iscatter_function) (const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                             ...
                             int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s) = NULL;

int (*MV2_Iscatter_intra_node_function) (const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                             void *recvbuf, int recvcount, MPI_Datatype recvtype,
                             int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s) = NULL;

extern unsigned long long PVAR_COUNTER_mv2_coll_iscatter_binomial_bytes_send;
extern unsigned long long PVAR_COUNTER_mv2_coll_iscatter_binomial_bytes_recv;
extern unsigned long long PVAR_COUNTER_mv2_coll_iscatter_binomial_count_send;
extern unsigned long long PVAR_COUNTER_mv2_coll_iscatter_binomial_count_recv;
extern unsigned long long PVAR_COUNTER_mv2_coll_iscatter_bytes_send;
extern unsigned long long PVAR_COUNTER_mv2_coll_iscatter_bytes_recv;
extern unsigned long long PVAR_COUNTER_mv2_coll_iscatter_count_send;
extern unsigned long long PVAR_COUNTER_mv2_coll_iscatter_count_recv;
...
/* Use binomial tree algorithm */
    ...
    if (is_homogeneous) {
        ...
        while (mask > 0) {
            if (relative_rank + mask < comm_size) {
                ...
                else
                {
                    ...
                    mpi_errno = MPIDU_Sched_cb2(&calc_send_count_non_root, ss, ((void *)(size_t)mask), s);
                    MPIR_ERR_CHECK(mpi_errno);
                    MPIR_SCHED_BARRIER(s);
        ...
        if (rank == root) {
          ...
          /* calculate the value of nbytes, the number of bytes in packed
            ...
            if (root == 0) {
                if (recvbuf != MPI_IN_PLACE) {
                    position = 0;
                    mpi_errno = MPIR_Typerep_pack(sendbuf, sendcount*comm_size,
                    ...
                    position = nbytes;
                    mpi_errno = MPIR_Typerep_pack(((char *) sendbuf + 
                                               extent*sendcount),
        ...
        position = 0;
        if (recvbuf != MPI_IN_PLACE) {
            mpi_errno = MPIR_Typerep_unpack(tmp_buf, tmp_buf_size, recvbuf,
                                         recvcount, recvtype, position,
                                         &position);
            MPIR_ERR_CHECK(mpi_errno);
        }
    }
