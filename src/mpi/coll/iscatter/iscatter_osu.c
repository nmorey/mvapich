...
int (*MVP_Iscatter_function)(const void *sendbuf, int sendcount,
                             MPI_Datatype sendtype, void *recvbuf,
                             int recvcount, MPI_Datatype recvtype, int root,
                             MPIR_Comm *comm_ptr, MPIR_Sched_t s) = NULL;

int (*MVP_Iscatter_intra_node_function)(const void *sendbuf, int sendcount,
                                        MPI_Datatype sendtype, void *recvbuf,
...
static int calc_send_count_root(MPIR_Comm *comm, int tag, void *state,
                                void *state2)
{
    struct shared_state *ss = state;
...
static int calc_send_count_non_root(MPIR_Comm *comm, int tag, void *state,
                                    void *state2)
{
    struct shared_state *ss = state;
    int mask = (int)(size_t)state2;
...
int MPIR_Iscatter_binomial(const void *sendbuf, int sendcount,
                           ...
                           MPIR_Sched_t s)
{
    ...
    MPIR_SCHED_CHKPMEM_MALLOC(ss, struct shared_state *,
                              sizeof(struct shared_state), mpi_errno,
    ...
    if (is_homogeneous) {
        ...
        if (relative_rank && !(relative_rank % 2)) {
            ...
            MPIR_SCHED_CHKPMEM_MALLOC(tmp_buf, void *, tmp_buf_size, mpi_errno,
        ...
        if (rank == root) {
            if (root != 0) {
                ...
                MPIR_SCHED_CHKPMEM_MALLOC(tmp_buf, void *, tmp_buf_size,
                ...
                if (recvbuf != MPI_IN_PLACE)
                    mpi_errno = MPIR_Sched_copy(
                        ...
                        ss->nbytes * (comm_size - rank), MPI_BYTE, s);
                else
                    mpi_errno = MPIR_Sched_copy(
                ...
                mpi_errno = MPIR_Sched_copy(
                    sendbuf, sendcount * rank, sendtype,
        ...
        while (mask < comm_size) {
            if (relative_rank & mask) {
                ...
                if (relative_rank % 2) {
                    mpi_errno = MPIR_Sched_recv(recvbuf, recvcount, recvtype,
                                                src, comm_ptr, s);
                    MPIR_ERR_CHECK(mpi_errno);
                    MPIR_SCHED_BARRIER(s);
                } else {
                    ...
                    mpi_errno =
                        MPIR_Sched_recv_status(tmp_buf, tmp_buf_size, MPI_BYTE,
                                               src, comm_ptr, &ss->status, s);
                    MPIR_ERR_CHECK(mpi_errno);
                    MPIR_SCHED_BARRIER(s);
        ...
        while (mask > 0) {
            if (relative_rank + mask < comm_size) {
                ...
                if ((rank == root) && (root == 0)) {
...
#endif
                    mpi_errno = MPIR_Sched_cb2(&calc_send_count_root, ss,
                                               ((void *)(size_t)mask), s);
                    MPIR_ERR_CHECK(mpi_errno);
                    MPIR_SCHED_BARRIER(s);

                    /* mask is also the size of this process's subtree */
                    mpi_errno = MPIR_Sched_send_defer(
                        ((char *)sendbuf + extent * sendcount * mask),
                        &ss->send_subtree_count, sendtype, dst, comm_ptr, s);
                    MPIR_ERR_CHECK(mpi_errno);
                    MPIR_SCHED_BARRIER(s);
                } else {
                    /* non-zero root and others */
                    mpi_errno = MPIDU_Sched_cb2(&calc_send_count_non_root, ss,
                                                ((void *)(size_t)mask), s);
                    MPIR_ERR_CHECK(mpi_errno);
                    MPIR_SCHED_BARRIER(s);
                    ...
                    /* mask is also the size of this process's subtree */
                    mpi_errno = MPIR_Sched_send_defer(
                        ((char *)tmp_buf + ss->nbytes * mask),
                        &ss->send_subtree_count, MPI_BYTE, dst, comm_ptr, s);
                    MPIR_ERR_CHECK(mpi_errno);
                    MPIR_SCHED_BARRIER(s);
                }
        ...
        if ((rank == root) && (root == 0) && (recvbuf != MPI_IN_PLACE)) {
            ...
            mpi_errno = MPIR_Sched_copy(sendbuf, sendcount, sendtype, recvbuf,
            ...
            /* for non-zero root and non-leaf nodes, copy from tmp_buf
               into recvbuf */
            mpi_errno = MPIR_Sched_copy(tmp_buf, ss->nbytes, MPI_BYTE, recvbuf,
                                        recvcount, recvtype, s);
            MPIR_ERR_CHECK(mpi_errno);
            MPIR_SCHED_BARRIER(s);
        }
        ...
        if (rank == root) {
            ...
            position = 0;
            mpi_errno = MPIR_Typerep_pack(sendbuf, 1, sendtype, position,
            ...
            if (root == 0) {
                if (recvbuf != MPI_IN_PLACE) {
                    ...
                    mpi_errno = MPIR_Typerep_pack(
                        sendbuf, sendcount * comm_size, sendtype, position,
                        tmp_buf, tmp_buf_size, &position);
                    MPIR_ERR_CHECK(mpi_errno);
                } else {
                    position = nbytes;
                    mpi_errno = MPIR_Typerep_pack(
                        ((char *)sendbuf + extent * sendcount),
                        sendcount * (comm_size - 1), sendtype, position,
                        tmp_buf, tmp_buf_size, &position);
                    MPIR_ERR_CHECK(mpi_errno);
                }
            } else {
                if (recvbuf != MPI_IN_PLACE) {
                    ...
                    mpi_errno = MPIR_Typerep_pack(
                        ((char *)sendbuf + extent * sendcount * rank),
                        sendcount * (comm_size - rank), sendtype, position,
                        tmp_buf, tmp_buf_size, &position);
                    MPIR_ERR_CHECK(mpi_errno);
                } else {
                    position = nbytes;
                    mpi_errno = MPIR_Typerep_pack(
                        ((char *)sendbuf + extent * sendcount * (rank + 1)),
                        sendcount * (comm_size - rank - 1), sendtype, position,
                        tmp_buf, tmp_buf_size, &position);
                    MPIR_ERR_CHECK(mpi_errno);
                }
                mpi_errno = MPIR_Typerep_pack(sendbuf, sendcount * rank,
                                              sendtype, position, tmp_buf,
                                              tmp_buf_size, &position);
                MPIR_ERR_CHECK(mpi_errno);
            }
        } else {
            MPIR_Pack_size_impl(recvcount * (comm_size / 2), recvtype,
                                &tmp_buf_size);
            MPIR_CHKLMEM_MALLOC(tmp_buf, void *, tmp_buf_size, mpi_errno,
                                "tmp_buf", MPL_MEM_COLL);
            ...
            /* calculate nbytes */
            position = 0;
            mpi_errno = MPIR_Typerep_pack(recvbuf, 1, recvtype, position,
                                          tmp_buf, tmp_buf_size, &position);
            MPIR_ERR_CHECK(mpi_errno);
        ...
        while (mask > 0) {
            if (relative_rank + mask < comm_size) {
                ...
                /* mask is also the size of this process's subtree */
                MPIR_PVAR_INC(iscatter, binomial, send, send_subtree_cnt,
                              MPI_BYTE);
        ...
        if (recvbuf != MPI_IN_PLACE) {
            mpi_errno =
                MPIR_Typerep_unpack(tmp_buf, tmp_buf_size, recvbuf, recvcount,
                                    recvtype, position, &position);
            MPIR_ERR_CHECK(mpi_errno);
        }
    }
...
static int MPIR_Iscatter_tune_helper_MVP(const void *sendbuf, int sendcount,
                                         MPI_Datatype sendtype, void *recvbuf,
                                         int recvcount, MPI_Datatype recvtype,
                                         int root, MPIR_Comm *comm_ptr,
                                         MPIR_Sched_t s)
{
    ...
    mpi_errno = MVP_Iscatter_function(sendbuf, sendcount, sendtype, recvbuf,
...
int MPIR_Iscatter_intra_MVP(const void *sendbuf, int sendcount,
                            MPI_Datatype sendtype, void *recvbuf, int recvcount,
                            ...
                            MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    while ((range < (mvp_size_iscatter_tuning_table - 1)) &&
           (comm_size > mvp_iscatter_thresholds_table[range].numproc)) {
        range++;
    }

    // Search for corresponding inter-leader function
    while ((range_threshold <
            (mvp_iscatter_thresholds_table[range].size_inter_table - 1)) &&
           (nbytes > mvp_iscatter_thresholds_table[range]
                         .inter_leader[range_threshold]
                         .max) &&
           (mvp_iscatter_thresholds_table[range]
                .inter_leader[range_threshold]
                .max != -1)) {
        range_threshold++;
    }
    ...
    while ((range_threshold_intra <
            (mvp_iscatter_thresholds_table[range].size_intra_table - 1))
           && (nbytes >
               mvp_iscatter_thresholds_table[range].intra_node[range_threshold_intra].max)
           &&
    (mvp_iscatter_thresholds_table[range].intra_node[range_threshold_intra].max
    ...
    */
    ...
    MVP_Iscatter_function = mvp_iscatter_thresholds_table[range]
                                .inter_leader[range_threshold]
                                .MVP_pt_Iscatter_function;

    MVP_Iscatter_intra_node_function = mvp_iscatter_thresholds_table[range]
                                           .intra_node[range_threshold_intra]
                                           .MVP_pt_Iscatter_function;

    /* There are currently no two-level nb-scatter functions hence
       setting to 0 by default */
    two_level_iscatter = mvp_iscatter_thresholds_table[range]
                             .is_two_level_iscatter[range_threshold];
    if (1 != two_level_iscatter) {
        mpi_errno = MPIR_Iscatter_tune_helper_MVP(sendbuf, sendcount, sendtype,
...
int MPIR_Iscatter_MVP(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                      ...
                      int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        ...
        mpi_errno =
            MPIR_Iscatter_intra_MVP(sendbuf, sendcount, sendtype, recvbuf,
                                    recvcount, recvtype, root, comm_ptr, s);
#else
