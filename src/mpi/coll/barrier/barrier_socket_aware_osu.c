...
int MPIR_socket_aware_shmem_barrier_old_MV2(MPIR_Comm * comm_ptr, 
                                            MPIR_Errflag_t *errflag)
{
    ...
    if (local_size > 1) {
        MPIR_MV2_SHMEM_COLL_Barrier_gather(intra_sock_size, intra_sock_rank,
                                             shmem_comm_rank);
    }
    ...
    if (intra_sock_rank == 0) {
        ...
        if(shmem_leader_commptr->dev.ch.shmem_coll_ok != 1)
	    {
            ...
            mpi_errno = MPIR_Pairwise_Barrier_MV2(global_sock_leader_ptr, 
                                                    errflag);
	    }
        else
        {
            ...
            if(shmem_leader_commptr->local_size > 1)
            {
                ...
                MPIR_MV2_SHMEM_COLL_Barrier_gather(leader_size, leader_rank,
                        leader_shmem_comm_rank);
            }
            ...
            if(shmem_leader_commptr->local_size > 1)
            {
                MPIR_MV2_SHMEM_COLL_Barrier_bcast(leader_size, leader_rank,
                        leader_shmem_comm_rank);
            }
        }
    }
    ...
    if (local_size > 1) {
        MPIR_MV2_SHMEM_COLL_Barrier_bcast(intra_sock_size, intra_sock_rank,
                                            shmem_comm_rank);
    }
    return mpi_errno;
}

int MPIR_socket_aware_shmem_barrier_MV2(MPIR_Comm * comm_ptr, 
                                        MPIR_Errflag_t *errflag)
{
    MPIR_Assert(comm_ptr->dev.ch.use_intra_sock_comm 
                && comm_ptr->dev.ch.shmem_coll_ok == 1);
    ...
    if (intra_sock_rank == 0) {
        ...
        else
        {
            ...
            if(shmem_leader_commptr->local_size > 1)
            {
                mv2_shm_barrier_gather(shmem_leader_shmemcomm
                                        ->dev.ch.shmem_info);
            }
            ...
            if(local_rank == 0)
            {
                ...
                if (comm_ptr->dev.ch.is_sharp_ok == 1 
                    ...
                    if (mpi_errno != MPI_SUCCESS) {
                        ...
                        mpi_errno = MPIR_Pairwise_Barrier_MV2(leader_commptr, 
                                                                errflag);
                        if (mpi_errno) {
            ...
            if(shmem_leader_commptr->local_size > 1)
            {
                mv2_shm_barrier_bcast(shmem_leader_shmemcomm
                                        ->dev.ch.shmem_info);
            }
        }
    }
