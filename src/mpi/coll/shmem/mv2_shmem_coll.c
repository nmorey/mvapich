 ...
 * specific functionality. We should define an MPID_ level interface that exposes
 * these functions to the MPI_ level if needed. For now most of these files
 * appear to only be used inside ch3 though. 
 */
...
cvars:
    - name        : MV2_KNOMIAL_INTER_LEADER_THRESHOLD
      category    : COLLECTIVE
      type        : int
      default     : (64 * 1024)
      class       : none
    ...
    - name        : MV2_KNOMIAL_INTER_NODE_FACTOR
      category    : COLLECTIVE
      type        : int
      default     : 4
      class       : none
    ...
    - name        : MV2_KNOMIAL_INTRA_NODE_FACTOR
      category    : COLLECTIVE
      type        : int
      default     : 4
      class       : none
    ...
    - name        : MV2_KNOMIAL_INTRA_NODE_THRESHOLD
      category    : COLLECTIVE
      type        : int
      default     : (128 * 1024)
      class       : none
    ...
    - name        : MV2_SHMEM_COLL_MAX_MSG_SIZE
      category    : COLLECTIVE
      type        : int
      default     : (128 * 1024)
      class       : none
    ...
    - name        : MV2_SHMEM_COLL_SPIN_COUNT
      category    : COLLECTIVE
      type        : int
      default     : 5
      class       : none
    ...
    - name        : MV2_USE_OSU_NB_COLLECTIVES
      category    : COLLECTIVE
      type        : boolean
      default     : 1
      class       : none
    ...
    - name        : MV2_ENABLE_SOCKET_AWARE_COLLECTIVES
      category    : COLLECTIVE
      type        : boolean
      default     : 0
      class       : none
      ...
      description : >-
        ...
        collective communication. The parameter MV2_USE_SHMEM_COLL
 ...
 */
#define MPID_MV2_PROGRESS_IF_NEEDED                                  \
...
struct coll_runtime mv2_coll_param = { 
    MPIR_ALLGATHER_SHORT_MSG,
 ...
 */
extern unsigned long long PVAR_COUNTER_mv2_num_2level_comm_requests;
extern unsigned long long PVAR_COUNTER_mv2_num_2level_comm_success;
extern unsigned long long PVAR_COUNTER_mv2_num_shmem_coll_calls;
...
int mv2_set_bcast_collective_algorithm()
{
    ...
    if (read_value != -1 &&
       (getenv("MV2_INTRA_BCAST_TUNING") != NULL ||
        ...
        PRINT_INFO(MPIR_Process.local_rank == 0, "User has set environment "
...
void MV2_Read_env_vars(void)
{
    ...
    mv2_use_osu_collectives = MV2_USE_OSU_COLLECTIVES;
    if (MV2_USE_OSU_COLLECTIVES != 1) {
        mv2_use_anl_collectives = 1;
    }

    mv2_use_osu_nb_collectives = MV2_USE_OSU_NB_COLLECTIVES;
    ...
    mv2_enable_shmem_collectives = MV2_USE_SHMEM_COLL;

    mv2_enable_shmem_allreduce = MV2_USE_SHMEM_ALLREDUCE;
...
#endif 
    mv2_enable_shmem_reduce = MV2_USE_SHMEM_REDUCE;
    ...
    mv2_enable_shmem_barrier = MV2_USE_SHMEM_BARRIER;
    if (MV2_SHMEM_COLL_NUM_COMM > 0) {
        mv2_g_shmem_coll_blocks = MV2_SHMEM_COLL_NUM_COMM;
    }
    if (MV2_SHMEM_COLL_MAX_MSG_SIZE > 0) {
        mv2_g_shmem_coll_max_msg_size = MV2_SHMEM_COLL_MAX_MSG_SIZE;
    }
    ...
    mv2_allgather_ranking = MV2_ALLGATHER_REVERSE_RANKING;
    if (MV2_ALLREDUCE_SHORT_MSG >= 0) {
        mv2_coll_param.allreduce_short_msg = MV2_ALLREDUCE_SHORT_MSG;
    }
    if (MV2_ALLGATHER_RD_THRESHOLD >= 0) {
        mv2_coll_param.allgather_rd_threshold = MV2_ALLGATHER_RD_THRESHOLD;
    }
    if (MV2_ALLGATHERV_RD_THRESHOLD != -1) {
        mv2_user_allgatherv_switch_point = MV2_ALLGATHERV_RD_THRESHOLD;
        mv2_tune_parameter = 1;
    }
    if (MV2_ALLGATHER_BRUCK_THRESHOLD >= 0) {
        mv2_coll_param.allgather_bruck_threshold = MV2_ALLGATHER_BRUCK_THRESHOLD;
    }
    if (MV2_ALLREDUCE_2LEVEL_MSG >= 0) {
        mv2_coll_param.allreduce_2level_threshold = MV2_ALLREDUCE_2LEVEL_MSG;
    }
    if (MV2_REDUCE_SHORT_MSG >= 0) {
        mv2_coll_param.reduce_short_msg = MV2_REDUCE_SHORT_MSG;
    }
    if (MV2_SHMEM_ALLREDUCE_MSG >= 0) {
        mv2_coll_param.shmem_allreduce_msg = MV2_SHMEM_ALLREDUCE_MSG;
    }
    if (MV2_REDUCE_2LEVEL_MSG >= 0) {
        mv2_coll_param.reduce_2level_threshold = MV2_REDUCE_2LEVEL_MSG;
    }
    if (MV2_SCATTER_SMALL_MSG != -1) {
        mv2_user_scatter_small_msg = MV2_SCATTER_SMALL_MSG;
        mv2_tune_parameter = 1;
    }
    if (MV2_SCATTER_MEDIUM_MSG != -1) {
        mv2_user_scatter_medium_msg = MV2_SCATTER_MEDIUM_MSG;
        mv2_tune_parameter = 1;
    }
#if defined(_MCST_SUPPORT_)
...
#endif                          /* #if defined(_MCST_SUPPORT_) */
    if (MV2_GATHER_SWITCH_PT != -1) {
        mv2_user_gather_switch_point = MV2_GATHER_SWITCH_PT;
        mv2_tune_parameter = 1;
    }
    ...
    if (MV2_SHMEM_REDUCE_MSG >= 0) {
        mv2_coll_param.shmem_reduce_msg = MV2_SHMEM_REDUCE_MSG;
    }
    if (MV2_INTRA_SHMEM_REDUCE_MSG >= 0) {
        mv2_coll_param.shmem_intra_reduce_msg = MV2_INTRA_SHMEM_REDUCE_MSG;
    }
    ...
    mv2_enable_shmem_bcast = MV2_USE_SHMEM_BCAST;
    ...
    mv2_use_two_level_gather = MV2_USE_TWO_LEVEL_GATHER;
    mv2_use_direct_gather = MV2_USE_DIRECT_GATHER;
    mv2_use_two_level_scatter = MV2_USE_TWO_LEVEL_SCATTER;
    mv2_use_direct_scatter = MV2_USE_DIRECT_SCATTER;
    if (MV2_USE_DIRECT_GATHER_SYSTEM_SIZE_SMALL > 0) {
        mv2_gather_direct_system_size_small = MV2_USE_DIRECT_GATHER_SYSTEM_SIZE_SMALL;
    }  else {
        mv2_gather_direct_system_size_small = MV2_GATHER_DIRECT_SYSTEM_SIZE_SMALL;
    }
    if (MV2_USE_DIRECT_GATHER_SYSTEM_SIZE_MEDIUM > 0) {
        mv2_gather_direct_system_size_medium = MV2_USE_DIRECT_GATHER_SYSTEM_SIZE_MEDIUM;
    } else {
        mv2_gather_direct_system_size_medium = MV2_GATHER_DIRECT_SYSTEM_SIZE_MEDIUM;
    }
    if (MV2_ALLTOALL_SMALL_MSG >= 0) {
        mv2_coll_param.alltoall_small_msg = MV2_ALLTOALL_SMALL_MSG;
    }
    if (MV2_ALLTOALL_THROTTLE_FACTOR != -1) {
        if (MV2_ALLTOALL_THROTTLE_FACTOR <= 1) {
            ...
            mv2_coll_param.alltoall_throttle_factor = MV2_ALLTOALL_THROTTLE_FACTOR;
            mv2_coll_param.alltoall_intra_throttle_factor = MV2_ALLTOALL_THROTTLE_FACTOR;
            mv2_coll_param.alltoall_large_msg_throttle_factor = MV2_ALLTOALL_THROTTLE_FACTOR;
        }
    }
    if (MV2_ALLTOALL_MEDIUM_MSG >= 0) {
        mv2_coll_param.alltoall_medium_msg = MV2_ALLTOALL_MEDIUM_MSG;
    }
    mv2_use_xor_alltoall = MV2_USE_XOR_ALLTOALL;
    ...
    if (MV2_KNOMIAL_INTER_LEADER_THRESHOLD > 0) {
        mv2_knomial_inter_leader_threshold = MV2_KNOMIAL_INTER_LEADER_THRESHOLD;
    }
    if (MV2_KNOMIAL_INTRA_NODE_THRESHOLD > 0
        && MV2_KNOMIAL_INTRA_NODE_THRESHOLD < mv2_g_shmem_coll_max_msg_size) {
        mv2_knomial_intra_node_threshold = MV2_KNOMIAL_INTRA_NODE_THRESHOLD;
    } else {
        mv2_knomial_intra_node_threshold = mv2_g_shmem_coll_max_msg_size;
    }
    mv2_scatter_ring_inter_leader_bcast = MV2_USE_SCATTER_RING_INTER_LEADER_BCAST;
    mv2_scatter_rd_inter_leader_bcast = MV2_USE_SCATTER_RD_INTER_LEADER_BCAST;
    mv2_knomial_inter_leader_bcast = MV2_USE_KNOMIAL_INTER_LEADER_BCAST;
    if (MV2_BCAST_TWO_LEVEL_SYSTEM_SIZE > 0) {
        mv2_bcast_two_level_system_size = MV2_BCAST_TWO_LEVEL_SYSTEM_SIZE;
    }
    if (MV2_USE_BCAST_SHORT_MSG > 0) {
        mv2_bcast_short_msg = MV2_USE_BCAST_SHORT_MSG;
    }
    ...
    if (MV2_SHMEM_COLL_SPIN_COUNT > 0) {
        mv2_shmem_coll_spin_count = MV2_SHMEM_COLL_SPIN_COUNT;
    }

    mv2_enable_knomial_2level_bcast = MV2_USE_KNOMIAL_2LEVEL_BCAST;

    mv2_knomial_2level_bcast_message_size_threshold = MV2_KNOMIAL_2LEVEL_BCAST_MESSAGE_SIZE_THRESHOLD;
    mv2_knomial_2level_bcast_system_size_threshold = MV2_KNOMIAL_2LEVEL_BCAST_SYSTEM_SIZE_THRESHOLD;


    mv2_intra_node_knomial_factor = MV2_KNOMIAL_INTRA_NODE_FACTOR;
    ...
    mv2_inter_node_knomial_factor = MV2_KNOMIAL_INTER_NODE_FACTOR;
    ...
    if (MV2_RED_SCAT_SHORT_MSG > 0) {
        mv2_red_scat_short_msg = MV2_RED_SCAT_SHORT_MSG;
    }

    if (MV2_RED_SCAT_LARGE_MSG > 0) {
            mv2_red_scat_long_msg = MV2_RED_SCAT_LARGE_MSG;
    }
    ...
    if ((value = getenv("MV2_SHARP_HCA_NAME")) != NULL) {
        mv2_sharp_hca_name = MPL_malloc(sizeof(value), MPL_MEM_COLL);
        MPIR_Memcpy(mv2_sharp_hca_name, value, sizeof(value));
    }
    ...
    mv2_enable_socket_aware_collectives = MV2_ENABLE_SOCKET_AWARE_COLLECTIVES;
    if (mv2_enable_socket_aware_collectives) {
        mv2_enable_topo_aware_collectives = 0;
     }

    mv2_use_socket_aware_allreduce = MV2_USE_SOCKET_AWARE_ALLREDUCE;
    ...
    mv2_use_socket_aware_sharp_allreduce = MV2_USE_SOCKET_AWARE_SHARP_ALLREDUCE;
    mv2_use_socket_aware_barrier = MV2_USE_SOCKET_AWARE_BARRIER;
    mv2_socket_aware_allreduce_max_msg = MV2_SOCKET_AWARE_ALLREDUCE_MAX_MSG;
    mv2_socket_aware_allreduce_min_msg = MV2_SOCKET_AWARE_ALLREDUCE_MIN_MSG;
...
void MPIR_MV2_SHMEM_COLL_Cleanup()
{
    /*unmap */
...
void MPIR_MV2_SHMEM_COLL_Unlink()
{
...
int MPIR_MV2_SHMEM_Helper_fn(int local_id, char **filename,
                                char *prefix, int *fd, size_t file_size)
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_HELPER_FN);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_HELPER_FN);
    ...
    char *mv2_kvs_name = (char *)MPL_malloc(kvs_name_sz + 1, MPL_MEM_STRINGS);
    UPMI_KVS_GET_MY_NAME(mv2_kvs_name, kvs_name_sz);

    /* Allocate memory for unique file name */
    *filename = (char *) MPL_malloc(sizeof (char) *
...
#define FSIZE_LIMIT 2147483640 /* 2G - c */
        {
            char *buf = (char *)MPL_calloc(file_size + 1, sizeof(char),
                                           MPL_MEM_BUFFER);
  ...
  fn_exit:
    MPL_free(mv2_kvs_name);
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_HELPER_FN);
    return mpi_errno;
...
int MPIR_MV2_SHMEM_COLL_init(int local_id)
{
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_COLL_INIT);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_COLL_INIT);
    int mpi_errno = MPI_SUCCESS;
    char *value;
    ...
    mv2_shmem_coll_num_procs = MPIR_Process.local_size;
    ...
    mpi_errno = MPIR_MV2_SHMEM_Helper_fn(local_id, &mv2_shmem_coll_file,
  ...
  fn_exit:
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_COLL_INIT);
    return mpi_errno;

  fn_fail:
    MPIR_MV2_SHMEM_COLL_Cleanup();
    goto fn_exit;
}

int MPIR_MV2_SHMEM_COLL_Mmap(int local_id)
{
    int i = 0;
    int j = 0;
    char *buf = NULL;
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_COLLMMAP);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_COLLMMAP);
...
#if defined(CKPT)
    if (smc_store_set) {
        MPIR_Memcpy(mv2_shmem_coll_obj.mmap_ptr, smc_store, mv2_shmem_coll_size);
        MPL_free(smc_store);
        smc_store_set = 0;
    }
#endif
    ...
    if (local_id == 0) {
        ...
        num_cntrl_bufs = 6;
#endif
        MPIR_Memset(buf, 0, (num_cntrl_bufs-1)*SHMEM_COLL_SYNC_ARRAY_SIZE + 
  ...
  fn_exit:
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_COLLMMAP);
    return mpi_errno;
...
int MPIR_MV2_SHMEM_COLL_finalize(int local_id, int num_local_nodes)
{
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_COLL_FINALIZE);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_COLL_FINALIZE);
    ...
    if (g_cr_in_progress) {
        ...
        if ((mv2_shmem_coll_obj.mmap_ptr != NULL) && (local_id == 0)) {
            smc_store = MPL_malloc(mv2_shmem_coll_size, MPL_MEM_COLL);
            PRINT_DEBUG(DEBUG_CR_verbose > 1, "mv2_shmem_coll_obj.mmap_ptr = %p\n", mv2_shmem_coll_obj.mmap_ptr);
            MPIR_Memcpy(smc_store, mv2_shmem_coll_obj.mmap_ptr, mv2_shmem_coll_size);
            smc_store_set = 1;
        }
    }
#endif
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_COLL_FINALIZE);
    return MPI_SUCCESS;
}

int MPIR_MV2_SHMEM_Sync(volatile int * volatile bar_array, int my_local_id, int num_local_procs)
{
    int mpi_errno = MPI_SUCCESS;
...
int MPIR_MV2_SMP_COLL_init()
{
    ...
    mpi_errno = MPIR_MV2_SHMEM_Helper_fn(local_id, &shmem_coll_init_file,
    ...
    if ((mpi_errno = MPIR_MV2_SHMEM_COLL_init(local_id)) != MPI_SUCCESS)
    {
        mpi_errno = MPIR_Err_create_code(MPI_SUCCESS, MPI_ERR_OTHER,
                __func__, __LINE__, MPI_ERR_OTHER, "**fail",
    ...
    if ((mpi_errno = MPIR_MV2_SHMEM_COLL_Mmap(local_id)) != MPI_SUCCESS)
    {
        mpi_errno = MPIR_Err_create_code(MPI_SUCCESS, MPI_ERR_OTHER,
                __func__, __LINE__, MPI_ERR_OTHER, "**fail", "%s",
                "SHMEM_COLL_Mmap failed");
    }
    mpi_errno = MPIR_MV2_SHMEM_Sync(bar_array, local_id, local_num_processes);
    if (mpi_errno != MPI_SUCCESS) {
        MPIR_ERR_POP(mpi_errno);
    }
...
int MPIR_MV2_SHMEM_Coll_get_free_block()
{
    int i = 0;
...
void MPIR_MV2_SHMEM_Coll_Block_Clear_Status(int block_id)
{
...
void MPIR_MV2_SHMEM_TREE_COLL_GetShmemBuf_optrels(int size, int rank, 
                                       int shmem_comm_rank, int target_rank,
                                       void **output_buf, int parent)
{
    int cnt = 0, err = 0;
    char *shmem_coll_buf = (char *) (&(shmem_coll->shmem_coll_buf));
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_TREE_SHMEM_COLL_GETSHMEMBUF_OPTRELS);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_TREE_SHMEM_COLL_GETSHMEMBUF_OPTRELS);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_TREE_SHMEM_COLL_GETSHMEMBUF_OPTRELS);
}

/* Shared memory gather: rank zero is the root always*/
void MPIR_MV2_SHMEM_COLL_GetShmemBuf_optrels(int size, int rank, int shmem_comm_rank, int target_rank,
                                       void **output_buf)
{
    int cnt = 0, err = 0;
    char *shmem_coll_buf = (char *) (&(shmem_coll->shmem_coll_buf));
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_COLL_GETSHMEMBUF_OPTRELS);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_COLL_GETSHMEMBUF_OPTRELS);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_COLL_GETSHMEMBUF_OPTRELS);
}


/* Shared memory gather: rank zero is the root always*/
void MPIR_MV2_SHMEM_COLL_GetShmemBuf(int size, int rank, int shmem_comm_rank,
                                       void **output_buf)
{
    int i = 1, cnt = 0, err = 0;
    char *shmem_coll_buf = (char *) (&(shmem_coll->shmem_coll_buf));
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_COLL_GETSHMEMBUF);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_COLL_GETSHMEMBUF);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_COLL_GETSHMEMBUF);
}

/* Shared memory bcast: rank zero is the root always*/
void MPIR_MV2_SHMEM_Bcast_GetBuf(int size, int rank,
                                   int shmem_comm_rank, void **output_buf)
{
    int i = 1, cnt = 0, err = 0;
    char *shmem_coll_buf = (char *) (&(shmem_coll->shmem_coll_buf) +
                                     mv2_g_shmem_coll_blocks * SHMEM_COLL_BLOCK_SIZE);
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_BCAST_GETBUF);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_BCAST_GETBUF);
    ...
    READBAR();
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_BCAST_GETBUF);
}

/* Shared memory bcast: rank zero is the root always*/
void MPIR_MV2_SHMEM_Bcast_Complete(int size, int rank, int shmem_comm_rank)
{
    int i = 1;
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_COLL_SETBCASTCOMPLETE);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_COLL_SETBCASTCOMPLETE);
    ...
    READBAR();
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_COLL_SETBCASTCOMPLETE);
}

void MPIR_MV2_SHMEM_TREE_COLL_SetGatherComplete_optrels(int size, int rank, int shmem_comm_rank, int target_rank, int parent)
{
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_TREE_SHMEM_COLL_SETGATHERCOMPLETE_OPTRELS);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_TREE_SHMEM_COLL_SETGATHERCOMPLETE_OPTRELS);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_TREE_SHMEM_COLL_SETGATHERCOMPLETE_OPTRELS);
}

void MPIR_MV2_SHMEM_COLL_SetGatherComplete_optrels(int size, int rank,
                                                     int shmem_comm_rank,
                                                     int target_rank)
{
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_COLL_SETGATHERCOMPLETE_OPTRELS);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_COLL_SETGATHERCOMPLETE_OPTRELS);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_COLL_SETGATHERCOMPLETE_OPTRELS);
}

void MPIR_MV2_SHMEM_COLL_SetGatherComplete(int size, int rank, 
                                             int shmem_comm_rank)
{
    int i = 1;
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_COLL_SETGATHERCOMPLETE);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_COLL_SETGATHERCOMPLETE);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_COLL_SETGATHERCOMPLETE);
}

void MPIR_MV2_SHMEM_COLL_Barrier_gather(int size, int rank, int shmem_comm_rank)
{
    int i = 1, cnt = 0, err = 0;
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_COLL_BARRIER_GATHER);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_COLL_BARRIER_GATHER);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_COLL_BARRIER_GATHER);
}

void MPIR_MV2_SHMEM_COLL_Barrier_bcast(int size, int rank, int shmem_comm_rank)
{
    int i = 1, cnt = 0, err = 0;

    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIR_MV2_SHMEM_COLL_BARRIER_BCAST);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIR_MV2_SHMEM_COLL_BARRIER_BCAST);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIR_MV2_SHMEM_COLL_BARRIER_BCAST);
}
                     ...
                     int send_on_device, int recv_on_device, int disp)
{
    ...
    if (send_on_device && *send_buf != MPI_IN_PLACE && mv2_device_host_sendbuf_size < sendsize) {
        if (mv2_device_host_send_buf) {
            ...
            MPL_free(mv2_device_host_send_buf);
        }
        mv2_device_host_sendbuf_size =
            sendsize < mv2_device_stage_block_size ? mv2_device_stage_block_size : sendsize;
        mv2_device_host_send_buf = 
                MPL_aligned_alloc(page_size, mv2_device_host_sendbuf_sizem, 
                                    MPL_MEM_BUFFER);
        if (NULL == mv2_device_host_send_buf) {
            mpi_errno = MPIR_Err_create_code(MPI_SUCCESS, MPI_ERR_OTHER,
                                             __func__, __LINE__, MPI_ERR_OTHER, "**fail",
    ...
    if (recv_on_device && mv2_device_host_recvbuf_size < recvsize) {
        if (mv2_device_host_recv_buf) {
            ...
            MPL_free(mv2_device_host_recv_buf);
        }
        mv2_device_host_recvbuf_size =
            recvsize < mv2_device_stage_block_size ? mv2_device_stage_block_size : recvsize;
        result = MPL_aligned_alloc(&mv2_device_host_recv_buf, page_size, mv2_device_host_recvbuf_size);
        if (NULL == mv2_device_host_recv_buf) {
            mpi_errno = MPIR_Err_create_code(MPI_SUCCESS, MPI_ERR_OTHER,
                                             __func__, __LINE__, MPI_ERR_OTHER, "**fail",
                                             "%s: %s", "posix_memalign", strerror(errno));
            return (mpi_errno);
    ...
    if (send_on_device && *send_buf != MPI_IN_PLACE) {
        if (send_on_device) {
            MV2_MPIDI_Memcpy_Device(mv2_device_host_send_buf, *send_buf, sendsize, deviceMemcpyDeviceToHost);
        }
    } else {
        if (recv_on_device) {
            MV2_MPIDI_Memcpy_Device(((char *) (mv2_device_host_recv_buf) + disp),
                             ((char *) (*recv_buf) + disp),
                     ...
                     int send_on_device, int recv_on_device)
{
    ...
    if (send_on_device && mv2_device_original_send_buf && send_buf != MPI_IN_PLACE) {
        if (!recv_on_device && !mv2_device_original_recv_buf) {
            MV2_MPIDI_Memcpy_Device(mv2_device_original_send_buf, *send_buf,
    ...
    if (recv_on_device && mv2_device_original_recv_buf) {
        MV2_MPIDI_Memcpy_Device(mv2_device_original_recv_buf, *recv_buf, recvsize, deviceMemcpyHostToDevice);
                       ...
                       int send_buf_on_device, int recv_buf_on_device, int rank)
{
    ...
    if (mv2_device_dev_srbuf_size < total_buf_size) {
        if (mv2_device_dev_sr_buf) {
            MV2_MPIDI_Free_Device(mv2_device_dev_sr_buf);
        }
        mv2_device_dev_srbuf_size = total_buf_size;
        MV2_MPIDI_Malloc_Device(mv2_device_dev_sr_buf, mv2_device_dev_srbuf_size);
    }
    ...
    if (send_buf_on_device && *send_buf != MPI_IN_PLACE) {
        ...
        if (mv2_device_host_send_peers < send_peers) {
            if (mv2_device_host_send_displs) {
                MPL_free(mv2_device_host_send_displs);
            }
            mv2_device_host_send_peers = send_peers;
            mv2_device_host_send_displs = MPL_malloc(sizeof (int) * 
                                    mv2_device_host_send_peers, MPL_MEM_COLL);
            MPIR_Memset((void *) mv2_device_host_send_displs, 0, sizeof (int) * mv2_device_host_send_peers);
        }
        ...
        if (mv2_device_host_sendbuf_size < total_send_size) {
            ...
            mv2_device_host_send_buf = 
                    MPL_aligned_alloc(page_size, mv2_device_host_sendbuf_size,
                                        MPL_MEM_BUFFER);
            if (NULL == mv2_device_host_send_buf) {
                mpi_errno = MPIR_Err_create_code(MPI_SUCCESS, MPI_ERR_OTHER,
                                                 __func__, __LINE__, MPI_ERR_OTHER,
    ...
    if (recv_buf_on_device && *recv_buf != MPI_IN_PLACE) {
        ...
        if (mv2_device_host_recv_peers < recv_peers) {
            if (mv2_device_host_recv_displs) {
                MPL_free(mv2_device_host_recv_displs);
            }
            mv2_device_host_recv_peers = recv_peers;
            mv2_device_host_recv_displs = MPL_malloc(sizeof (int) * 
                            mv2_device_host_recv_peers, MPL_MEM_COLL);
            MPIR_Memset(mv2_device_host_recv_displs, 0, sizeof (int) * mv2_device_host_recv_peers);
        }
        /*allocate buffer to stage the data */
        if (mv2_device_host_recvbuf_size < total_recv_size) {
            ...
            mv2_device_host_recv_buf = 
                    MPL_aligned_alloc(page_size, mv2_device_host_recvbuf_size,
                                        MPL_MEM_BUFFER);
            if (NULL == mv2_device_host_recv_buf) {
                mpi_errno = MPIR_Err_create_code(MPI_SUCCESS, MPI_ERR_OTHER,
                                                 __func__, __LINE__, MPI_ERR_OTHER,
    ...
    if (send_buf_on_device && *send_buf != MPI_IN_PLACE) {
        for (i = 0; i < send_peers; i++) {
            MV2_MPIDI_Memcpy_Device((void *) ((char *) mv2_device_dev_sr_buf
        ...
        MV2_MPIDI_Memcpy_Device(mv2_device_host_send_buf, mv2_device_dev_sr_buf,
    ...
    if (recv_buf_on_device && *recv_buf != MPI_IN_PLACE) {
        ...
        if (!recv_type_contig) {
            ...
            MV2_MPIDI_Memcpy_Device(mv2_device_host_recv_buf, mv2_device_dev_sr_buf,
                       ...
                       int send_buf_on_device, int recv_buf_on_device, int rank)
{
    ...
    if (send_buf_on_device && *send_buf != MPI_IN_PLACE) {
        MPIR_Assert(mv2_device_original_send_buf != NULL);
        *send_buf = mv2_device_original_send_buf;
    ...
    if (recv_buf_on_device && *recv_buf != MPI_IN_PLACE) {
        MV2_MPIDI_Memcpy_Device(mv2_device_dev_sr_buf, *recv_buf, total_recv_size, deviceMemcpyHostToDevice);
        for (i = 0; i < recv_peers; i++) {
            if (send_buf_on_device && *send_buf == MPI_IN_PLACE && i == rank)
                continue;
            MV2_MPIDI_Memcpy_Device((void *) ((char *) mv2_device_original_recv_buf
...
void DEVICE_COLL_Finalize()
{
    ...
    if (mv2_device_allgather_store_buf) {
        ...
        MPL_free(mv2_device_allgather_store_buf);
        mv2_device_allgather_store_buf = NULL;
    }

    if (mv2_device_coll_pack_buf_size) {
        MV2_MPIDI_Free_Device(mv2_device_coll_pack_buf);
        mv2_device_coll_pack_buf_size = 0;
    }

    if (mv2_device_coll_unpack_buf_size) {
        MV2_MPIDI_Free_Device(mv2_device_coll_unpack_buf);
                    ...
                    int disp, int procs_in_sendbuf, int comm_size)
{
    ...
    if (!sendtype_iscontig && send_copy_size > mv2_device_coll_pack_buf_size) {
        MV2_MPIDI_Free_Device(mv2_device_coll_pack_buf);
        MV2_MPIDI_Malloc_Device(mv2_device_coll_pack_buf, send_copy_size);
        mv2_device_coll_pack_buf_size = send_copy_size;
    }
    if (!recvtype_iscontig && recvsize > mv2_device_coll_unpack_buf_size) {
        MV2_MPIDI_Free_Device(mv2_device_coll_unpack_buf);
        MV2_MPIDI_Malloc_Device(mv2_device_coll_unpack_buf, recvsize);
                                  ...
                                  MPIR_Comm * comm_ptr)
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIDI_CH3I_LIMIC_GATHER_START);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIDI_CH3I_LIMIC_GATHER_START);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIDI_CH3I_LIMIC_GATHER_START);
    return mpi_errno;
}
...
int MPIDI_CH3I_LIMIC_Gather_Complete(int rank, int comm_size, int shmem_comm_rank)
{
    int i = 1, cnt = 0, err = 0;
    int mpi_errno = MPI_SUCCESS;
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPIDI_CH3I_LIMIC_COLL_GATHER_COMPLETE);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPIDI_CH3I_LIMIC_COLL_GATHER_COMPLETE);
    ...
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPIDI_CH3I_LIMIC_COLL_GATHER_COMPLETE);
    return (mpi_errno);
}

int MPIR_Limic_Gather_OSU(void *recvbuf,
                          int recvbytes,
                          void *sendbuf, int sendbytes, MPIR_Comm * shmem_comm_ptr)
{
    int mpi_errno = MPI_SUCCESS;
    MPI_Comm shmem_comm;
    int shmem_comm_rank;
    ...
    local_rank = MPIR_Comm_rank(shmem_comm_ptr);
    local_size = MPIR_Comm_size(shmem_comm_ptr);
...
#endif
    {
        MPIR_Memcpy(shmem->queue[shmem->local_rank].shm_slots[windex]->buf, in_buf, len);
    }
...
#if defined(CHANNEL_MRAIL_GEN2) || defined(CHANNEL_NEMESIS_IB)
static inline int mv2_post_zcpy_mid_request(MPIR_Comm *leader_commptr,
                                            shmem_info_t *shmem)
{
    int mpi_errno = MPI_SUCCESS;
...
int mv2_flush_zcpy_mid_request(shmem_info_t *shmem)
{
    int mpi_errno = MPI_SUCCESS;
    MPI_Status status;
...
int mv2_post_zcpy_end_request(MPIR_Comm *leader_commptr,
                                            shmem_info_t *shmem)
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_Request *req_ptr = NULL;
...
int mv2_flush_zcpy_end_request(shmem_info_t *shmem)
{
    int mpi_errno = MPI_SUCCESS;
    MPI_Status status; 
...
#endif
            {
                MPIR_Memcpy(shmem->queue[root].shm_slots[windex]->buf, buf, len);
            }
            WRITEBAR();
...
#endif
            {
                MPIR_Memcpy(buf, shmem->queue[root].shm_slots[rindex]->buf, len);
            }
        }
    } 
    shmem->write++;
                                ...
                                MPIR_Comm *comm_ptr)
{
    ...
    ptr = shmem->buffer;
    shmem->queue = (shm_queue_t *) MPL_malloc(sizeof (shm_queue_t *)
            * max_local_size, MPL_MEM_COLL);
    shmem->max_local_size = max_local_size;
    for (k = 0; k < max_local_size; k++) {
        shmem->queue[k].shm_slots = (shm_slot_t **)
            MPL_malloc(mv2_shm_window_size * sizeof (shm_slot_t *), MPL_MEM_COLL);
        ...
        if(leader_commptr->local_size > 1) {
            ...
            if(shmem->bcast_exchange_rdma_keys == 1) {
                ...
                if(expected_recv_count > 0) {
                    remote_handle_info_parent = MPL_malloc(sizeof(shm_coll_pkt)*1, 
                                                            MPL_MEM_COLL);
                    i=0;
                    do {
            ...
            if(leader_commptr->rank != root) {
...
#endif
                {
                    ...
                    MPIR_Memcpy(shmem->queue[intra_node_root].shm_slots[windex]->buf, buf, len);
                }
                WRITEBAR();
                shmem->queue[intra_node_root].shm_slots[windex]->psn = shmem->write;
                WRITEBAR();
...
#endif
                {
                    MPIR_Memcpy(buf, shmem->queue[intra_node_root].shm_slots[windex]->buf, len);
                }
            }
        } else {
#if defined(_ENABLE_CUDA_)
...
#endif
        {
            MPIR_Memcpy(buf, shmem->queue[intra_node_root].shm_slots[rindex]->buf, len);
        }
    }
    shmem->write++;
    shmem->read++;
    ...
    if (IS_SHMEM_WINDOW_FULL(shmem->write, shmem->tail)   ||
       ((mv2_shm_window_size - windex < 2) || (mv2_shm_window_size - rindex < 2))) {
        ...
        if (shmem->local_size > 1) {
             MPIR_MV2_SHMEM_COLL_Barrier_gather(local_size, local_rank,
                                             shmem_comm_rank);
        } 
        ...
        if (shmem->local_size > 1) {
             MPIR_MV2_SHMEM_COLL_Barrier_bcast(local_size, local_rank,
                                            shmem_comm_rank);
        }
        shmem->half_full_complete = 0; 
        shmem->write++;
        ...
        if(leader_commptr->local_size > 1) {
            ...
            if(shmem->reduce_exchange_rdma_keys == 1) {
               ...
               if(expected_recv_count > 0) { 
                    ...
                    pkt_array = MPL_malloc(sizeof(shm_coll_pkt)*expected_recv_count, 
                                            MPL_MEM_COLL); 
                    request_array = (MPIR_Request **) 
                            MPL_malloc(sizeof(MPIR_Request*)*expected_recv_count,
                                        MPL_MEM_COLL); 
                    status_array = 
                            MPL_malloc(sizeof(MPI_Status)*expected_recv_count,
                                        MPL_MEM_COLL); 
               ...
               if(shmem->reduce_expected_recv_count > 0) { 
                   inter_node_reduce_status_array = MPL_malloc(sizeof(int)*
