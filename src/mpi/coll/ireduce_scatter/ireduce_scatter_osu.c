                                        ...
                                        MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (is_commutative) {
        ...
        if (MPL_is_pof2(comm_size, NULL) && is_block_regular) {
            ...
            mpi_errno = 
                MPIR_Ireduce_scatter_intra_sched_noncommutative(sendbuf, 
                                                     recvbuf, recvcount, 
                                                     datatype, op, comm_ptr, s);
        } else {
            /* noncommutative and (non-pof2 or block irregular), use recursive doubling. */
            mpi_errno = 
                MPIR_Ireduce_scatter_intra_sched_recursive_doubling(sendbuf, 
                                                     recvbuf, recvcount, 
                                                     datatype, op, comm_ptr, s);
        }
    }
                                        ...
                                        MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        ...
        mpi_errno = MPIR_Ireduce_scatter_inter_sched_auto(sendbuf, recvbuf,
