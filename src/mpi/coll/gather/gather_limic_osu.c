                                         ...
                                         MPIR_Errflag_t *errflag)
{
    ...
    if (intra_sock_rank == 0) {
        if (intra_node_leader_rank == 0) {
            ...
            if (rank == root) {
                intra_tmp_buf = MPL_malloc(recvcnt * MPL_MAX(recvtype_extent,
                            recvtype_true_extent) * local_size, MPL_MEM_COLL);
            } else {
                intra_tmp_buf = MPL_malloc(sendcnt * MPL_MAX(sendtype_extent,
                            sendtype_true_extent) * local_size, MPL_MEM_COLL);
            }
        } else {
        ...
        if (shmem_commptr->dev.ch.is_socket_uniform != 1) {
            ...
            if (intra_node_leader_rank == 0) {
                ...
                displs = MPL_malloc(sizeof (int) * intra_node_leader_comm_size,
                                        MPL_MEM_COLL);
                recvcnts = MPL_malloc(sizeof (int) * 
                                            intra_node_leader_comm_size,
                                        MPL_MEM_COLL);
                if (!displs || !recvcnts) {
                    mpi_errno = MPIR_Err_create_code(MPI_SUCCESS,
                            MPIR_ERR_RECOVERABLE,
    ...
    if (intra_sock_rank == 0) {
        if (intra_node_leader_rank == 0) {
            ...
            if (rank == root) {
                ...
                intra_tmp_buf = MPL_malloc(send_nbytes * local_size, 
                                            MPL_MEM_COLL);
            }
                                         ...
                                         MPIR_Errflag_t *errflag)
{
    ...
    if ((!sendtype_iscontig) && (sendbuf != MPI_IN_PLACE)) {
        ...
        MPIR_CHKLMEM_MALLOC(local_sendbuf, void *, send_nbytes, mpi_errno,
                            "local_sendbuf", MPL_MEM_COLL);
        MPIR_Typerep_pack(sendbuf, sendcnt, sendtype, position,
                            local_sendbuf, send_nbytes, &position);
    }
    else {
        local_sendbuf = (void *)sendbuf;
        send_nbytes = nbytes;
                                     ...
                                     MPIR_Errflag_t *errflag)
{
     ...
     * algorithm */
        ...
        mpi_errno = MPIR_Limic_Gather_Scheme_PT_LINEAR(sendbuf, sendcnt, 
                                                       sendtype,
                                                       recvbuf, recvcnt, 
                                                       recvtype,
                                                       root, comm_ptr,
     ...
     * algorithm */
        ...
        mpi_errno = MPIR_Limic_Gather_Scheme_LINEAR_PT(sendbuf, sendcnt, 
                                                       sendtype,
                                                       recvbuf, recvcnt, 
                                                       recvtype,
                                                       root, comm_ptr,
