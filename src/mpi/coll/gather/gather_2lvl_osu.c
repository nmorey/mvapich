                                            ...
                                            MPIR_Errflag_t *errflag)
{
    ...
    if (rank == root) {
     ...
     if((g_use_limic2_coll) && (shmem_commptr->dev.ch.use_intra_sock_comm == 1)
         ...
         &&((num_scheme == USE_GATHER_PT_PT_BINOMIAL)
            ...
            mpi_errno = MV2_Gather_intra_node_function(sendbuf, sendcnt, 
                                                    sendtype,
...
#endif/*#if defined(_SMP_LIMIC_)*/
    {
        if (local_rank == 0) {
            ...
            if (rank == root) {
                tmp_buf = MPL_malloc(recvcnt * MPL_MAX(recvtype_extent,
                            recvtype_true_extent) * local_size, MPL_MEM_COLL);
            } else {
                tmp_buf = MPL_malloc(sendcnt * MPL_MAX(sendtype_extent,
                            sendtype_true_extent) *
                        local_size, MPL_MEM_COLL);
            }
            if (tmp_buf == NULL) {
    ...
    if (comm_ptr->dev.ch.is_uniform != 1) {
        if (local_rank == 0) {
            ...
            if (leader_comm_rank == leader_root && root != leader_of_root) {
                ...
                if(rank == root) {
                    leader_gather_buf = MPL_malloc(recvcnt *
                                                MPL_MAX(recvtype_extent,
                                                recvtype_true_extent) *
                                                comm_size, MPL_MEM_COLL);
                } else {
                    leader_gather_buf = MPL_malloc(sendcnt *
                                                MPL_MAX(sendtype_extent,
                                                sendtype_true_extent) *
                                                comm_size, MPL_MEM_COLL);
                }
        ...
        if (local_rank == 0) {
            if (leader_comm_rank == leader_root && root != leader_of_root) {
                 ...
                 */
                leader_gather_buf = MPL_malloc(nbytes * comm_size, 
                                                MPL_MEM_COLL);
                if (leader_gather_buf == NULL) {
                    mpi_errno = MPIR_Err_create_code(MPI_SUCCESS,
                                                     MPIR_ERR_RECOVERABLE,
            ...
            if (root == leader_of_root) {
                if (comm_ptr->dev.ch.is_global_block != 1 
                    ...
                    if (leader_comm_rank == leader_root) {
                        reorder_tmp_buf = MPL_malloc(nbytes * comm_size, 
                                                        MPL_MEM_COLL);
                    }
    ...
    if ((local_rank == 0) && (root != rank)
        && (leader_of_root == rank)) {
        MPIR_PVAR_INC(gather, two_level_direct, 
                        send, nbytes * comm_size, MPI_BYTE);
    ...
    if (rank == root && local_rank != 0) {
        ...
        MPIR_PVAR_INC(gather, two_level_direct, recv, 
                        recvcnt * comm_size, recvtype);
        if (comm_ptr->dev.ch.is_global_block != 1 
                || comm_ptr->dev.ch.is_blocked != 1) {
            /* cyclic process mapping */
            MPIR_Assert (comm_ptr->dev.ch.rank_list != NULL);
            void * reorder_tmp_buf = MPL_malloc(nbytes * comm_size, 
                                                MPL_MEM_COLL);
