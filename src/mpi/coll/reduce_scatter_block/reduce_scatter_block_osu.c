    ...
    MPIR_Errflag_t *errflag)
{
    ...
    void* tmp_sendbuf;
    void* tmp_recvbuf;
    MPIR_CHKLMEM_MALLOC(tmp_sendbuf, char *, mpi_buf_size, mpi_errno, "tmp_sendbuf",
                        MPL_MEM_COLL);
    MPIR_CHKLMEM_MALLOC(tmp_recvbuf, char *, mpi_buf_size, mpi_errno, "tmp_recvbuf",
                        MPL_MEM_COLL);
    ...
    while (nread < elem_per_rank) {
        ...
        for(dist = comm_size-1; dist >= 0; dist--) {
            ...
            if (dist < comm_size-1) {
                MPIR_Reduce_local(tmp_recvbuf, tmp_sendbuf, count, datatype, op);
            }
