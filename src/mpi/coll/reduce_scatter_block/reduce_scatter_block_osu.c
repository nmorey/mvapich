...
int MPIR_Reduce_scatter_block_ring_2lvl_MVP(const void *sendbuf, void *recvbuf,
                                            int recvcount,
                                            ...
                                            MPIR_Errflag_t *errflag)
{
    ...
    if (comm_ptr->dev.ch.rank_list == NULL) {
        return MPIR_Reduce_scatter_block_ring_MVP(
            sendbuf, recvbuf, recvcount, datatype, op, comm_ptr, errflag);
    }
    ...
    while (nread < elem_per_rank) {
        ...
        for (dist = comm_size - 1; dist >= 0; dist--) {
            ...
            unsigned long elem_offset =
                elem_per_rank * (unsigned long)src_rank + nread;
            const char *buf = (const char *)input_buf + elem_offset * extent;

            /* copy next set of bytes for this chunk from input buffer into
             * sendbuf */
            MPIR_Localcopy(buf, count, datatype, tmp_sendbuf, count, datatype);

            /* merge the blocks via reduce operation */
            if (dist < comm_size - 1) {
                MPIR_Reduce_local(tmp_recvbuf, tmp_sendbuf, count, datatype,
                                  op);
            }
...
int MPIR_Reduce_scatter_block_ring_MVP(const void *sendbuf, void *recvbuf,
                                       int recvcount, MPI_Datatype datatype,
                                       MPI_Op op, MPIR_Comm *comm_ptr,
                                       MPIR_Errflag_t *errflag)
{
                                  ...
                                  MPIR_Errflag_t *errflag)
{
    ...
    if (!is_commutative) {
        mpi_errno = MPIR_Reduce_scatter_block_impl(
            sendbuf, recvbuf, recvcount, datatype, op, comm_ptr, errflag);
    } else {
        int range = 0;
        ...
        while (
            (range < (mvp_size_red_scat_block_tuning_table - 1)) &&
            (comm_size > mvp_red_scat_block_thresholds_table[range].numproc)) {
            range++;
        }
        /* Search for corresponding inter-leader function */
        while ((range_threshold <
                (mvp_red_scat_block_thresholds_table[range].size_inter_table -
                 1)) &&
               (nbytes > mvp_red_scat_block_thresholds_table[range]
                             .inter_leader[range_threshold]
                             .max) &&
               (mvp_red_scat_block_thresholds_table[range]
                    .inter_leader[range_threshold]
                    .max != -1)) {
            range_threshold++;
        }
        ...
        /* Set inter-leader pt */
        MVP_Red_scat_block_function = mvp_red_scat_block_thresholds_table[range]
                                          .inter_leader[range_threshold]
                                          .MVP_Red_scat_block_function;

        mpi_errno = MVP_Red_scat_block_function(
            sendbuf, recvbuf, recvcount, datatype, op, comm_ptr, errflag);
    }
