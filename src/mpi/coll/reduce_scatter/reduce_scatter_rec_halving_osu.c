                                        ...
                                        MPIR_Errflag_t *errflag)
{
	...
	if (rank < 2 * rem) {
		if (rank % 2 == 0) {	/* even */
			MPIR_PVAR_INC(reduce_scatter, rec_halving, 
                                      send, total_count, datatype);
			...
			MPIR_PVAR_INC(reduce_scatter, rec_halving, recv, 
			...
			/* do the reduction on received data. since the
			   ...
			   the operation is commutative or not. */
                        MPIR_MV2_Reduce_local(tmp_recvbuf, tmp_results, 
	...
	if (newrank != -1) {
		...
		MPI_Status status;
		MPIR_CHKLMEM_MALLOC(newcnts, int *, pof2 * sizeof (int),
                                     mpi_errno, "newcnts", MPL_MEM_COLL);
		MPIR_CHKLMEM_MALLOC(newdisps, int *, pof2 * sizeof (int), 
                                    mpi_errno, "newdisps", MPL_MEM_COLL);
		...
		while (mask > 0) {
			...
			{
				MPIR_PVAR_INC(reduce_scatter, rec_halving, 
                                                send, send_cnt, datatype);
				MPIR_PVAR_INC(reduce_scatter, rec_halving, 
                                                recv, recv_cnt, datatype);
			...
			if ((send_cnt == 0) && (recv_cnt != 0)) {
				mpi_errno = MPIC_Waitall(1, &request, 
                                                         &status, errflag);
				if (mpi_errno) {
					/* for communication errors, 
                                         * just record the error but continue */
			...
			if (recv_cnt) {
                              MPIR_MV2_Reduce_local((char *) tmp_recvbuf +
                                                        newdisps[recv_idx] * extent, 
                                                    (char *) tmp_results +
                                                        newdisps[recv_idx] * extent, 
                                                    (MPI_Aint) recv_cnt,
