...
int MPIR_Reduce_scatter_noncomm_MV2(const void *sendbuf,
                                           void *recvbuf,
                                           const int *recvcnts,
                                           ...
                                           MPIR_Errflag_t *errflag)
{
	...
	MPIR_CHKLMEM_MALLOC(tmp_buf0, void *, true_extent * total_count, 
                                mpi_errno,
						"tmp_buf0", MPL_MEM_COLL);
	MPIR_CHKLMEM_MALLOC(tmp_buf1, void *, true_extent * total_count, 
                                                mpi_errno,
	...
	for (i = 0; i < comm_size; ++i) {
		mpi_errno =
			MPIR_Localcopy((char *) (sendbuf == MPI_IN_PLACE 
                                                    ? recvbuf : sendbuf) +
                                                 (i * true_extent * block_size),
                                                  block_size, datatype,
                                                 (char *) tmp_buf0 +
                                                    (mirror_permutation(i, 
                                                        log2_comm_size) *
	...
	for (k = 0; k < log2_comm_size; ++k) {
		...
		mpi_errno = MPIC_Sendrecv(outgoing_data 
                                            + send_offset * true_extent,
                                          size, datatype, peer,
		...
		if (rank > peer) {
			/* higher ranked value 
                        ...
                        MPIR_MV2_Reduce_local(incoming_data
                                                + recv_offset * true_extent,
                                              outgoing_data
                                                + recv_offset * true_extent
                                              , (MPI_Aint) size,
			...
			/* lower ranked value 
                        ...
                        MPIR_MV2_Reduce_local(outgoing_data
                                                + recv_offset * true_extent,
                                              incoming_data
                                                + recv_offset * true_extent
                                              , (MPI_Aint) size,
	...
	result_ptr =
		...
		recv_offset * true_extent;
	mpi_errno =
		MPIR_Localcopy(result_ptr, size, datatype, recvbuf, 
                                size, datatype);
  fn_exit:
