                                  ...
                                  MPIR_Errflag_t *errflag)
{
	...
	if (MPI_IN_PLACE == sendbuf) {
		...
		if (root == rank) {
			mpi_errno = MPIR_Reduce_MV2(MPI_IN_PLACE, tmprbuf, 
        ...
        MPIR_CHKLMEM_MALLOC(tmprbuf_free, void *, total_count *
                (MPL_MAX(extent, true_extent)),
                mpi_errno, "receive buffer", MPL_MEM_COLL);
        tmprbuf = (void *) ((char *) tmprbuf_free - true_lb);

		mpi_errno = MPIR_Reduce_MV2(sendbuf, tmprbuf, total_count,						     datatype, op, root, comm_ptr, 
                                            errflag);
	}
