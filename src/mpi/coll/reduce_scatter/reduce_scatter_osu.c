    ...
    if (pof2 == comm_size && is_block_regular) {
		...
		while (mask < comm_size) {
		    ...
		    /* At step 1, processes exchange (n-n/p) amount of
		        adata; at step 2, (n-2n/p) amount of data; at step 3, (n-4n/p)
			...
			if (dst_tree_root + mask > comm_size) {
			    ...
			    if (received) {
		        	if (is_commutative || (dst_tree_root < my_tree_root)) {
                                    ...
                                    MPIR_MV2_Reduce_local(tmp_results, tmp_recvbuf,
                                                         (MPI_Aint) blklens[0],
                                                         datatype, uop
#ifdef HAVE_CXX_BINDING
                                                         , is_cxx_uop
                                   ...
                                   MPIR_MV2_Reduce_local(((char *) tmp_results +
                                                            dis[1] * extent),
                                                         ((char *) tmp_recvbuf +
                                                            dis[1] * extent),
                                                         (MPI_Aint) blklens[1],
							...
							MPIR_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (mv2_enable_device && recv_mem_type) {
        ...
        if (sendbuf == MPI_IN_PLACE) {
            ...
            recv_host_buf = (char*) MPL_malloc(recvcnts[rank]*type_size,
                                               MPL_MEM_COLL);
        }
