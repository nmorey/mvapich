    ...
    MPIR_Errflag_t *errflag)
{
    ...
    while (nread < max_elem_per_rank) {
        ...
        for(dist = comm_size-1; dist >= 0; dist--) {
            ...
            if (dist < comm_size-1) {
                MPIR_Reduce_local(tmp_recvbuf, tmp_sendbuf, 
                                    send_count, datatype, op);
            }
            ...
            if (dist > 0) {
                ...
                MPIC_Irecv(tmp_recvbuf, recv_count, datatype, rank_left,  
                            0, comm_ptr, &request[0]);
                MPIC_Isend(tmp_sendbuf, send_count, datatype, rank_right, 
                            0, comm_ptr, &request[1], errflag);
