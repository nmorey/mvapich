        ...
        MPIR_Errflag_t *errflag)
{
    ...
    while (nread < max_elem_per_rank) {
        ...
        for(dist = comm_size-1; dist >= 0; dist--) {
            ...
            if (dist > 0) {
                MPIR_PVAR_INC(reduce_scatter, ring_2lvl, send, 
                              send_count, datatype);
                MPIR_PVAR_INC(reduce_scatter, ring_2lvl, recv, 
                              recv_count, datatype);
                /* exchange data with neighbors */
                MPIC_Irecv(tmp_recvbuf, recv_count, datatype, 
                            rank_left,  0, comm_ptr, &request[0]);
                MPIC_Isend(tmp_sendbuf, send_count, datatype, 
                            rank_right, 0, comm_ptr, &request[1], errflag);
