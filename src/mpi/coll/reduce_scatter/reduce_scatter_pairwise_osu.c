                                      ...
                                      MPIR_Errflag_t *errflag)
{
	...
	for (i = 1; i < comm_size; i++) {
		...
		if (sendbuf != MPI_IN_PLACE)
		{
			MPIR_PVAR_INC(reduce_scatter, pairwise, send, 
                                        recvcnts[dst], datatype);
			MPIR_PVAR_INC(reduce_scatter, pairwise, recv, 
                                        recvcnts[rank], datatype);
			mpi_errno =
				MPIC_Sendrecv(((char *) sendbuf 
                                                + disps[dst] * extent),
		...
		{
		    MPIR_PVAR_INC(reduce_scatter, pairwise, 
                                    send, recvcnts[dst], datatype);
		    MPIR_PVAR_INC(reduce_scatter, pairwise, 
		...
		if (is_commutative || (src < rank)) {
			if (sendbuf != MPI_IN_PLACE) {
                            MPIR_MV2_Reduce_local(tmp_recvbuf, recvbuf, 
                                                  (MPI_Aint) recvcnts[rank],
                                                  datatype, uop
#ifdef HAVE_CXX_BINDING
                                                  , is_cxx_uop
#endif
                            ...
                            MPIR_MV2_Reduce_local(tmp_recvbuf, 
                                                  ((char *) recvbuf
                                                    + disps[rank] * extent),
                                                  (MPI_Aint) recvcnts[rank],
                                                  datatype, uop
			...
			if (sendbuf != MPI_IN_PLACE) {
                            MPIR_MV2_Reduce_local(recvbuf, tmp_recvbuf,
                                                  (MPI_Aint) recvcnts[rank],
                                                  datatype, uop
#ifdef HAVE_CXX_BINDING
                                                  , is_cxx_uop
			    ...
			    mpi_errno = MPIR_Localcopy(tmp_recvbuf, 
                                                            recvcnts[rank],
                                                            datatype, recvbuf,
                                                            recvcnts[rank], 
                                                            datatype);
                MPIR_ERR_CHECK(mpi_errno);
                            ...
                            MPIR_MV2_Reduce_local(((char *) recvbuf
                                                    + disps[rank] * extent),
                                                  tmp_recvbuf,
                                                  (MPI_Aint) recvcnts[rank],
                                                  datatype, uop
