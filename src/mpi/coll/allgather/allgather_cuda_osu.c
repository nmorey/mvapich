...
extern void *mvp_device_allgather_store_buf;
extern int mvp_device_allgather_store_buf_size;
...
#undef FUNCNAME
#define FUNCNAME MPIR_Allgather_cuda_intra_MVP
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Allgather_cuda_intra_MVP(const void *sendbuf, int sendcount,
                                  ...
                                  MPIR_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    int mask, dst_tree_root, my_tree_root, send_offset, recv_offset;
    int comm_size_is_pof2;
    MPI_Status status;
    int page_size = 0;
    int result, max_size;
    MPIR_Request *recv_req;
    ...
    max_size = mvp_device_allgather_store_buf_size <
                       recvcount * comm_size * recvtype_extent ?
                   recvcount * comm_size * recvtype_extent :
                   mvp_device_allgather_store_buf_size;

    if (mvp_device_allgather_store_buf_size < max_size ||
        !mvp_device_allgather_store_buf) {
        ...
        mvp_device_allgather_store_buf =
            MPL_aligned_alloc(page_size, max_size, MPL_MEM_OTHER);
        if (NULL == mvp_device_allgather_store_buf) {
            mpi_errno = MPIR_Err_create_code(
                MPI_SUCCESS, MPI_ERR_OTHER, FCNAME, __LINE__, MPI_ERR_OTHER,
                "**fail", "%s: %s", "posix_memalign", strerror(errno));
            MPIR_ERR_POP(mpi_errno);
        }
        ibv_device_register(mvp_device_allgather_store_buf, max_size);
        mvp_device_allgather_store_buf_size = max_size;
    }

    if (recvcount * recvtype_extent >
            mvp_device_allgather_rd_limit * comm_size ||
        ...
        /* This synchronization is needed because MPIR_Localcopy calls
         * cudamemcpy on the default stream (0) but subsequent MPI_Isend/Irecv
         * calls access GPU buffers using non-default streams which don't wait
         * for the initial local copy to complete*/
        if (mvp_enable_device && mvp_device_initialized &&
            ...
            MVP_MPID_Device_StreamWaitEvent(stream_d2h,
                                            cuda_nbstream_sync_event, 0);
        }
        ...
        MPIR_PVAR_INC(allgather_cuda, intra, recv, recvcount * recvtype_extent,
                      MPI_BYTE);
        mpi_errno = MPIC_Irecv(((char *)mvp_device_allgather_store_buf +
                                jnext * recvcount * recvtype_extent),
                               recvcount * recvtype_extent, MPI_BYTE, left,
                               MPIR_ALLGATHER_TAG, comm_ptr, &recv_req);
        MPIR_PVAR_INC(allgather_cuda, intra, send, recvcount * recvtype_extent,
                      MPI_BYTE);
        mpi_errno =
            MPIC_Isend(((char *)recvbuf + j * recvcount * recvtype_extent),
                       recvcount * recvtype_extent, MPI_BYTE, right,
                       MPIR_ALLGATHER_TAG, comm_ptr, &send_req, errflag);
        ...
        MVP_MPID_Memcpy_Device_Async(
            ...
            (void *)((char *)mvp_device_allgather_store_buf +
                     jnext * recvcount * recvtype_extent),
            recvcount * recvtype_extent, deviceMemcpyHostToDevice, stream_h2d);
        ...
        for (i = 2; i < comm_size - 1; i++) {
            ...
            mpi_errno =
                MPIC_Isend(((char *)mvp_device_allgather_store_buf +
                            j * recvcount * recvtype_extent),
                           recvcount, recvtype, right, MPIR_ALLGATHER_TAG,
                           comm_ptr, &send_req, errflag);
            mpi_errno = MPIC_Waitall(1, &recv_req, &status, errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue
                 */
            ...
            MVP_MPID_Memcpy_Device_Async(
                ...
                (void *)((char *)mvp_device_allgather_store_buf +
                         jnext * recvcount * recvtype_extent),
                recvcount * recvtype_extent, deviceMemcpyHostToDevice,
                stream_h2d);
        ...
        if (i < comm_size) {
            ...
            mpi_errno = MPIC_Irecv(
                ...
                recvcount, recvtype, left, MPIR_ALLGATHER_TAG, comm_ptr,
                &recv_req);
            MPIR_PVAR_INC(allgather_cuda, intra, send, recvcount, recvtype);
            mpi_errno =
                MPIC_Isend(((char *)mvp_device_allgather_store_buf +
        ...
        if (dst < comm_size) {
            MVP_MPID_Memcpy_Device(
                (void *)((char *)mvp_device_allgather_store_buf +
                         rank * recvcount * recvtype_extent),
                (void *)((char *)recvbuf + rank * recvcount * recvtype_extent),
                recvcount * recvtype_extent, deviceMemcpyDeviceToHost);

            MPIR_PVAR_INC(allgather_cuda, intra, recv, (mask)*recvcount,
                          recvtype);
            mpi_errno = MPIC_Irecv(
                ((char *)mvp_device_allgather_store_buf + recv_offset),
                (mask)*recvcount, recvtype, dst, MPIR_ALLGATHER_TAG, comm_ptr,
                &recv_req);
            MPIR_PVAR_INC(allgather_cuda, intra, send, curr_cnt, recvtype);
            mpi_errno = MPIC_Isend(
                ((char *)mvp_device_allgather_store_buf + send_offset),
                curr_cnt, recvtype, dst, MPIR_ALLGATHER_TAG, comm_ptr,
                &send_req, errflag);
            ...
            MVP_MPID_Memcpy_Device_Async(
                (void *)((char *)recvbuf + recv_offset),
                (void *)((char *)mvp_device_allgather_store_buf + recv_offset),
                (mask)*recvcount * recvtype_extent, deviceMemcpyHostToDevice,
                stream_h2d);
            ...
            if (dst < comm_size) {
                if (mask == comm_size / 2) {
                    MPIR_PVAR_INC(allgather_cuda, intra, recv, (mask)*recvcount,
                                  recvtype);
                    mpi_errno = MPIC_Irecv(
                        ((char *)recvbuf + recv_offset), (mask)*recvcount,
                        recvtype, dst, MPIR_ALLGATHER_TAG, comm_ptr, &recv_req);
                } else {
                    MPIR_PVAR_INC(allgather_cuda, intra, recv, (mask)*recvcount,
                                  recvtype);
                    mpi_errno = MPIC_Irecv(
                        ((char *)mvp_device_allgather_store_buf + recv_offset),
                        (mask)*recvcount, recvtype, dst, MPIR_ALLGATHER_TAG,
                        comm_ptr, &recv_req);
                }
                MPIR_PVAR_INC(allgather_cuda, intra, send, curr_cnt, recvtype);
                mpi_errno = MPIC_Isend(
    ...
    MVP_MPID_Device_EventRecord(cuda_nbstream_sync_event, stream_h2d);
