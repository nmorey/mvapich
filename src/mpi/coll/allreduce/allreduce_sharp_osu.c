...
extern MPIR_T_pvar_timer_t PVAR_TIMER_mv2_coll_timer_allreduce_sharp;

extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_sharp;

#if defined(_SHARP_SUPPORT)
                        ...
                        MPIR_Errflag_t *errflag)
{
    ...
    if (is_contig == 1) {
        ...
        if (sendbuf != MPI_IN_PLACE) {
            ...
            reduce_spec.sbuf_desc.buffer.ptr = 
                MPL_malloc(reduce_spec.sbuf_desc.buffer.length);
    ...
    sharp_comm = ((sharp_info_t *)comm_ptr->dev.ch.sharp_coll_info)
