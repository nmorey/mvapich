...
extern MPIR_T_pvar_timer_t PVAR_TIMER_mv2_coll_timer_allreduce_shm_rs;
extern MPIR_T_pvar_timer_t 
    PVAR_TIMER_mv2_coll_timer_allreduce_reduce_scatter_allgather_colls;

extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_shm_rs;
extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_pt2pt_rs_bytes_send;
extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_pt2pt_rs_bytes_recv;
extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_pt2pt_rs_count_send;
extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_pt2pt_rs_count_recv;
extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_pt2pt_ring;
extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_pt2pt_ring_inplace;
extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_pt2pt_ring_wrapper;
extern unsigned long long 
    PVAR_COUNTER_mv2_coll_allreduce_reduce_scatter_allgather_colls;
        ...
        /* use recursive doubling */
            ...
            while (mask < pof2) {
                ...
                mpi_errno = MPIC_Sendrecv((char *) recvbuf +
                    disps[send_idx] * extent, send_cnt, datatype,
                    MPIR_Rank_list_mapper(comm_ptr, dst), MPIR_ALLREDUCE_TAG,
...
/* TODO: this is similar to the func: MPIR_Allreduce_pt2pt_rs_MV2
                             ...
                             MPIR_Errflag_t *errflag)
{
    ...
    MPIR_T_PVAR_COUNTER_INC(MV2, mv2_coll_allreduce_pt2pt_ring, 1);
...

    {
        ...
        for (i = 1; i < comm_size; i++) {
            ...
            if (i == 1 && sendbuf != MPI_IN_PLACE)
                send_chunk  = (void*) sendbuf + ((rank - (i-1) + comm_size) 
                    % comm_size)*chunk_size;
            else
                send_chunk  = recvbuf 
            ...
            */
            ...
            mpi_errno = MPID_Irecv(recv_chunk, chunk_count, datatype, 
                left, 11, comm_ptr,
                    context_id, &recv_req_ptr);
            MPIR_ERR_CHECK(mpi_errno);

            MPIR_PVAR_INC(allreduce, pt2pt_ring, send, chunk_count, datatype);
            mpi_errno = MPID_Isend(send_chunk, chunk_count, datatype, 
                right, 11,
            ...
            MPIR_MV2_Reduce_local(comp_chunk, recv_chunk, (MPI_Aint) chunk_count,
                                  datatype, uop
#ifdef HAVE_CXX_BINDING
                                  , is_cxx_uop
#endif
        ...
        */
        ...
        for (i = 1; i < comm_size; i++) {
            ...
            recv_chunk = ((rank - (i-1) + comm_size)%comm_size)*chunk_size 
                + recvbuf;
            MPIR_PVAR_INC(allreduce, pt2pt_ring, recv, chunk_count, datatype);
            mpi_errno = MPID_Irecv(recv_chunk, chunk_count, datatype, 
                left, 11, comm_ptr, context_id, &recv_req_ptr);
            MPIR_ERR_CHECK(mpi_errno);

            send_chunk = ((rank - (i-2) + comm_size)%comm_size)*chunk_size 
                + recvbuf;
            MPIR_PVAR_INC(allreduce, pt2pt_ring, send, chunk_count, datatype);
                             ...
                             MPIR_Errflag_t *errflag)
{
    ...
    if (count % comm_size != 0 || sendbuf == MPI_IN_PLACE ||
            count < comm_size || !is_commutative) {
        mpi_errno = MPIR_Allreduce_pt2pt_rs_MV2(sendbuf, recvbuf, count, 
        datatype, op, comm_ptr, errflag);
        return mpi_errno;
    }
    MPIR_TIMER_START(coll,allreduce,pt2pt_ring_inplace);
    MPIR_T_PVAR_COUNTER_INC(MV2, mv2_coll_allreduce_pt2pt_ring_inplace, 1);
    //memset(recvbuf, 0, type_size*count);
...

    {
            ...
            and doing compute on chunk=%d \n",
                   i, ((rank - (i-1) + comm_size) % comm_size), right, ((rank -
                            i + comm_size) % comm_size), left, ((rank - i +
                                comm_size) % comm_size));
            */
            MPIR_PVAR_INC(allreduce, pt2pt_ring_inplace, recv, 
                chunk_count, datatype);
            mpi_errno = MPID_Irecv(recv_chunk, chunk_count, 
                datatype, left, 11, comm_ptr, context_id, &recv_req_ptr);
            MPIR_ERR_CHECK(mpi_errno);

            MPIR_PVAR_INC(allreduce, pt2pt_ring_inplace, send, 
                chunk_count, datatype);
            mpi_errno = MPID_Isend(send_chunk, chunk_count, datatype, right, 11,
                                   comm_ptr, context_id, &send_req_ptr);
            MPIR_ERR_CHECK(mpi_errno);
            ...
            /* now do the computation with some MPI_Test (TODO) in between */
            MPIR_MV2_Reduce_local(recv_chunk, comp_chunk, (MPI_Aint) chunk_count,
                                          datatype, uop
#ifdef HAVE_CXX_BINDING
                                          , is_cxx_uop
#endif
        ...
        */
        ...
        for (i = 1; i < comm_size; i++) {
            ...
            MPIR_PVAR_INC(allreduce, pt2pt_ring_inplace, recv, 
                chunk_count, datatype);
            mpi_errno = MPID_Irecv(recv_chunk, chunk_count, datatype, left, 
                11, comm_ptr, context_id, &recv_req_ptr);
            MPIR_ERR_CHECK(mpi_errno);
            ...
            MPIR_PVAR_INC(allreduce, pt2pt_ring_inplace, send, 
                chunk_count, datatype);
            mpi_errno = MPID_Isend(send_chunk, chunk_count, datatype, right, 
                11, comm_ptr, context_id, &send_req_ptr);
            MPIR_ERR_CHECK(mpi_errno);
...
int MPIR_Allreduce_pt2pt_ring_wrapper_MV2(const void *sendbuf, void *recvbuf,
        int count, MPI_Datatype datatype, MPI_Op op, MPIR_Comm * comm_ptr,
        MPIR_Errflag_t *errflag) {
    MPIR_TIMER_START(coll,allreduce,pt2pt_ring_wrapper);
    MPIR_T_PVAR_COUNTER_INC(MV2, mv2_coll_allreduce_pt2pt_ring_wrapper, 1);
    ...
    if (sendbuf != MPI_IN_PLACE)
        mpi_errno = MPIR_Allreduce_pt2pt_ring_MV2(sendbuf, recvbuf, 
            new_count, datatype, op, comm_ptr, errflag);
    else
        mpi_errno = MPIR_Allreduce_pt2pt_ring_inplace_MV2(sendbuf, recvbuf, 
            new_count, datatype, op, comm_ptr, errflag);
    ...
    if (sendbuf != MPI_IN_PLACE) {
        mpi_errno = MPIR_Allreduce_pt2pt_rs_MV2(sendbuf+new_count*sendtype_size,
                recvbuf+new_count*sendtype_size, remaining_count, datatype,
                op, comm_ptr, errflag);
        MPIR_TIMER_END(coll,allreduce,pt2pt_ring_wrapper);
        return mpi_errno;
    } else {
        mpi_errno = MPIR_Allreduce_pt2pt_rs_MV2(MPI_IN_PLACE,
                recvbuf+new_count*sendtype_size, remaining_count, datatype,
                op, comm_ptr, errflag);
        MPIR_TIMER_END(coll,allreduce,pt2pt_ring_wrapper);
        return mpi_errno;
                            ...
                            MPIR_Errflag_t *errflag)
{
    ...
    MPIR_T_PVAR_COUNTER_INC(MV2, 
        mv2_coll_allreduce_reduce_scatter_allgather_colls, 1);
    int comm_size, rank;
    int mpi_errno = MPI_SUCCESS;
    int i, *cnts, *disps;
    MPI_Aint true_lb, true_extent, extent;
    ...
    if (count < comm_size) {
        mpi_errno = MPIR_Allreduce_pt2pt_rs_MV2(sendbuf, recvbuf, 
...

    {
       ...
       MPIR_CHKLMEM_MALLOC(tmp_buf, void *, 
        ...
        /* adjust for potential negative lower bound in datatype */
       tmp_buf = (void *) ((char *) tmp_buf - true_lb);
       MPIR_CHKLMEM_MALLOC(disps, int *, comm_size * sizeof (int),
                           mpi_errno, "displacements", MPL_MEM_COLL);
       disps[0] = 0;
       for (i = 1; i < comm_size; i++) {
           disps[i] = disps[i - 1] + cnts[i - 1];
       ...
       if (sendbuf != MPI_IN_PLACE) {
            mpi_errno =
               MPIR_Reduce_scatter_MV2(sendbuf, tmp_recvbuf, 
                cnts, datatype, op, comm_ptr, errflag);
            MPIR_ERR_CHKANDJUMP((mpi_errno), mpi_errno, MPI_ERR_OTHER,
                                "**fail");
       } else {
            mpi_errno =
               MPIR_Reduce_scatter_MV2(MPI_IN_PLACE, recvbuf, cnts, 
                datatype, op, comm_ptr, errflag);
            MPIR_ERR_CHKANDJUMP((mpi_errno), mpi_errno, MPI_ERR_OTHER,
                                "**fail");
        ...
        if (count % comm_size == 0) {
            MPIR_Allgather_MV2(MPI_IN_PLACE, cnts[rank], datatype, recvbuf, 
                cnts[rank], datatype, comm_ptr, errflag);
       } else {
            MPIR_Allgatherv_MV2(MPI_IN_PLACE, cnts[rank], datatype, recvbuf, 
