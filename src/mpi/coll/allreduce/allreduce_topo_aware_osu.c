...
extern unsigned long long 
    PVAR_COUNTER_mv2_coll_allreduce_topo_aware_hierarchical;
                                          ...
                                          MPIR_Errflag_t *errflag)
{
    ...
    MPIR_T_PVAR_COUNTER_INC(MV2, mv2_coll_allreduce_topo_aware_hierarchical, 1);
    ...
    if (local_rank == 0 && leader_commptr->local_size > 1) {
        ...
        if (in_buf == sendbuf) {
            ...
            mpi_errno = 
                MPIR_Allreduce_topo_aware_inter_node_helper_MV2(MPI_IN_PLACE,
    ...
    for (i = mv2_num_intra_node_comm_levels; i >= 0; --i) {
        MPIR_Comm_get_ptr(shmem_commptr->dev.ch.topo_comm[i], topo_comm_ptr);
        if (topo_comm_ptr && topo_comm_ptr->local_size > 1) {
            mpi_errno = MPIR_Shmem_Bcast_MV2(in_buf, count, datatype, 0, 
