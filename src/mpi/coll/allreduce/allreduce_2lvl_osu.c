...
extern MPIR_T_pvar_timer_t PVAR_TIMER_mv2_coll_timer_allreduce_shm_intra;
extern MPIR_T_pvar_timer_t PVAR_TIMER_mv2_coll_timer_allreduce_intra_p2p;
extern MPIR_T_pvar_timer_t PVAR_TIMER_mv2_coll_timer_allreduce_2lvl;

extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_shm_intra;
extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_intra_p2p;
extern unsigned long long PVAR_COUNTER_mv2_coll_allreduce_2lvl;

int (*MV2_Allreduce_function)(const void *sendbuf,
                             void *recvbuf,
                             int count,
                             MPI_Datatype datatype,
                             MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t
*errflag)=NULL;
                             ...
                             MPIR_Errflag_t *errflag)
{
    ...
    if (count * (MPL_MAX(extent, true_extent))
        >= mv2_g_shmem_coll_max_msg_size) {
                mpi_errno =
                    MPIR_Reduce_intra_smp(sendbuf, recvbuf, 
                    count, datatype, op, 0,
    ...
    if (local_rank == 0) {
        ...
        if (local_size > 1) {
            ...
            MPIR_MV2_SHMEM_COLL_GetShmemBuf(local_size, local_rank,
                                              shmem_comm_rank,
                                              (void *) &shmem_buf);
            if (is_commutative) {
                ...
                MPIR_MV2_SHMEM_COLL_SetGatherComplete(local_size,
                                                        local_rank,
                                                        shmem_comm_rank);
            }
        }
        ...
        MPIR_ERR_CHKANDJUMP((mpi_errno), mpi_errno, MPI_ERR_OTHER,
                            "**fail");
        MPIR_MV2_SHMEM_COLL_SetGatherComplete(local_size, local_rank,
                                                    shmem_comm_rank);
    }
                             ...
                             MPIR_Errflag_t *errflag)
{
    ...
    if (local_rank == 0) {
        ...
        || (MV2_Allreduce_intra_function == &MPIR_Allreduce_reduce_p2p_MV2) ) {
            mpi_errno =
                MV2_Allreduce_intra_function(sendbuf, recvbuf, 
                                        count, datatype,
                                        op, comm_ptr, errflag);
        }
        else {
        ...
        if (local_size != total_size) {
            ...
            {
                mpi_errno =
                    MPIR_Allreduce_pt2pt_rs_MV2(MPI_IN_PLACE, recvbuf, count, 
                    datatype, op, leader_commptr, errflag);
            }
    ...
    mpi_errno = MPIR_Shmem_Bcast_MV2(recvbuf, count, 
                    datatype, 0, shmem_commptr, errflag);
    if (mpi_errno) {
        /* for communication errors, just record the error but continue */
        *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
        MPIR_ERR_SET(mpi_errno, MPI_ERR_OTHER, "**fail");
