...
int MPIR_Reduce_tree_shmem_MV2_optrels(const void *sendbuf, void *recvbuf,
                                       ...
                                       MPIR_Errflag_t *errflag)
{
    ...
    if (sendbuf != MPI_IN_PLACE && (local_rank<mv2_use_shmem_num_trees)) {
        ...
        if (sendbuf == recvbuf || recvbuf == NULL) {
            tmp_buf = recvbuf;
            recvbuf = MPL_malloc(count * MPL_MAX(extent, true_extent),
                                    MPL_MEM_COLL);
            buf_allocated = 1;
        }
    ...
    if(local_rank<mv2_use_shmem_num_trees) {
        for (i = local_rank+mv2_use_shmem_num_trees; i < local_size;
                    i=i+mv2_use_shmem_num_trees) {
            MPIR_MV2_SHMEM_TREE_COLL_GetShmemBuf_optrels(local_size,
                                        local_rank, shmem_comm_rank, i,
                                        (void *)&shmem_buf, local_rank);
            ...
            MPIR_MV2_SHMEM_TREE_COLL_SetGatherComplete_optrels(local_size,
                                    local_rank, shmem_comm_rank, i, local_rank);
        }
    } else {
        MPIR_MV2_SHMEM_TREE_COLL_GetShmemBuf_optrels(local_size, local_rank,
                                                       shmem_comm_rank, 0,
                                                       (void *)&shmem_buf,
                                                       parent);
        ...
        MPIR_ERR_CHECK(mpi_errno);
        MPIR_MV2_SHMEM_TREE_COLL_SetGatherComplete_optrels(local_size,
                                        local_rank, shmem_comm_rank, 0, parent);
    }
    ...
    if (local_rank==0) {
        for (i = 1;i < mv2_use_shmem_num_trees; i++) {
            ...
            MPIR_MV2_SHMEM_TREE_COLL_SetGatherComplete_optrels(local_size,
                                            local_rank, shmem_comm_rank, i, 0);
        }
        ...
        MPIR_ERR_CHECK(mpi_errno);
        MPIR_MV2_SHMEM_TREE_COLL_SetGatherComplete_optrels(local_size,
                                             local_rank, shmem_comm_rank, 0, 0);
    }
...
int MPIR_Reduce_shmem_MV2_optrels(const void *sendbuf, void *recvbuf, int count,
                                  ...
                                  MPIR_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (local_rank == 0) {
        ...
        for (i = 1; i < local_size; i++) {
            MPIR_MV2_SHMEM_COLL_GetShmemBuf_optrels(local_size, local_rank,
            ...
            MPIR_MV2_SHMEM_COLL_SetGatherComplete_optrels(local_size,
                    local_rank, shmem_comm_rank, i);
        }
    } else {
        MPIR_MV2_SHMEM_COLL_GetShmemBuf_optrels(local_size, local_rank,
        ...
        MPIR_ERR_CHECK(mpi_errno);
        MPIR_MV2_SHMEM_COLL_SetGatherComplete_optrels(local_size, local_rank,
                shmem_comm_rank, 0);
    }
