...
{                                                                           \
    ...
    mv2_##_cname##_indexed_thresholds_table = MPL_malloc(                   \
        sizeof(mv2_##_cname##_indexed_tuning_table *) * nconf,              \
                MPL_MEM_COLL);                                              \
    table_ptrs = MPL_malloc(                                                \
        sizeof(mv2_##_cname##_indexed_tuning_table *) * nconf,              \
                MPL_MEM_COLL);                                              \
    mv2_size_##_cname##_indexed_tuning_table = MPL_malloc(                  \
            sizeof(int) * nconf, MPL_MEM_COLL);                             \
    mv2_##_cname##_indexed_table_ppn_conf = MPL_malloc(                     \
            sizeof(int) * nconf, MPL_MEM_COLL);
...
#define MV2_COLL_TUNING_FINISH_TABLE(_cname)                        \
    ...
    mv2_##_cname##_indexed_thresholds_table[0] = MPL_malloc(       \
        sizeof (mv2_##_cname##_indexed_tuning_table) *              \
        agg_table_sum, MPL_MEM_COLL);                               \
    MPIR_Memcpy(mv2_##_cname##_indexed_thresholds_table[0],         \
        table_ptrs[0],                                              \
        sizeof(mv2_##_cname##_indexed_tuning_table) *               \
    ...
    for (i = 1; i < nconf; i++) {                                   \
        ...
        MPIR_Memcpy(mv2_##_cname##_indexed_thresholds_table[i],     \
            table_ptrs[i],                                          \
            sizeof(mv2_##_cname##_indexed_tuning_table) *           \
            mv2_size_##_cname##_indexed_tuning_table[i]);           \
    }                                                               \
