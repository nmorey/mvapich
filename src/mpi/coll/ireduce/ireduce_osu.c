...
extern unsigned long long PVAR_COUNTER_mv2_coll_ireduce_sharp;
extern MPIR_T_pvar_timer_t PVAR_COUNTER_mv2_coll_timer_ireduce_sharp;
                             ...
                             MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if ((MV2_Ireduce_function == MPIR_Ireduce_intra_sched_reduce_scatter_gather) &&
        (HANDLE_GET_KIND(op) == HANDLE_KIND_BUILTIN) && (count >= pof2))
    {
        mpi_errno = MPIR_Ireduce_intra_sched_reduce_scatter_gather(sendbuf,
                        recvbuf, count, datatype, op, root, comm_ptr, s);
    }
    else {
