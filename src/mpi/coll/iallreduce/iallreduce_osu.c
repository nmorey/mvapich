...
extern unsigned long long PVAR_COUNTER_mv2_coll_iallreduce_sharp;
...
int MPIR_Sharp_Iallreduce_MV2 (const void *sendbuf, void *recvbuf, int count,
                        MPI_Datatype datatype, MPI_Op op, MPIR_Comm * comm_ptr, int *errflag, MPIR_Request **req)
{
    ...
    if (is_contig == 1) {
        ...
        if (sendbuf != MPI_IN_PLACE) {
            ...
            is_inplace = 1;
            reduce_spec.sbuf_desc.buffer.ptr    = MPL_malloc(reduce_spec.sbuf_desc.buffer.length);
            MPIR_Memcpy(reduce_spec.sbuf_desc.buffer.ptr, recvbuf, reduce_spec.sbuf_desc.buffer.length);
        }
				    ...
				    MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (MV2_Iallreduce_function == 
        ...
        if (count >= pof2) {
            ...
            mpi_errno = MPIR_Iallreduce_intra_sched_naive(sendbuf, recvbuf,
                                                count, datatype,
                                                op, comm_ptr, s);
        }
    }
