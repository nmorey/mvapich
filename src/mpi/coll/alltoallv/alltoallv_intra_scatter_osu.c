                             ...
                             MPIR_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (sendbuf == MPI_IN_PLACE) {
        ...
        sendbuf_tmp = (char *)MPL_malloc(total_count *
                                        (MPL_MAX(recv_extent, true_extent)),
                                        MPL_MEM_COLL);
    ...
    MPIR_CHKLMEM_MALLOC(sreqarray, MPIR_Request **, 
                        bblock*sizeof(MPIR_Request*),
                        mpi_errno, "sreqarray", MPL_MEM_COLL);

    MPIR_CHKLMEM_MALLOC(sstarray, MPI_Status *, 
                        bblock*sizeof(MPI_Status),
                        mpi_errno, "sstarray", MPL_MEM_COLL);

    MPIR_CHKLMEM_MALLOC(rreqarray, MPIR_Request **, 
                        bblock*sizeof(MPIR_Request*),
                        mpi_errno, "rreqarray", MPL_MEM_COLL);

    MPIR_CHKLMEM_MALLOC(rstarray, MPI_Status *, 
                        bblock*sizeof(MPI_Status),
                        mpi_errno, "rstarray", MPL_MEM_COLL);

    mpi_errno = MPIR_Localcopy(((char *) sendbuf_tmp 
                                    + sdispls_tmp[rank] * send_extent),
    ...
    for (i = 0, ii=0; ii<comm_size; ii+=bblock) {
        ...
        for ( i=0; i<ss; i++ ) {
            ...
            MPIR_PVAR_INC(alltoallv, intra_scatter, recv, 
                         recvcnts[src], recvtype);
