 ...
 * a base case */
int MPIR_Allgather_intra_osu_direct(const void *sendbuf, MPI_Aint sendcnt,
                                    ...
                                    MPIR_Comm *comm_ptr, MPIR_Errflag_t errflag)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        MPIR_T_MVP_PVAR_COLL_SET_INC(allgather, direct, recv, recvcnt,
                                     recvtype);
    ...
    for (i = 0; i < size; i++) {
        ...
        MPIR_T_MVP_PVAR_COLL_SET_INC(allgather, direct, send, scnt, stype);
...
int MPIR_Allgather_intra_osu_direct_spread(
    const void *sendbuf, MPI_Aint sendcnt, MPI_Datatype sendtype, void *recvbuf,
    MPI_Aint recvcnt, MPI_Datatype recvtype, MPIR_Comm *comm_ptr,
    MPIR_Errflag_t errflag)
{
    ...
    for (i = 1; i < size; i++) {
        ...
        /* post receive */
        MPIR_T_MVP_PVAR_COLL_SET_INC(allgather, directspread, recv, recvcnt,
                                     recvtype);
    ...
    for (i = 1; i < size; i++) {
        ...
        MPIR_T_MVP_PVAR_COLL_SET_INC(allgather, directspread, send, scnt,
                                     stype);
