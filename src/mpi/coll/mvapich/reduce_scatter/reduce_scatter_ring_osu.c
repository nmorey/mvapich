...
int MPIR_Reduce_scatter_intra_osu_ring(const void *sendbuf, void *recvbuf,
                                       ...
                                       MPIR_Errflag_t errflag)
{
    ...
    int i;
    size_t max_elem_per_rank, elem_per_buffer;
    size_t nread = 0;
    ...
    rank_left = (rank - 1 + comm_size) % comm_size;
    rank_right = (rank + 1 + comm_size) % comm_size;
    ...
    max_elem_per_rank = (size_t)max_recv_count;

    /* compute number of whole elements that can fit in the buffer */
    elem_per_buffer = mpi_buf_size / true_extent;
    ...
    while (nread < max_elem_per_rank) {
        ...
        for (dist = comm_size - 1; dist >= 0; dist--) {
            ...
            send_count = send_count < 0 ? 0 : send_count;
            send_count =
                send_count > elem_per_buffer ? elem_per_buffer : send_count;
            recv_count = recv_count < 0 ? 0 : recv_count;
            recv_count =
                recv_count > elem_per_buffer ? elem_per_buffer : recv_count;
            ...
            if (dist > 0) {
                MPIR_T_MVP_PVAR_COLL_SET_INC(reduce_scatter, ring, send,
                                             send_count, datatype);
                MPIR_T_MVP_PVAR_COLL_SET_INC(reduce_scatter, ring, recv,
                                             recv_count, datatype);
