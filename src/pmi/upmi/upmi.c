...
void UPMI_lock_init(void) { pthread_mutex_init(&upmi_lock, NULL); }

void UPMI_lock_destroy(void) { pthread_mutex_destroy(&upmi_lock); }

void UPMI_lock(void) { pthread_mutex_lock(&upmi_lock); }

void UPMI_unlock(void) { pthread_mutex_unlock(&upmi_lock); }
                 ...
                 int length)
{
    ...
    int vallen;
    pmi_ret_val =
        PMI2_KVS_Get(kvsname, PMI2_ID_NULL, key, value, length, &vallen);
#else
    UPMI_lock();
...
#elif USE_PMI2_API
    pmi_ret_val =
        PMI2_Nameserv_lookup(service_name, info_ptr, port, sizeof port);
#else
    UPMI_lock();
...
#elif USE_PMI2_API
    pmi_ret_val = PMI2_Job_Spawn(
        count, cmds, argcs, argvs, maxprocs, info_keyval_sizes,
        (const struct MPID_Info **)info_keyval_vectors, preput_keyval_size,
        (const struct MPID_Info **)preput_keyval_vector, jobId, jobIdSize,
        errors);
#else
    UPMI_lock();
    pmi_ret_val = PMI_Spawn_multiple(
        count, cmds, argvs, maxprocs, info_keyval_sizes,
        (const struct PMI_keyval_t **)info_keyval_vectors, preput_keyval_size,
        (const struct PMI_keyval_t *)preput_keyval_vector, errors);
    UPMI_unlock();
#endif
    return pmi_ret_val;
}
...
int UPMI_GET_LOCAL_SIZE(int *size)
{
    ...
    PMIX_INFO_LOAD(&info[0], PMIX_OPTIONAL, &val_optional, PMIX_BOOL);
    if (PMIX_SUCCESS !=
        (rc = PMIx_Get(&proc, PMIX_LOCAL_SIZE, NULL, 0, &val))) {
        return convert_err(rc);
    }
    *size = val->data.uint32;
    return convert_err(rc);
