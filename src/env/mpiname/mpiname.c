...
{
    printf("Usage: [OPTION]...\n");
    printf("Print MPI library information.  With no OPTION, the output is the "
           "same as -v.\n\n");
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "acdhnorv")) != -1) {
        switch (c) {
            case 'a':
                s_lineElements = PRINT_DEVICE | PRINT_NAME |
                                 PRINT_RELEASE_DATE | PRINT_VERSION;
    ...
    if (s_stackElements & PRINT_COMPILERS) {
        s_stackElements ^= PRINT_COMPILERS;
        printf("Compilation\nCC: %s\nCXX: %s\nF77: %s\nFC: %s\n\n", MPINAME_CC,
               MPINAME_CXX, MPINAME_F77, MPINAME_FC);
    }
