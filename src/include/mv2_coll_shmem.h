...
#ifndef _MV2_COLL_SHMEM_
#define _MV2_COLL_SHMEM_
...
#define SHMEM_COLL_BLOCK_SIZE (MPIR_Process.local_size * mv2_g_shmem_coll_max_msg_size)
...
#endif /* defined(CHANNEL_MRAIL_GEN2) || defined(_MV2_CH4_OVERRIDE_) */


/* SHEMEM_COLL functions */
/* TODO: most of these are not MPIDI_CH3I_ functions and should be moved to the
 * MPIR_MV2_ or MPID_ namespace as they are device independent or device level. 
 *
...
void MPIR_MV2_SHMEM_COLL_Unlink(void);
void MPIR_MV2_SHMEM_COLL_Barrier_gather(int, int, int);
void MPIR_MV2_SHMEM_COLL_Barrier_bcast(int, int, int);
...
void MPIR_MV2_SHMEM_COLL_SetGatherComplete(int size, int rank,
                                             int shmem_comm_rank);
void MPIR_MV2_SHMEM_COLL_GetShmemBuf(int, int, int, void**);
void MPIR_MV2_SHMEM_COLL_SetGatherComplete(int, int, int);
void MPIR_MV2_SHMEM_COLL_GetShmemBuf_optrels(int, int, int, int, void**);
void MPIR_MV2_SHMEM_COLL_SetGatherComplete_optrels(int, int, int, int);
void MPIR_MV2_SHMEM_TREE_COLL_GetShmemBuf_optrels(int, int, int, int, void**, int);
void MPIR_MV2_SHMEM_TREE_COLL_SetGatherComplete_optrels(int, int, int, int, int);
void MPIR_MV2_SHMEM_Bcast_GetBuf(int, int, int, void**);
void MPIR_MV2_SHMEM_Bcast_Complete(int ,int , int);
void MPIR_MV2_SHMEM_Coll_Block_Clear_Status(int block_id); 
int MPIR_MV2_SHMEM_Coll_get_free_block();
/* TODO - the below function should ideally be in a header for SMP collective operations.
 * This is relevant when multiple intra-node channels exist. Leaving this here for now*/
...
void mv2_shm_reduce(shmem_info_t *shmem, char *buf, int len, 
                    int count, int root, MPI_User_function *uop,
...
int MPIR_MV2_SMP_COLL_init(void);
