...
hash_table_t *create_hash_table(int size, char *name)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        MVP_INIT_LIST_HEAD(&(ht->buckets[i].list));
    }
...
void dump_hash_table(hash_table_t *ht)
{
    ...
    for (i = 0; i < ht->length; i++) {
        ...
        if (mvp_list_empty(&bkt->list))
            continue;

        mvp_list_for_each_entry(cfile, &(bkt->list), list)
        {
            if (flag) {
                printf("================   bucket %d:\n", i);
                flag = 0;
            ...
            // printf("\tckpt-%d-rank-%d : chunk_num %d, chunk_to_write %d,
            // has_last_chunk %d\n",
...
#else
ckpt_file_t *hash_table_create_record(hash_table_t *ht, int ckpt_id,
                                      int proc_rank)
#endif
{
#if HASH_FROM_FILENAME
    ...
    {
#if HASH_FROM_FILENAME
        if (cfile->filename_len == namelen &&
            strncmp(cfile->filename, fname, namelen) == 0)
#else
    ...
    mvp_list_add_tail(&cfile->list, &bkt->list);
...
NOTE::: caller should be serialized , so be called by the IB-thread.  IB thread
poll the RQ, and for each chunk-RR-rqst, add a reference to the ckpt-file, then
add a work-item to the work-queue. The io-thread-pool then fetch work-item from
the work-queue, and deals with each chunk
*/
#if HASH_FROM_FILENAME
ckpt_file_t *hash_table_get_record(hash_table_t *ht, const char *fname,
                                   int is_last_chunk)
#else
ckpt_file_t *hash_table_get_record(hash_table_t *ht, int ckpt_id, int proc_rank,
                                   int is_last_chunk)
#endif
{
#if HASH_FROM_FILENAME
...
/*
find an entry matching (ckpt id, proc rank), decrease its reference
"chunk_to_write" for this entry If all chunks belonging to this entry have
finished, including the last_chunk, then remove this entry from hash table
Input:
    is_release:  0: called from io-thr; 1: called by fuse-release to close the
file Return: 0:  after deref, some other chunks remains to be written, or the
last-chunk hasn't been written 1:  all chunks of this ckpt-file have been
written. Have released this record
*/
...
int hash_table_put_record(hash_table_t *ht, ckpt_file_t *cfile, int is_release)
{
    ...
    dbg("Enter:: cfile %s: ref = %d, fd=%d, can-rel=%d, has-last-chunk=%d\n",
        cfile->filename, atomic_read(&cfile->ref), cfile->fd,
        cfile->can_release, cfile->has_last_chunk);

    if (atomic_read(&cfile->ref) > 0)
        atomic_dec(&cfile->ref);

    if (atomic_read(&cfile->ref) >
        0) //! atomic_dec_and_test( &cfile->ref) )  // ref > 0
    {      // the ref not zero
        // if( !is_release ) // calling from io-thr
        // cfile->rcv_pending_RR_num--;
        // dbg("cfile (%d, %d) ref = %d, still active\n", cfile->ckpt_id,
        // cfile->proc_rank, cfile->ref );
        pthread_mutex_unlock(&cfile->mutex);
        return 0;
    }
    ...
    // if(!cfile->can_release && !cfile->has_last_chunk ) /// (can-rel=0,
    // last-chunk=0)
    if (!cfile->can_release || // user hasn't call close()
        (cfile->adv_size > 0 &&
         !cfile->has_last_chunk) // user has posted some data to file, but
                                 // hasn't finished writing
        )                        ///// (can-rel=0, last-chunk=0)
    {
        dbg("can't rel(%s):: cfile->adv_size=%d, cfile->can_rel=%d, "
            "cfile->has_last_chk=%d\n",
            cfile->filename, cfile->adv_size, cfile->can_release,
            cfile->has_last_chunk);
        pthread_mutex_unlock(&cfile->mutex);
        return 0;
    }
    /// Has finished all data, but OS still has reference  (can-rel=0,
    /// last-chunk=1)
    // can free all data, close the "fd", but keep the "cfile" struct
    if (!cfile->can_release) {
        if (!mvp_list_empty(&cfile->chunk_list)) {
            error("free cfile: ckpt-%d-proc-%d: chunk-list not empty!!!\n",
                  cfile->ckpt_id, cfile->proc_rank);
            ckpt_chunk_t *ck;
            struct list_head *pos, *n;
            mvp_list_for_each_safe(pos, n, &(cfile->chunk_list))
            {
                ck = mvp_list_entry(pos, ckpt_chunk_t, list);
    ...
    mvp_list_del(&cfile->list);

    /// free the ckpt-file record
    dbg("\n*** Now, free -- (%s)(%d,%d) : total chunk_num %d, pending chunks "
        "%d, has_last_chunk %d, adv-size=%d\n\n",
        cfile->filename, cfile->ckpt_id, cfile->proc_rank,
        cfile->rcv_total_chunk_num, cfile->rcv_pending_RR_num,
        cfile->has_last_chunk, cfile->adv_size);
...
#else
    unsigned long x =
        ((unsigned long)cfile->ckpt_id) * 0x9e370001UL + cfile->proc_rank;
#endif
    ...
    ////////////////////////
    /////??? when is a ckpt-file finished: has_last_chunk=1 &&
    /// rcv_pending_RR_num==0 && rcv_ready_chunk_num==0
    pthread_mutex_lock(&bkt->mutex);
    if (atomic_dec_and_test(
            &cfile->chunk_to_write) // true if: the counter becoems 0
        && cfile->has_last_chunk) {
        mvp_list_del(&cfile->list);
        /// unlock
        pthread_mutex_unlock(&bkt->mutex);

        printf("\tRelease ckpt-%d-rank-%d : chunk_num %d, chunk_to_write %d, "
               "has_last_chunk %d, size=%ld\n",
               cfile->ckpt_id, cfile->proc_rank, cfile->chunk_num,
               atomic_read(&cfile->chunk_to_write), cfile->has_last_chunk,
               cfile->write_size);
        /// close the ckpt-file
        if (cfile->fd) {
            fsync(cfile->fd);
...
if the record with given file-name exists, lock the bucket and return a pointer
of the bucket; if the record with (fname) doesn't exist, unlock the bucket and
return NULL;
**/
hash_bucket_t *htbl_find_lock_bucket(hash_table_t *ht, const char *fname,
                                     int namelen)
{
#if HASH_FROM_FILENAME
