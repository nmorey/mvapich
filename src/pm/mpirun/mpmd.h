...
/*
 * Save the list of specification read in the configuration file in the plist
 * used by mpirun_rsh.
 */
process *save_plist(config_spec_t *cfg_list, int nprocs);
/**
 * Add to the argument passed to mpispawn the name of the executable and its
 * arguments.
 *
 */
...
/**
 * Add to the argument passed to mpispawn the name of the executable and its
 * arguments. compatible with srun based launch
 */
char *add_srun_argv(char *mpispawn_env, char *exe, char *args, int tmp_i);

/**
 * Count the number of arguments of an executable. In mpirun_rsh we need this
 * information in order to send it to mpispawn.
 */
