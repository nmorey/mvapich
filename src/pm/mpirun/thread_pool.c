...
// struct thread_pool*  tp_init_thread_pool(struct thread_pool* pool,  int
// numthr, void*(func)(void*), char* name )
struct thread_pool *tp_create_thread_pool(int numthr, void *(func)(void *),
                                          char *name)
{
    ...
    /// now bitmap is no longer needed in a thread-pool, replaced by a
    /// work-queue
                                                     ...
                                                     // )
{
    ...
    /*    /// tell all threads to exit
        for(i=0; i<pool->num_threads; i++){
            pool->arg1[i] = arg_invalid;  // pass an invalid args, tell thread i
       to terminate sem_post( &pool->sem[i] );    // awake thread i
            //dbg(" resume thread %d\n", i);
        }
    */
