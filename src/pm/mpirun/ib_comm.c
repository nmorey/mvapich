...
static inline int find_qp_from_conn_array(int qpn, struct ib_connection *conn,
                                          int num_con, int *conidx, int *qpidx)
{
...
int put_HCA(struct ib_HCA *hca)
{
    if (atomic_sub_and_test(
            1, &hca->ref)) { // no connection is referring it, release the HCA
    }
    return 0;
}
...
int ib_HCA_init(struct ib_HCA *hca, int is_srv)
{
    ...
    ret = mvp_mpirun_ibv_dlopen_init();
    ...
    dev_list = mpirun_ibv_ops.get_device_list(
        &ret); // has "ret" num of devices in the system
    if (!dev_list) {
    ...
    {
        if (strcmp(g_devname, mpirun_ibv_ops.get_device_name(dev_list[i])) ==
            0) {
    ...
    /*    g_device = ret>1? g_device: 0;
        ...
        if( ! hca->context ){
            ...
            mpirun_ibv_ops.free_device_list( dev_list ); // free all other
       devices goto err_out_1;
        }    */
    ...
    ret = fcntl(hca->comp_channel->fd, F_SETFL,
                i | O_NONBLOCK); // set the fd to be non-blocking
    if (ret < 0) {
    ...
    // get CQ
    hca->cq = mpirun_ibv_ops.create_cq(
        hca->context, hca->rx_depth + 1, // rx_depth+1,  /* CQE depth */
    ...
    struct ibv_srq_init_attr srqattr = {
        .attr = {.max_wr = g_rx_depth, .max_sge = 1}};
    ...
    if (ib_connection_buf_init(hca, sendbuf_size, recvbuf_size, rdmabufsize) !=
        0) {
...
void ib_HCA_destroy(struct ib_HCA *hca)
{
    ...
    mvp_mpirun_ibv_dlopen_finalize();
}
...
*/
int ib_connection_init_1(struct ib_connection *conn, int num_qp,
                         struct ib_HCA *hca) // , int rx_depth)
{
    ...
    for (i = 0; i < conn->num_qp; i++) {
        ...
        qp_init_attr.cap.max_send_wr = 64, // 4,
            qp_init_attr.cap.max_recv_wr = hca->rx_depth,
        qp_init_attr.cap.max_send_sge = 1, qp_init_attr.cap.max_recv_sge = 1;
...
int ib_connection_init_2(struct ib_connection *conn)
{
    ...
    for (i = 0; i < conn->num_qp; i++) {
        ...
        qp_attr.rq_psn =
            conn->remote_ep[i].psn; ///***** RQ-psn, match remote send-psn
        qp_attr.max_dest_rd_atomic =
            4; // num of outstanding "inbound" rdma-read / atomic, as responder
        ...
        printf("  local QP %d (qpn %x) match remote QP:  ", i,
               conn->qp[i]->qp_num);
    ...
    for (i = 0; i < conn->num_qp; i++) {
        ...
        qp_attr.max_rd_atomic =
            4; // max outstanding rdma-read/ atomic, as initiator
...
*/
// int   ib_connection_buf_init(struct ib_connection* conn, int sendsize, int
// recvsize, int rdmasize)
int ib_connection_buf_init(struct ib_HCA *hca, int sendsize, int recvsize,
                           int rdmasize)
{
    int slot_size =
        sendslot_size; // 64; // max 256 slots, so max size = 64 * 256 = 16 KB

    hca->send_buf = create_ib_buffer(sendsize, slot_size, "send-buf");
    hca->send_buf->mr = mpirun_ibv_ops.reg_mr(hca->pd, hca->send_buf->addr,
                                              sendsize, IBV_ACCESS_LOCAL_WRITE);
    ...
    hca->recv_buf->mr = mpirun_ibv_ops.reg_mr(hca->pd, hca->recv_buf->addr,
                                              recvsize, IBV_ACCESS_LOCAL_WRITE);
    // dbg("recv-buf: addr=%p, size=%d, mr=%p\n", hca->recv_buf->addr,
    //       recvsize, hca->recv_buf->mr );

    /// max num of RQ WQE, determined by recv-buf size
    hca->max_rq_wqe = (hca->recv_buf->num_slot < hca->rx_depth) ?
                          hca->recv_buf->num_slot :
                          hca->rx_depth;

    slot_size =
        rdmaslot_size; // 1*1024*1024; // 1M per slot in RDMA buf. max 256 slots
    if (rdmasize > 0) {
        hca->rdma_buf = create_ib_buffer(rdmasize, slot_size, "rdma-buf");
        hca->rdma_buf->mr = mpirun_ibv_ops.reg_mr(
            hca->pd, hca->rdma_buf->addr, rdmasize,
            IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE |
                IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_ATOMIC);
        printf("%s: Create RDMA-buf: size=%d, slotsize=%d\n", __func__,
               rdmasize, rdmaslot_size);
...
/*
post a send-req to qp[idx], the buf to send is at send_buf[slot], with
size="size"
*/
int ib_connection_post_send(struct ib_connection *conn, int qp_index,
                            int sbuf_slot, int size)
{
    struct ib_HCA *hca = conn->hca;

    /// ibv_send:  wc->wr_id =   connection-id | qp-id | buf-slot-id
    struct ibv_sge list = {
        .addr = (uint64_t)(hca->send_buf->addr +
                           sbuf_slot * hca->send_buf->slot_size),
        .length = size,
    ...
    //  dbg("post-send at qp %d(qpn %x), send slot %d, size %d, msg=\"%s\", ret
    //  %d\n",
    //      qp_index, conn->qp[qp_index]->qp_num, sbuf_slot,  size,
    //      ib_buffer_slot_addr( hca->send_buf, sbuf_slot),  i );
...
*/
int ib_connection_post_RR(struct ib_connection *conn, int qpidx,
                          ib_packet_t *rrpkt)
{
    struct ib_HCA *hca = conn->hca;
    ...
    struct ibv_sge list = {
        .addr =
            (uint64_t)addr, // fetch remote data, store the data to this "addr"
    ...
    dbg("post RR: qp %d(qpn %x) (%s) %ld@%ld, lbuf-slot %d, rmt-addr %p, "
        "rmt-bufid %d, rkey 0x%x, ret %d\n",
        qpidx, conn->qp[qpidx]->qp_num, rrpkt->RR.filename, rrpkt->RR.size,
        rrpkt->RR.offset, slot, rrpkt->RR.raddr, rrpkt->RR.rbuf_id,
        rrpkt->RR.rkey, i);
...
*/
int ib_connection_send(struct ib_connection *conn, int qpidx, void *buf,
                       int size)
{
    struct ib_HCA *hca = conn->hca;
    void *addr;
    int i =
        get_buf_slot(hca->send_buf, &addr, 0); // addr now points to slot "i"

    memcpy(addr, buf,
           size < hca->send_buf->slot_size ? size : hca->send_buf->slot_size);
...
*/
int ib_connection_post_recv(struct ib_connection *conn, int qpidx,
                            int rbuf_slot, int size)
{
    struct ib_HCA *hca = conn->hca;

    struct ibv_sge list = {
        .addr = (uint64_t)(hca->recv_buf->addr +
                           rbuf_slot * hca->recv_buf->slot_size),
        .length = size,
...
*/
int ib_connection_post_srq_recv(struct ib_HCA *hca, int qpidx, int rbuf_slot,
                                int size)
{
    struct ibv_sge list = {
        .addr = (uint64_t)(hca->recv_buf->addr +
...
int ib_connection_fillup_srq(struct ib_HCA *hca) // struct ib_connection* conn)
{
    ...
    if (hca->rq_wqe <= 20) {
        ...
        for (i = 0; i < fill; i++) {
            ib_connection_post_srq_recv(hca, 0, slot % hca->max_rq_wqe,
                                        hca->recv_buf->slot_size);
            slot++;
        }
...
int ib_connection_send_RR_rqst(struct ib_connection *conn, int qpidx,
                               int rbufid, int rprocid, int rckptid, int size,
                               int offset, int is_last_chunk)
{
    struct ib_HCA *hca = conn->hca;
    ...
    // sprintf((void*)pkt->RR.raddr, " raddr=%p, rbufid=%d, rkey=0x%x,
    // size=%d\n", pkt->RR.raddr,
    ...
    dbg(" rqst RR of rdma-bufid %d= %s, free-slots=%d\n", pkt->RR.rbuf_id,
        pkt->RR.raddr, hca->rdma_buf->free_slots);
...
int ib_connection_send_chunk_RR_rqst(struct ib_connection *conn,
                                     ckpt_file_t *cfile, ckpt_chunk_t *chunk,
                                     void *arg)
{
    struct ib_HCA *hca = conn->hca;
    struct ib_packet *pkt;
    ...
    if (chunk->curr_pos > 0)
        pkt->RR.raddr =
            (uint64_t)ib_buffer_slot_addr(hca->rdma_buf, chunk->bufid);
    else
        pkt->RR.raddr = 0;
    ...
    dbg("send chunk-RR: (%s) %ld@%ld,  rdma-bufid %d,free-slots=%d\n",
        cfile->filename, chunk->curr_pos, chunk->offset, chunk->bufid,
        hca->rdma_buf->free_slots);
...
*/
int exam_cqe(struct ib_connection *conn, int qpidx, struct ibv_wc *wc,
             int server, struct thread_pool *tpool)
{
    struct ib_HCA *hca = conn->hca;
    uint64_t arg1, arg2;
    ...
    switch (wc->opcode) {
        ...
        case IBV_WC_RDMA_READ: // finished a RDMA-READ, only on server
            ...
            dbg("finished RDMA-READ (%s) %ld@%ld,rbufid=%d, lbufid=%d\n", //
                pkt->RR.filename, pkt->RR.size, pkt->RR.offset, pkt->RR.rbuf_id,
                pkt->RR.lbuf_id); // pkt->RR.laddr );
            // dump_ib_packet(pkt);

            /// wake up the iothread who is waiting for this RR to complete
            sem_t *sem = (sem_t *)pkt->RR.larg1;
            if (sem)
                sem_post(sem); // wake up the iothread, who is waiting for the
                               // RR to complete
            else {
                error(" Error:: RR-WC, but sem invalid!! rbuf-id=%d, "
                      "lbuf-id=%d\n",
        ...
        case IBV_WC_RECV: // receive an inbound message
            ...
            if (is_server(server)) {
                ...
                if (pkt->command == rqst_terminate) {
                    ...
                    /* has gotten a RR rqst, add a ref to a record about this
                       RR-ckpt-file into a hash table, then add one
                       work-item(for one chunk) to work-queue. One io-thread
                       will wake up and fetch this work-item, lookup the hash
                       table to locate the ckpt-file record, open the ckpt file
                       if needed, and perform RR, write the RR data to this file
                     */
                    // ckpt_file_t *cfile = hash_table_get_record(ht_cfile,
                    // pkt->RR.rckptid, pkt->RR.rprocid, pkt->RR.larg1 );
                    // ckpt_file_t *cfile =
                    // hash_table_get_record(ht_cfile, "file", pkt->RR.larg1 );
                    ckpt_file_t *cfile;
                    cfile = hash_table_get_record(ht_cfile, pkt->RR.filename,
                                                  pkt->RR.is_last_chunk);
                    // dump_hash_table( ht_cfile );
                    cfile->adv_size += pkt->RR.size;
                    arg1 = (((uint64_t)qpidx) << 32) | slot;
                    arg2 = (uint64_t)conn;
                    /// insert the rqst to queue, one iothr will unblock and
                    /// work on it
                    workqueue_enqueue3(tpool->queue, pkt, sizeof(*pkt), arg1,
                                       arg2, (unsigned long)cfile);
                ...
                if (pkt->command == reply_RR) {
                    ...
                    // dbg("  RR reply : procid=%d, buf=%p, bufid = %d, size =
                    // %d\n",
                    //  pkt->RR.rprocid, pkt->RR.raddr, pkt->RR.rbuf_id,
                    //  pkt->RR.size );

                    if (pkt->RR.size > 0) // only free valid rdma-buf chunk
                        free_buf_slot(hca->rdma_buf, pkt->RR.rbuf_id, 0);

                    dbg("Finished RR: (%s) %ld@%ld, free rdma-buf id %d, "
                        "free-slots=%d\n",
                        pkt->RR.filename, pkt->RR.size, pkt->RR.offset,
                        pkt->RR.rbuf_id, hca->rdma_buf->free_slots);
            ...
            // sprintf(ib_buffer_slot_addr(conn->send_buf, i), "qp %d slot %d",
            // qpidx, i ); ret = ib_connection_post_send( conn, qpidx, i, 64 );
...
int ib_server_loop(struct ib_connection *connarray, struct thread_pool *tp)
{
    ...
    int num_conn = atomic_read(
        &hca->ref); // num of connections using this HCA, = num of client nodes
    struct pollfd pfd;

    // num_wc = 8; // 256; //hca->max_rq_wqe + num_conn *
    // connarray[0].num_qp*16; wc_array = malloc( num_wc * sizeof(struct
    // ibv_wc*) );
    ...
    // while( ! all_connection_finished(connarray, atomic_read( &hca->ref) ) )
    {
        ...
        do {
            ...
            for (i = 0; i < ne; i++) {
                ...
                if (find_qp_from_conn_array(wc->qp_num, connarray,
                                            atomic_read(&hca->ref), &connidx,
                                            &qpidx) != 0) {
...
int ib_client_loop(struct ib_connection *connarray)
{
    ...
    /*    //if( g_client_rank == 0 )
       ...
       g_num_srv); slot = get_buf_slot( hca->send_buf, (void**)&pkt, 0 );
                pkt->command = rqst_terminate;
                ib_connection_post_send( &connarray[i], qpidx, slot,
       sizeof(*pkt) ); connarray[i].status = connection_terminated;
            }
        }    */
...
err_out_1:
    ...
    for (i = 0; i < num_conn; i++) {
        ...
        // conn->recv_cqe, conn->hca->total_cqe);
    }
    // free(wc_array);
    return ret;
}
...
*/
void pass_hash_table(hash_table_t *ht) { ht_cfile = ht; }
