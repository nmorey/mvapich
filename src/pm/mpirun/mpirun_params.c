 ...
 *
 * mpirun [-debug] [-xterm] -np N [-hostfile hfile | h1 h2 h3 ... hN] a.out
 * [args]
 */
...
{
    fprintf(stderr,
            "usage: mpirun_rsh [-v] [-sg group] [-launcher rsh|ssh|srun] "
            "[-debug] -[tv] [-xterm] [-show] [-legacy] [-export|-export-all] "
            "-np N "
#if defined(CKPT) && defined(CR_FTB)
            "[-sparehosts sparehosts_file] "
#endif
            "(-hostfile hfile | h1 h2 ... hN) a.out args | -config configfile "
    ...
    fprintf(stderr,
            "\tlauncher   => "
            "one of rsh, ssh, or srun for connecing (ssh is default)\n");
    ...
    fprintf(stderr, "\texport     => "
                    "automatically export environment to remote processes\n");
    fprintf(stderr, "\texport-all => "
                    "automatically export environment to remote processes even "
                    "if already set remotely\n");
    fprintf(stderr, "\tnp         => "
                    "specify the number of processes\n");
    fprintf(stderr,
            "\tppn        => "
            "specify the number of processes to allocate to eachnode\n");
    ...
    fprintf(stderr, "\ta.out      => "
                    "name of MPI binary\n");
    fprintf(stderr, "\targs       => "
                    "arguments for MPI binary\n");
    fprintf(stderr, "\tconfig     => "
                    "name of file containing the exe information: each line "
                    "has the form -n numProc : exe args\n");
#if defined(CKPT) && defined(CR_FTB)
    fprintf(stderr,
            "\tsparehosts => "
            "file containing the spare hosts for migration, one per line\n");
#endif
    fprintf(stderr, "\n");
}
