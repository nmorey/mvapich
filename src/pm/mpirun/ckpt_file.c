...
ckpt_file_t *alloc_ckpt_file()
{
    ...
    MVP_INIT_LIST_HEAD(&cfile->list);
    MVP_INIT_LIST_HEAD(&cfile->chunk_list);
...
int free_ckpt_file(ckpt_file_t *cfile)
{
    ...
    if (!mvp_list_empty(&cfile->chunk_list)) {
        ...
        /*mvp_list_for_each_entry( ck, &(cfile->chunk_list), list )
           {
           //// free the buf-chunk
...
Check buf-content.
***/
unsigned long check_chunk_content(ckpt_file_t *cfile, ckpt_chunk_t *chunk,
                                  unsigned long size)
{
    ...
    dbg("****  (%s) %ld@%ld: lbuf-id=%d:  found %ld err(s)\n", cfile->filename,
        size, chunk->offset, chunk->bufid, err);

    return err;
}

/**
io-thr has finished RR a chunk, now store this chunk to the corresponding
ckpt-file. rrpkt: the ib-pkt that containing info about this chunk
**/
int add_chunk_to_ckpt_file(ckpt_file_t *cfile, struct ib_packet *rrpkt)
{
    ...
    mvp_list_for_each_entry(ck, &(cfile->chunk_list), list)
    {
        if (chunk->offset < ck->offset) {
            ...
            mvp_list_add(&chunk->list, ck->list.prev);
            break;
        }
    }
    if (!flag) { // should insert to tail of list
        mvp_list_add_tail(&chunk->list, &(cfile->chunk_list));
    }
...
void dump_ckpt_file(ckpt_file_t *cfile)
{
    ...
    int i = 0;
    // pthread_mutex_lock( &cfile->mutex );
    printf("ckpt-file (%s)(ckptid %d, proc-rank %d): ref=%d, has_last=%d, "
           "can-release=%d, adv_size=%ld "
           "chunklist:: \n",
           cfile->filename, cfile->ckpt_id, cfile->proc_rank,
           atomic_read(&cfile->ref), cfile->has_last_chunk, cfile->can_release,
           cfile->adv_size);

    mvp_list_for_each_entry(ck, &(cfile->chunk_list), list)
    {
        printf("\tChunk %d:  bufid %d, %ld@%ld, curr_pos %lu\n", i, ck->bufid,
               ck->size, ck->offset, ck->curr_pos);
        i++;
    }
    //  pthread_mutex_unlock( &cfile->mutex );
}
    ...
    while (!found) // must block until we get a chunk with that offset
    {
        ...
        mvp_list_for_each_entry(ck, &(cfile->chunk_list), list)
        {
            ...
            if (ck->offset == offset) {  // find the chunk
                mvp_list_del(&ck->list); // rm the chunk from chunk-list
                pthread_mutex_unlock(&cfile->mutex);
                // found = 1;
                return ck;
            }
