...
/*
 * This function is in part equal to the one present in mpiexec project from
 * OSC. Read the heterogeneous config file, making sure it's proper and all the
...
process *parse_config(char *configfile, int *nprocs)
{
    ...
    while (fgets(buf, sizeof(buf), fp)) {
...

        {
            ...
            if (*cq != ':')
                error("%s: line %d: no ':' separating executable", __func__,
                      line);
            ...
            *cq = 0; /* colon -> 0, further parsing easier */
            ...
            if (!*cq || *cq == '#')
                error("%s: line %d: no executable after the ':'", __func__,
                      line);
        ...
        if (*cp == '-') {
            if (*++cp == 'n') {
                ...
                for (cp = cq; *cp && isspace((int)*cp); cp++)
                    ;
                if (*cp)
                    error("%s: line %d: junk after \"-n <num>\"", __func__,
                          line);
        ...
        push(lastPtrRef, exe, args,
             numprocs); // Add node at the last pointer in the list
        lastPtrRef = &((*lastPtrRef)->next);
    }
    if (fp != stdin)
        fclose(fp);

    /*TODO Some check if the -np is specified and if the list of executables is
     * empty */
...
process *save_plist(config_spec_t *cfg_list, int nprocs)
{
    ...
    /* int i;
       for (i = 0; i < nprocs; i++) {
       printf("===============\n %s %s %d \n",plist[i].executable_name,
       plist[i].executable_args,plist[i].remote_pid);
       } */
    return plist;
}
...
char *add_argv(char *mpispawn_env, char *exe, char *args, int tmp_i)
{
    ...
    // free(mpispawn_env);
    mpispawn_env = tmp;
    /*The args are as a single string, instead in mpispawn we need to pass each
     * word as a single argument. */
    if (args != NULL) {
 ...
 * When mpmd is activated in spawn_one we send the list of hosts and for each
 * host we send the executable name, the number of argument and the list of
 * argument. The host_list has the following form:
 * host1:numProc:pid1:pid2..:pidN:exe:argc:arg1:..:argN
 */
...
char *create_host_list_mpmd(process_groups *pglist, process *plist)
{
    ...
    for (k = 0; k < pglist->npgs; k++) {
        /* Make a list of hosts, the number of processes on each host and the
         * executable and args. */
        /* NOTE: RFCs do not allow : or ; in hostnames */
        if (host_list)
            host_list = mkstr("%s:%s:%d", host_list, pglist->data[k].hostname,
                              pglist->data[k].npids);
        else
            host_list =
                mkstr("%s:%d", pglist->data[k].hostname, pglist->data[k].npids);
        if (!host_list) {
            error("ALLOCATION ERROR IN BUILD HOST_LIST \n");
        }
        for (n = 0; n < pglist->data[k].npids; n++) {
            host_list =
                mkstr("%s:%d", host_list, pglist->data[k].plist_indices[n]);

            if (!host_list) {
                error("ALLOCATION ERROR IN BUILD HOST_LIST \n");
            }
        }
        // We use the first of the plist in the group. In each group of
        // processes the exe is the same.
 ...
 * The host_list has the following form:
 * host1:numProc:pid1:pid2..:pidN:exe:argc:arg1:..:argN
 */

/*void parse_host_list_mpmd(int i, int mt_nnodes, char *host_list, char **host,
int** ranks, int *np, char **exe, char ***argv)
{
    int j = 0, k;
     while (i > 0) {
                if (i == mt_nnodes)
