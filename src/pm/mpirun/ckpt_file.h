...
#define HASH_FROM_FILENAME                                                     \
    1 // if this is defined, ckpt-files are hashed to htable using its
      // file-name;
...
typedef struct ckpt_chunk_s {
    ...
    struct list_head list; // link to all chunks belonging to a ckpt file
                           // (ckptid, proc_rank)
...
typedef struct ckpt_file_s {
    ...
    pthread_mutex_t io_mtx; // only 1 io-thr can write to this file at given
                            // time
    ...
    int can_release;    // Indicate: release() is called, and can release this
                        // struct if last_chunk is finished
    ...
    unsigned long adv_size; // advertised size of this file. Whenever a chunk is
                            // posted, adv_size is increased
    ...
    struct list_head chunk_list; // for recv: link all ready ckpt-chunks
                                 // belongint to this ckptfile
    ...
    sem_t sem; // for recv: those who want to read data wait on this sem, for RR
               // to fetch the chunk;
    ...
    struct timeval tend;
    // for sender: those who has data to write wait on this sem, for a buf-chunk
    // to be avail
} ckpt_file_t;
