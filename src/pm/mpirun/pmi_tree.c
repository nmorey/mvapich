...
#define CR_DBG(args...)                                                        \
    do {                                                                       \
        fprintf(stderr, "\t [mtid %d][%s: line %d]", mt_id, __FILE__,          \
                __LINE__);                                                     \
        fprintf(stderr, args);                                                 \
    } while (0)
#else
...
/* #define dbg(fmt, args...)  do{ \
    fprintf(stderr,"%s: [mt_id %d]: "fmt, __func__, mt_id,
...
int read_size(int fd, void *msg, int size)
{
    ...
    while (n < size) {
        ...
        if (rc < 0) {
            ...
            if (!cr_mig_src) {
#endif
                PRINT_ERROR("Unexpected End-Of-File on file descriptor %d. MPI "
                            "process died?\n",
                            fd);
#if defined(CKPT) && defined(CR_FTB)
            }
...
/**
 * Some message is not sent by all local nodes, in those cases the aggregation
 * fails. Some other message may need to be delivered soon. We check for those
 * messages and we send them without aggregating.
 */
static char *flushing_messages[] = {"sharedFilename", "MVPBUF", "bcast",
                                    "allgather", NULL};
...
int check_pending_puts(void)
{
    ...
    /* Check if one of the messages in the pending list is listed in
       flushing_messages. If so, we send them now */
     ...
     * this batched put logic relies on having exactly NCHILD pushes, which we
     * won't have. The bcast push is delayed, which then get's combined with an
     * allgather push later and changes the offset. This eventually propegates
     * to the last piece of the allgather which has no partner and is lost.
     *
     * This also violates the PMI standard, which has never required that PUSHs
     * be symetric. Until we can find a better design (or some kind of on-demand
     * flush from the requesting process kind of like a page fault) we are
     * removing this logic and flushing everything
     */
    ...
    while (iter && !msg_flush) {
        ...
        while ((flushing_messages[i] != NULL) && (!msg_flush)) {
            if (strstr(iter->kvc_key, flushing_messages[i])) {
                msg_flush = 1;
            }
            i++;
        }
    ...
    switch (strlen(command)) {
        ...
        case 4: /* init */
            if (0 == strcmp(command, "init")) {
                if (pmi_version[0] == PMI_VERSION && pmi_version[1] == '\0' &&
                    ...
                    pmi_subversion[1] == '\0') {
                    sprintf(resp,
                            "cmd=response_to_init pmi_version=%c "
        ...
        case 7: /* initack */
            if (0 == strcmp(command, "initack")) {
                ...
                if (i == NCHILD) {
                    dbg("*********** Error::  got initack for child-%d of %d, "
                        "no match\n",
                        i, NCHILD);
                    rv = ERR_DEF;
                    goto exit_err;
        ...
        case 8: /* finalize */
            if (0 == strcmp(command, "finalize")) {
                ...
                PRINT_DEBUG(DEBUG_FT_verbose,
                            "barrier_count = %d, NCHILD = %d, MPISPAWN_NCHILD "
                            "= %d, exclude_spare = %d\n",
                            barrier_count, NCHILD, MPISPAWN_NCHILD,
                            exclude_spare);
        ...
        case 9: /* get_maxes */
            if (0 == strcmp(command, "get_maxes")) {
                sprintf(resp,
                        "cmd=maxes kvsname_max=%d keylen_max=%d "
                        "vallen_max=%d\n",
        ...
        case 10: /* get_appnum, get_result, put_result, barrier_in */
            if (0 == strcmp(command, "get_result")) {
                ...
                PRINT_DEBUG(
                    DEBUG_FT_verbose,
                    "MPISPAWN_HAS_PARENT = %d, barrier_count = %d, NCHILD = "
                    "%d, MPISPAWN_NCHILD = %d, exclude_spare = %d\n",
                    MPISPAWN_HAS_PARENT, barrier_count, NCHILD, MPISPAWN_NCHILD,
                    exclude_spare);
                if (barrier_count == (NCHILD + MPISPAWN_NCHILD - exclude_spare))
#else
                if (barrier_count == (NCHILD + MPISPAWN_NCHILD))
#endif
                {
                    ...
                    if (MPISPAWN_HAS_PARENT) {
                        ...
                        CR_DBG(
                            "[mt_id %d on %s] ***rank %d GOTO barrier_out \n",
                            mt_id, my_hostname, rank);
                        goto barrier_out;
                    }
                }
        ...
        case 17: /* get_universe_size */
            if (0 == strcmp(command, "get_universe_size")) {
                ...
                int uni_size_value =
                    -1; /* MPIR_UNIVERSE_SIZE_NOT_SET defined in mpiimpl.h */
                if ((uni_size_str = getenv("MVP_UNIVERSE_SIZE")) != NULL) {
                    uni_size_value = atoi(uni_size_str);
                    if (uni_size_value <= 0) {
                        uni_size_value = -1; /* MPIR_UNIVERSE_SIZE_NOT_SET
                                                defined in mpiimpl.h */
                    }
                }
...
int mtpmi_init(void)
{
    ...
    if (env2int("MPISPAWN_USE_TOTALVIEW") == 1) {
        ...
        if (MPISPAWN_NCHILD) {
            all_pinfo =
                (process_info_t *)malloc(process_info_s * nchild_subtree);
            if (!all_pinfo) {
                mpispawn_abort(ERR_MEM);
            }
...
int mtpmi_processops(void)
{
    ...
    while (rv == 0) {
        ...
        /*        if( ready>0 )
                dbg("select() ret ready=%d, cleanup=%d, cr_mig_tgt=%d,
        ...
        if (ready == 0) {
            ...
            if (MPISPAWN_HAS_PARENT &&
                ...
                if (rv != 0) {
                    //                     PRINT_ERROR("handle_mt_peer() failed
                    //                     on file descriptor %d with rank
                    //                     %d\n", MPISPAWN_PARENT_FD,
                    //                     hdr.msg_rank);
                }
            }
            ...
            for (i = 0; i < MPISPAWN_NCHILD; i++) {
                if (FD_ISSET(MPISPAWN_CHILD_FDS[i], &child_socks)) {
                    ...
                    if (rv != 0) {
                        //                     PRINT_ERROR("handle_mt_peer()
                        //                     failed on file descriptor %d with
                        //                     rank %d\n",
        ...
        if (MPISPAWN_HAS_PARENT && FD_ISSET(MPISPAWN_PARENT_FD, &child_socks)) {
            ...
            rv = handle_mt_peer(MPISPAWN_PARENT_FD, &hdr);
            if (rv != 0) {
                //             PRINT_ERROR("handle_mt_peer() failed on file
                //             descriptor %d with rank %d\n",
        ...
        for (i = 0; rv == 0 && ready > 0 && i < MPISPAWN_NCHILD; i++) {
            if (FD_ISSET(MPISPAWN_CHILD_FDS[i], &child_socks)) {
                ready--;
                read_size(MPISPAWN_CHILD_FDS[i], &hdr, msg_hdr_s);
                dbg(">mtpmi_process(v) MT_CHILD (%d of %d): msg-len=%d, "
                    "ready=%d\n\n",
...
#endif
                        PRINT_ERROR("Error while reading PMI socket. MPI "
                                    "process died?\n");
                        mpispawn_abort(MPISPAWN_PMI_READ_ERROR);
#if defined(CKPT) && defined(CR_FTB)
                    }
...
char *handle_spawn_request(int fd, char *buf, int buflen)
{
    ...
    fp = fopen(fname, "w");
    for (j = 0; j < spcnt; j++) {
        /* Continue to readline until it receives the end of the command
         * indicated by endcmd. It first read the size of the message and then
         * the message. It writes each message in the tmp file. */
        do {
