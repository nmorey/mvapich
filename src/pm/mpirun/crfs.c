...
extern struct thread_pool *iopool; // only crfs-client needs to expose a
                                   // aggre-io-pool to the FS itself
...
static inline void crfs_fullpath(char *fpath, const char *path)
{
    ...
    strncat(fpath, path,
            MAX_PATH_LEN); // ridiculously long paths will break here

    log_msg(
        "    crfs_fullpath:  rootdir = \"%s\", path = \"%s\", fpath = \"%s\"\n",
...
/**
 * Initialize filesystem * The return value will passed in the private_data
 ...
 * Introduced in version 2.3
 */
int crfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
                 off_t offset, struct fuse_file_info *fi)
{
    ...
    //    log_msg("\ncrfs_readdir(path=\"%s\", buf=0x%08x, filler=0x%08x,
    //    offset=%lld, fi=0x%08x)\n",
    //      path, buf, filler, offset, fi);
    dbg("    (path=\"%s\", buf=0x%08x, filler=0x%08x, offset=%lld, "
        "fi=0x%08x)\n",
        path, buf, filler, offset, fi);
...
int crfs_getattr(const char *path, struct stat *statbuf)
{
    ...
    if (crfs_mode == MODE_MIG) { // at srv-side: proc-migration needs the
                                 // file-size to be large enough
        // to restart a proc
...
// returned by read.
int crfs_read(const char *path, char *buf, size_t size, off_t in_offset,
              struct fuse_file_info *fi)
{
    /***********************
    Attention:  fuse uses multi-thread internally to perform read. So, it's
    possible that multi-threads come here at same time, not in order of
    "offset". Can disable this multi-thread behavior by: "./crfs xx yy  -s"
    ***********************/
    int retstat = 0;

    // log_msg("\ncrfs_read(path=\"%s\", buf=0x%08x, size=%d, offset=%lld,
    // fi=0x%08x, ret=%d)\n",
    ...
    while (to_read > 0) {
        ...
        if (chunk) {
            if (offset <
                chunk->offset + chunk->curr_pos) { // this should never happen
            ...
            if (offset >=
                (chunk->offset + chunk->size)) { // this should never happen...
            ...
            ///////
            // dbg("before copy:  to copy (%d, %d), chunk is::  ", offset,
            // to_read); dump_chunk( chunk );
            ////////////
            cpsize = chunk->size -
                     chunk->curr_pos; // has this much avail data in this chunk
            ...
            offset += cpsize;
            ////////
            // dbg("After copy:  to copy (%d, %d), chunk is::  ", offset,
            // to_read); dump_chunk( chunk );
            //////////////////////
            ...
            if (cfile->curr_chunk == NULL) // the ckpt-file is completed, ret
                break;
            if (cfile->curr_chunk->size ==
                0) { // a dummy chunk: signify the end of file
 ...
 * Changed in version 2.2
 */
int crfs_write(const char *path, const char *buf, size_t size, off_t offset,
               struct fuse_file_info *fi)
{
    int retstat = 0;

    // log_msg("\ncrfs_write(path=\"%s\", buf=0x%08x, size=%d, offset=%lld,
    // fi=0x%08x)\n",
    ...
    while (to_write > 0) {
        ...
        if (chunk) {
            ...
            // offset); dump_chunk( chunk );
            ////////////
            cpsize = chunk->size -
                     chunk->curr_pos; // has this much avail space in this chunk
            ...
            offset += cpsize;
            ////////
            // dbg("After copy:  to copy (%d@%d), chunk is::  ", to_write,
            // offset); dump_chunk( chunk );
            ...
            chunk->offset = offset; // this chunk is at this logical-offset of
                                    // the whole file
...
int crfs_fsync(const char *path, int datasync, struct fuse_file_info *fi)
{
    ...
    //  cfile->adv_size, us/1000 );

    // if( crfs_mode == CRFS_CLIENT ) /// at ckpt-fs(client-side): flush the
    // "curr_chunk"
    if (crfs_mode ==
        MODE_WRITEAGGRE) /// at ckpt-fs(client-side): flush the "curr_chunk"
    {
        ...
        if (chunk) {
            ...
            atomic_inc(&minfo.chunk_cnt);
            workqueue_enqueue(iopool->queue, chunk, sizeof(*chunk), 0, 0);
            //////retstat = pwrite(cfile->fd, chunk->buf, chunk->curr_pos,
            /// chunk->offset);
            free(chunk);
            cfile->curr_chunk = NULL;
        }
...
int crfs_flush(const char *path, struct fuse_file_info *fi)
{
    ...
    filename[128 - 1] = 0;
    hash_bucket_t *bkt =
        htbl_find_lock_bucket(g_ht_cfile, filename, strlen(filename));
    ...
    if (crfs_mode == MODE_WRITEAGGRE) // at ckpt-fs(client-side)
    {
        ...
        // hash_bucket_t* bkt = htbl_find_lock_bucket(g_ht_cfile, filename,
        // strlen(filename)); if( bkt )
...
int crfs_release(const char *path, struct fuse_file_info *fi)
{
    ...
    if (crfs_mode == MODE_MIG) // at mig-FS serv side: for migration
    {
        // pthread_mutex_lock( &cfile->mutex );
        if (!mvp_list_empty(
                &cfile->chunk_list)) { /// clear all remaining chunk-bufs
            error("free cfile: ckpt-%d-proc-%d: chunk-list not empty!!!\n",
                  cfile->ckpt_id, cfile->proc_rank);
            dump_ckpt_file(cfile);
            mvp_list_for_each_entry_safe(ck, tmpck, &(cfile->chunk_list), list)
            {
                //// free the buf-chunk
                free_buf_slot(hca.rdma_buf, ck->bufid, 0);
                mvp_list_del(&ck->list);
                free(ck);
            }
        }
        ck = cfile->curr_chunk;
    ...
    } else // at ckpt-fs(client side), only with write-aggre
    {
        ...
        if (ck) // this should never happen!!!!
        {
            ...
            printf("\t%s: %s: flush final chunk: (%ld@%ld)\n", __func__,
                   cfile->filename, ck->curr_pos, ck->offset);
            ck->is_last_chunk = 1;
    ...
    /// an iothr may have cleaned the hash-tbl to remove this file
    hash_bucket_t* bkt = htbl_find_lock_bucket(g_ht_cfile, filename,
    strlen(filename)); if( bkt ) {    // has located the cfile-record, and
    locked the hosting hash-bucket if( hash_table_put_record( g_ht_cfile, cfile,
    1)>0 )
        {
            //dbg(" [%s] fi=%p, set fi->fh to NULL...\n", cfile->filename, fi );
            fi->fh = NULL;
...
At ckpt-fs(client) side, set the "target of migration", so client can connect to
it. Usually the "path" can be simply set as "/". Or, the "path" must be an
existing file in the FS.
**/
int crfs_setxattr(const char *path, const char *name, const char *value,
                  size_t size, int flags)
{
    ...
    } else if (strcmp(name, "migration.state") == 0) // a migration is finished
    {
        ...
        {
            if (mig_state == 1 &&
                mig_role == ROLE_MIG_SRC) { // has an active mig
                mig_state = 0;
                ibcli_end_mig(&minfo);
                memset(&minfo, 0, sizeof(minfo));
            } else
...
struct fuse_operations crfs_oper = {
    ...
    .flush = crfs_flush, // when app calls close(), translates to: flush(); then
                         // release()
...
{
    fprintf(stderr,
            "Usage: crfs  [real-work-dir] [mount-Point] <-opt1> <-opt2> ");
}
...
int crfs_main(int pfd, int argc, char *argv[])
{
    ...
    ////////// check the params:   ./crfs [real-work-dir] [mount-Point] <-opt1>
    ///<-opt2>
    if (argc < 3) {
        crfs_usage();
        return -1;
