...
static int mig_ftb_init()
{
    ...
    strcpy(ftb_cinfo.client_name, "MVP_MIGRATE");

    /* sessionid should be <= 16 bytes since client_jobid is 16 bytes. */
    snprintf(ftb_cinfo.client_jobid, FTB_MAX_CLIENT_JOBID, "%d",
             16); // sessionid
    ...
    ret = FTB_Declare_publishable_events(ftb_handle, NULL, mig_ftb_events,
                                         1); // max events in mig_ftb_events
    if (ret != FTB_SUCCESS)
...
err_connect:
    fprintf(stderr, "FTB_Connect() in MVP_MIGRATE failed with %d\n", ret);
    ret = -1;
    goto exit_connect;

err_declare_events:
    fprintf(stderr,
            "FTB_Declare_publishable_events() in MVP_MIGRATE failed with %d\n",
            ret);
    ret = -2;
...
int main(int argc, char *argv[])
{
    fprintf(stderr, "Error: please enable FTB to use mvp_trigger\n");
    return 1;
}
