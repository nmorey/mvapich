...
amdirs=". src/mpl src/pmi osu_benchmarks"
...
fn_getpvars() {
    echo_n "Extracting performance variables (pvar) ... "
    if test ! -x maint/extractmvppvars ; then
        fn_maint_configure
    fi

    if ./maint/extractmvppvars ; then
        echo "done"
    else
        echo "failed"
        error "unable to extract performance variables"
        exit 1
    fi
}
