...
mvp_pvar_file = "src/mpi_t/mvp_coll_pvars.txt"
mvp_pvar_arr = []
...
def main():
    ...
    G.algos = load_coll_algos("src/mpi/coll/mvapich_coll_algorithms.txt")
    ...
    init_mvp_pvar(mvp_pvar_file)
    dump_algo_pvar_decl(mvp_pvar_file)
    for a in coll_names:
        dump_coll_pvar_decl(mvp_pvar_file, a, "blocking")
        dump_coll_pvar_decl(mvp_pvar_file, a, "nonblocking")
        dump_coll(a, "blocking")
        dump_coll(a, "nonblocking")
        dump_coll(a, "persistent")
    end_mvp_pvar(mvp_pvar_file)
...
def dump_allcomm_auto_blocking(name):
    ...
    def dump_cnt_algo_blocking(algo, commkind):
        ...
        dump_split(3, "MPIR_T_PVAR_COUNTER_INC(MVP, %s, 1);" % (algo_to_pvar_counter(algo)))
...
def dump_mpir(name, blocking_type):
    ...
    if (func_name in mvp_pvar_arr):
        pvar_call_counter = "mvp_%s_call_counter" % (func_name)
        pvar_timer = "mvp_%s_timer" % (func_name)
        G.out.append("MVP_COLL_PVAR_UPDATE_START(%s, %s);" % (pvar_call_counter, pvar_timer))
    ...
    G.out.append("")
    if (func_name in mvp_pvar_arr):
        G.out.append("MVP_COLL_PVAR_UPDATE_END(%s);" % (pvar_timer))
        G.out.append("")
    G.out.append("return mpi_errno;")
    dump_close("}")
...
def init_mvp_pvar(f):
    with open(f, "w") as Out:
        print("=== BEGIN_MPI_T_MVP_PVAR_INFO_BLOCK ===", file=Out)
        print("pvars:", file=Out)

def end_mvp_pvar(f):
    with open(f, "a") as Out:
        print("=== END_MPI_T_MVP_PVAR_INFO_BLOCK ===", file=Out)

def dump_coll_pvar_decl(f, fn_name, blocking_type):
    coll = get_func_name(fn_name, blocking_type)
    Coll = coll.capitalize()
    mvp_pvar_arr.append(coll)
    with open(f, "a") as Out:
        print("    - type : COUNTER", file=Out)
        print("      module : MVP", file=Out)
        print("      datatype :  MPI_UNSIGNED_LONG_LONG", file=Out)
        print("      name :  mvp_%s_call_counter" % (coll), file=Out)
        print("      verbosity : MPI_T_VERBOSITY_USER_BASIC", file=Out)
        print("      bind :  MPI_T_BIND_NO_OBJECT", file=Out)
        print("      flags : MPIR_T_PVAR_FLAG_READONLY | MPIR_T_PVAR_FLAG_SUM", file=Out)
        print("      category : %s Collective" % (Coll), file=Out)
        print("      description : >-", file=Out)
        print("        Number of calls to %s" % (Coll), file=Out)
        print("", file=Out)
        print("    - type : TIMER", file=Out)
        print("      module : MVP", file=Out)
        print("      datatype :  MPI_DOUBLE", file=Out)
        print("      name :  mvp_%s_timer" % (coll), file=Out)
        print("      verbosity : MPI_T_VERBOSITY_USER_BASIC", file=Out)
        print("      bind :  MPI_T_BIND_NO_OBJECT", file=Out)
        print("      flags :  MPIR_T_PVAR_FLAG_SUM", file=Out)
        print("      category : %s Collective" % (Coll), file=Out)
        print("      description : >-", file=Out)
        print("        Total time spent in %s" % (Coll), file=Out)
        print("", file=Out)

def algo_to_slug(algo):
    (func_name, commkind) = algo['func-commkind'].split('-')
    return "%s_%s_%s" % (func_name, commkind, algo['name'])

def algo_to_pvar_counter(algo):
    return "mvp_%s_call_counter" % (algo_to_slug(algo))

def dump_algo_pvar_decl(f):
    with open(f, "a") as Out:
        for key in G.algos:
            for algo in G.algos[key]:
                (func_name, commkind) = algo['func-commkind'].split('-')
                algo['func-commkind'].split('-')
                slug = algo_to_slug(algo)
                pvar = algo_to_pvar_counter(algo)
                print("    - type : COUNTER", file=Out)
                print("      module : MVP", file=Out)
                print("      datatype :  MPI_UNSIGNED_LONG_LONG", file=Out)
                print("      name :  %s" % (pvar), file=Out)
                print("      verbosity : MPI_T_VERBOSITY_USER_BASIC", file=Out)
                print("      bind :  MPI_T_BIND_NO_OBJECT", file=Out)
                print("      flags : MPIR_T_PVAR_FLAG_READONLY | MPIR_T_PVAR_FLAG_SUM", file=Out)
                print("      category : %s Algorithms" % (key), file=Out)
                print("      description : >-", file=Out)
                print("        Number of calls to %s" % (slug), file=Out)
                print("", file=Out)
