                          ...
                          size_t iov_offset, int dir)
{
    ...
    for (i = 0; i < iov_count; i++) {
        ...
        size_t len = ofi_iov_bytes_to_copy(hmem_iov + i, &buf_size, &iov_offset,
                                           &hmem_buf);
        if (!len)
            continue;

        ret = ucr_copy_hmem(local_desc, buf + data_size, hmem_buf, len, dir);
        if (ret)
            return ret;
                      ...
                      int dir)
{
    ...
    if (desc) {
        iface = ((struct ucr_mr *)desc)->iface;
        device = ((struct ucr_mr *)desc)->device;
        flags = ((struct ucr_mr *)desc)->hmem_flags;
        hmem_handle = ((struct ucr_mr *)desc)->hmem_handle;
    }
    ...
    if (flags & OFI_HMEM_DATA_DEV_REG_HANDLE) {
        ...
        if (dir == OFI_COPY_IOV_TO_BUF) {
            ofi_hmem_dev_reg_copy_from_hmem(iface, (uint64_t)hmem_handle,
                                            host_buf, dev_buf, size);
        } else {
            ofi_hmem_dev_reg_copy_to_hmem(iface, (uint64_t)hmem_handle, dev_buf,
                                          host_buf, size);
        }
        return FI_SUCCESS;
    }
