dnl Configury specific to the libfabric mverbs provider
...
AC_DEFUN([FI_MVERBS_CONFIGURE],[
	...
	MVERBS_HAVE_QUERY_EX=0
	AS_IF([test $verbs_ibverbs_happy -eq 1],[
		AC_CHECK_DECL([ibv_query_device_ex],
			[MVERBS_HAVE_QUERY_EX=1],[],
			[#include <infiniband/verbs.h>])
		])
	AC_DEFINE_UNQUOTED([MVERBS_HAVE_QUERY_EX],[$MVERBS_HAVE_QUERY_EX],
	...
	AC_DEFINE_UNQUOTED([MVERBS_HAVE_XRC],[$MVERBS_HAVE_XRC],
		[Whether infiniband/verbs.h has XRC support or not])

	#See if we have rdma-core rdma_establish support
	MVERBS_HAVE_RDMA_ESTABLISH=0
	AS_IF([test $verbs_ibverbs_happy -eq 1 && \
	       test $verbs_rdmacm_ex_happy -eq 1],[
		AC_CHECK_DECL([rdma_establish],
			[MVERBS_HAVE_RDMA_ESTABLISH=1],[],
			[#include <rdma/rdma_cma.h>])
		])
	AC_DEFINE_UNQUOTED([MVERBS_HAVE_RDMA_ESTABLISH],[$MVERBS_HAVE_RDMA_ESTABLISH],
		[Whether rdma/rdma_cma.h has rdma_establish() support or not])

	#See if we have rdma-core dmabuf mr support
	MVERBS_HAVE_DMABUF_MR=0
	AS_IF([test $verbs_ibverbs_happy -eq 1],[
		AC_CHECK_DECL([ibv_reg_dmabuf_mr],
			[MVERBS_HAVE_DMABUF_MR=1],[],
			[#include <infiniband/verbs.h>])
		])
	AC_DEFINE_UNQUOTED([MVERBS_HAVE_DMABUF_MR],[$MVERBS_HAVE_DMABUF_MR],
	...
	# below.
	mverbs_CPPFLAGS=$verbs_ibverbs_CPPFLAGS
	mverbs_LDFLAGS=$verbs_ibverbs_LDFLAGS
	mverbs_LIBS="$verbs_rdmacm_LIBS $verbs_ibverbs_LIBS"
	AC_SUBST(mverbs_CPPFLAGS)
	AC_SUBST(mverbs_LDFLAGS)
	AC_SUBST(mverbs_LIBS)
])
...
dnl
AC_DEFUN([FI_MVERBS_DOUBLE_CHECK_LIBIBVERBS],[
