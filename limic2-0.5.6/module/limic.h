...
typedef enum { CPY_TX, CPY_RX } limic_copy_flag;

int limic_map_and_copy(limic_request *req, struct page **maplist,
    ...
    while ((pg_num <= lu->nr_pages) && (len > 0)) {
        ...
        if (flag == CPY_TX) {
            if (copy_from_user(kaddr + offset, buf, pcount)) {
                printk(
                    "LiMIC: (limic_map_and_copy) copy_from_user() is failed\n");
                return -EFAULT;
            }
        } else if (flag == CPY_RX) {
            if (copy_to_user(buf, kaddr + offset, pcount)) {
                printk(
                    "LiMIC: (limic_map_and_copy) copy_to_user() is failed\n");
                return -EFAULT;
            }
        }
...
struct page **limic_get_pages(limic_user *lu, int rw)
{
    ...
    err = get_user_pages(lu->tsk, mm, lu->va, pgcount, (rw == READ), 0, maplist,
                         NULL);
    up_read(&mm->mmap_sem);
